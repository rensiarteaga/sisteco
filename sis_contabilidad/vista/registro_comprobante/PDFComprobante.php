...
{   
	...
	function Header(){
		...
		if($_SESSION['id_comprobante']==202001){
			$this->Cell(40,3.5,'$us.',1,1,'C');
		}else{
	...
	function Footer(){
		...
		$suma_total_debe=0.00;
		$suma_total_haber=0.00;
		...
		if((number_format($suma_total_debe,2))==(number_format($suma_total_haber,2))){
			$this->MultiCell(214,3,'               Son: '.$_SESSION['PDF_sum_total_literal'].' '.$vdebe[0][8],'LRB');
		}else{
			$this->MultiCell(214,3,'          Los montos de débitos y créditos no son iguales. D-'.number_format($suma_total_debe,2).' H-'.number_format($suma_total_haber,2),'LRB');
	    }
		...
		if ($_SESSION['desc_clases']=='COMPROBANTE PAGO'){
			...
			for($i=0;$i<=3;$i++){
				...
				//$firmas[$i][2]=$firmas_obtenidas[1][$i]; //para frimas de otro periodo
			...
			$firmas[0][0]=$firmas_obtenidas[0][0];
			$firmas[0][1]='PREPARADO POR:';
			$firmas[0][2]='';
			$firmas[0][3]='';
			
			
			$firmas[1][0]=$firmas_obtenidas[0][1];
			$firmas[1][1]=$firmas_obtenidas[1][1];
			$firmas[1][2]='';
			$firmas[1][3]='';
			
			
			$firmas[2][0]='JEFE CONTABILIDAD CENTRAL INTERINO';
			$firmas[2][1]='CONSULTOR DE LINEA';
			$firmas[2][2]='';
			$firmas[2][3]='';
