...
function main(){
	...
	elemento={pagina:new pagina_registro_comprobante(idContenedor,direccion,paramConfig,sw_vista),idContenedor:idContenedor};
...
function pagina_registro_comprobante(idContenedor,direccion,paramConfig , sw_vista){
	...
	var ds = new Ext.data.Store({
		...
		'desc_cbte',
		'sw_activo_fijo',
		'cbtes_depen','variacion_tc','sw_bancariza' //ago2015
		,'sw_omite_fe'//oct2016
		]),
	...
	function render_cbtes_depen(value,cell,record,row,colum,store){
		if(store.getAt(row).data['cbtes_depen'] > 0){
			...
			if(store.getAt(row).data['sw_bancariza']=='si'){//ago2015
				return  '<span style="color:red;"><b>' + value +' (B)</b></span>'  
			}else{
		...
		if(store.getAt(row).data['cbtes_depen'] == 0){
			if (sw_vista!='cbte_fe'){
              ...
              if(store.getAt(row).data['sw_bancariza']=='si'){//ago2015
					if(store.getAt(row).data['sw_omite_fe']=='total'){//ago2015
						return  '<span style="color:green;"><b>' + value +'(B)</b></span>' 
					}else{

						if(store.getAt(row).data['sw_omite_fe']=='parcial'){//ago2015
							return  '<span style="color:orange;"><b>' + value +'(B)</b></span>' 
						}
						else
							return '<b>'+  value+' (B)</b>'
				    }
	...
	Atributos[16]={
		...
		filtro_0:true,
		//filtro_1:true,
		filterColValue:'PERSON_15.apellido_paterno#PERSON_15.apellido_materno#PERSON_15.nombre',
		id_grupo:0
	};
...
console.log(sw_vista);
	if(sw_vista=='cbte_fe'){
		var config={titulo_maestro:'registro_comprobante',grid_maestro:'grid-'+idContenedor,urlHijo:'../../../sis_contabilidad/vista/registro_transaccion2/registro_transaccion_fe.php?idSub='+decodeURI('Transaccion Detalle')+'&'};
	}else{
	...
	if(sw_vista=='libro_diario' || sw_vista=='cbte_fe' ){ //sept2016
	...
	this.btnEliminar=function(){
		...
		if(NumSelect==1){
			...
			if(sw){
				Ext.MessageBox.prompt('Inserta la Observacion:', "Una vez eliminado el Comprobante 'No podra ser recuperado y revertira los procesos del subsistema "+getSubsistema(g_subsistema)+"' Esta seguro de Continuar ",observacion_eliminar)
			}
		}
		if(NumSelect==0){
		...
		if(NumSelect>1){
			Ext.MessageBox.alert('Estado', 'Selecione un solo registro a la vez.')
		}
	}
	...
	var paramFunciones={
		...
		Formulario:{
			...
			height:450,width:550,minWidth:150,minHeight:200,columnas:['95%'],
			grupos:[
	...
	this.EnableSelect=function(sm,row,rec){
		...
		if(sw_vista=='cbte_fe'){
  			if(rec.data.sw_omite_fe=='total' || rec.data.sw_omite_fe=='parcial'){
  				CM_getBoton('finalizar_fe-'+idContenedor).disable();
  				CM_getBoton('omitir_fe-'+idContenedor).disable();
  				CM_getBoton('corregir_fe-'+idContenedor).enable();
  				
  			}else{
  				CM_getBoton('finalizar_fe-'+idContenedor).enable();
  				CM_getBoton('omitir_fe-'+idContenedor).enable();
  				CM_getBoton('corregir_fe-'+idContenedor).disable();
  	  		}
		}
	...
	function mostrar_respuesta(resp){
		...
		if(sw_vista!='cbte_fe'){
	     ...
	     Ext.MessageBox.alert("Validacion ","Omision de Cbte realizado ");
	     ds.load({params:{//start:0,
				limit: paramConfig.TamanoPagina,CantFiltros:paramConfig.CantFiltros,
		m_ids_depto:g_ids_depto,
		m_fecha_inicio:g_fecha_inicio,
	...
	function btn_validar(){
		...
		if(NumSelect!=0){
			...
			if(datas_edit.variacion_tc!=1){
	               mensaje_validacion='Existe variacion en T/C respecto al dia de ayer. Esta seguro de Validar Comprobante?.';
					
				}else{
					 mensaje_validacion='Esta seguro de Validar Comprobante?.';
				}

			Ext.MessageBox.confirm("Atencion",mensaje_validacion,function(btn){if(btn=='yes'){
				Ext.MessageBox.show({
	...
	function btn_validar_igualar(){
		...
		if(NumSelect!=0){
			...
			datas_edit=sm.getSelected().data;
			if(datas_edit.variacion_tc!=1){
	               mensaje_validacion='Existe variacion en T/C respecto al dia de ayer. Esta seguro de Validar e Igualar Comprobante?';
					
				}else{
					 mensaje_validacion='Esta seguro de Validar e Igualar Comprobante?';
				}
	...
	function btn_omitir_fe(){
		var sm=getSelectionModel();
		
		var NumSelect=sm.getCount();
		if(NumSelect!=0){
			datas_fe=sm.getSelected().data;
			g_comprobante=datas_fe.id_comprobante;
			/*g_id_moneda=g_id_moneda;
			g_simbolo=g_simbolo;
			g_titulo=datas_edit.titulo_cbte;
			g_cbte=datas_edit.momento_cbte;*/
		
			Ext.MessageBox.confirm("Atencion","Esta seguro de Omitir este cbte para FE?",function(btn){if(btn=='yes'){
				Ext.MessageBox.show({
					title: 'Validando comprobante...',
					msg:"<div><img src='../../../lib/ext-yui/resources/images/default/grid/loading.gif'/> Modificando Comprobante...</div>",
					width:150,
					height:200,
					closable:false
				});
				Ext.Ajax.request({
					url:direccion+"../../../control/comprobante/ActionMarcarCbteFE.php",
					success:mostrar_respuesta,
					params:{accion:'omitir',id_comprobante:datas_fe.id_comprobante},
					failure:ClaseMadre_conexionFailure,
					timeout:paramConfig.TiempoEspera
				});
			} });
		...
		if(NumSelect!=0){
			...
			Ext.MessageBox.confirm("Atencion","Esta seguro de Omitir este cbte para FE?",function(btn){if(btn=='yes'){
				...
				Ext.Ajax.request({
					url:direccion+"../../../control/comprobante/ActionMarcarCbteFE.php",
					success:mostrar_respuesta,
					params:{accion:'finalizar',id_comprobante:datas_fe.id_comprobante},
					failure:ClaseMadre_conexionFailure,
					timeout:paramConfig.TiempoEspera
				});
			} });
	...
	function btn_corregir_fe(){
		...
		if(NumSelect!=0){
			...
			Ext.MessageBox.confirm("Atencion","Esta seguro de corregir este cbte para FE?",function(btn){if(btn=='yes'){
				Ext.MessageBox.show({
					title: 'Verificando comprobante...',
					msg:"<div><img src='../../../lib/ext-yui/resources/images/default/grid/loading.gif'/> Verificando Comprobante...</div>",
					width:150,
					height:200,
					closable:false
				});
				Ext.Ajax.request({
					url:direccion+"../../../control/comprobante/ActionMarcarCbteFE.php",
					success:mostrar_respuesta,
					params:{accion:'corregir',id_comprobante:datas_fe.id_comprobante},
					failure:ClaseMadre_conexionFailure,
					timeout:paramConfig.TiempoEspera
				});
			} });
	...
	if(sw_vista=='cbte_fe'){
		this.AdicionarBoton('../../../lib/imagenes/print.gif','Imprimir Comprobante',btn_reporte_comprobante,true,'reporte_comprobante','');
		this.AdicionarBoton('../../../lib/imagenes/cross.gif','Omitir todo el Cbte en FE',btn_omitir_fe,true,'omitir_fe','');
		this.AdicionarBoton('../../../lib/imagenes/book_next.png','Finalizar FE',btn_fin_fe,true,'finalizar_fe');
		this.AdicionarBoton('../../../lib/imagenes/book_previous.png','Corregir FE',btn_corregir_fe,true,'corregir_fe');
		this.AdicionarBotonCombo(fecha_inicio,'Inicio');
		this.AdicionarBotonCombo(fecha_fin,'Fin');
	}
