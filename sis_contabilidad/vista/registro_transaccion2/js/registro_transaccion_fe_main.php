...
function main(){
	...
	var elemento={pagina:new pagina_registro_transaccion(idContenedor,direccion,paramConfig,idContenedorPadre),idContenedor:idContenedor};
...
function pagina_registro_transaccion(idContenedor,direccion,paramConfig,idContenedorPadre){
	...
	var var_importe_debe_flujo; 
	var var_importe_haber_flujo;
	...
	var Trasaccion = Ext.data.Record.create([		
		...
		'sw_rega',
		'disponibilidad',
		'importe_debe_flujo', 'importe_haber_flujo' //oct2016
		]); 
	...
	function render_moneda_16(value,cell,record,row,colum,store){
		...
		*/
		if(maestro.sw_omite_fe!=''){
			return  '<span style="color:blue;">' +var_importe_debe_flujo.formatMoneda(value)+'</span>'
		}else{
			return  '<span style="color:blue;">' +var_importe_haber_flujo.formatMoneda(value)+'</span>'
		}
	}
   function render_moneda_17(value,cell,record,row,colum,store){
	 	/*if(record.data['disponibilidad'] == 1){
		return  '<span style="color:blue;">' +importe_haber_flujo.formatMoneda(value)+'</span>'}	
		if(record.data['disponibilidad'] == 0){return '<span style="color:red;">' +var_importe_haber.formatMoneda(value)+'</span>'}*/
	   if(maestro.sw_omite_fe!=''){
			return  '<span style="color:blue;">' +var_importe_haber_flujo.formatMoneda(value)+'</span>'
		}else{
			return  var_importe_haber.formatMoneda(value)
		}
 	}
	...
	Atributos[8]={
			validacion:{
				name:'importe_debe_flujo',
				fieldLabel:'ajuste Debe',
				allowBlank:true,
				align:'right', 
				maxLength:50,
				minLength:0,
			...
			//defecto:'45,123,489.15',
			filtro_0:true,
			filterColValue:'importe_debe_flujo',
			save_as:'importe_debe_flujo'
		};
		
		Atributos[9]={
			validacion:{
				name:'importe_haber_flujo',
				fieldLabel:'Ajuste Haber',
				allowBlank:true,
				align:'right', 
				maxLength:50,
				minLength:0,
			...
			filtro_0:true,
			id_grupo:5,
			filterColValue:'importe_haber_flujo',
			save_as:'importe_haber_flujo'
		};
	...
	///////////////////////////////////
	var paramMenu={guardar:{crear:true,separador:false},nuevo:{crear:false,separador:true},editar:{crear:false,separador:false},eliminar:{crear:false,separador:false},actualizar:{crear:false,separador:false},excel:{crear:false,separador:false}};
	...
	var paramFunciones={
		...
		Save:{url:direccion+'../../../control/transaccion/ActionGestionarGuardarRegistroTransacionFE.php'},
		ConfirmSave:{url:direccion+'../../../control/transaccion/ActionGestionarGuardarRegistroTransacionFE.php'},
	...
	this.reload=function(params,sw_editar){
		...
		if(maestro.sw_omite_fe!=''){
			this.getBotonNombre('guardar').disable();
		}else{
			this.getBotonNombre('guardar').enable();
		}
	};
	...
	function InitRegistroTransaccion(){
		...
		var_importe_haber_flujo=ClaseMadre_getComponente('importe_haber_flujo');
		var_importe_debe_flujo=ClaseMadre_getComponente('importe_debe_flujo');
...
/*	var tb = this.getBarra()
		...
		var filtro_replica=new Ext.form.NumberField({
		id:'transac_filtro_'+idContenedor,
		width:20,
		allowDecimals:false,
		maxLength:2
		...
		tb.add('','Replicar: ','',filtro_replica,'',{
			...
			//cls:'remove',
			tooltip:'Replicar',
			handler:btn_actualizar_replica
		}
		);
				
		function btn_actualizar_replica(){
			...
			if(NumSelect!=0){
				...
				if(filtro_replica.value=='undefined' || filtro_replica.value==undefined || parseFloat(filtro_replica.value)<1){
					alert ('Necesita definir la cantidad de réplicas a efectuar. Valor mínimo 1');
				}else{

						Ext.Ajax.request({
							url:direccion+"../../../control/transaccion/ActionGuardarReplicaTransaccion.php",
							method:'POST',
							params:{cantidad_ids:'1',id_transaccion:SelectionsRecord.data.id_transaccion,
							cantidad:filtro_replica.value
								},
							success:esteSuccess,
