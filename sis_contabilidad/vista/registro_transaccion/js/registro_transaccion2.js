...
function render_id_presupuesto(value, p, record){ 	rf = ds_presupuesto.getById(value);
	if(rf!=null){record.data['id_presupuesto'] =rf.data['id_presupuesto'];record.data['desc_presupuesto'] =rf.data['desc_presupuesto'];}
...
Atributos[18]={
	...
	if(params.momento_cbte==0){
		...
		this.getGrid().colModel.setHidden(this.getColumnNum('id_presupuesto'),true);
		this.getGrid().colModel.setHidden(this.getColumnNum('id_partida'),true);
		this.getGrid().colModel.setHidden(this.getColumnNum('id_fina_regi_prog_proy_acti'),false);
		this.getGrid().colModel.setHidden(this.getColumnNum('id_unidad_organizacional'),false);
	...
	if(params.momento_cbte==1||params.momento_cbte==2){
		this.getGrid().colModel.setHidden(this.getColumnNum('id_presupuesto'),false);
		this.getGrid().colModel.setHidden(this.getColumnNum('id_partida'),false);
		this.getGrid().colModel.setHidden(this.getColumnNum('id_fina_regi_prog_proy_acti'),true);
		this.getGrid().colModel.setHidden(this.getColumnNum('id_unidad_organizacional'),true);
	...
	componentes_grid[2].store.baseParams={m_id_parametro:params.id_parametro,sw_reg_comp:'si',m_sw_ingreso:sw_ingreso};
...
//rtestrigción de la fecha 
 //componentes[12]minValue=params.fecha_cbte;
	...
	componentes[2].on('select',f_filtrar_partida);			
	componentes_grid[2].on('beforeselect',f_filtrar_partida_grid);	
...
//	componentes_grid[4].on('beforeselect',f_filtrar_partida_grid);		
	...
	this.btnEdit=function(){
		...
		componentes[3].store.baseParams={sw_reg_comp:'si',m_id_fuente_financiamiento:var_record.data.id_fuente_financiamiento,m_id_parametro_conta:var_id_parametro_conta};
		componentes[3].modificado=true;
		componentes[4].store.baseParams={sw_reg_comp:'si',m_id_fuente_financiamiento:var_record.data.id_fuente_financiamiento,m_id_fina_regi_prog_proy_acti:var_record.data.id_fina_regi_prog_proy_acti,m_id_parametro_conta:var_id_parametro_conta};
	...
	function f_filtrar_epe( combo, record, index ){
				if(sw_filtrar=='si'){
				 componentes[3].store.baseParams={sw_reg_comp:'si',m_id_fuente_financiamiento:record.data.id_fuente_financiamiento,m_id_parametro_conta:var_id_parametro_conta,m_sw_ingreso:sw_ingreso};
				 componentes[3].modificado=true;
	...
	function f_filtrar_epe_grid( combo, record, index ){
				...
				if(sw_filtrar=='si'){
				 componentes_grid[3].store.baseParams={sw_reg_comp:'si',m_id_fuente_financiamiento:record.data.id_fuente_financiamiento,m_id_parametro_conta:var_id_parametro_conta,m_sw_ingreso:sw_ingreso};
	...
	function f_filtrar_partida( combo, record, index ){
			if(sw_filtrar=='si'){
		 				var_tipo_pres= record.data['tipo_press'];
						var_id_presupuesto= record.data['id_presupuesto'];
...
var tpl_id_moneda_reg=new Ext.Template('<div class="search-item">','<b><i>{nombre}</i></b>','<br><FONT COLOR="#B5A642"><b>Abrev: </b>{simbolo}</FONT>','</div>');
	...
	getSelectionModel().on('rowselect',function( SM,rowIndex){	sw_filtro="insert";
		...
		var_tipo_pres= var_record.data.tipo_pres;		
		componentes_grid[10].setDisabled(false);	
		componentes_grid[3].store.baseParams={sw_reg_comp:'si',m_id_fuente_financiamiento:var_record.data.id_fuente_financiamiento,m_id_parametro_conta:var_id_parametro_conta};
		componentes_grid[3].modificado=true;
		componentes_grid[4].store.baseParams={sw_reg_comp:'si',m_id_fuente_financiamiento:var_record.data.id_fuente_financiamiento,m_id_fina_regi_prog_proy_acti:var_record.data.id_fina_regi_prog_proy_acti,m_id_parametro_conta:var_id_parametro_conta};
		componentes_grid[4].modificado=true;
