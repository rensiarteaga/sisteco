...
function paginaEEFFPeriodo(idContenedor,direccion,paramConfig,maestro,idContenedorPadre)
{
	...
	var ds = new Ext.data.Store({
		...
		proxy: new Ext.data.HttpProxy({url: direccion+'../../../control/eeff/ActionListarEEFFPeriodo.php'}),
	...
	{		 
		...
		var data_maestro=[ ['EEFF ',g_id_eeff_desc ,'Detalle Nivel',g_nivel],
		                   ...
		                   ['Gestión',g_id_parametro_desc, 'Periodo(s)',g_ids_periodo],
		                   ['Dpto(s)', g_depto]
						    ];
...
// txt mes_01
	...
	this.btnActualizar=function(){
		...
		if (g_ids_periodo==""){g_mensaje += '- Periodo(s)'};
	...
	function btn_imprimir(){
		...
		if (g_mensaje=="")
		{
			window.open(direccion+'../../../control/eeff/ActionEEFFPeriodo.php?'+data);
		}else{
			Ext.MessageBox.alert("Falta indicar:", g_mensaje);
		}
	}		
