<?php 
/**
 * Nombre:		  	    cuenta_arb_main.php
 * Propósito: 			pagina que arranca la configuracion de la vista
 * Autor:				Generado Automaticamente
 * Fecha creación:		2008-07-07 11:38:59
 * Fecha Modificacion   24/08/2016 AVQ.
 *
 */
session_start();
?>
...
function pagina_cuenta_arb(idContenedor,direccion,paramConfig){
	...
	var cmpIdPartidaDebe,cmpIdPartidaHaber;
	var g_id_gestion='';
	var Dialog;
	var copiados; 
	...
	var ds_partida=new Ext.data.Store({
		proxy:new Ext.data.HttpProxy({url:direccion+'../../../../sis_presupuesto/control/partida/ActionListarPartida.php'}),
		reader:new Ext.data.XmlReader({record:'ROWS',id:'id_partida',totalRecords:'TotalCount'},['id_partida','codigo_partida','nombre_partida','sw_transaccional','tipo_partida','desc_parametro','desc_partida','desc_par'])
	});

	var ds_partida_haber=new Ext.data.Store({
		proxy:new Ext.data.HttpProxy({url:direccion+'../../../../sis_presupuesto/control/partida/ActionListarPartida.php'}),
		reader:new Ext.data.XmlReader({record:'ROWS',id:'id_partida',totalRecords:'TotalCount'},['id_partida','codigo_partida','nombre_partida','sw_transaccional','tipo_partida','desc_parametro','desc_partida','desc_par'])
	});
	...
	//FUNCIONES RENDER
	function renderTipoPartida(value, p, record)
	{						
		if(value == 1)
		{return "Recursos"}
		if(value == 2)
	...
	var resultTplPartida=new Ext.Template('<div class="search-item">','<b><i>{codigo_partida}</i></b>','<br><FONT COLOR="#B5A642"><b>Partida: </b>{nombre_partida}</FONT>','<br><FONT COLOR="#B5A642"><b>Tipo: </b>{tipo_partida}</FONT>','<br><FONT COLOR="#B5A642"><b>Gestion: </b>{desc_parametro}</FONT>','</div>');
		
	var resultTplPartidaHaber=new Ext.Template('<div class="search-item">','<b><i>{codigo_partida}</i></b>','<br><FONT COLOR="#B5A642"><b>Partida: </b>{nombre_partida}</FONT>','<br><FONT COLOR="#B5A642"><b>Tipo: </b>{tipo_partida}</FONT>','<br><FONT COLOR="#B5A642"><b>Gestion: </b>{desc_parametro}</FONT>','</div>');
	...
	Atributos[20]={
			validacion:{
				...
				fieldLabel:'Categoria FE',
				typeAhead:false,
				loadMask:true,
				allowBlank:true,
				triggerAction:'all',
				store:new Ext.data.SimpleStore({fields:['id','valor'],data:[['ope','OPE - Operacion'],['dis','DIS - Distribucion'],['fin','FIN - Financiamiento'],['efe','EFE - Efectivo']]}),
				valueField:'id',
				displayField:'valor',
				lazyRender:true,
				forceSelection:true,
	...
	Atributos[22]={
			validacion:{
				name:'id_partida_flu_debe',
				fieldLabel:'Al debe',
				emptyText:'Partida...',
				store:ds_partida,
				desc:'desc_partida_debe',
				...
				loadMask:true,
				forceSelection:true,
				tpl:resultTplPartida,
				mode:'remote',
				renderer:renderTipoPartida,
			...
			tipo:'ComboBox',
			id_grupo:4,
			save_as:'id_partida_flu_debe'
		};
	Atributos[23]={
			validacion:{
				name:'id_partida_flu_haber',
				fieldLabel:'Al Haber',
				emptyText:'Partida...',
				store:ds_partida_haber,
				desc:'desc_partida_haber',
				valueField:'id_partida',
				displayField:'desc_par',
				queryParam:'filterValue_0',
				filterCol:'PARTID.codigo_partida',
	...
	Atributos[24]={
			validacion:{
				...
				loadMask:true,
				triggerAction:'all',
				store:new Ext.data.SimpleStore({fields:['id','valor'],data:[['credito_fiscal','CREDITO FISCAL'],['caif_sum','CAIF SUM'],['caif_sum_imp','CAIF SUM IMP']]}),
				valueField:'id',
				displayField:'valor',
				lazyRender:true,
				forceSelection:true,
	...
	var paramFunciones={
		...
		Formulario:{html_apply:'dlgInfo-'+idContenedor,
			height:'75%',
			width:'70%',columnas:['50%','50%'],
				minWidth:150,minHeight:200,	closable:true,titulo:'Plan de cuentas',
			    grupos:[{
					tituloGrupo:'Datos',
					columna:0,
					id_grupo:0},
					{
					tituloGrupo:'Actualizacion',
					columna:0,
					id_grupo:1},
					{
					tituloGrupo:'Sigma',
	...
	function iniciarEventos(){
		...
		cmpIdPartidaDebe=getComponente('id_partida_flu_debe');
		cmpIdPartidaHaber=getComponente('id_partida_flu_haber');
		cmpSwCaif=getComponente('sw_caif');
		cmpSwSiet=getComponente('sw_siet');
		CM_ocultarGrupo('Actualizacion');
		CM_ocultarGrupo('Sigma');
		CM_ocultarGrupo('EEFF');
		CM_ocultarGrupo('Partida CAIF');
		...
		var onSwTransaccionalSelect=function(e){
			...
			if(id==1){
				...
				CM_mostrarGrupo('Actualizacion');
				CM_mostrarGrupo('Sigma');
				CM_mostrarGrupo('EEFF');
				CM_mostrarGrupo('Partida CAIF');
				...
				mostrarComponente(cmpIdPartidaDebe);
				cmpIdPartidaDebe.allowBlank=true;
				cmpIdPartidaDebe.setValue('');
				mostrarComponente(cmpIdPartidaHaber);
				cmpIdPartidaHaber.allowBlank=true;
				cmpIdPartidaHaber.setValue('');
			}
			else{
				CM_mostrarGrupo('Datos');
				CM_ocultarGrupo('Actualizacion');
				...
				cmpIdCuentaSigma.allowBlank=true;
				cmpIdCuentaSigma.setValue('');
				ocultarComponente(cmpIdPartidaDebe);
				cmpIdPartidaDebe.allowBlank=true;
				cmpIdPartidaDebe.setValue('');
				ocultarComponente(cmpIdPartidaHaber);
				cmpIdPartidaHaber.allowBlank=true;
				cmpIdPartidaHaber.setValue('');
			}
		};
