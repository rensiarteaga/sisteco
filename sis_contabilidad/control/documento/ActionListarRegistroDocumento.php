...
//Parámetros del filtro
	...
	$res = $Custom->ListarRegistroDocumento($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$hidden_ep_id_financiador,$hidden_ep_id_regional,$hidden_ep_id_programa,$hidden_ep_id_proyecto,$hidden_ep_id_actividad);
		$importe_total=0;
			...
			//2016
			$importe_descuento=0;
			$importe_exportaciones=0;
			$importe_ventas_gravadas_tasa_0=0;
			$importe_ndc_importe_total_orig=0;
	if($res)
	{
		...
		foreach ($Custom->salida as $f)
		{
			...
			$xml->add_nodo('ndc_fecha_documento_orig',$f["ndc_fecha_documento_orig"]);
			$xml->add_nodo('ndc_nro_autorizacion_factura_orig',$f["ndc_nro_autorizacion_factura_orig"]);
			$xml->add_nodo('ndc_nro_documento_orig',$f["ndc_nro_documento_orig"]);
			$xml->add_nodo('ndc_importe_total_orig',$f["ndc_importe_total_orig"]);
			...
			//2016
			$importe_descuento=$importe_descuento+$f["importe_descuento"];
			$importe_exportaciones=$importe_exportaciones+$f["importe_exportaciones"];
			$importe_ventas_gravadas_tasa_0=$importe_ventas_gravadas_tasa_0+$f["importe_ventas_gravadas_tasa_0"];
			$importe_ndc_importe_total_orig=$importe_ndc_importe_total_orig+$f["ndc_importe_total_orig"];
			$xml->fin_rama();
		}
		if($m_sw_reg_documento=='si' && $total_registros)
		{
			...
			//2016
			$xml->add_nodo('importe_descuento',$importe_descuento);
			$xml->add_nodo('importe_exportaciones',$importe_exportaciones);
			$xml->add_nodo('importe_ventas_gravadas_tasa_0',$importe_ventas_gravadas_tasa_0);
			$xml->add_nodo('tipo_compra',"");
			//para notas de debito credito
			$xml->add_nodo('ndc_fecha_documento_orig',"");
			$xml->add_nodo('ndc_nro_autorizacion_factura_orig',"");
			$xml->add_nodo('ndc_nro_documento_orig',"");
			$xml->add_nodo('ndc_importe_total_orig',$importe_ndc_importe_total_orig);
			$xml->fin_rama();
		}
		$xml->mostrar_xml();
	}
