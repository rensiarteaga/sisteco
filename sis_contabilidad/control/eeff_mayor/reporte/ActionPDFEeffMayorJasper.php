<?php
	...
	$nombre_archivo = 'ActionPDFEeffMayorJasper.php';
	...
	if($_SESSION['autentificado']=='SI'){
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST'){
  		...
  		if($sw_vista=='cuenta_tc' || $sw_vista=='mayor_tc'){
  			$sw_actualizacion = $tipo_cambio;
  		}
...
/*if($_SESSION["ss_id_usuario"]==120){
	echo '$sw_vista'.$sw_vista.' ,id_ges = '.$id_gestion .' ,$id_depto='.$id_depto .' ,$sw_estado_cbte='.$sw_estado_cbte
	.',$sw_actualizacion'.$sw_actualizacion.' ,$sw_orden='.$sw_orden.' ,$sw_cuenta='.$sw_cuenta.' ,$sw_auxiliar='.$sw_auxiliar.' ,$sw_partida='.$sw_partida.' , $sw_epe='.$sw_epe.
	' ,$id_cuenta_inicial='.$id_cuenta_inicial.' ,$id_cuenta_final='.$id_cuenta_final.', $ids_cuenta='.$ids_cuenta;
	exit;
}*/
		if($tipo_reporte=='xls'){
			if($sw_vista=='mayor' || $sw_vista=='mayor_tc'){
				$reporte=new ReportDriver('repeeff_Mayor_xls.jasper','sci',$tipo_reporte);
			}else{
			...
			$tipo_rep = 'pdf';
			if($sw_vista=='mayor' || $sw_vista=='mayor_tc'){
				if($tipo_reporte=='pdf1' or $tipo_reporte=='pdf5'){
					$reporte=new ReportDriver('repeeff_Mayor.jasper','sci',$tipo_rep);
				}
				if($tipo_reporte=='pdf2' or $tipo_reporte=='pdf6'){
					$reporte=new ReportDriver('repeeff_Mayor_res.jasper','sci',$tipo_rep);
				}
				if($tipo_reporte=='pdf3'){
					$reporte=new ReportDriver('repeeff_Mayor_ppto.jasper','sci',$tipo_rep);
				}
				if($tipo_reporte=='pdf4'){
					$reporte=new ReportDriver('repeeff_Mayor_rpto.jasper','sci',$tipo_rep);
				}
				if($tipo_reporte=='pdf7'){
					$reporte=new ReportDriver('repeeff_Mayor_res_sf.jasper','sci',$tipo_rep);
				}
			}else{	
		...
		if($sw_vista=='mayor' || $sw_vista=='mayor_tc'){
			...
			if($tipo_reporte=='pdf5' or $tipo_reporte=='pdf6'){
				$sw_ctapar = 'B';
				$reporte->addParametro('sw_ctapar',$sw_ctapar);
			}
			if($tipo_reporte=='pdf7'){
				$reporte->addParametro('pagina_inicial',$tipo_cambio,'Integer');
			}
		}else{
