...
class cls_DBDocumento
{
	...
	function ListarRegistroDocumento($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad)
	{
		...
		//2016
		$this->var->add_def_cols('importe_descuento','numeric');
		$this->var->add_def_cols('importe_exportaciones','numeric');
		$this->var->add_def_cols('importe_ventas_gravadas_tasa_0','numeric');
		$this->var->add_def_cols('tipo_compra','integer');
		//para notas de debito-credito
		$this->var->add_def_cols('ndc_fecha_documento_orig','date');
		$this->var->add_def_cols('ndc_nro_autorizacion_factura_orig','varchar');
		$this->var->add_def_cols('ndc_nro_documento_orig','bigint');
		$this->var->add_def_cols('ndc_importe_total_orig','numeric');
	...
	function ListarDocumentoIva($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad,$id_periodo_subsis,$id_moneda,$sw_debito_credito,$id_depto,$tipo_documento)
	{
		...
		if($id_periodo_subsis<1845){
			$this->nombre_funcion = 'sci.__28feb2016_f_tct_documento_iva_sel';
		}else{
		...
		if($id_periodo_subsis<1845){
			...
			//2016
			$this->var->add_def_cols('importe_descuento','numeric');
			//$this->var->add_def_cols('importe_total_desc','numeric');
			$this->var->add_def_cols('tipo_compra','integer');
			$this->var->add_def_cols('id_documento','bigint');
			...
			if ($sw_debito_credito == 7 || $sw_debito_credito == 8 ){
				$this->var->add_def_cols('ndc_nro_documento_orig','bigint');
				$this->var->add_def_cols('ndc_nro_autorizacion_factura_orig','varchar');
				$this->var->add_def_cols('ndc_fecha_documento_orig','date');
				$this->var->add_def_cols('ndc_importe_total_orig','numeric');
			...
			)
	{
		...
		//2016
		$this->var->add_param($importe_descuento);
		$this->var->add_param($importe_exportaciones);
		$this->var->add_param($importe_ventas_gravadas_tasa_0);
		$this->var->add_param("'$ndc_fecha_documento_orig'");
		$this->var->add_param("'$ndc_nro_autorizacion_factura_orig'");
		$this->var->add_param($ndc_nro_documento_orig);
		$this->var->add_param($ndc_importe_total_orig);
		//Ejecuta la función
