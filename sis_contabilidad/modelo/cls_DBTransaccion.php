...
{  
	...
	{    
		...
		$this->var->add_def_cols('importe_debe_flujo','numeric');//oct2016
		$this->var->add_def_cols('importe_haber_flujo','numeric');//oct2016
		//Ejecuta la función de consulta
		$res = $this->var->exec_query();
...
//exit;
	...
	function ReplicarRegistroTransacion($id_transaccion ,$cantidad)
	{
		$this->salida = "";
		$this->nombre_funcion = 'f_tct_transaccion_iud';
		$this->codigo_procedimiento = "'CT_REPLITRA_UPD'";
		...
		$this->var->add_param("NULL");//($id_oec);
		$this->var->add_param("NULL");//("'$concepto_tran'");
		$this->var->add_param("NULL");//("$id_fina_regi_prog_proy_acti");
		$this->var->add_param("NULL");//($importe_debe);
		$this->var->add_param("NULL");//($importe_haber);
		$this->var->add_param("NULL");//($id_moneda);
		$this->var->add_param("NULL");//("'$fecha_trans'");
		$this->var->add_param("NULL");//($tipo_Cambio);
		$this->var->add_param("NULL");//("'$tipo_cambio_origen'");
		$this->var->add_param("NULL");//("$importe_ejecucion");
	...
	function ModificarGestionarRegistroTransacionFE($id_transaccion,$concepto_tran,$id_auxiliar ,$id_comprobante,$id_oec,$id_orden_trabajo,$id_partida,$id_cuenta,$id_presupuesto,$importe_debe_flujo,$importe_haber_flujo,$importe_gasto,$importe_recurso)
	{
		$this->salida = "";
		$this->nombre_funcion = 'f_tct_gestionar_transaccion_iud';
		$this->codigo_procedimiento = "'CT_REGTRAFE_UPD'";
		...
		//$this->var->add_param("$id_partida_cuenta");
		$this->var->add_param("$id_presupuesto");
		$this->var->add_param("$importe_debe_flujo");
		$this->var->add_param("$importe_haber_flujo");
		$this->var->add_param("$importe_gasto");
		$this->var->add_param("$importe_recurso");
