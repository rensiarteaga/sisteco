...
{   
	...
	function Footer(){
		...
		$this->Cell(200,3,$this->preview_text($_SESSION['PDF_glosa'],165),'T',0);
		$this->SetFont('Arial','',4);
		$this->Cell(8,3,$_SESSION['id_comprobante'],'T',1);
		$this->SetFont('Arial','BI',6);
		$vdebe=$_SESSION['PDF_transaccion_debe'];
	    ...
	    if(("$suma_total_debe")==("$suma_total_haber")){
	       $this->MultiCell(214,3,'               Son: '.$_SESSION['PDF_sum_total_literal'].' '.$vdebe[0][8],'LRB');
	    }else{
	    	 $this->MultiCell(214,3,'          Los montos de débitos y créditos no son iguales','LRB');
	    }
		...
		if ($_SESSION['desc_clases']=='COMPROBANTE PAGO'){
			...
			if ($_SESSION[ss_id_usuario]!=120){
				for($i=0;$i<=3;$i++){
					...
					$firmas[$i][4]=$firmas_obtenidas[1][$i];
				}
			}else{
 				$firmas[0][0]='';
		  		$firmas[0][1]='Centro Responsable';
				$firmas[0][2]='Firma Autorizada';
				$firmas[0][3]='Firma Autorizada';
				$firmas[0][4]='FIRMA INTERESADO';
				$firmas[1][0]='';
				$firmas[1][1]='UDAETA ORELLANA EDDSON MIRKO';
				$firmas[1][2]='CARDENAS CORDOVA JOHNNY FERNANDO';
				$firmas[1][3]='PARDO IRIARTE JAVIER ROLANDO';
				//$firmas[1][3]='PARDO IRIARTE JAVIER ROLANDO';
				//$firmas[1][2]='SANCHEZ GUZMAN FELIPE';
				//$firmas[1][3]='CACERES MOREIRA HANS JURGEN';
				//$firmas[1][3]='PARDO IRIARTE JAVIER ROLANDO';
				$firmas[1][4]='';
				$firmas[2][0]='';
				$firmas[2][1]='JEFE CONTABILIDAD CENTRAL';
				$firmas[2][2]='ANALISTA CREDITOS';
				$firmas[2][3]='GERENTE NACIONAL DE DESARROLLO EMPRESARIAL Y ECONOMIA';
				
				//$firmas[2][2]='JEFE PRESUPUESTOS';
				//$firmas[2][3]='GERENTE NAL. DE DESARROLLO EMPRESARIAL Y ECONOMIA';
				$firmas[2][4]='';
				//CARDENAS CORDOVA JOHNNY FERNANDO //RESPONSABLE CONTABILIDAD PROYECTOS; PARDO IRIARTE JAVIER ROLANDO; GERENTE NAL. DE DESARROLLO EMPRESARIAL Y ECONOMIA
			}
			...
			$this->SetFontsSizes(array(5,5,5,5));
			$this->SetSpaces(array(3,3,3,3));
			$this->SetWidths(array(59,63,46,46));
			...
			$this->MultiTabla($firmas[0],0,3,4,5);
			$this->Cell(59,10,'',1,0,'C');
			$this->Cell(63,10,'',1,0,'C');
			$this->Cell(46,10,'',1,0,'C');
			$this->Cell(46,10,'',1,1,'C');
			$this->MultiTabla($firmas[1],0,3,4,5);
			$this->MultiTabla($firmas[2],0,3,4,5);
		}
