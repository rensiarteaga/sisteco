...
function pagina_registro_transaccion(idContenedor,direccion,paramConfig,idContenedorPadre){
  ...
  var Trasaccion = Ext.data.Record.create([		
	...
	function render_id_oec(value, p, record){rf = ds_oec.getById(value);
...
//rtestrigción de la fecha 
 //componentes[12]minValue=params.fecha_cbte;
	...
	this.btnEdit=function(){
		...
		alert("La moneda de registro tiene que ser igual a la moneda de listado");
		
 		}
		else if(sw_filtrar=='si'){activar(9,9);activar(11,11);
		...
		componentes[7].store.baseParams={sw_reg_comp:'si',m_id_partida:var_record.data.id_partida,sw_transaccional:1};
