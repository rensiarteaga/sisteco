...
function pagina_cuenta_arb(idContenedor,direccion,paramConfig){
	...
	var ds_partida=new Ext.data.Store({
		...
		reader:new Ext.data.XmlReader({record:'ROWS',id:'id_partida',totalRecords:'TotalCount'},['id_partida','codigo_partida','nombre_partida','sw_transaccional','tipo_partida','desc_parametro','desc_partida'])
	});
	...
	var resultTplPartida=new Ext.Template('<div class="search-item">','<b><i>{codigo_partida}</i></b>','<br><FONT COLOR="#B5A642"><b>Partida: </b>{desc_partida}</FONT>','<br><FONT COLOR="#B5A642"><b>Gestion: </b>{desc_parametro}</FONT>','</div>');
		
	var resultTplPartidaHaber=new Ext.Template('<div class="search-item">','<b><i>{codigo_partida}</i></b>','<br><FONT COLOR="#B5A642"><b>Partida: </b>{desc_partida}</FONT>','<br><FONT COLOR="#B5A642"><b>Gestion: </b>{desc_parametro}</FONT>','</div>');
	...
	Atributos[20]={
			validacion:{
				...
				fieldLabel:'Flujo de Efectivo',
				typeAhead:false,
				loadMask:true,
				allowBlank:true,
				triggerAction:'all',
				store:new Ext.data.SimpleStore({fields:['id','valor'],data:[['dis','Disponible'],['inv','Inversion'],['fin','Financiamiento'],['ope','Operativa']]}),
				valueField:'id',
				displayField:'valor',
				lazyRender:true,
				forceSelection:true,
