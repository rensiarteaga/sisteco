...
function Cabecera($matrizCabecera)
{
    ...
    for ($i=0; $i< count($matrizCabecera); $i++) {
	  ...
	  $this->Ln(3);
		...
		$this->Cell(105,3,"CONCEPTO",1,0,'L',0);//concepto_cbte
		//$this->Cell(60,3,"DESCOMPOSICIÓN",1,0,'L',0);//desc_componentes
		$this->Cell(20,3,"DEBE",1,0,'L',0);//importe_debe
		$this->Cell(20,3,"HABER",1,0,'L',0);//importe_haber
		$this->Cell(20,3,"SALDO",1,0,'L',0);//saldo
	  ...
	  //echo $Custom->query; exit();
	   ...
	   if($res){
					...
					{	 
						...
						if ($row[4]=="0" && $row[2]=='-1' ) {
							...
							if ($data[ $this->contador+1][2]!="-1") {
												if($this->mayor_importe_debe>0 ||$this->mayor_importe_haber>0){
													/*insertar totales*/
												$this->SetFont('Arial','',6);
												$this->SetXY(8, $this->posicionY+1);  
												$this->MultiCell(140,3,"TOTAL",'T',0,'L');	
												//$this->MultiCell(15,3, $this->posicionY,1,0,'J');	
												
												$this->SetXY(148, $this->posicionY+1); 
												$this->MultiCell(20,3,number_format($this->mayor_importe_debe,2),'T','R');	 
												$this->SetXY(168,$this->posicionY+1); 
												
												$this->MultiCell(20,3,number_format($this->mayor_importe_haber,2),'T','R');	 
												/*
						...
						{  
						   $this->Cell(15,3,$row[3],'0',0,'L',$fill);	
						   $this->Cell(20,3,$row[4],'0',0,'C',$fill);	 
						   if (strlen($row[5])>75){$this->Cell(105,3,substr($row[5],0,75)." ...",'0',0,'J',$fill); }
						      else {$this->Cell(105,3,$row[5],'0',0,'J',$fill); }
						   $this->Cell(20,3,number_format($row[7],2),'0',0,'R',$fill);	 
						   $this->Cell(20,3,number_format($row[8],2),'0',0,'R',$fill);	 
						   $this->Cell(20,3,number_format($row[9],2),'0',0,'R',$fill);	  
						  ...
						  $fill=!$fill;
						   ...
						   $comprobante=ceil(40/40);
						   $componente=ceil(40/40);
						   ...
						   $posicionY_sig=$this->posicionY+ (ceil(40/40))*$this->diferencialY;
						   $this->Ln();
