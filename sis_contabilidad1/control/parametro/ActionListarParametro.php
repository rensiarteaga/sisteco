...
Propósito:				Permite realizar el listado en tct_parametro
...
//Parámetros del filtro
	...
	$cond->add_criterio_extra('param.estado_gestion',$m_estado);
	
	if($m_sw_reg_documento=="si"){
		$cond->add_criterio_extra("PARAM.id_parametro",$m_id_parametro);
	}

	if($m_vigente){
		$cond->add_criterio_extra("GESTIO.estado_vigente","''si''");
	}
	...
	if($res)
	{
		...
		foreach ($Custom->salida as $f)
		{
			...
			$xml->add_nodo('porcen_iva',$f["porcen_iva"]);
			$xml->add_nodo('porcen_it',$f["porcen_it"]);
			$xml->add_nodo('porcen_servicio',$f["porcen_servicio"]);
			$xml->add_nodo('porcen_bien',$f["porcen_bien"]);
			$xml->add_nodo('porcen_remesa',$f["porcen_remesa"]);
			...
			$xml->add_nodo('desc_unidad_organizacional',$f["nombre_unidad"]);
			$xml->add_nodo('desc_estado',$f["desc_estado"]);
            $xml->fin_rama();
		}
		$xml->mostrar_xml();
	}
