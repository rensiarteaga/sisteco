 ...
 if ($reporte_excel=='si')
	{	$Excel = new GestionarExcel();
		...
		$emp_afps_detalle = $Custom->RepEmpleadoAFPsDetalle($cant,$m_id_planilla,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);
		 ...
		 for($h=0;$h<=$num_afiliados;$h++){
		    ...
		    if($v_emp_afps[$h]['sw_jub55']=='si'){
		 	...
		 	if($v_emp_afps[$h]['may65']=='si'){
				$sum_devengado_may65=$sum_devengado_may65+$v_emp_afps[$h]['valor'];
			}
				
			if($v_emp_afps[$h]['jub65']=='si'){
				$sum_jubilado_may65=$sum_jubilado_may65+$v_emp_afps[$h]['valor'];
			}
		}
	    ...
	    $v_sum_afp=$Custom->salida;
					  ...
					  $total_salario_devengadob=$sum_devengado-$sum_devengado_may65+$sum_jubilado_may65; 
	 	 			  $sum_tot_ap_lab=($total_salario_devengado * 0.1)+($total_salario_devengadob * 0.0171)+($v_importe * 0.005)+($v_importe * 0.005);
	 	              $sum_tot_ap_pat=($total_salario_devengadob * 0.0171)+($v_importe * 0.02)+($v_importe * 0.03);
                     ...
                     $datos_resumen[$i][3]=array(doc_id=>'(-) Salario Por Tramite de Jubilación (A)',valor=>$sum_salario_tram_jub);
                     $datos_resumen[$i][4]=array(doc_id=>'TOTAL SALARIO DEVENGADO (A):',valor=>$total_salario_devengado);
                    
                     $datos_resumen[$i][5]=array(doc_id=>'(-) Salario Trabajadores Activos > 65 años (B)',valor=>$sum_devengado_may65);
                     $datos_resumen[$i][6]=array(doc_id=>'TOTAL SALARIO DEVENGADO (B):',valor=>$total_salario_devengadob);
                   
                   // $datos_resumen[$i][5]=array(doc_id=>'Incapacidad Temporal:',valor=>$v_afps[0][8]);
                     $datos_resumen[$i][7]=array(doc_id=>'TOTAL SALARIO COTIZABLE:',valor=>$v_importe);
                     $datos_resumen[$i][8]=array(doc_id=>'APORTE LABORAL:',valor=>'');
                    $datos_resumen[$i][9]=array(doc_id=>'Total Aporte Cuenta Individual',dias_cot=>'10.00%',valor=>$total_salario_devengado * 0.1);
                    $datos_resumen[$i][10]=array(doc_id=>'Total Seguro de Riesgo Comun',dias_cot=>'1.71%',valor=>$v_importe * 0.0171);
                    $datos_resumen[$i][12]=array(doc_id=>'Total Comisión Administradora'.$v_ob_afps[$i]['nombre_afp'],dias_cot=>'0.50%',valor=>$v_importe * 0.005);
                    $datos_resumen[$i][13]=array(doc_id=>'Total Aporte Solidario:',dias_cot=>'0.50%',valor=>$v_importe * 0.005);
                    
                    $datos_resumen[$i][14]=array(doc_id=>'TOTAL APORTE LABORAL:',valor=>$sum_tot_ap_lab);
 	                $datos_resumen[$i][15]=array(doc_id=>'APORTE PATRONAL:',valor=>'');
                    $datos_resumen[$i][16]=array(doc_id=>'Total Seguro Riesgo Profesional:',dias_cot=>'1.71%',valor=>$v_importe * 0.0171);
                    
                    $datos_resumen[$i][17]=array(doc_id=>'Total Aporte Patronal y Vivienda:',dias_cot=>'2.00%',valor=>$v_importe * 0.02);
                    $datos_resumen[$i][18]=array(doc_id=>'Total Aporte Solidario:',dias_cot=>'3.00%',valor=>$v_importe * 0.03);
                    
 	                $datos_resumen[$i][19]=array(doc_id=>'TOTAL APORTE PATRONAL:',valor=>$sum_tot_ap_pat);
 	                
                    $datos_resumen[$i][20]=array(doc_id=>'TOTAL APORTE LABORAL Y PATRONAL SIP',valor=>$sum_total);*/
	 	              ...
	 	              $sum_salario_tram_jub=$sum_salario_tram_jub+$sum_jubilado_may65;
	 	              $datos_resumen[$i][3]=array(doc_id=>'(-) Salario Por Tramite de Jubilación (A)',valor=>$sum_salario_tram_jub);
	 	              $datos_resumen[$i][4]=array(doc_id=>'TOTAL SALARIO DEVENGADO (A):',valor=>$total_salario_devengado);
	 	              
	 	              $datos_resumen[$i][5]=array(doc_id=>'(-) Salario Trabajadores Activos > 65 años (B)',valor=>$sum_devengado_may65-$sum_jubilado_may65);
	 	              $datos_resumen[$i][6]=array(doc_id=>'TOTAL SALARIO DEVENGADO (B):',valor=>$total_salario_devengadob);
	 	              ...
	 	              $datos_resumen[$i][8]=array(doc_id=>'APORTE LABORAL:',valor=>'');
	 	              $datos_resumen[$i][9]=array(doc_id=>'Total Aporte Cuenta Individual',dias_cot=>'10.00%',valor=>$total_salario_devengado * 0.1);
	 	              $datos_resumen[$i][10]=array(doc_id=>'Total Seguro de Riesgo Comun',dias_cot=>'1.71%',valor=>$sum_devengado-$sum_devengado_may65);
	 	              $datos_resumen[$i][12]=array(doc_id=>'Total Comisión Administradora'.$v_ob_afps[$i]['nombre_afp'],dias_cot=>'0.50%',valor=>$v_importe * 0.005);
	 	              $datos_resumen[$i][13]=array(doc_id=>'Total Aporte Solidario:',dias_cot=>'0.50%',valor=>$v_importe * 0.005);
	 	              
	 	              $datos_resumen[$i][14]=array(doc_id=>'TOTAL APORTE LABORAL:',valor=>$sum_tot_ap_lab);
	 	              $datos_resumen[$i][15]=array(doc_id=>'APORTE PATRONAL:',valor=>'');
	 	              $datos_resumen[$i][16]=array(doc_id=>'Total Seguro Riesgo Profesional:',dias_cot=>'1.71%',valor=>$sum_devengado-$sum_devengado_may65);
