		...
		if($node=='id'){
			...
			if($res)
			{
				foreach ($Custom->salida as $f)
				{
					...
					$tmp['prioridad'] = utf8_encode($f["prioridad"]);
					$tmp['area'] = utf8_encode($f["area"]);
					$tmp['correspondencia'] = utf8_encode($f["correspondencia"]);
					//4may12: Mercedes Zambrana
					$tmp['url_archivo'] = utf8_encode($f["url_archivo"]);
					$tmp['extension'] = utf8_encode($f["extension"]);
					$nodes[] = $tmp;
				}
header("Content-Type:text/json; charset=".$_SESSION["CODIFICACION_HEADER"]);
				if(sizeof($nodes)>0){
