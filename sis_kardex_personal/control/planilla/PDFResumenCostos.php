...
{    //Cargar los datos
	...
	{	//Posición: a 1,5 cm del final
  	...
  	/////////////////////////////////////////////////////////////////////////////
  	function LoadData($id_planilla){     
	$Custom=new cls_CustomDBKardexPersonal();
	$Custom->ResumenCostosPersonal(0,0,'desc_presupuesto','asc','0=0',NULL,NULL,NULL,NULL,NULL,$id_planilla);
	$var1=$Custom->salida;
	return $var1;
	}
		...
		$w=array(73,15,32);
	        ...
	        foreach($data as $row){
	        	$relleno=0;
					...
					$this->Cell($w[1],5,number_format($row[3],2),1,0,'R',$relleno);//desc_ret_incor
					$this->Cell($w[1],5,number_format($row[4],2),1,0,'R',$relleno);//desc_ret_incor
					$this->Cell($w[1],5,number_format($row[5],2),1,0,'R',$relleno);//desc_ret_incor
					$this->Cell($w[1],5,number_format($row[6],2),1,0,'R',$relleno);//desc_ret_incor
					$this->Cell($w[1],5,number_format($row[7],2),1,0,'R',$relleno);//desc_ret_incor
					$this->Cell($w[1],5,number_format($row[8],2),1,0,'R',$relleno);//desc_ret_incor
					$this->Cell($w[1],5,number_format($row[9],2),1,0,'R',$relleno);//desc_ret_incor
					$this->Cell($w[1],5,number_format($row[10],2),1,0,'R',$relleno);//desc_ret_incor
					$this->Cell($w[1],5,number_format($row[11],2),1,0,'R',$relleno);//desc_ret_incor
					$this->Cell($w[1],5,number_format($row[12],2),1,0,'R',$relleno);//desc_ret_incor
					$this->Cell($w[1],5,number_format($row[13],2),1,0,'R',$relleno);//desc_ret_incor
					$this->Ln(5);
					$i=$i+1;
					...
					$this->Cell($w[1],5,number_format($suma_normal,2),1,0,'R',$relleno);//desc_ret_incor
					$this->Cell($w[1],5,number_format($suma_extra,2),1,0,'R',$relleno);//desc_ret_incor
					$this->Cell($w[1],5,number_format($suma_nocturno,2),1,0,'R',$relleno);//desc_ret_incor
					$this->Cell($w[1],5,number_format($suma_subtotal,2),1,0,'R',$relleno);//desc_ret_incor
					$this->Cell($w[1],5,number_format($suma_aguin,2),1,0,'R',$relleno);//desc_ret_incor
					$this->Cell($w[1],5,number_format($suma_subsid,2),1,0,'R',$relleno);//desc_ret_incor
					$this->Cell($w[1],5,number_format($suma_inctmp,2),1,0,'R',$relleno);//desc_ret_incor
					$this->Cell($w[1],5,number_format($suma_reintnc,2),1,0,'R',$relleno);//desc_ret_incor
					$this->Cell($w[1],5,number_format($suma_carga_soc,2),1,0,'R',$relleno);//desc_ret_incor
					$this->Cell($w[1],5,number_format($suma_indemniz,2),1,0,'R',$relleno);//desc_ret_incor
					$this->Cell($w[1],5,number_format($suma_total_gral,2),1,0,'R',$relleno);//desc_ret_incor
