...
function pagina_historico_asignacion_det(idContenedor,direccion,paramConfig,maestro,idContenedorPadre){ 
	...
	ds_cargo=new Ext.data.Store({
		...
		reader:new Ext.data.XmlReader({
			...
			id:'id_cargo',
			totalRecords:'TotalCount'
		},['id_cargo','codigo_cargo','nombre_cargo','numero_item','tipo_item','desc_escala_salarial','desc_tipo_contrato'])
	});	
	
	var tplCargo=new Ext.Template(
			'<div class="search-item">',
			'N° Item:{numero_item}<br>',
			'<b><i>{codigo_cargo}-{nombre_cargo}</i></b><br>',
			'Escala:<b><i>{desc_escala_salarial}</i></b><br>',
			'Tipo Contrato:<i>{desc_tipo_contrato}</i>',
			'</div>'
			);	
	...
	vectorAtributos[11]={
		...
		vectorAtributos[13]={
				validacion:{
					...
					loadMask:true,
					triggerAction:'all',
					store:new Ext.data.SimpleStore({fields:['valor'],data:[['si'],['no']]}),
					valueField:'valor',
					displayField:'valor',
					grid_visible:true,
					grid_editable:false,
