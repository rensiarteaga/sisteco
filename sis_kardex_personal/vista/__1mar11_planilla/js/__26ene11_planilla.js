...
// txt fecha_reg
	...
	this.EnableSelect=function(sm,row,rec){
		...
		if(rec.data.desc_tipo_planilla=='sueldo'){
	...
	*/
		...
		if(NumSelect!=0){
			...
			if(SelectionsRecord.data.estado_anticipo!='' && SelectionsRecord.data.estado_anticipo!=null && SelectionsRecord.data.estado_anticipo!=undefined && SelectionsRecord.data.estado_anticipo!='pagado'){
				...
				if(SelectionsRecord.data.estado=='calculado'||SelectionsRecord.data.estado=='comprometido'){
					...
					alert('Es necesario proceder con el calculo de la planilla previamente');
				}
			}
		}else{var enableSelect=this.EnableSelect;
			Ext.MessageBox.alert('Estado', 'Antes debe seleccionar un item.');
	...
	this.AdicionarBoton('../../../lib/imagenes/copy.png','Presupuesto',btn_planilla_presupuesto,true,'planilla_presupuesto','');
