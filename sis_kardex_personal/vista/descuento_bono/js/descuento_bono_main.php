...
var elemento={idContenedor:idContenedor,pagina:new pagina_descuento_bono(idContenedor,direccion,paramConfig,maestro,idContenedorPadre)};
...
function pagina_descuento_bono(idContenedor,direccion,paramConfig,maestro,idContenedorPadre){
	...
	ds=new Ext.data.Store({
		...
		'estado_reg','desc_tipo_descuento_bono','desc_empleado','desc_moneda','cuotas',
...
vectorAtributos[4]= {
		validacion: {
			name:'cuotas',
			emptyText:'Pago en Cuotas',
			fieldLabel:'Pago en Cuotas',
			allowBlank:false,
			typeAhead: true,
			loadMask: true,
			triggerAction: 'all',
...
vectorAtributos[5]={
	...
	vectorAtributos[12]={
		...
		filtro_0:true,
		filtro_1:true,
		filterColValue:'EMPBONO.fecha_fin',
		dateFormat:'m-d-Y',
		defecto:'',
		save_as:'fecha_fin'
	};	
	...
	function iniciarEventosFormularios(){	
		...
		txt_num_cuotas=getComponente('num_cuotas');
		txt_total=getComponente('monto_total');
		txt_cuota=getComponente('valor_por_cuota');
		txt_cuotas=getComponente('cuotas');
		
		var onCuotas=function(e){
			if(e.value=='si'){
		...
		txt_cuotas.on('select', onCuotas);
		var onNum=function(e){
			if(getComponente('monto_total').getValue()>0){
				getComponente('valor_por_cuota').setValue(getComponente('monto_total').getValue()/e.value);
			}else{
				getComponente('valor_por_cuota').setValue(0);
			}
		}
		
		var onTotal=function(e,r,o){
			if(getComponente('num_cuotas').getValue()>0){
				getComponente('valor_por_cuota').setValue(e.value/getComponente('num_cuotas').getValue());
			}else{
				getComponente('valor_por_cuota').setValue(0);
			}
		}
		...
		txt_total.on('blur',onTotal);
		txt_num_cuotas.on('blur',onNum);
	...
	this.btnEdit=function(){
		...
		if(NumSelect!=0)
		{
			...
			if(SelectionsRecord.data.cuotas=='no'){
				ocultarComponente(getComponente('monto_total'));
				ocultarComponente(getComponente('num_cuotas'));
				ocultarComponente(getComponente('monto_faltante'));
