...
var elemento={pagina:new pagina_empleado_familiar(idContenedor,direccion,paramConfig),idContenedor:idContenedor};
 ...
 * Fecha creación:		2007-10-18 09:06:57
 */
function pagina_empleado_familiar(idContenedor,direccion,paramConfig){
	var vectorAtributos=new Array;
	var ds,ds_persona;
	var elementos=new Array();
	var layout_empleado_familiar;
	...
	var config={titulo_maestro:'Empleado',grid_maestro:'grid-'+idContenedor,urlHijo:'../../../sis_kardex_personal/vista/relacion_familiar/relacion_familiar.php'};
	layout_empleado_familiar=new DocsLayoutMaestroDeatalle(idContenedor);
	layout_empleado_familiar.init(config);
	
	// INICIAMOS HERENCIA //
	/// HEREDAMOS DE LA CLASE MADRE
	this.pagina=Pagina;
	//-- pagina (array PARAMETROS DE CONFIGURACION, array DEFINICION DE ATRIBUTOS, SELECTION MODEL, DATA STORE, COLUM MODEL)
	this.pagina(paramConfig,vectorAtributos,ds,layout_empleado_familiar,idContenedor);
	...
	function btn_empleado_tpm_frppa()
	{
		...
		if(NumSelect!=0)
		{
			...
			layout_empleado_familiar.loadWindows(direccion+'../../empleado_tpm_frppa/empleado_tpm_frppa_det.php?'+data,'Empleados Estructura Programática',ParamVentana);
            layout_empleado_familiar.getVentana().on('resize',function(){layout_empleado_familiar.getLayout().layout()})
		}
		else
		{
	...
	function iniciarEventosFormularios()
	{
		...
		{
			if(_CP.getPagina(layout_empleado_familiar.getIdContentHijo()).pagina.limpiarStore())
			{
				//_CP.getPagina(layout_empleado_familiar.getIdContentHijo()).pagina.limpiarStore();
				_CP.getPagina(layout_empleado_familiar.getIdContentHijo()).pagina.bloquearMenu()
			}
	...
	this.EnableSelect=function(sm,row,rec)
	{
		...
		_CP.getPagina(layout_empleado_familiar.getIdContentHijo()).pagina.reload(rec.data);
		_CP.getPagina(layout_empleado_familiar.getIdContentHijo()).pagina.desbloquearMenu();	
	...
	this.getLayout=function(){
		return layout_empleado_familiar.getLayout()
	};
	...
	layout_empleado_familiar.getLayout().addListener('layout',this.onResize);//aregla la forma en que se ve el grid dentro del layout
