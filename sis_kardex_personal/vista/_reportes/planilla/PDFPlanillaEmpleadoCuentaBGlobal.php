...
{       
   ...
   // $detalle1=$_SESSION['PDF_lista_planilla_empleado_cb'];
    $detalle='';
    	...
    	if ($tipo_reporte=='BONMES'){
    		...
    		if ($tipo_reporte=='BONO_TRA'){
    			...
    			if ($tipo_reporte=='REFRIGERIO'){
    				$this->SetFont('Arial','B',8);
    			   $detalle=$_SESSION["desc_refrigerio"]; //'Reposicion de Refrigerio '. $cabecera[0]['numero'].' : ';
    			}else{//06.06.2014
    				if($tipo_reporte=='PRIMA'){
    					$detalle='Prima gestion 2014';
    				}
    			}
    		}
    	}
    	...
    	if($tipo_reporte=='PRIMA' ){
    		...
    		if($tipo_reporte=='REFRIGERIO'){
    			...
    			if($tipo_reporte=='BONMES'|| $tipo_reporte=='BONO_TRA'){
    				if($cabecera[0]['periodo']=='Enero'){
    					$ges_an=$cabecera[0]['gestion'];
    					$ges_an=$ges_an-1;
    					$this->Cell(0,5,$detalle.' '.$cabecera[0]['periodo_anterior'].'-'.$ges_an,0,1,'C');
    				}else{
   ...
   if($_SESSION["tipo_pago"]=='LIQPAG'){
   	$this->Cell(20,3.5,'LIQ. PAGABLE',1,$salto,'C');
   	    ...
   	    if($_SESSION["tipo_pago"]=='REFRIGERIO'|| $_SESSION["tipo_pago"]=='BONMES'|| $_SESSION["tipo_pago"]=='BONO_TRA'|| $_SESSION["tipo_pago"]=='PRIMA'){
   	    	
   	    	$this->Cell(30,3.5,'IMPORTE BONO',1,$salto,'C');
   	    }else{
...
function Footer()
{
 ...
 /*if($_SESSION["reporte"]=='firma'){
	...
	$tamano_monto=20;
 	if($_SESSION["tipo_pago"]=='REFRIGERIO' || $_SESSION["tipo_pago"]=='BONMES' || $_SESSION["tipo_pago"]=='BONO_TRA' || $_SESSION["tipo_pago"]=='BONO_TE' || $_SESSION["tipo_pago"]=='PRIMA'  ){
 		$tamano_monto=30;
 	}
 	
 	$pdf->SetWidths(array(15,130,30,$tamano_monto,0));
    ...
    $pdf->SetFormatNumber(array(0,0,0,1,0));
 	 		...
 	 		$sum_parc_lugar=0;
 	 		$salto='no';
 	 		/*echo "ddd".$detalle[0]["nombre_banco"];
 	 		exit;*/
 	 		...
 	 		for ($i=0;$i<count($detalle);$i++){
 	 			...
 	 			/**** esto es  para muchosbancos y cuando se pagaba por cheques ***/
