...
//echo $cabecera[0]['gestion'].''. $cabecera[0]['periodo']; exit;


if($cabecera[0]['gestion']==2012 ){
	$cant_columnas=39; //gestion 2015
}else{
 if($cabecera[0]['gestion']==2013   && $cabecera[0]['periodo']!='Octubre' ){
 	   
 	
 		$cant_columnas=39; 
  }else {
  	
  	if($cabecera[0]['gestion']==2014   && $cabecera[0]['periodo']='Diciembre' ){
  		$cant_columnas=41;
  	}else{
  		if($cabecera[0]['gestion']>=2016 &&  ($cabecera[0]['periodo']!='Septiembre' || $cabecera[0]['periodo']!='Octubre' ||$cabecera[0]['periodo']!='Noviembre' ||$cabecera[0]['periodo']!='Diciembre'  ))
  			$cant_columnas=44;
  		else
  	
        $cant_columnas=43; //gestion 2015
  	}
  }
}


$tam_det=count($detalle)/$cant_columnas;

//echo 'detalle'.count($detalle).'valor'.$cant_columnas; exit;
...
$pdf->AddPage();
	...
	for ($i=$indice;$i<($indice+$cant_columnas);$i++){
		...
		switch ($detalle[$i]['codigo']){
			...
			case 'DESATR':  $desatr=$detalle[$i]['valor'];
			  ...
			  //modificacion para planilla de octubre/2013
			   ...
			   case 'TRCOOP':  $trcoop=$detalle[$i]['valor'];
			   break;
		}
	}
	$indice=$indice+$cant_columnas;
...
if (is_null($bonant) || $bonant<=0) {
   ...
   if (is_null($reintbext) || $reintbext<=0) {
	...
	$pdf->Cell(10,3,'','',0,'R');  
		...
		$pdf->Cell(50,3,'REINTEGRO ENE-JUN/2015',0,'L');
		$pdf->Cell(10,3,'','',0,'R');
		$pdf->Cell(30,3,'','',0,'R');
	...
	//+$descvar
	+$compdep+$descap+$apsib+$descuota+$desunico+$trcoop;
