    		...
    		if($tipo_reporte=='REFRIGERIO'){
    			...
    			if($tipo_reporte=='BONMES'|| $tipo_reporte=='BONO_TRA' || $tipo_reporte=='BONO_TE'){
					...
					if($cabecera[0]['periodo']=='Enero'){
						$gestion_ant=$cabecera[0]['gestion'];
						$gestion_ant=$gestion_ant-1;
						$this->Cell(0,5,$detalle.' '.$cabecera[0]['periodo_anterior'].'-'.$gestion_ant,0,1,'C');
					}else{
 ...
 if($_SESSION["reporte"]=='firma'){
    ...
    $pdf->SetFormatNumber(array(0,0,0,1,0));
 	 		...
 	 		for ($i=0;$i<count($detalle);$i++){
 	 		 ...
 	 		 $pdf->SetFont('Arial','BI',8);
			 if ($bandera){
 	 			$pdf->Cell(135,4.5,'TOTAL xxxx','RTB',0,'R');
                $pdf->Cell($tamano_monto,4.5,number_format($sum_parc,2),'TB',1,'R');
             }else{
             	$pdf->Cell(175,4.5,'TOTAL ','RTB',0,'R');
             	$pdf->Cell($tamano_monto,4.5,number_format($sum_parc,2),'TB',1,'R');
             	...
             	$pdf->SetFont('Arial','BI',8);
             	if($_SESSION["tipo_pago"]=='LIQPAG' ){
             		//$pdf->Cell($tamano_monto,3.5,'TOTAL**','RTB',0,'R');
             		//$pdf->Cell(40,3.5,'','RTB',0,'R');
             		$pdf->Cell(40,3.5,number_format($sum_detalle,2),'TB',1,'R');
             		$pdf->AddPage();
             		$pdf->Ln(2);
