...
class cls_DBTipoColumna
{
	 ...
	 * Fecha de creación:		2010-08-10 17:59:44
	 */
	function InsertarColumnaTipo($id_columna_tipo,$id_parametro_kardex,$id_partida,$nombre,$valor,$tipo_dato,$id_moneda,$tipo_aporte,$estado_reg,$fecha_reg,$cotizable,$descripcion,$descuento_incremento,$observacion,$formula,$id_tipo_descuento_bono,$codigo,$id_cuenta_pasivo,$id_auxiliar_pasivo,$compromete,$id_tipo_obligacion)
	{
		$this->salida = "";
	...
	function ValidarColumnaTipo($operacion_sql,$id_columna_tipo,$id_parametro_kardex,$id_partida,$nombre,$valor,$tipo_dato2,$id_moneda,$tipo_aporte,$estado_reg,$fecha_reg,$cotizable,$descripcion,$descuento_incremento,$observacion,$formula,$movimiento_contable,$prorratea)
	{
		...
		if($operacion_sql=='insert' || $operacion_sql=='update')
		{
			if($operacion_sql == 'update')
			{
				...
				//validar movimiento_contable
				$tipo_dato->_reiniciar_valor();
				$tipo_dato->set_MaxLength(300);
				$tipo_dato->set_Columna("movimiento_contable");

				if(!$valid->verifica_dato($tipo_dato->TipoDatoText(), "movimiento_contable", $movimiento_contable))
				{
					$this->salida = $valid->salida;
					return false;
				}
				...
				$tipo_dato->set_Columna("prorratea");

				if(!$valid->verifica_dato($tipo_dato->TipoDatoText(), "prorratea", $prorratea))
				{
					$this->salida = $valid->salida;
					return false;
				}
