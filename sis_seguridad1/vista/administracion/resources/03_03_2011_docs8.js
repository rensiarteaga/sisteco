	...
	return{
		...
		conexionFailure:function(resp1,resp2,resp3,resp4){
			...
			if(resp.status==04&&!sw){
				...
				var mensaje="<p> No se encontro el Action requerido</p>";
			...
			if(!sw&&resp.responseXML&& resp.responseXML.documentElement){
				...
				if(resp.status==-1&&!sw){
					...
					var mensaje = "<p>Tiempo de espera agotado</p>";
				...
				if (resp.status==0&&!sw){
					...
					var mensaje = "<p> Fallo en su conexión de Internet</p>";
		...
		loadWindows:function(url,title,param,idContenedor){
			...
			for(var i=0;i<pagHijo.length;i++){
				if(pagHijo[i].url==_url[0]){
					...
					if(paginaHijo.pagina.setReload!=false){
					...
					//si el reload no esta habilidato creamos una nueva instancia del objeto	
						//var ele = Ext.DomQuery.selectNode('body')
						//var elefor = document.getElementById("dlgInfo-"+pagHijo[i].idContenedor)
						
						//var eleVen = document.getElementById('v-'+pagHijo[i].idContenedor)
						//var eleCon = document.getElementById(pagHijo[i].idContenedor)
						//var eleLayout = document.getElementById('layout-'+pagHijo[i].idContenedor)
					    paginaHijo.pagina.Destroy()
					
						
						//ele.removeChild(elefor);
						//eleCon.removeChild(eleLayout)
						delete  eleLayout;
					
						//ele.removeChild(eleVen);
			            ...
			            //Ext.DomHelper.overwrite(pagHijo[i].idContenedor,'')
			            //console.log(pagHijo[i].ventana.destroy)
			           
						pagHijo[i].ventana.show();
						DinaMain.carga_menu(_url[1],pagHijo[i].idContenedor);
			...
			if(!sw){
				...
				ventana.addListener('beforehide',function(x){
					...
					if(paginaHijo.pagina.limpiarStore){paginaHijo.pagina.limpiarStore()};
