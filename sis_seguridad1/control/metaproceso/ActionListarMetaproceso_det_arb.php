...
Fecha de Creación:		2007-10-26 16:42:36
Versión:				1.0.0
...
//Parámetros del filtro
	...
	if($txt_envio_alerta>0){
		
		$criterio_filtro = $criterio_filtro ." AND METAPR.id_metaproceso not in (select id_metaproceso from tsg_metaproceso_envio_alerta where id_envio_alerta=$txt_envio_alerta)";
		
	}
	if($m_id_rol>0){
		
		$criterio_filtro = $criterio_filtro ." AND METAPR.id_metaproceso not in (select id_metaproceso from tsg_rol_metaproceso where id_rol=$m_id_rol)";
