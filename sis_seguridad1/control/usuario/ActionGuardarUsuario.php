...
if($_SESSION["autentificado"]=="SI")
{
	...
	for($j = 0;$j < $cont; $j++)
	{
		if ($get)
		{
			...
			$txt_id_nivel_seguridad= $_GET["id_nivel_seguridad_$j"];
			$fecha_inactivacion= $_GET["txt_fecha_inactivacion_$j"];
		...
		else
		{
			...
			$txt_id_nivel_seguridad= $_POST["id_nivel_seguridad_$j"];
			$fecha_inactivacion= $_POST["txt_fecha_inactivacion_$j"];
		}
//echo 'id_nivel seguridad'.$txt_id_nivel_seguridad; exit;
		...
		{//echo $j;
			...
			$res = $Custom -> InsertarUsuario($hidden_id_usuario, $txt_id_persona, $txt_login, $txt_contrasenia, $txt_fecha_registro, $txt_hora_registro, $txt_fecha_ultima_modificacion, $txt_hora_ultima_modificacion, $txt_estado_usuario, $txt_estilo_usuario, $txt_filtro_avanzado,$txt_fecha_expiracion,$txt_autentificacion,$txt_id_nivel_seguridad);
		...
		else
		{
			...
			if($fecha_inactivacion!='' && $fecha_inactivacion!=null){
				$res=$Custom->InactivarUsuario($hidden_id_usuario,$fecha_inactivacion);
				if(!$res){
					$resp= new cls_manejo_mensajes(true,"406");
					$resp->mensaje_error= $Custom->salida[1];
					$resp->origen=$Custom->salida[2];
			      ...
			      if($_POST["txt_contrasenia_nueva_$j"]=='' and $_POST["txt_estilo_usuario_$j"]!=''){
					///////////////////////Modificación////////////////////
					  //Validación de datos (del lado del servidor)
							...
							$res = $Custom->ModificarUsuario($hidden_id_usuario, $txt_id_persona, $txt_login, $txt_contrasenia, $txt_fecha_registro, $txt_hora_registro, $txt_fecha_ultima_modificacion, $txt_hora_ultima_modificacion, $txt_estado_usuario, $txt_estilo_usuario, $txt_filtro_avanzado,$txt_fecha_expiracion, $txt_autentificacion,$txt_id_nivel_seguridad);
							if(!$res)
							{
								//Se produjo un error
								$resp = new cls_manejo_mensajes(true, "406");
