...
class cls_DBProcesoMotivo
{
	...
	var $nombre_archivo = "cls_DBProcesoMotivo.php";
	...
	function ListarProcesoMotivo($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad)
	{
		...
		$this->salida ="";
		$this->nombre_funcion = 'f_taf_proceso_motivo_consultas';
		$this->codigo_procedimiento = "'AF_PRO_MOT_SEL'";
		...
		$this->var->add_def_cols('id_motivo','integer');
		$this->var->add_def_cols('descripcion','text');
		$this->var->add_def_cols('id_proceso','integer');
		$this->var->add_def_cols('des_proceso','varchar');
	...
	function ContarListaPaginadaProcesoMotivo($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad)
	{
		$this->salida = "";
		$this->nombre_funcion = 'f_taf_proceso_motivo_consultas';
		$this->codigo_procedimiento = "'AF_PRO_MOT_SEL_COUNT'";
	...
	/*
	**********************************************************
	Nombre de la función:	CrearProcesoMotivo($descripcion)

	Propósito:				Se utiliza esta función para insertar un nuevo proceso motivo en la base de datos
	Parámetros:				$descripcion	-->	desc del motivo
	...
	**********************************************************
	*/
	function CrearProcesoMotivo($id_motivo,$descripcion,$id_proceso)
	{
		$this->salida = "";
		$this->nombre_funcion = 'f_taf_proceso_motivo';
		$this->codigo_procedimiento = "'AF_PRO_MOT_INS'";
		...
		$this->var->add_param('NULL');//id_proceso_motivo
	...
	/*************************************************************************************/
	/*Nombre de la función:	EliminarProcesoMotivo($id_motivo)

	Propósito:				Se utiliza esta función para eliminar motivo de procesos de la base de datos
	Parámetros:				$id_motivo	-->	identificador del motivo del proceso
	...
	function  EliminarProcesoMotivo($id_motivo)
	{
		...
		$this->salida="";
		$this->nombre_funcion = 'f_taf_proceso_motivo';
		$this->codigo_procedimiento = "'AF_PRO_MOT_DEL'";

		$this->var = new cls_middle($this->nombre_funcion,$this->codigo_procedimiento,$this->decodificar);

		//Carga parámetros específicos (no incluyen los parámetros fijos)
		$this->var->add_param('NULL');//id_fina_regi
		$this->var->add_param($id_motivo);//id_MOTIVO
	...
	function  ModificarProcesoMotivo($id_motivo,$descripcion,$id_proceso)
	{
		$this->salida="";
		$this->nombre_funcion = 'f_taf_proceso_motivo';
		$this->codigo_procedimiento = '\'AF_PRO_MOT_UPD\'';
		
		$this->var = new cls_middle($this->nombre_funcion,$this->codigo_procedimiento,$this->decodificar);
		
		$this->var->add_param('NULL');//id_fina_regi_
		$this->var->add_param($id_motivo);//id_proceso_motivo
		$this->var->add_param("'$descripcion'");//descripcion
		$this->var->add_param($id_proceso);//id_proceso
	...
	function ValidarProcesoMotivo($operacion_sql,$id_motivo,$descripcion,$id_proceso)
	{
		...
		switch ($operacion_sql) {
			case 'insert' || 'update':
				{
					...
					if(!$valid->verifica_dato($this->matriz_validacion[2] ,"id_proceso", $id_proceso))
					{
			...
			case 'update':
				{
					if(!$valid->verifica_dato($this->matriz_validacion[0],"id_motivo",$id_motivo))
					{
						$this->salida = $valid->resp;
						return false;
					}
	...
	function cargar_param_valid()
	{
		...
		$this->matriz_validacion[0]['Columna'] = "id_motivo";
		...
		$this->matriz_validacion[2]['Columna'] = "id_proceso";
