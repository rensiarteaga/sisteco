 ...
 * Propï¿½sito:				es la interfaz del modelo del Sistema de Activos Fijos
 * todos los metodos existentes pasan por aqui
 * Fecha de Creaciï¿½n:		01-06-2007
 * Autor:					
 *
 */
...
//			 include_once("cls_DBDetalleActivoFijoReporte.php");
			 include_once("cls_DBCodigoBarras.php");
			 include_once("cls_DBActivacionGestion.php");
			 include_once("cls_DBReporteActivoFijoResponsableCustodio.php");
			 include_once("cls_DBActivoFijoDepreciacionComprobante.php");
			 include_once("cls_DBUbicacion.php");
			 include_once("cls_DBUnidadConstructiva.php");
			 include_once ("cls_DBTipoActivoCuenta.php");
			 include_once("cls_DBActivoFijoComprobante.php");
			 include_once("cls_DBContraCuenta.php");
			 
			 include_once("cls_DBPlanCuentas.php");
			 
			 include_once("cls_DBObservacionesVarias.php");
			 include_once("cls_DBDepreciacionGestion.php");
			 include_once("cls_DBDepreciacionGestionDetalle.php");
			 
			 include_once("cls_DBCbteDepto.php");
			 include_once("cls_DBCbteDeptoDetalle.php");
			 include_once("cls_DBDepreciacionDepto.php");
	...
	/////////////// FIN Mï¿½TODOS DEPRECIACIï¿½N/////////////////////

	/////////////   Mï¿½TODO PARAMETROS GENERALES /////////////////////////////
...
function ContarParamGral($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad)
	...
	function DevolucionActivoProceso($id_activo_fijo_proceso)
	{
		...
		$res = $dbActivoFijoProceso -> DevolucionActivoProceso($id_activo_fijo_proceso);
	...
	function CambiarActivoFijo($id_activo_fijo,$id_unidad_constructiva,$id_ubicacion,$ubicacion_fisica,$observaciones,$descripcion_larga,$id_estado_funcional,$descripcion,$num_factura)
	{
		$this->salida = "";
		$dbActivoFijo = new cls_DBActivoFijo($this->decodificar);
		$res = $dbActivoFijo -> CambiarActivoFijo($id_activo_fijo,$id_unidad_constructiva,$id_ubicacion,$ubicacion_fisica,$observaciones,$descripcion_larga,$id_estado_funcional,$descripcion,$num_factura);
		$this->salida = $dbActivoFijo->salida;
		$this->query = $dbActivoFijo->query;
		return $res;
	}
	...
	function ModificarActivoFijoGrupoProceso($id_activo_fijo_proceso,$id_grupo_proceso,$id_activo_fijo,$monto_revalorizacion,$vida_util_revalorizacion,$fecha_ini_dep,$observaciones,$asignar)
	{
		$this->salida = "";
		$dbActivoFijoProceso = new cls_DBActivoFijoProceso($this->decodificar);
		$res = $dbActivoFijoProceso ->ModificarActivoFijoGrupoProceso($id_activo_fijo_proceso,$id_grupo_proceso,$id_activo_fijo,$monto_revalorizacion,$vida_util_revalorizacion,$fecha_ini_dep,$observaciones,$asignar);
		$this->salida = $dbActivoFijoProceso->salida;
		$this->query = $dbActivoFijoProceso->query;
		return $res;
	}
...
*/
	...
	////////////////////////////////CODIGO DE BARRAS Silvia Ximena Ortiz Fernï¿½ndez//////////////////
	...
	{	
		$this->salida = "";
		$dbActivoFijo = new cls_DBCodigoBarras($this->decodificar);
		$res = $dbActivoFijo ->ListarCodigoBarrasPrueba($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);
		$this->salida = $dbActivoFijo->salida;
		$this->query = $dbActivoFijo->query;
		return $res;
	}
	function ContarCodigoBarrasPrueba($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad)
	{
		...
		$this->query = "";
		$dbActivoFijo= new cls_DBCodigoBarras($this->decodificar);
		$res = $dbActivoFijo ->ContarCodigoBarrasPrueba($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);
		$this->salida = $dbActivoFijo->salida;
		$this->query = $dbActivoFijo->query;
		return $res;
	}
	...
	/////////////   Mï¿½TODO EMPLEADO CARGO UNIDAD ORGANIZACIONAL/////////////////////////////
...
//28-11/2008
	...
	function AccionesGrupoProceso($id_grupo_proceso,$accion,$id_persona)
	{
		...
		$res = $dbGrupoProceso ->AccionesGrupoProceso($id_grupo_proceso,$accion,$id_persona);
		$this->salida = $dbGrupoProceso ->salida;
		$this->query = $dbGrupoProceso->query;
		return $res;
	}
	...
	function InsertarProcesoTipoCuenta($txt_des_proceso,$hidden_id_proceso,$debe_haber,$campo_importe)
	{
		$this->salida = "";
		$dbProceso = new cls_DBProcesoTipoCuenta($this->decodificar);
		$res = $dbProceso ->InsertarProcesoTipoCuenta($txt_des_proceso,$hidden_id_proceso,$debe_haber,$campo_importe);
		$this->salida = $dbProceso ->salida;
		$this->query = $dbProceso ->query;
		return $res;
	}
	
	function ModificarProcesoTipoCuenta($hidden_id_proceso_tipo_cuenta,$txt_des_proceso,$hidden_id_proceso,$debe_haber,$campo_importe)
	{
		$this->salida = "";
		$dbProceso = new cls_DBProcesoTipoCuenta($this->decodificar);
		$res = $dbProceso ->ModificarProcesoTipoCuenta($hidden_id_proceso_tipo_cuenta,$txt_des_proceso,$hidden_id_proceso,$debe_haber,$campo_importe);
		$this->salida = $dbProceso ->salida;
		$this->query = $dbProceso ->query;
		return $res;
	}
	...
	function EliminarTipoCuentaCuenta($id)
	{
		...
		//Aï¿½adido por Silvia Ximena Ortiz Fernï¿½ndez
	...
	function ListarActivoFijoEmpleadoDetalle2($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad)
	{
		$this->salida = "";
		$dbTipoCuenta = new cls_DBActivoFijoReporte($this->decodificar);
		$res = $dbTipoCuenta ->ListarActivoFijoEmpleadoDetalle2($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);
		$this->salida = $dbTipoCuenta ->salida;
		$this->query = $dbTipoCuenta ->query;
		return $res;
	}
	...
	function ListarActivoBaja($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad)
	{
		$this->salida = "";
		$dbTipoCuenta = new cls_DBActivoFijoReporte($this->decodificar);
		$res = $dbTipoCuenta ->ListarActivoBaja($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);
		$this->salida = $dbTipoCuenta ->salida;
		$this->query = $dbTipoCuenta ->query;
		return $res;
	}
	...
	function Cabecera_rep_det_dep($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad,$id_activo_fijo,$fecha_desde,$fecha_hasta)
	{	
		$this->salida = "";
		$dbDetDepreciacion = new cls_DBDepreciacion($this->decodificar);
		$res = $dbDetDepreciacion ->Cabecera_rep_det_dep($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad,$id_activo_fijo,$fecha_desde,$fecha_hasta);
		$this->salida = $dbDetDepreciacion ->salida;
		$this->query = $dbDetDepreciacion ->query;
		return $res;
	}	
	
	function Cuerpo_rep_det_dep($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad,$id_activo_fijo_det,$fecha_desde,$fecha_hasta)
	{	
		$this->salida = "";
		$dbDetDepreciacion = new cls_DBDepreciacion($this->decodificar);
		$res = $dbDetDepreciacion ->Cuerpo_rep_det_dep($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad,$id_activo_fijo_det,$fecha_desde,$fecha_hasta);
		$this->salida = $dbDetDepreciacion ->salida;
		$this->query = $dbDetDepreciacion ->query;
		return $res;
	}	
	...
	{	
		$this->salida = "";
		$dbDetDepreciacion = new cls_DBDepreciacion($this->decodificar);
		$res = $dbDetDepreciacion ->SumaDatosDetDep($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad,$id_activo_fijo,$fecha_desde,$fecha_hasta);
		$this->salida = $dbDetDepreciacion ->salida;
		$this->query = $dbDetDepreciacion ->query;
		return $res;
	}
	...
	{	$this->salida = "";
		$dbDetActivoFijo = new cls_DBActivoFijoReporte($this->decodificar);
		$res = $dbDetActivoFijo ->ReporteDetalleAFNom($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);
		$this->salida = $dbDetActivoFijo ->salida;
		$this->query = $dbDetActivoFijo ->query;
		return $res;
	}
	...
	{	$this->salida = "";
		$dbDetActivoFijo = new cls_DBActivoFijoReporte($this->decodificar);
		$res = $dbDetActivoFijo ->ReporteDetalleAFDesc($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);
		$this->salida = $dbDetActivoFijo ->salida;
		$this->query = $dbDetActivoFijo ->query;
		return $res;
	}
	
	///////////////////////////////////////Reporte activacion gestion////////////////////
	...
	{	
		$this->salida = "";
		$dbActGestion = new cls_DBActivacionGestion($this->decodificar);
		$res = $dbActGestion ->Cabecera_rep_act_gestion($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad,$id_activo_fijo,$gestion, $id_tipo_activo, $id_sub_tipo_activo);
		$this->salida = $dbActGestion ->salida;
		$this->query = $dbActGestion ->query;
		return $res;
	}	
	...
	{	
		$this->salida = "";
		$dbActGestion = new cls_DBActivacionGestion($this->decodificar);
		$res = $dbActGestion ->Cuerpo_rep_act_gestion($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad,$id_activo_fijo,$gestion, $id_tipo_activo, $id_sub_tipo_activo);
		$this->salida = $dbActGestion ->salida;
		$this->query = $dbActGestion ->query;
		return $res;
	...
	//////////////////////////fin activacion gestion/////////////////////////////////
	
	//13 de junio de 2012 - se adiciono la funcion al Custom porque no se encontraba registrada
	...
	{	
		$this->salida = "";
		$dbActResp = new cls_DBActivoFijoProceso($this->decodificar);
		$res = $dbActResp ->ObtieneResponsableAF($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);
		$this->salida = $dbActResp ->salida;
		$this->query = $dbActResp ->query;
		return $res;
	...
	/***********************************Reporte Activo Fijo Responsable Custodio*****************************/
	/*
	 * Author: 	       Daniel Sanchez Torrico
	*  Fecha Creacion: 18/10/2012
	*  Modificado:     04/02/2013
	*/
	/**********************************************************************************************************/
	function ReporteActivoFijoResponsableCustodio($cant, $puntero, $sortdir, $sortcol, $codigo_procedimiento, $criterio_filtro,$criterio_filtro_2 = ''){
		
		$this->salida = "";
		$dbAcFiResCus = new cls_DBReporteActivoFijoResponsableCustodio($this->decodificar);
		$res = $dbAcFiResCus ->ReporteActivoFijoResponsableCustodio($cant,$puntero,$sortdir,$sortcol, $codigo_procedimiento, $criterio_filtro,$criterio_filtro_2);
		$this->salida = $dbAcFiResCus ->salida;
		$this->query = $dbAcFiResCus ->query;
		return $res;
	...
	/***********************************Reporte Activo Fijo Depreciacion*****************************/
	/*
	 * Author: 	Daniel Sanchez Torrico
	* Fecha: 	Creacion: 23/11/2012
	*/
	/**********************************************************************************************************/
	
	function ReporteActivosFijosDepreciacionNuevo($cant, $puntero, $sortdir, $sortcol, $criterio_filtro){
	
		$this->salida = "";
		$dbAcFiDeprec = new cls_DBActivoFijoDepreciacionComprobante($this->decodificar);
		$res = $dbAcFiDeprec ->ReporteActivoFijoDepreciacionNuevo($cant, $puntero, $sortdir, $sortcol, $criterio_filtro);
		//$res = $dbAcFiDeprec ->ReporteActivoFijoDepreciacionNuevoAuditoria($cant, $puntero, $sortdir, $sortcol, $criterio_filtro);
		$this->salida = $dbAcFiDeprec ->salida;
		$this->query = $dbAcFiDeprec ->query;
		return $res;
	...
	/***********************************Registro Activo Fijo Comprobantes*****************************/
	/*
	 * Author: 	Daniel Sanchez Torrico
	...
	*/
	...
	function RegistrarActivosFijosDepreciacionComprobantes($cant, $puntero, $sortdir, $sortcol, $criterio_filtro,$id_grupo_dep,$fecha_hasta,$cod_regional){
		...
		$this->salida = "";
		$dbAcFiDepreComp = new cls_DBActivoFijoDepreciacionComprobante($this->decodificar);
		$res = $dbAcFiDepreComp ->RegistrarActivosFijosDepreciacionComprobantes($cant, $puntero, $sortdir, $sortcol, $criterio_filtro,$id_grupo_dep,$fecha_hasta,$cod_regional);
		$this->salida = $dbAcFiDepreComp ->salida;
		$this->query = $dbAcFiDepreComp ->query;
		return $res;
	
	}
	/***********************************Listar Regionales Activo Fijo Depreciacion*****************************/
	/*
	 * Author: 	Daniel Sanchez Torrico
	...
	*/
	...
	function ListarRegionalesActivoFijoDepreciacion($cant, $puntero, $sortdir, $sortcol, $criterio_filtro){
		...
		$this->salida = "";
		$dbAcFiDepreComp = new cls_DBActivoFijoDepreciacionComprobante($this->decodificar);
		$res = $dbAcFiDepreComp ->ListarRegionalesActivoFijoDepreciacion($cant, $puntero, $sortdir, $sortcol, $criterio_filtro);
		$this->salida = $dbAcFiDepreComp ->salida;
		$this->query = $dbAcFiDepreComp ->query;
		return $res;
	...
	function ModificarDepreciacionCbtesRegional($id_depre_cbte_reg,$id_depto,$regional,$h_id_depto_aux)
	{
		$this->salida = "";
		$dbAcFiDeprec = new cls_DBActivoFijoDepreciacionComprobante($this->decodificar);
		$res = $dbAcFiDeprec->ModificarDepreciacionCbtesregional($id_depre_cbte_reg,$id_depto,$regional,$h_id_depto_aux);
		$this->salida = $dbAcFiDeprec ->salida;
		$this->query = $dbAcFiDeprec ->query;
		return $res;
	}
	...
	function CountDepreciacionCbtesRegional($cant,$puntero,$sortcol,$sortdir,$criterio_filtro)
	{
		$this->salida = "";
		$dbAcFiDeprec = new cls_DBActivoFijoDepreciacionComprobante($this->decodificar);
		$res = $dbAcFiDeprec->CountDepreciacionCbtesRegional($cant,$puntero,$sortcol,$sortdir,$criterio_filtro);
		$this->salida = $dbAcFiDeprec ->salida;
		$this->query = $dbAcFiDeprec ->query;
		return $res;
	}
	/***********************************Reporte Detalle Activo Fijo *****************************/
	/*
	 * Author: 	Daniel Sanchez Torrico
	* Fecha: 	Creacion: 09/04/2013
	*/
	function ReporteDetalleActivoFijoAnalisis($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$criterio_cabeceras){	
		$this->salida = "";
		$dbDetActivoFijo = new cls_DBActivoFijoReporte($this->decodificar);
		$res = $dbDetActivoFijo ->ReporteDetalleActivoFijoAnalisis($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$criterio_cabeceras);
		$this->salida = $dbDetActivoFijo ->salida;
		$this->query = $dbDetActivoFijo ->query;
		return $res;
	...
	function ListarRegionalesDepreciacionNuevo($cant, $puntero, $sortdir, $sortcol, $criterio_filtro)
	{
		$this->salida = "";
		$dbAcFiDepreComp = new cls_DBActivoFijoDepreciacionComprobante($this->decodificar);
		$res = $dbAcFiDepreComp->ListarRegionalesDepreciacionNuevo($cant, $puntero, $sortdir, $sortcol, $criterio_filtro);
		$this->salida = $dbAcFiDepreComp ->salida;
		$this->query = $dbAcFiDepreComp ->query;
		return $res;
	}
	/*
	 * 	ACTIVO FIJO UBICACION FISICA
	 * proposito:implementar las funcionalidad de la tabla actif.taf_ubicacion
	 * Autor: unknow
	 * Fecha: 05/07/2013
	 */
	function ListarUbicacionFisica($cant,$puntero,$sortcol,$sortdir,$criterio_filtro)
	{
		...
		$res = $dbUbicacion ->ListarUbicacionFisica($cant,$puntero,$sortcol,$sortdir,$criterio_filtro);
	...
	function ContarListaUbicacionFisica($cant,$puntero,$sortcol,$sortdir,$criterio_filtro)
	{
		$this->salida = "";
		$dbUbicacion= new cls_DBUbicacion($this->decodificar);
		$res = $dbUbicacion ->ContarListaUbicacionFisica($cant,$puntero,$sortcol,$sortdir,$criterio_filtro);
		$this->salida = $dbUbicacion->salida;
		$this->query = $dbUbicacion->query;
		return $res;
	}
	function ValidarUbicacion($operacion_sql,$id_ubicacion,$codigo,$estado,$fecha_reg,$id_lugar, $ubicacion)
	{
		$this->salida = "";
		$dbUbicacion= new cls_DBUbicacion($this->decodificar);
		$res = $dbUbicacion ->ValidarUbicacion($operacion_sql,$id_ubicacion,$codigo,$estado,$fecha_reg,$id_lugar, $ubicacion);
		$this->salida = $dbUbicacion->salida;
		$this->query = $dbUbicacion->query;
		return $res;
	}
	function  CrearUbicacion($id_ubicacion,$codigo,$estado,$fecha_reg,$id_lugar, $ubicacion)
	{
		$this->salida = "";
		$dbUbicacion= new cls_DBUbicacion($this->decodificar);
		$res = $dbUbicacion ->CrearUbicacion($id_ubicacion,$codigo,$estado,$fecha_reg,$id_lugar, $ubicacion);
		$this->salida = $dbUbicacion->salida;
		$this->query = $dbUbicacion->query;
		return $res;
	}
	function ModificarUbicacion($id_ubicacion,$codigo,$estado,$fecha_reg,$id_lugar, $ubicacion)
	{
		$this->salida = "";
		$dbUbicacion= new cls_DBUbicacion($this->decodificar);
		$res = $dbUbicacion ->ModificarUbicacion($id_ubicacion,$codigo,$estado,$fecha_reg,$id_lugar, $ubicacion);
		$this->salida = $dbUbicacion->salida;
		$this->query = $dbUbicacion->query;
		return $res;
	}
	function EliminarUbicacion($id_ubicacion)
	{
		$this->salida = "";
		$dbUbicacion= new cls_DBUbicacion($this->decodificar);
		$res = $dbUbicacion ->EliminarUbicacion($id_ubicacion);
		$this->salida = $dbUbicacion->salida;
		$this->query = $dbUbicacion->query;
		return $res;
	}
	
	/****************Inicio  de las funciones nuevas 23/08/2013******************************/
	function PDFDetalleAsociacionTipoEP($cant,$puntero,$sortcol,$sortdir,$criterio_filtro)
	{
		$this->salida = "";
		$dbAsociacion = new cls_DBActivoFijoComprobante($this->decodificar);
		$res = $dbAsociacion->PDFDetalleAsociacionTipoEP($cant,$puntero,$sortcol,$sortdir,$criterio_filtro);
		$this->salida = $dbAsociacion ->salida;
		$this->query = $dbAsociacion ->query;
		return $res;
	}
	...
	{
		$this->salida = "";
		$dbAsociacion = new cls_DBTipoActivoCuenta($this->decodificar);
		$res = $dbAsociacion->CountTipoActivoCuenta($cant,$puntero,$sortcol,$sortdir,$criterio_filtro);
		$this->salida = $dbAsociacion ->salida;
		$this->query = $dbAsociacion ->query;
		return $res;
	}
	function ListarTipoActivoCuenta($cant,$puntero,$sortcol,$sortdir,$criterio_filtro)
	{
		$this->salida = "";
		$dbAsociacion = new cls_DBTipoActivoCuenta($this->decodificar);
		$res = $dbAsociacion->ListarTipoActivoCuenta($cant,$puntero,$sortcol,$sortdir,$criterio_filtro);
		$this->salida = $dbAsociacion ->salida;
		$this->query = $dbAsociacion ->query;
		return $res;
	}
	function EliminarTipoActivoCuenta($id_tipo_activo_cuenta)
	{
		$this->salida = "";
		$dbAsociacion = new cls_DBTipoActivoCuenta($this->decodificar);
		$res = $dbAsociacion->EliminarTipoActivoCuenta($id_tipo_activo_cuenta);
		$this->salida = $dbAsociacion ->salida;
		$this->query = $dbAsociacion ->query;
		return $res;
	}
	function InsertarTipoActivoCuenta($id_tipo_activo_cuenta,$id_tipo_activo,$codigo_programa,$descripcion_programa,$cuenta_activo,$cuenta_depacum,$cuenta_gasto,$cuenta_activo_auxiliar,$cuenta_dep_acumulada_auxiliar,$cuenta_gasto_auxiliar,$id_tension)
	{
		$this->salida = "";
		$dbAsociacion = new cls_DBTipoActivoCuenta($this->decodificar);
		$res = $dbAsociacion->InsertarTipoActivoCuenta($id_tipo_activo_cuenta,$id_tipo_activo,$codigo_programa,$descripcion_programa,$cuenta_activo,$cuenta_depacum,$cuenta_gasto,$cuenta_activo_auxiliar,$cuenta_dep_acumulada_auxiliar,$cuenta_gasto_auxiliar,$id_tension);
		$this->salida = $dbAsociacion ->salida;
		$this->query = $dbAsociacion ->query;
		return $res;
	}
	function ModificarTipoActivoCuenta($id_tipo_activo_cuenta,$id_tipo_activo,$codigo_programa,$descripcion_programa,$cuenta_activo,$cuenta_depacum,$cuenta_gasto,$cuenta_activo_auxiliar,$cuenta_dep_acumulada_auxiliar,$cuenta_gasto_auxiliar,$id_tension)
	{
		$this->salida = "";
		$dbAsociacion = new cls_DBTipoActivoCuenta($this->decodificar);
		$res = $dbAsociacion->ModificarTipoActivoCuenta($id_tipo_activo_cuenta,$id_tipo_activo,$codigo_programa,$descripcion_programa,$cuenta_activo,$cuenta_depacum,$cuenta_gasto,$cuenta_activo_auxiliar,$cuenta_dep_acumulada_auxiliar,$cuenta_gasto_auxiliar,$id_tension);
		$this->salida = $dbAsociacion ->salida;
		$this->query = $dbAsociacion ->query;
		return $res;
	}

	//funcion1 extra, listado de cuentas segun la gestion contable actual
	function ContarCuentasContablesGestion($cant,$puntero,$sortcol,$sortdir,$criterio_filtro)
	{
		$this->salida = "";
		$dbAsociacion = new cls_DBTipoActivoCuenta($this->decodificar);
		$res = $dbAsociacion->ContarCuentasContablesGestion($cant,$puntero,$sortcol,$sortdir,$criterio_filtro);
		$this->salida = $dbAsociacion ->salida;
		$this->query = $dbAsociacion ->query;
		return $res;
	}
	//funcion2 extra, listado de cuentas segun la gestion contable actual
	function ListarCuentasContablesGestion($cant,$puntero,$sortcol,$sortdir,$criterio_filtro)
	{
		$this->salida = "";
		$dbAsociacion = new cls_DBTipoActivoCuenta($this->decodificar);
		$res = $dbAsociacion->ListarCuentasContablesGestion($cant,$puntero,$sortcol,$sortdir,$criterio_filtro);
		$this->salida = $dbAsociacion ->salida;
		$this->query = $dbAsociacion ->query;
		return $res;
	}
	...
	{
		$this->salida = "";
		$dbContraCuenta= new cls_DBContraCuenta($this->decodificar);
		$res = $dbContraCuenta->CountContraCuenta($cant,$puntero,$sortcol,$sortdir,$criterio_filtro);
		$this->salida = $dbContraCuenta ->salida;
		$this->query = $dbContraCuenta ->query;
		return $res;
	}
	function ListarContraCuenta($cant,$puntero,$sortcol,$sortdir,$criterio_filtro)
	{
		$this->salida = "";
		$dbContraCuenta= new cls_DBContraCuenta($this->decodificar);
		$res = $dbContraCuenta->ListarContraCuenta($cant,$puntero,$sortcol,$sortdir,$criterio_filtro);
		$this->salida = $dbContraCuenta ->salida;
		$this->query = $dbContraCuenta ->query;
		return $res;
	}
	function InsertarContraCuenta($id_contra_cuenta,$id_gestion,$id_regional,$id_cuenta,$id_cuenta_aux,$id_proceso,$id_importe)
	{
		$this->salida = "";
		$dbContraCuenta= new cls_DBContraCuenta($this->decodificar);
		$res = $dbContraCuenta->InsertarContraCuenta($id_contra_cuenta,$id_gestion,$id_regional,$id_cuenta,$id_cuenta_aux,$id_proceso,$id_importe);
		$this->salida = $dbContraCuenta ->salida;
		$this->query = $dbContraCuenta ->query;
		return $res;
	}
	function ModificarContraCuenta($id_contra_cuenta,$id_gestion,$id_regional,$id_cuenta,$id_cuenta_aux,$id_proceso,$id_importe)
	{
		$this->salida = "";
		$dbContraCuenta= new cls_DBContraCuenta($this->decodificar);
		$res = $dbContraCuenta->ModificarContraCuenta($id_contra_cuenta,$id_gestion,$id_regional,$id_cuenta,$id_cuenta_aux,$id_proceso,$id_importe);
		$this->salida = $dbContraCuenta ->salida;
		$this->query = $dbContraCuenta ->query;
		return $res;
	}
	function EliminarContraCuenta($id_contra_cuenta)
	{
		$this->salida = "";
		$dbContraCuenta= new cls_DBContraCuenta($this->decodificar);
		$res = $dbContraCuenta->EliminarContraCuenta($id_contra_cuenta);
		$this->salida = $dbContraCuenta ->salida;
		$this->query = $dbContraCuenta ->query;
		return $res;
	}
	...
	{
		$this->salida = "";
		$dbActivoFijoComprobante= new cls_DBActivoFijoComprobante($this->decodificar);
		$res = $dbActivoFijoComprobante->CountActivoFijoComprobante($cant,$puntero,$sortcol,$sortdir,$criterio_filtro);
		$this->salida = $dbActivoFijoComprobante ->salida;
		$this->query = $dbActivoFijoComprobante ->query;
		return $res;
	}
	function ListarActivoFijoComprobante($cant,$puntero,$sortcol,$sortdir,$criterio_filtro)
	{
		$this->salida = "";
		$dbActivoFijoComprobante= new cls_DBActivoFijoComprobante($this->decodificar);
		$res = $dbActivoFijoComprobante->ListarActivoFijoComprobante($cant,$puntero,$sortcol,$sortdir,$criterio_filtro);
		$this->salida = $dbActivoFijoComprobante ->salida;
		$this->query = $dbActivoFijoComprobante ->query;
		return $res;
	}
	function InsertarActivoFijoComprobante($id_grupo_proceso)
	{
		$this->salida = "";
		$dbActivoFijoComprobante= new cls_DBActivoFijoComprobante($this->decodificar);
		$res = $dbActivoFijoComprobante->InsertarActivoFijoComprobante($id_grupo_proceso);
		$this->salida = $dbActivoFijoComprobante ->salida;
		$this->query = $dbActivoFijoComprobante ->query;
		return $res;
	}
	function EliminarActivoFijoComprobante($id_activo_fijo_comprobante)
	{
		$this->salida = "";
		$dbActivoFijoComprobante= new cls_DBActivoFijoComprobante($this->decodificar);
		$res = $dbActivoFijoComprobante->EliminarActivoFijoComprobante($id_activo_fijo_comprobante);
		$this->salida = $dbActivoFijoComprobante ->salida;
		$this->query = $dbActivoFijoComprobante ->query;
		return $res;
	}
	...
	function RegistrarComprobantesAlta($id_grupo_proceso)
	{
		$this->salida = "";
		$dbActivoFijoComprobante= new cls_DBActivoFijoComprobante($this->decodificar);
		$res = $dbActivoFijoComprobante->RegistrarComprobantesAlta($id_grupo_proceso);
		$this->salida = $dbActivoFijoComprobante ->salida;
		$this->query = $dbActivoFijoComprobante ->query;
		return $res;
	}
	function RegistrarComprobantesBaja($id_grupo_proceso)
	{
		$this->salida = "";
		$dbActivoFijoComprobante= new cls_DBActivoFijoComprobante($this->decodificar);
		$res = $dbActivoFijoComprobante->RegistrarComprobantesBaja($id_grupo_proceso);
		$this->salida = $dbActivoFijoComprobante ->salida;
		$this->query = $dbActivoFijoComprobante ->query;
		return $res;
	}
	...
	{
		$this->salida = "";
		$dbTipoActivoCuenta= new cls_DBTipoActivoCuenta($this->decodificar);
		$res = $dbTipoActivoCuenta->CountActivoFijoDistribucion($cant,$puntero,$sortcol,$sortdir,$criterio_filtro);
		$this->salida = $dbTipoActivoCuenta ->salida;
		$this->query = $dbTipoActivoCuenta ->query;
		return $res;
	}
	function ListarActivoFijoDistribucion($cant,$puntero,$sortcol,$sortdir,$criterio_filtro)
	{
		$this->salida = "";
		$dbTipoActivoCuenta= new cls_DBTipoActivoCuenta($this->decodificar);
		$res = $dbTipoActivoCuenta->ListarActivoFijoDistribucion($cant,$puntero,$sortcol,$sortdir,$criterio_filtro);
		$this->salida = $dbTipoActivoCuenta ->salida;
		$this->query = $dbTipoActivoCuenta ->query;
		return $res;
	}

	/*funciones para la modificacion del campo tension en actif.taf_activo_fijo**/
	function ModificarActivoFijoTension($id_activo_fijo,$tension,$tipo_bien)
	{
		$this->salida = "";
		$dbActivoFijo = new cls_DBActivoFijo($this->decodificar);
		$res = $dbActivoFijo ->ModificarActivoFijoTension($id_activo_fijo,$tension,$tipo_bien);
		$this->salida = $dbActivoFijo->salida;
		$this->query = $dbActivoFijo->query;
		return $res;
	}
	/*fin funciones para la modificacion del campo tension en actif.taf_activo_fijo_iud*/
	
	function PDFRespaldoDepreciacion($cant, $puntero, $sortdir, $sortcol, $criterio_filtro)
	{
		$this->salida = "";
		$dbAcFiDepreComp = new cls_DBActivoFijoDepreciacionComprobante($this->decodificar);
		$res = $dbAcFiDepreComp ->PDFRespaldoDepreciacion($cant, $puntero, $sortdir, $sortcol, $criterio_filtro);
		$this->salida = $dbAcFiDepreComp ->salida;
		$this->query = $dbAcFiDepreComp ->query;
		return $res;
	}
	...
	function ReporteActivoFijoFormulario605($cant, $puntero, $sortdir, $sortcol, $criterio_filtro)
	{
		$this->salida = "";
		$dbAcFiDepreComp = new cls_DBActivoFijoDepreciacionComprobante($this->decodificar);
		$res = $dbAcFiDepreComp ->ReporteActivoFijoFormulario605($cant, $puntero, $sortdir, $sortcol, $criterio_filtro);
		$this->salida = $dbAcFiDepreComp ->salida;
		$this->query = $dbAcFiDepreComp ->query;
		return $res;
	}
	...
	//añadido 02/05/2014
	//grupo depreciacion contabilizado
	function ContarDepreciacion2Contabilizado($cant ,$puntero,$sortcol,$sortdir,$criterio_filtro)
	{
		$this->salida = "";
		$dbDepreciacion = new cls_DBDepreciacion($this->decodificar);
		$res = $dbDepreciacion ->ContarDepreciacion2Contabilizado($cant ,$puntero,$sortcol,$sortdir,$criterio_filtro);
		$this->salida = $dbDepreciacion ->salida;
		$this->query = $dbDepreciacion ->query;
		return $res;
	}
	function ListarDepreciacion2Contabilizado($cant,$puntero,$sortcol,$sortdir,$criterio_filtro)
	{
		$this->salida = "";
		$dbDepreciacion = new cls_DBDepreciacion($this->decodificar);
		$res = $dbDepreciacion ->ListarDepreciacion2Contabilizado($cant ,$puntero,$sortcol,$sortdir,$criterio_filtro);
		$this->salida = $dbDepreciacion ->salida;
		$this->query = $dbDepreciacion ->query;
		return $res;
	}
	
	//fin depreciacion contabilizado
	
	function ReporteActivosFijosDepreciacion2($cant, $puntero, $sortdir, $sortcol, $criterio_filtro)
	{
		$this->salida = "";
		$dbAcFiDeprec = new cls_DBActivoFijoDepreciacionComprobante($this->decodificar);
		$res = $dbAcFiDeprec ->ReporteActivosFijosDepreciacion2($cant, $puntero, $sortdir, $sortcol, $criterio_filtro);
		$this->salida = $dbAcFiDeprec ->salida;
		$this->query = $dbAcFiDeprec ->query;
		return $res;
	}
	//detalle histrico depreciacion activo
	function ActionPDFActivoFijoHistorialDepreciaciones($cant,$puntero,$sortcol,$sortdir,$criterio_filtro)
	{
		$this->salida = "";
		$dbDepreciacion = new cls_DBDepreciacion($this->decodificar);
		$res = $dbDepreciacion ->ActionPDFActivoFijoHistorialDepreciaciones($cant,$puntero,$sortcol,$sortdir,$criterio_filtro);
		$this->salida = $dbDepreciacion ->salida;
		$this->query = $dbDepreciacion ->query;
		return $res;
	}
	...
	{
		$this->salida = "";
		$dbDetallePlanCtas = new cls_DBPlanCuentas($this->decodificar);
		$res = $dbDetallePlanCtas->ListarPlanCuentasArb($cant, $puntero, $sortcol, $sortdir, $criterio_filtro, $id_financiador, $id_regional, $id_programa, $id_proyecto, $id_actividad);
		$this->salida = $dbDetallePlanCtas ->salida;
		$this->query = $dbDetallePlanCtas ->query;
		return $res;
	}
	function InsertarPlanCuentas($id_plan_ctas,$id_plan_ctas_fk,$codigo,$descripcion,$estado,$id_tipo,$id_cta_activo,$id_aux_activo,$id_depacum,$id_aux_depacum,$id_gasto,$id_aux_gasto,$id_gestion,$nivel,$programa,$tension,$tipo_bien)
	{
		$this->salida = "";
		$dbDetallePlanCtas = new cls_DBPlanCuentas($this->decodificar);
		$res = $dbDetallePlanCtas->InsertarPlanCuentas($id_plan_ctas,$id_plan_ctas_fk,$codigo,$descripcion,$estado,$id_tipo,$id_cta_activo,$id_aux_activo,$id_depacum,$id_aux_depacum,$id_gasto,$id_aux_gasto,$id_gestion,$nivel,$programa,$tension,$tipo_bien);
		$this->salida = $dbDetallePlanCtas ->salida;
		$this->query = $dbDetallePlanCtas ->query;
		return $res;
	}
	function ModificarPlanCuentas($id_plan_ctas,$id_plan_ctas_fk,$codigo,$descripcion,$estado,$id_tipo,$id_cta_activo,$id_aux_activo,$id_depacum,$id_aux_depacum,$id_gasto,$id_aux_gasto,$id_gestion,$nivel,$programa,$tension,$tipo_bien)
	{
		$this->salida = "";
		$dbDetallePlanCtas = new cls_DBPlanCuentas($this->decodificar);
		$res = $dbDetallePlanCtas->ModificarPlanCuentas($id_plan_ctas,$id_plan_ctas_fk,$codigo,$descripcion,$estado,$id_tipo,$id_cta_activo,$id_aux_activo,$id_depacum,$id_aux_depacum,$id_gasto,$id_aux_gasto,$id_gestion,$nivel,$programa,$tension,$tipo_bien);
		$this->salida = $dbDetallePlanCtas ->salida;
		$this->query = $dbDetallePlanCtas ->query;
		return $res;
	}
	function EliminarPlanCuentas($id_plan_cuentas)
	{
		$this->salida = "";
		$dbDetallePlanCtas = new cls_DBPlanCuentas($this->decodificar);
		$res = $dbDetallePlanCtas->EliminarPlanCuentas($id_plan_cuentas);
		$this->salida = $dbDetallePlanCtas ->salida;
		$this->query = $dbDetallePlanCtas ->query;
		return $res;
	}
	function ReportePlanCuentas($cant, $puntero, $sortcol, $sortdir, $criterio_filtro)
	{
		$this->salida = "";
		$dbDetallePlanCtas = new cls_DBPlanCuentas($this->decodificar);
		$res = $dbDetallePlanCtas->ReportePlanCuentas($cant, $puntero, $sortcol, $sortdir, $criterio_filtro);
		$this->salida = $dbDetallePlanCtas ->salida;
		$this->query = $dbDetallePlanCtas ->query;
		return $res;
	}
	...
	function ReporteTipoSubtipoActivoFijo($cant, $puntero, $sortdir, $sortcol, $criterio_filtro)
	{
		$this->salida = "";
		$this->query = "";
		$dbTipoActivo = new cls_DBActivoFijoReporte($this->decodificar);
		$res = $dbTipoActivo->ReporteTipoSubtipoActivoFijo($cant, $puntero, $sortdir, $sortcol, $criterio_filtro);
		$this->salida = $dbTipoActivo->salida;
		$this->query = $dbTipoActivo->query;
		return $res;
	}
	...
	function ContarObservacionesVarias($cant,$puntero,$sortcol,$sortdir,$criterio_filtro)
	{
		$this->salida = "";
		$this->query = "";
		$dbTipoActivo = new cls_DBObservacionesVarias($this->decodificar);
		$res = $dbTipoActivo->ContarObservacionesVarias($cant,$puntero,$sortcol,$sortdir,$criterio_filtro);
		$this->salida = $dbTipoActivo->salida;
		$this->query = $dbTipoActivo->query;
		return $res;
	}
	function ListarObservacionesVarias($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$maestro_id_activo_fijo)
	{
		$this->salida = "";
		$this->query = "";
		$dbTipoActivo = new cls_DBObservacionesVarias($this->decodificar);
		$res = $dbTipoActivo->ListarObservacionesVarias($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$maestro_id_activo_fijo);
		$this->salida = $dbTipoActivo->salida;
		$this->query = $dbTipoActivo->query;
		return $res;
	}
	function ValidarObservacionesVarias($sql, $id_obs_var, $txt_observaciones, $txt_estado)
	{
		$this->salida = "";
		$this->query = "";
		$dbTipoActivo = new cls_DBObservacionesVarias($this->decodificar);
		$res = $dbTipoActivo->ValidarObservacionesVarias($sql, $id_obs_var, $txt_observaciones, $txt_estado);
		$this->salida = $dbTipoActivo->salida;
		$this->query = $dbTipoActivo->query;
		return $res;
	}
	function InsertarObservacionesVarias($id_obs_var,$id_activo,$txt_observaciones, $txt_estado)
	{
		...
		$this->query = "";
		$dbTipoActivo = new cls_DBObservacionesVarias($this->decodificar);
		$res = $dbTipoActivo->InsertarObservacionesVarias($id_obs_var,$id_activo,$txt_observaciones, $txt_estado);
		$this->salida = $dbTipoActivo->salida;
		$this->query = $dbTipoActivo->query;
		return $res;
	}
	function ModificarObservacionesVarias($id_obs_var, $id_activo, $txt_observaciones, $txt_estado)
	{
		...
		$this->query = "";
		$dbTipoActivo = new cls_DBObservacionesVarias($this->decodificar);
		$res = $dbTipoActivo->ModificarObservacionesVarias($id_obs_var, $id_activo, $txt_observaciones, $txt_estado);
		$this->salida = $dbTipoActivo->salida;
		$this->query = $dbTipoActivo->query;
		return $res;
	}
	function EliminarObservacionesVarias($id_observacion)
	{
		...
		$this->query = "";
		$dbTipoActivo = new cls_DBObservacionesVarias($this->decodificar);
		$res = $dbTipoActivo->EliminarObservacionesVarias($id_observacion);
		$this->salida = $dbTipoActivo->salida;
		$this->query = $dbTipoActivo->query;
		return $res;
	}
	function ContarControlObservacionesVarios($cant,$puntero,$sortcol,$sortdir,$criterio_filtro)
	{
		...
		$this->query = "";
		$dbTipoActivo = new cls_DBObservacionesVarias($this->decodificar);
		$res = $dbTipoActivo->ContarControlObservacionesVarios($cant,$puntero,$sortcol,$sortdir,$criterio_filtro);
		$this->salida = $dbTipoActivo->salida;
		$this->query = $dbTipoActivo->query;
		return $res;
	}
	function ListarControlObservacionesVarios($cant,$puntero,$sortcol,$sortdir,$criterio_filtro)
	{
		...
		$this->query = "";
		$dbTipoActivo = new cls_DBObservacionesVarias($this->decodificar);
		$res = $dbTipoActivo->ListarControlObservacionesVarios($cant,$puntero,$sortcol,$sortdir,$criterio_filtro);
		$this->salida = $dbTipoActivo->salida;
		$this->query = $dbTipoActivo->query;
		return $res;
	}
	...
	function ContarDepreciacionGestion($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad)
	{
		$this->salida = "";
		$this->query = "";
		$dbDepreciacionGestion = new cls_DBDepreciacionGestion($this->decodificar);
		$res = $dbDepreciacionGestion->ContarDepreciacionGestion($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);
		$this->salida = $dbDepreciacionGestion->salida;
		$this->query = $dbDepreciacionGestion->query;
		return $res;
	}
	function ListarDepreciacionGestion($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad)
	{
		...
		$this->query = "";
		$dbDepreciacionGestion = new cls_DBDepreciacionGestion($this->decodificar);
		$res = $dbDepreciacionGestion->ListarDepreciacionGestion($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);
		$this->salida = $dbDepreciacionGestion->salida;
		$this->query = $dbDepreciacionGestion->query;
		return $res;
	}
	function InsertarDepreciacionGestion($id_gestion_ini,$id_gestion_fin,$proyecto,$id_depto,$estado,$mes_ini,$mes_fin,$id_proyecto)
	{
		...
		$this->query = "";
		$dbDepreciacionGestion = new cls_DBDepreciacionGestion($this->decodificar);
		$res = $dbDepreciacionGestion->InsertarDepreciacionGestion($id_gestion_ini,$id_gestion_fin,$proyecto,$id_depto,$estado,$mes_ini,$mes_fin,$id_proyecto);
		$this->salida = $dbDepreciacionGestion->salida;
		$this->query = $dbDepreciacionGestion->query;
		return $res;
	}
	function ModificarDepreciacionGestion($id_depreciacion_gestion,$id_gestion_ini,$id_gestion_fin,$proyecto,$id_depto,$estado,$mes_ini,$mes_fin,$id_proyecto)
	{
		...
		$this->query = "";
		$dbDepreciacionGestion = new cls_DBDepreciacionGestion($this->decodificar);
		$res = $dbDepreciacionGestion->ModificarDepreciacionGestion($id_depreciacion_gestion,$id_gestion_ini,$id_gestion_fin,$proyecto,$id_depto,$estado,$mes_ini,$mes_fin,$id_proyecto);
		$this->salida = $dbDepreciacionGestion->salida;
		$this->query = $dbDepreciacionGestion->query;
		return $res;
	}
	function EliminarDepreciacionGestion($id_depreciacion_gestion)
	{
		...
		$this->query = "";
		$dbDepreciacionGestion = new cls_DBDepreciacionGestion($this->decodificar);
		$res = $dbDepreciacionGestion->EliminarDepreciacionGestion($id_depreciacion_gestion);
		$this->salida = $dbDepreciacionGestion->salida;
		$this->query = $dbDepreciacionGestion->query;
		return $res;
	}
	function ReporteDepreciacionGestionActivosFijos($cant, $puntero, $sortdir, $sortcol, $criterio_filtro)
	{
		...
		$this->query = "";
		$dbDepreciacionGestion = new cls_DBDepreciacionGestion($this->decodificar);
		$res = $dbDepreciacionGestion->ReporteDepreciacionGestionActivosFijos($cant, $puntero, $sortdir, $sortcol, $criterio_filtro);
		$this->salida = $dbDepreciacionGestion->salida;
		$this->query = $dbDepreciacionGestion->query;
		return $res;
	}
	function ContarDepreciacionGestionDetalle($cant ,$puntero,$sortcol,$sortdir,$criterio_filtro)
	{
		$this->salida = "";
		$this->query = "";
		$dbDepreciacionGestion = new cls_DBDepreciacionGestionDetalle($this->decodificar);
		$res = $dbDepreciacionGestion->ContarDepreciacionGestionDetalle($cant ,$puntero,$sortcol,$sortdir,$criterio_filtro);
		$this->salida = $dbDepreciacionGestion->salida;
		$this->query = $dbDepreciacionGestion->query;
		return $res;
	}
	function ListarDepreciacionGestionDetalle($cant,$puntero,$sortcol,$sortdir,$criterio_filtro)
	{
		...
		$this->query = "";
		$dbDepreciacionGestion = new cls_DBDepreciacionGestionDetalle($this->decodificar);
		$res = $dbDepreciacionGestion->ListarDepreciacionGestionDetalle($cant,$puntero,$sortcol,$sortdir,$criterio_filtro);
		$this->salida = $dbDepreciacionGestion->salida;
		$this->query = $dbDepreciacionGestion->query;
		return $res;
	}
	function EliminarDepreciacionGestionDetalle($id_depreciacion_gestion_detalle)
	{
		...
		$this->query = "";
		$dbDepreciacionGestion = new cls_DBDepreciacionGestionDetalle($this->decodificar);
		$res = $dbDepreciacionGestion->EliminarDepreciacionGestionDetalle($id_depreciacion_gestion_detalle);
		$this->salida = $dbDepreciacionGestion->salida;
		$this->query = $dbDepreciacionGestion->query;
		return $res;
	}
	...
	function ReporteActivosFijosRespaldoCbtesBaja($cant, $puntero, $sortdir, $sortcol, $criterio_filtro)
	{
		$this->salida = "";
		$dbActivoFijoProceso = new cls_DBActivoFijoProceso($this->decodificar);
		$res = $dbActivoFijoProceso->ReporteActivosFijosRespaldoCbtesBaja($cant, $puntero, $sortdir, $sortcol, $criterio_filtro);
		$this->salida = $dbActivoFijoProceso->salida;
		$this->query = $dbActivoFijoProceso->query;
		return $res;
	}
	...
	{
		$this->salida = "";
		$dbCbteDepto = new cls_DBCbteDepto($this->decodificar);
		$res = $dbCbteDepto->ContarCbteDepto($cant,$puntero,$sortcol,$sortdir,$criterio_filtro);
		$this->salida = $dbCbteDepto->salida;
		$this->query = $dbCbteDepto->query;
		return $res;
	}
	...
	function ListarCbteDepto($cant,$puntero,$sortcol,$sortdir,$criterio_filtro)
	{
		$this->salida = "";
		$dbCbteDepto = new cls_DBCbteDepto($this->decodificar);
		$res = $dbCbteDepto->ListarCbteDepto($cant,$puntero,$sortcol,$sortdir,$criterio_filtro);
		$this->salida = $dbCbteDepto->salida;
		$this->query = $dbCbteDepto->query;
		return $res;
	}
	...
	function InsertarCbteDepto($id_depto,$descripcion,$id_regional)
	{
		$this->salida = "";
		$dbCbteDepto = new cls_DBCbteDepto($this->decodificar);
		$res = $dbCbteDepto->InsertarCbteDepto($id_depto,$descripcion,$id_regional);
		$this->salida = $dbCbteDepto->salida;
		$this->query = $dbCbteDepto->query;
		return $res;
	}
	...
	function ModificarCbteDepto($id,$id_depto,$descripcion,$id_regional)
	{
		$this->salida = "";
		$dbCbteDepto = new cls_DBCbteDepto($this->decodificar);
		$res = $dbCbteDepto->ModificarCbteDepto($id,$id_depto,$descripcion,$id_regional);
		$this->salida = $dbCbteDepto->salida;
		$this->query = $dbCbteDepto->query;
		return $res;
	}
	...
	function ContarRegionalCbte($cant,$puntero,$sortcol,$sortdir,$criterio_filtro)
	{
		$this->salida = "";
		$dbCbteDepto = new cls_DBCbteDepto($this->decodificar);
		$res = $dbCbteDepto->ContarRegionalCbte($cant,$puntero,$sortcol,$sortdir,$criterio_filtro);
		$this->salida = $dbCbteDepto->salida;
		$this->query = $dbCbteDepto->query;
		return $res;
	}
	
	function ListarRegionalCbte($cant,$puntero,$sortcol,$sortdir,$criterio_filtro)
	{
		$this->salida = "";
		$dbCbteDepto = new cls_DBCbteDepto($this->decodificar);
		$res = $dbCbteDepto->ListarRegionalCbte($cant,$puntero,$sortcol,$sortdir,$criterio_filtro);
		$this->salida = $dbCbteDepto->salida;
		$this->query = $dbCbteDepto->query;
		return $res;
	}
	...
	function EliminarCbteDepto($id)
	{
		$this->salida = "";
		$dbCbteDepto = new cls_DBCbteDepto($this->decodificar);
		$res = $dbCbteDepto->EliminarCbteDepto($id);
		$this->salida = $dbCbteDepto->salida;
		$this->query = $dbCbteDepto->query;
		return $res;
	}
	...
	{
		$this->salida = "";
		$dbCbteDeptoDetalle = new cls_DBCbteDeptoDetalle($this->decodificar);
		$res = $dbCbteDeptoDetalle->ContarCbteDeptoDetalle($cant,$puntero,$sortcol,$sortdir,$criterio_filtro);
		$this->salida = $dbCbteDeptoDetalle->salida;
		$this->query = $dbCbteDeptoDetalle->query;
		return $res;
	}
	...
	function ListarCbteDeptoDetalle($cant,$puntero,$sortcol,$sortdir,$criterio_filtro)
	{
		$this->salida = "";
		$dbCbteDeptoDetalle = new cls_DBCbteDeptoDetalle($this->decodificar);
		$res = $dbCbteDeptoDetalle->ListarCbteDeptoDetalle($cant,$puntero,$sortcol,$sortdir,$criterio_filtro);
		$this->salida = $dbCbteDeptoDetalle->salida;
		$this->query = $dbCbteDeptoDetalle->query;
		return $res;
	}
	...
	function InsertarCbteDeptoDetalle($id_cbte,$descripcion,$id_ep)
	{
		$this->salida = "";
		$dbCbteDeptoDetalle = new cls_DBCbteDeptoDetalle($this->decodificar);
		$res = $dbCbteDeptoDetalle->InsertarCbteDeptoDetalle($id_cbte,$descripcion,$id_ep);
		$this->salida = $dbCbteDeptoDetalle->salida;
		$this->query = $dbCbteDeptoDetalle->query;
		return $res;
	}
	...
	function ModificarCbteDeptoDetalle($id,$id_cbte,$descripcion,$id_ep)
	{
		$this->salida = "";
		$dbCbteDeptoDetalle = new cls_DBCbteDeptoDetalle($this->decodificar);
		$res = $dbCbteDeptoDetalle->ModificarCbteDeptoDetalle($id,$id_cbte,$descripcion,$id_ep);
		$this->salida = $dbCbteDeptoDetalle->salida;
		$this->query = $dbCbteDeptoDetalle->query;
		return $res;
	}
	...
	function EliminarCbteDeptoDetalle($id)
	{
		$this->salida = "";
		$dbCbteDeptoDetalle = new cls_DBCbteDeptoDetalle($this->decodificar);
		$res = $dbCbteDeptoDetalle->EliminarCbteDeptoDetalle($id);
		$this->salida = $dbCbteDeptoDetalle->salida;
		$this->query = $dbCbteDeptoDetalle->query;
		return $res;
	}
	...
	{
		$this->salida = "";
		$dbCbteDeptoDetalle = new cls_DBDepreciacionDepto($this->decodificar);
		$res = $dbCbteDeptoDetalle->ListarDepreciacionDepto($cant,$puntero,$sortcol,$sortdir,$criterio_filtro);
		$this->salida = $dbCbteDeptoDetalle->salida;
		$this->query = $dbCbteDeptoDetalle->query;
		return $res;
	}
	...
	function ListarDepreciacionDeptoDetalle($cant,$puntero,$sortcol,$sortdir,$criterio_filtro)
	{
		$this->salida = "";
		$dbCbteDeptoDetalle = new cls_DBDepreciacionDepto($this->decodificar);
		$res = $dbCbteDeptoDetalle->ListarDepreciacionDeptoDetalle($cant,$puntero,$sortcol,$sortdir,$criterio_filtro);
		$this->salida = $dbCbteDeptoDetalle->salida;
		$this->query = $dbCbteDeptoDetalle->query;
		return $res;
	}
	...
	function countDepreciacionDeptoDetalle($cant,$puntero,$sortcol,$sortdir,$criterio_filtro)
	{
		$this->salida = "";
		$dbCbteDeptoDetalle = new cls_DBDepreciacionDepto($this->decodificar);
		$res = $dbCbteDeptoDetalle->countDepreciacionDeptoDetalle($cant,$puntero,$sortcol,$sortdir,$criterio_filtro);
		$this->salida = $dbCbteDeptoDetalle->salida;
		$this->query = $dbCbteDeptoDetalle->query;
		return $res;
	}
	...
	function PDFRespaldoDepreciacionDepto($cant, $puntero, $sortdir, $sortcol, $criterio_filtro)
	{
		$this->salida = "";
		$dbCbteDeptoDetalle = new cls_DBDepreciacionDepto($this->decodificar);
		$res = $dbCbteDeptoDetalle->PDFRespaldoDepreciacionDepto($cant, $puntero, $sortdir, $sortcol, $criterio_filtro);
		$this->salida = $dbCbteDeptoDetalle->salida;
		$this->query = $dbCbteDeptoDetalle->query;
		return $res;
	}
	...
	function obtenerCabeceraDepreciacionDepto($cant, $puntero, $sortdir, $sortcol, $criterio_filtro)
	{
		$this->salida = "";
		$dbCbteDeptoDetalle = new cls_DBDepreciacionDepto($this->decodificar);
		$res = $dbCbteDeptoDetalle->obtenerCabeceraDepreciacionDepto($cant, $puntero, $sortdir, $sortcol, $criterio_filtro);
		$this->salida = $dbCbteDeptoDetalle->salida;
		$this->query = $dbCbteDeptoDetalle->query;
		return $res;
	}
	...
	function RegistrarDepreciacionDepto($id_grupo_dep,$id_deprec_depto)
	{
		$this->salida = "";
		$dbCbteDeptoDetalle = new cls_DBDepreciacionDepto($this->decodificar);
		$res = $dbCbteDeptoDetalle->RegistrarDepreciacionDepto($id_grupo_dep,$id_deprec_depto);
		$this->salida = $dbCbteDeptoDetalle->salida;
		$this->query = $dbCbteDeptoDetalle->query;
		return $res;
	}
