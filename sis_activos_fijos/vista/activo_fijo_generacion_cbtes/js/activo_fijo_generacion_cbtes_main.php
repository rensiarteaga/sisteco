<?php
/**
 * Nombre:		  	    activo_fijo_generacion_cbtes_main.php
...
var maestro={id_grupo_proceso:<?php echo $maestro_id_grupo_proceso;?>,proceso:'<?php echo $maestro_proceso;?>'};
idContenedorPadre='<?php echo $idContenedorPadre;?>';
var elemento={idContenedor:idContenedor,pagina:new pagina_generacion_cbtes(idContenedor,direccion,paramConfig,maestro,idContenedorPadre)};
...
/**
* Nombre:				activo_fijo_generacion_cbtes.js  	  
* Propósito: 			pagina objeto principal		
* Autor:				Elmer Velasquez	
* Fecha creación:		01/02/2013
*/
function pagina_generacion_cbtes(idContenedor,direccion,paramConfig,maestro,idContenedorPadre)
{
	...
	var ds =  new Ext.data.Store({
		...
		proxy: new Ext.data.HttpProxy({url: direccion+'../../../control/activo_fijo_generacion_cbtes/ActionListarActivoFijoComprobante.php'}),
		...
		'estado',
		'tipo_comprobante','monto_actual','id_tipo_activo_cuenta','id_contra_cuenta',
		'depto_contable' 
		]),
	...
	/*Atributos[6] = {
			validacion:{
				...
				allowBlank:false,			
				emptyText:'Origen...',
				desc: 'presupuesto', //indica la columna del store principal ds del que proviane la descripcion
			...
			form:true,
			filtro_0:true,
			//filterColValue:'tab_4.presupuesto',
			id_grupo:0
		};*/
	/*Atributos[7] = {
			validacion:{
	...
	var paramFunciones={
			btnEliminar:{url:direccion+'../../../control/activo_fijo_generacion_cbtes/ActionEliminarActivoFijoComprobante.php'},
			Formulario:{
	...
	this.btnEdit=function(){
		...
		function btn_reporte_asociacion_af()
		{
			...
			var SelectionsRecord=sm.getSelected();	
				...
				{	
					...
					window.open(direccion+'../../../control/activo_fijo_generacion_cbtes/reporte/ActionPDFDetalleAsociacionTipoEP.php?'+data);
				}
				else
				{
				...
				{	
					...
					{	
						if(confirm('¿Esta Seguro de Generar los Comprobantes?'))
						{
							...
							Ext.Ajax.request({
							url:direccion+"../../../control/activo_fijo_generacion_cbtes/generacion_comprobantes/ActionGenerarComprobantesAlta.php",																									
							method:'POST',
		...
		function btnComprobantesBaja()
		{
			...
			if(NumSelect==1)
			{
				...
				if(confirm(confirmacion))
				{
					...
					Ext.Ajax.request({
						url:direccion+"../../../control/activo_fijo_generacion_cbtes/generacion_comprobantes/ActionGenerarComprobantesBaja.php",
						method:'POST',
						params:{cantidad_ids:'1',id_grupo_proceso:SelectionsRecord.data.id_grupo_proceso},
						success:esteSuccess,
						failure:ClaseMadre_conexionFailure,
		...
		function enable(sm,row,rec)
		{
			...
			if(aux.substring(0,4)=='ALTA')
			{
				...
				CM_getBoton('genComprobantesBaja-'+idContenedor).disable();
				CM_getBoton('genComprobantes-'+idContenedor).enable();
			}
			else
			{
				if(aux.substring(0,4)=='BAJA')
				{
					CM_getBoton('genComprobantes-'+idContenedor).disable();
					CM_getBoton('genComprobantesBaja-'+idContenedor).enable();
				}
			}
