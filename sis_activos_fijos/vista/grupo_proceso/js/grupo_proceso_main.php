<?php 
/**
 * Nombre:		  	    grupo_proceso_main.php
 * Propósito: 			pagina que arranca la configuracion de la vista
 * Autor:				Mercedes Zambrana Meneses
 * Fecha creación:		2010-07-07
 *
 */
session_start();
?>
...
function main(){
    ...
    echo "var estado_pagina='$estado_pagina';"
	?>
var fa=false;
<?php if($_SESSION["ss_filtro_avanzado"]!=''){echo 'fa='.$_SESSION["ss_filtro_avanzado"].';';}?>
var paramConfig={TamanoPagina:_CP.getConfig().ss_tam_pag,TiempoEspera:_CP.getConfig().ss_tiempo_espera,CantFiltros:1,FiltroEstructura:false,FiltroAvanzado:fa};
idContenedorPadre='<?php echo $idContenedorPadre;?>';
var elemento={pagina:new pagina_grupo_proceso(idContenedor,direccion,paramConfig, usuario,estado_pagina),idContenedor:idContenedor};
...
function pagina_grupo_proceso(idContenedor,direccion,paramConfig,usuario,estado_pagina){
	...
	Atributos[11]={
			...
			queryParam: 'filterValue_0',
			filterCol:'presup.desc_presupuesto#PRESUP.desc_epe#PRESUP.nombre_unidad',
			typeAhead:false,
...
/* rac 17/11/2016 se comenta para que deje pasar sin generar comprobantes
		...
		if(estado_pagina=='borrador'){
			...
			this.AdicionarBoton('../../../lib/imagenes/report.png','Comprobantes Activos Fijos',btn_respaldoCbte_pdf,true,'reporteCbtePDF','Respaldo Cbte.');
		}
		else if(estado_pagina=='en_prestamo'){
			this.AdicionarBoton('../../../lib/imagenes/det.ico','Solicitar Correccion',btn_correccion,true,'corregir_proceso','Correccion');
		...
		function btn_respaldoCbte_pdf()
		{
			...
			var NumSelect=sm.getCount();
					...
					if(NumSelect==1){
						...
						var aux=new String(SelectionsRecord.data['codigo_proceso']);
						var data='m_id_grupo_proceso='+SelectionsRecord.data.id_grupo_proceso;
						data=data+'&txt_proceso='+aux.substring(0,4);
						data=data+'&txt_id_proceso='+SelectionsRecord.data.id_proceso;
						data=data+'&fecha_proceso='+SelectionsRecord.data.fecha_contabilizacion;
						data=data+'&departamento='+SelectionsRecord.data.desc_depto_ori;
						data=data+'&descripcion='+SelectionsRecord.data.descripcion;
						window.open(direccion+'../../../../sis_activos_fijos/control/_reportes/respaldo_cbtes/ActionPDFRespaldoCbtes.php?tipo_reporte=pdf&'+data);
					}
					else if(NumSelect>1) {
						Ext.MessageBox.alert('Estado', 'Tiene mas de un registro seleccionado.');
					}	
		...
		function enable(sm,row,rec){
			...
			if(estado_pagina=='finalizado')
			{
				if(	aux.substring(0,4)=='BAJA' || aux.substring(0,4)=='ALTA')
					CM_getBoton('reporteCbtePDF-'+idContenedor).enable();
				else
					CM_getBoton('reporteCbtePDF-'+idContenedor).disable();
			}		
		}
