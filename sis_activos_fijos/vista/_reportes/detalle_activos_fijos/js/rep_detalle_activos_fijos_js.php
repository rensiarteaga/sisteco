//<script>
function GenerarReporteDetalleActivosFijos()
{	var vectorAtributos = new Array;
   ...
   ds_estado_funcional = new Ext.data.Store({
	// asigna url de donde se cargaran los datos
	proxy: new Ext.data.HttpProxy({url: '../../../control/estado_funcional/ActionListaEstadoFuncional.php?origen=filtro'}),
	reader: new Ext.data.XmlReader({
		record: 'ROWS',
   ...
   /////////// txt codigo//////
	...
	vectorAtributos[7] = paramId_unidad_organizacional;
...
vectorAtributos[8] = paramId_estado_funcional;
...
var paramEstado = {
	validacion: {
		...
		loadMask: true,
		triggerAction: 'all',
		//store: new Ext.data.SimpleStore({fields: ['id', 'desc'],data : Ext.rep_detalle_activos_fijosCombo.estado }),
		store: new Ext.data.SimpleStore({fields: ['id', 'desc'],data : [
		                                                                ['%', 'Todos'],
...
vectorAtributos[10] = paramFecha_compra1;
////////// txt fecha_compra//////
var paramFecha_compra2 = {
	validacion:{
		name: 'fecha_compra',
...
vectorAtributos[11] = paramFecha_compra2;


var paramUbicacionFisica = {
		validacion:{
	...
	vectorAtributos[12] = paramUbicacionFisica;
...
function formatDate(value){
	...
	// ---------- Inicia Layout ---------------//
	var config = {
		titulo_maestro:"Parámetros Para el Detalle de Activos "
	};
	...
	ds_estado_funcional.addListener('loadexception',  ClaseMadre_conexionFailure);
	...
	function iniciarEventosFormularios()
	{
		...
		combo_estado_funcional = ClaseMadre_getComponente('id_estado_funcional');
	...
	{	var titulo = "Reporte Detalle de Activos Fijos "+ ContPes;
		ContPes ++;
		return titulo;
	}
	...
	var paramFunciones = {
		...
		Formulario:{
			...
			url:'../../../control/_reportes/activo_fijo_detalle/ActionPDFActivoFijoDetalle.php',
...
{	//obj_proceso = new ProcesoDepreciacion();
	obj_pagina = new GenerarReporteDetalleActivosFijos();
}
