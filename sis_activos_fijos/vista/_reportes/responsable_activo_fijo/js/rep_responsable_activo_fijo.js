/**
 * Nombre:		  	    GenerarReporteAsignacionActivoFijo
   ...
   //proxy:new Ext.data.HttpProxy({url:direccion+'../../../../../sis_parametros/control/depto/ActionListarDepartamento.php?oc=si'}),
	...
	vectorAtributos[0]={
		validacion:{
			...
			loadMask:true,
			triggerAction:'all',
			store:new Ext.data.SimpleStore({fields:['id','valor'],data:[['0','Item'],['1','Clasificación']]}),
			onSelect: function(record){ClaseMadre_getComponente('sw_item_clasificacion').setValue(record.data.id);
				...
				CM_ocultarGrupo('Clasificación Item');
				SiBlancosGrupo(1);
				NoBlancosGrupo(2);
...
//	vectorAtributos[6] = param_id_item;
	...
	layout_resp_activos_fijos=new DocsLayoutProceso(idContenedor);
	layout_resp_activos_fijos.init(config);
	////////////////////////
	// INICIAMOS HERENCIA //
	////////////////////////
	...
	this.pagina(paramConfig,vectorAtributos,layout_resp_activos_fijos,idContenedor);
   ...
   //para que los hijos puedan ajustarse al tamaño
	this.getLayout=function(){return layout_resp_activos_fijos.getLayout();};
