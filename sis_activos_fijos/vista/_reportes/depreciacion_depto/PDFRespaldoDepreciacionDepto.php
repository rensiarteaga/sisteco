...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$cad= '  GESTION :  '.$_SESSION['cabecera_respaldo_deptocbte'][1];
		$this->SetX(90);
		$this->SetFont('Arial','B',6);
		$this->Cell(6,3,'  PERIODO :  '.$_SESSION['cabecera_respaldo_deptocbte'][0].'     '.$cad,0,0,'L');
		...
		$this->SetFont('Arial','',6);
		$this->Cell(50,3,utf8_decode($_SESSION['cabecera_respaldo_deptocbte'][5]),0,1,'L');
		...
		$this->SetFont('Arial','',6);
		$this->Cell(50,3,utf8_decode($_SESSION["cabecera_respaldo_deptocbte"][4]),0,1,'L');	 
	
		$this->SetY($y_1);
		$this->SetX(120);
		$this->SetFont('Arial','B',6);
		$this->Cell(20,3,'Departamento:',0,0,'L');
		$this->SetFont('Arial','',6);
		//$this->Cell(50,3,utf8_decode($_SESSION['cabecera_respaldo_deptocbte'][3]),0,1,'L');
		$this->Cell(60,3,$_SESSION['cabecera_respaldo_deptocbte'][6],0,1,'L'); 
		 
		$this->SetX(120);
		$this->SetFont('Arial','B',6);
		$this->Cell(20,3,'Usuario: ',0,0,'L');
		$this->SetFont('Arial','',6);
		$this->Cell(60,3,utf8_decode($_SESSION['cabecera_respaldo_deptocbte'][2]),0,1,'L');
...
$detalle = $_SESSION["pdf_depdepto_respaldo"];
...
for ($i=0; $i<count($detalle); $i++)
{
	...
	if($flag)
	{
		...
		$pdf->SetFont('Arial','',4);
		$pdf->Cell(50,3,$detalle[$i][20],0,1,'');
	...
	if($id_plan == $id_plan2)
	{
		...
		if ($i < $filas -1)
		{
			...
			$pdf->SetFont ( 'Arial', '', 4 );
			$pdf->Cell ( 50, 3, $detalle [$i + 1] [19], 0, 1, '' );
			$pdf->Ln ( 1 );
			...
			$pdf->SetFont ( 'Arial', '', 4 );
			$pdf->Cell ( 50, 2, $detalle [$i + 1] [20], 0, 1, '' );
			$pdf->Ln ( 1 );
		...
		else
		{
			...
			$pdf->SetFont ( 'Arial', 'B', 5 );
			$pdf->Text ( 95, 247, $_SESSION ['cabecera_respaldo_deptocbte'][2] );
		}
	}
