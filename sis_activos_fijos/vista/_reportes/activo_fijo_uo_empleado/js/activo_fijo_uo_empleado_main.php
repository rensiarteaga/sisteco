...
var elemento={pagina:new pagina_activo_fijo_uo(idContenedor,direccion,paramConfig),idContenedor:idContenedor};
...
/**
* Nombre:		  	    pagina_activo_fijo_uo.js
* Propósito: 			pagina objeto principal
* Autor:				AVQ
* Fecha creación:		30/07/2010
*/
function pagina_activo_fijo_uo(idContenedor,direccion,paramConfig)
{
	...
	//DATA STORE's
	 ...
	 var ds_unidad_organizacional = new Ext.data.Store({proxy: new Ext.data.HttpProxy({url: direccion+'../../../../../sis_kardex_personal/control/unidad_organizacional/ActionListarUnidadOrganizacional.php'}),
			...
			['id_unidad_organizacional','nombre_unidad','nombre_cargo','centro','cargo_individual','descripcion','fecha_reg','id_nivel_organizacional','estado_reg','nombre_nivel']),baseParams:{start:0,limit: paramConfig.TamanoPagina,CantFiltros:paramConfig.CantFiltros,sw_presto:1,tipo_vista:'reporte_activo_fijo_uo'}
	});

	
	
	
	var ds_gestion = new Ext.data.Store({proxy: new Ext.data.HttpProxy({url: direccion+'../../../../../sis_parametros/control/gestion/ActionListarGestion.php'}),reader: new Ext.data.XmlReader({record: 'ROWS',id: 'id_gestion',totalRecords: 'TotalCount'}, ['id_gestion','estado','fecha','id_gestion_subsistema','id_subsistema','id_gestion','gestion'])});
...
//	function render_id_empleado(value, p, record){return String.format('{0}', record.data['desc_empleado'])}
	...
	var resultTplGestion = new Ext.Template('<div class="search-item">','<b>Gestión: {gestion}</b>','</div>');
...
vectorAtributos[3]={
	...
	vectorAtributos[4]= {
		validacion: {
			...
			loadMask: true,
			triggerAction: 'all',
			//store: new Ext.data.SimpleStore({fields: ['ID','valor'],data : Ext.activo_fijoCombo.estado}),
			store: new Ext.data.SimpleStore({fields: ['ID','valor'],data : [['%','Todos'],['activo','Activos'],['inactivos','Inactivos']]}),
			valueField:'ID',
			displayField:'valor',
			lazyRender:true,
			forceSelection:true,
  ...
  /*  vectorAtributos[8]={
	...
	*/
	...
	var paramFunciones = {
		...
		height:'100%',
		url:direccion+'../../../../control/_reportes/activo_fijo_uo_empleado/ActionPDFActivoFijoUOEmpleado.php',
     ...
     */
		/*var onEpSelect = function(e){
			...
			//ds_unidad_organizacional.baseParams.tipo_vista="reporte_activo_fijo_uo";
