...
if($_SESSION['autentificado']=="SI")
{
	...
	{	//Si encuenta responsable para el filtro lo reemplaza por nombre, apellido_paterno y apellido_materno
		$aux=str_replace('responsable','PER.nombre#PER.apellido_paterno#PER.apellido_materno',$_GET["filterCol_$i"]);
		$cond->add_condicion_filtro($_GET["filterCol_$i"], $_GET["filterValue_$i"], $_GET["filterAvanzado_$i"]);
	}
	if($post)
	{	//Si encuenta responsable para el filtro lo reemplaza por nombre, apellido_paterno y apellido_materno
		$aux=str_replace('responsable','PER.nombre#PER.apellido_paterno#PER.apellido_materno',$_POST["filterCol_$i"]);
	...
	if($f["id_cotizacion_det"]>0){
		$xml->add_nodo('descripcion', $f["desc_cotdet"]);
	}else{
