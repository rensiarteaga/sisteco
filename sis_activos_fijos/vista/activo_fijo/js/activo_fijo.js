...
function pagina_activo_fijo(idContenedor,direccion,paramConfig,usuario){
	...
	var ds_cotizacion_det= new Ext.data.Store({proxy: new Ext.data.HttpProxy({url: direccion+'../../../../sis_adquisiciones/control/cotizacion_det/ActionListarCotizacionDet.php'}),
		reader:new Ext.data.XmlReader({record:'ROWS',id:'id_cotizacion_det',totalRecords: 'TotalCount'},['id_cotizacion_det','id_item_aprobado','item','codigo','abreviatura','precio','especificaciones_tecnicas'])
	});
	...
	var tpl_id_cotizacion_det=new Ext.Template('<div class="search-item">','<b>{item}</b><br>','<FONT COLOR="#B5A642">{codigo}</FONT>','<b>{precio}</b><br>','<b>{especificaciones_tecnicas}</b><br>','</div>');
	...
	Atributos[7] = {
		validacion:{
			...
			onSelect:function(record){
				...
				getComponente('id_cotizacion_det').setRawValue(record.data.item);
				getComponente('descripcion').setValue(record.data.item);
				getComponente('id_cotizacion_det').collapse();
				v_item=record.data.id_item_aprobado;
				Ext.Ajax.request({
					url:direccion+"../../../../sis_activos_fijos/control/activo_fijo_tpm_frppa/ActionListarEPActivoFijo.php?filtro_af=1&id_item="+record.data.id_item_aprobado+"&id_cotizacion_det="+record.data.id_cotizacion_det,
					method:'GET',
					success:  cargar_epuo,
	...
	Atributos[44]={
		validacion:{
			...
			fieldLabel: 'Ubicación Descriptiva',
			allowBlank: true,
			maxLength:300,
			minLength:0,
			selectOnFocus:true,
...
//   });
		...
		this.AdicionarBoton("../../../lib/imagenes/menu-show.gif",'<b>Estructura Programática<b>',btnEp,true,'ep','Estructura Programática');
