...
function PaginaActivoFijo(){
	...
	h_txt_fecha_fin_gar, h_txt_fecha_ini_dep, h_txt_fecha_compra, h_txt_orden_compra, 	h_txt_codigo,id_tipo_activo,id_sub_tipo_activo,cod_tipo_activo;
	...
	vectorAtributos[4] = paramOrden_compra;
	...
	var paramVida_util_restante = {
		validacion:{
			...
			fieldLabel: 'Vida Útil Restante  (meses)',
			allowBlank: false,
			maxLength:500,
			minLength:0,
			selectOnFocus:true,
	...
	var paramDepreciacion_acum_ant = {
		validacion:{
			...
			fieldLabel: 'Depreciación Acumulada anterior',
			allowBlank: false,
			maxLength:500,
			minLength:0,
			selectOnFocus:true,
	...
	var paramDepreciacion_periodo = {
		validacion:{
			...
			fieldLabel: 'Depreciación período',
			allowBlank: false,
			maxLength:500,
			minLength:0,
			selectOnFocus:true,
	...
	var paramMonto_compra_mon_orig = {
		validacion:{
			...
			fieldLabel: 'Valor de Incorporación',
			allowBlank: false,
			maxLength:500,
			minLength:0,
			selectOnFocus:true,
	...
	//////////////////////////////////////////
	var paramVida_util_restante_2 = {
		validacion:{
			name: 'vida_util_restante_2',
		...
		tipo: 'NumberField',
		filtro_0:true,
		filterColValue:'AF.vida_util_restante_2',
		filtro_1:true,
		save_as:'txt_vida_util_restante_2',
		defecto: 0,
		id_grupo: 8
	}
	vectorAtributos[42] = paramVida_util_restante_2;
	...
	var paramFunciones = {
		...
		Formulario:{
			...
			columnas:['47%','47%'],
			grupos:[
			{	tituloGrupo:'Información General del Activo Fijo',columna:0,	id_grupo:0	},
			{	tituloGrupo:'Datos de adquisición',columna:0,	id_grupo:1	},
			{	tituloGrupo:'Datos de adquisición en moneda principal',	columna:0,	id_grupo:2	},
			{	tituloGrupo:'Datos de depreciación',columna:0,	id_grupo:3	},
			{	tituloGrupo:'Datos vigentes',columna:1,	id_grupo:4	},
			{	tituloGrupo:'Garantía',	columna:1,	id_grupo:5	},
			{	tituloGrupo:'Foto Activo Fijo',columna:1,	id_grupo:6	},
			{	tituloGrupo:'Datos Depreciación',columna:1,	id_grupo:7	},
			{	tituloGrupo:'Datos Depreciación Histórica',columna:1,	id_grupo:8	},
	...
	function iniciarEventosFormularios(){
		...
		h_txt_responsable=ClaseMadre_getComponente('responsable');
		...
		h_txt_vida_util_restante_2=ClaseMadre_getComponente('vida_util_restante_2');
		...
		var onTipoActivoSelect = function(e){
			...
			//Obtener el código del tipo de activo
		...
		function get_datos_subtipo(){
			...
			if(combo_sub_tipo_activo.store.getById(id_sub_tipo_activo).data!=undefined){
				...
				h_txt_vida_util_2.setValue(data.vida_util);
				h_txt_vida_util_restante_2.setValue(data.vida_util);
		...
		/*function cargar_sub_tipo_activo_data(resp){
			...
			if(resp.responseXML != undefined && resp.responseXML != null && resp.responseXML.documentElement != null && resp.responseXML.documentElement != undefined){
				...
				h_txt_vida_util_2.setValue(root.getElementsByTagName('vida_util')[0].firstChild.nodeValue);
				h_txt_vida_util_restante_2.setValue(root.getElementsByTagName('vida_util')[0].firstChild.nodeValue);
		...
		if(resp.responseXML != undefined && resp.responseXML != null && resp.responseXML.documentElement != null && resp.responseXML.documentElement != undefined)
		{
			...
			if(root.getElementsByTagName('monto')[0].firstChild != undefined && root.getElementsByTagName('monto')[0].firstChild != null)
			{
				...
				if(h_txt_monto_compra_2.getValue()==''){
					h_txt_monto_compra_2.setValue(root.getElementsByTagName('monto')[0].firstChild.nodeValue);
				}
				if(h_txt_monto_actual_2.getValue()==''){
					h_txt_monto_actual_2.setValue(root.getElementsByTagName('monto')[0].firstChild.nodeValue);
				}
				...
				h_txt_monto_compra_2.setValue('');
				h_txt_monto_actual_2.setValue('');
			}
		}
		}
		function monto_compra2_ini()
		{
			//Carga el monto actual 2 con el monto compra_2 la primera vez
			h_txt_monto_actual_2.setValue(h_txt_monto_compra_2.getValue());
		}

		function depreciacion_acum2_ini()
		{
			//Carga la depreciación_acum_ant2 2 con la depreciacion_acum_ant la primera vez
			h_txt_depreciacion_acum_ant_2.setValue(h_txt_depreciacion_acum_2.getValue());
		}
		...
		h_txt_monto_compra_2.on('blur',monto_compra2_ini);
		h_txt_depreciacion_acum_2.on('blur',depreciacion_acum2_ini);
	}
	...
	this.AdicionarBoton("../../../lib/imagenes/menu-show.gif",'<b>Definición de Características<b>',btnCaracteristicas,true,'caracteristicas','Definición de Características');
	...
	this.AdicionarBoton("../../../lib/imagenes/menu-show.gif",'<b>Asignación a Responsables<b>',btnResponsables,true,'responsables','Asignación a Responsables');
