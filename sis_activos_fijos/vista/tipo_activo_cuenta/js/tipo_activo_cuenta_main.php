<?php 
/**
 * Nombre:		  	  	tipo_activo_cuenta_main.php 
 * Prop�sito: 			
 * Autor:				unknow
...
var elemento={pagina:new pagina_tipo_activo_cuenta(idContenedor,direccion,paramConfig),idContenedor:idContenedor};
 ...
 * Prop�sito: 	vista,parametrizacion de cuentas ACTIF-CONIN		
 * Autor:		Elmer Velasquez			
 * Fecha creaci�n:		01/02/2013
 */

function pagina_tipo_activo_cuenta(idContenedor,direccion,paramConfig) 
{	
	...
	var ds = new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({url: direccion+'../../../control/tipo_activo_cuenta/ActionListarTipoActivoCuenta.php'}),  
		...
		'descripcion',
		'nombre_cuenta_activo','nombre_cuenta_activo_auxiliar','id_cta_activo','id_cta_activo_auxiliar',
		'nombre_cuenta_dep_acumulada','nombre_cuenta_dep_acumulada_auxiliar',
		'nombre_cuenta_gasto','nombre_cuenta_gasto_auxiliar',
		'id_cta_dep_acum','id_cta_dep_acum_auxiliar','id_cta_gasto','id_cta_gasto_auxiliar'
		]),remoteSort:true});
	//DATA STORE COMBOS
	var ds_tipo = new Ext.data.Store({
		// asigna url de donde se cargar�n los datos
	
		proxy: new Ext.data.HttpProxy({url: direccion+'../../../control/tipo_activo/ActionListaTipoActivoEP.php'}),
		reader: new Ext.data.XmlReader({
			record: 'ROWS',
			id: 'id_tipo_activo',
	...
	function render_id_cta_activo(value, p, record){return String.format('{0}', record.data['nombre_cuenta_activo'])};
	...
	var tpl_cta_aux=new Ext.Template('<div class="search-item">','<b>C�digo: </b><FONT COLOR="#0000ff">{codigo_auxiliar}</FONT><br>','<b>Auxiliar: </b><FONT COLOR="#0000ff">{nombre_auxiliar}</FONT><br>','</div>');
	function render_id_cta_activo_auxiliar(value, p, record){return String.format('{0}', record.data['nombre_cuenta_activo_auxiliar'])};
	...
	Atributos[4]={
			validacion:{
				...
				queryParam: 'filterValue_0',
				filterCol:'tipo.codigo#tipo.descripcion',
				typeAhead:false,
				forceSelection:true,
				mode:'remote',
				queryDelay:50,
			...
			tipo: 'ComboBox',
			filtro_0:false,
			filterColValue:'tipo.codigo#tipo.descripcion',
			id_grupo:0,
	...
	Atributos[5]={
		...
		tipo:'ComboBox',
		filtro_0:false,
		filterColValue:'vep.codigo_programa#vep.nombre_programa',
	...
	id_grupo:1,
	filtro_0:true,
	filterColValue:'cta_dep_acum.nro_cuenta#cta_dep_acum.nombre_cuenta',
	form: true,
	...
	Atributos[7]={
			...
			name:'id_cta_activo_auxiliar',
			desc:'nombre_cuenta_activo_auxiliar',
			store:ds_auxiliar,
			...
			{
				ClaseMadre_getComponente('id_cta_activo_auxiliar').setValue(record.data.id_auxiliar); 
				ClaseMadre_getComponente('aux_cuenta_activo').setValue(record.data.id_auxiliar); 
				ClaseMadre_getComponente('id_cta_activo_auxiliar').collapse();
			},
			forceSelection:false,
			mode:'remote',
			queryDelay:250,
			...
			resizable:true,
			minChars:3,
			renderer:render_id_cta_activo_auxiliar,
			triggerAction:'all',
			grid_visible:false,
			grid_editable:true,
			width:300,
	...
	Atributos[9]={
			...
			name:'id_cta_dep_acum',
			desc:'nombre_cuenta_dep_acumulada',
	...
	Atributos[10]={
			...
			allowBlank:true,
			emptyText:'Codigo Activo Auxiliar',
			name:'id_cta_dep_acum_auxiliar',
			desc:'nombre_cuenta_dep_acumulada_auxiliar',
			...
			onSelect: function(record)
			{
				ClaseMadre_getComponente('id_cta_dep_acum_auxiliar').setValue(record.data.id_auxiliar); 
				ClaseMadre_getComponente('aux_cta_dep_acumulada').setValue(record.data.id_auxiliar); 
				ClaseMadre_getComponente('id_cta_dep_acum_auxiliar').collapse();
			},
			forceSelection:false,
			mode:'remote',
			queryDelay:250,
		...
		Atributos[13]={
				...
				name:'id_cta_gasto_auxiliar',
				desc:'nombre_cuenta_gasto_auxiliar',
				...
				onSelect: function(record)
				{
					ClaseMadre_getComponente('id_cta_gasto_auxiliar').setValue(record.data.id_auxiliar); 
					ClaseMadre_getComponente('aux_cta_gasto').setValue(record.data.id_auxiliar); 
					ClaseMadre_getComponente('id_cta_gasto_auxiliar').collapse();
				},
				forceSelection:false,
				mode:'remote',
				queryDelay:250,
	...
	var paramFunciones={ 
		btnEliminar:{url:direccion+'../../../control/tipo_activo_cuenta/ActionEliminarTipoActivoCuenta.php'},
		Save:{url:direccion+'../../../control/tipo_activo_cuenta/ActionGuardarTipoActivoCuenta.php'},
		ConfirmSave:{url:direccion+'../../../control/tipo_activo_cuenta/ActionGuardarTipoActivoCuenta.php'},
	...
	//para iniciar eventos en el formulario
		...
		combo_cuenta_activo_aux = getComponente('id_cta_activo');
		...
		//combo dep acum
		combo_cuenta_dep_acum_aux=getComponente('id_cta_dep_acum');
		...
		//combo cta gasto
		combo_cuenta_gasto_aux=getComponente('id_cta_gasto');
