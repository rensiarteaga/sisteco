<?php
/**
 * Nombre del archivo:	ActionEliminaActivoFijo.php
 * Propósito:			Permite eliminar registros de Activos Fijos
...
$nombre_archivo = 'ActionEliminaActivoFijo.php';
...
if($_SESSION['autentificado']=="SI")
{
	...
	for($j = 0;$j < $cont; $j++)
	{
		...
		if ($id_activo_fijo == "undefined" || $id_activo_fijo =="")
		{
			...
			$resp->mensaje_error = "No existe el Activo Fijo especificado para eliminar.";
			$resp->origen = "ORIGEN = $nombre_archivo";
			$resp->proc = "PROC = $nombre_archivo";
			$resp->nivel = "NIVEL = 4";
			echo $resp->get_mensaje();
		...
		{
			//Eliminación
			$res = $Custom-> EliminarActivoFijo($id_activo_fijo);
			if(!$res)
			{
				$resp = new cls_manejo_mensajes(true, "503");
