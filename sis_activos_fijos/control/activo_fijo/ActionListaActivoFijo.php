...
if($_SESSION['autentificado']=="SI")
{
	...
	if($estado_activo!=''){
		...
		//añadido 28/04/2014
		elseif (trim($estado_activo) == 'PROYECTO' and ($proyecto!='' && isset($proyecto)) )
		{
			$tipo_af_bien='';
			$criterio_filtro=$criterio_filtro."  AND AF.estado in (''codificado'') AND (AF.tipo_af_bien=''activo'' OR AF.tipo_af_bien=''bien_resp'')  AND AF.proyecto=''si'' ";
		}
		//fin 28/04/2014
		elseif(trim($estado_activo) == 'DEPAF' && ($proyecto != ''))
		{
			$tipo_af_bien='';
			$criterio_filtro=$criterio_filtro."  AND AF.estado in (''alta'') AND (AF.tipo_af_bien=''activo'' OR AF.tipo_af_bien=''activo'')  AND AF.proyecto=''no'' ";
		}
		//fin 28/04/2014
		else
		{
	...
	if($id_depto!=''){//para vista de grupo_proceso
		$criterio_filtro=$criterio_filtro." AND AF.id_depto=$id_depto";
	}
	if($asignado=='si'){//para vista de grupo_proceso
		$criterio_filtro=$criterio_filtro." AND AFEMP.id_empleado is not null";
	}
	if($asignado=='no'){
		$criterio_filtro=$criterio_filtro." AND AF.id_deposito is not null and AFEMP.id_empleado is null";
	}
	...
	//Verifica si el xml serï¿½ para llenar un combo o no
	if($origen == 'filtro'){
		$xml->add_rama('ROWS');
	...
	$xml->add_nodo('desc_cotdet', $f["desc_cotizacion_detalle"]);	
	...
	$xml->add_nodo('id_ubicacion_fisica', $f["id_ubicacion"]);
	$xml->add_nodo('desc_ubicacion_fisica', $f["desc_ubicacion_fisica"]);
	
	$xml->add_nodo('tension', $f["tension"]);
	$xml->add_nodo('tipo_bien_adt', $f["bienes_produccion"]);
	$xml->add_nodo('tipo_bien_gen', $f["bienes_produccion"]);
