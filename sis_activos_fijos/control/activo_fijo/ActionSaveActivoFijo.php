...
if($_SESSION['autentificado']=="SI")
{
	...
	for($j = 0;$j < $cont; $j++)
	{
		if ($get)
		{
			...
			$tension=$_GET["txt_tension_$j"];
			$id_tension2=$_GET["txt_tipo_bien_adt_$j"];
			$id_tension3=$_GET["txt_tipo_bien_gen_$j"];
		}
		else
		{
			...
			$tension=$_POST["txt_tension_$j"];
			$id_tension2=$_POST["txt_tipo_bien_adt_$j"];
			$id_tension3=$_POST["txt_tipo_bien_gen_$j"];
		}
		
		//verifica cual tension enviada no es nula
		$tipo_bien=trim($id_tension2.$id_tension3);
		...
		if($num_clones>0){ 
			...
			if ($id_activo_fijo == "undefined" || $id_activo_fijo == ""){	
				...
				if ( (strlen($tipo_bien) > 0) ){
					$res =$Custom->ModificarActivoFijoTension($id_activo_fijo,$tension,$tipo_bien);
