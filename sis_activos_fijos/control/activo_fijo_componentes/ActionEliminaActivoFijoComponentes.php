<?php
/**
 * Nombre del archivo:	ActionEliminaActivoFijoComponentes.php
 * Propósito:			Permite eliminar registros de Subtipos de Activos
...
$nombre_archivo = 'ActionEliminaActivoFijoComponentes.php';
...
{ 
	...
	for($j = 0;$j < $cont; $j++)
	{
		...
		{
			$resp = new cls_manejo_mensajes(true, "406");
			$resp->mensaje_error = "MENSAJE ERROR = No existe el Componente especificado para eliminar.";
			$resp->origen = "ORIGEN = $nombre_archivo";
			$resp->proc = "PROC = $nombre_archivo";
			$resp->nivel = "NIVEL = 4";
			echo $resp->get_mensaje();
		...
		{
			//Eliminación
			$res = $Custom-> EliminarActivoFijoComponentes($hidden_id_componente);
			if(!$res)
			{
				$resp = new cls_manejo_mensajes(true, "406");
