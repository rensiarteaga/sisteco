...
**********************************************************
Nombre de archivo:	    ActionEliminarGrupoProceso.php
Propósito:				Permite eliminar registros de la tabla de Proceso
Tabla:					taf_proceso
Parámetros:				$hidden_id_persona	--> id de la persona
...
$nombre_archivo = 'ActionEliminarGrupoProceso.php';
...

{
	...
	for($j = 0;$j < $cont; $j++)
	{
		...
		{
			$resp = new cls_manejo_mensajes(true, "406");
			$resp->mensaje_error = "MENSAJE ERROR = No existe el Grupo Proceso indicado.";
			$resp->origen = "ORIGEN = $nombre_archivo";
			$resp->proc = "PROC = $nombre_archivo";
			$resp->nivel = "NIVEL = 4";
			echo $resp->get_mensaje();
		...
		{	//Eliminación
			$res = $Custom-> EliminarGrupoProceso($id_grupo_proceso);
			if(!$res)
			{
				$resp = new cls_manejo_mensajes(true, "406");
