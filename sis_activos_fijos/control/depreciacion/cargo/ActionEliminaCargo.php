...
**********************************************************
Nombre de archivo:	    ActionEliminaCargo.php
...
$nombre_archivo = 'ActionEliminaCargo.php';
...

{
	...
	for($j = 0;$j < $cont; $j++)
	{
		...
		{
			$resp = new cls_manejo_mensajes(true, "406");
			$resp->mensaje_error = "MENSAJE ERROR = No existe Cargo especificado para eliminar.";
			$resp->origen = "ORIGEN = $nombre_archivo";
			$resp->proc = "PROC = $nombre_archivo";
			$resp->nivel = "NIVEL = 4";
			echo $resp->get_mensaje();
		...
		{	//Eliminación
			$res = $Custom-> EliminarCargo($hidden_id_cargo);
			if(!$res)
			{
				$resp = new cls_manejo_mensajes(true, "406");
