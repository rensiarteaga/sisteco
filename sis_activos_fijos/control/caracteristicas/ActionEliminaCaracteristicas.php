...
**********************************************************
Nombre de archivo:	    ActionEliminaCaracteristicas.php
Propósito:				Permite eliminar registros de la tabla de Caracteristicas
Tabla:					taf_caracteristicas
Parámetros:				$hidden_id_caracteristica	--> id de Caracteristicas
...
$nombre_archivo = 'ActionEliminaCaracteristicas.php';
...
if($_SESSION['autentificado']=="SI")
{
	...
	for($j = 0;$j < $cont; $j++)
	{
		if ($get)
		{
			...
			//$txt_id_usuario_asignacion = $_GET["txt_id_usuario_asignacion_$j"];
		}
		else
		{
			$hidden_id_caracteristica = $_POST["hidden_id_caracteristica_$j"];
			//$txt_id_usuario_asignacion = $_POST["txt_id_usuario_asignacion_$j"];
		}

		if ($hidden_id_caracteristica == "undefined" || $hidden_id_caracteristica =="")
		{
			$resp = new cls_manejo_mensajes(true, "406");
			$resp->mensaje_error = "MENSAJE ERROR = No existe caracteristica especificada para eliminar.";
			$resp->origen = "ORIGEN = $nombre_archivo";
			$resp->proc = "PROC = $nombre_archivo";
			$resp->nivel = "NIVEL = 4";
			echo $resp->get_mensaje();
		...
		{	//Eliminación
			$res = $Custom-> EliminarCaracteristicas($hidden_id_caracteristica);
			if(!$res)
			{
				$resp = new cls_manejo_mensajes(true, "406");
