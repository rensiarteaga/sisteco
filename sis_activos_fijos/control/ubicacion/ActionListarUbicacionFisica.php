...
Propï¿½sito:			Permite realizar el listado en taf_ubicacion
Tabla:					taf_ubicacion
Parï¿½metros:			$cant
						$puntero
						$sortcol
...
//Parï¿½metros del filtro
	...
	if($origen =='filtro'){$criterio_filtro=$criterio_filtro.'  AND UBI.id_ubicacion > 0';}
	...
	if($res)
	{
		...
		{
			$xml->add_rama('ROWS');
			$xml->add_nodo('id_ubicacion','%');
			$xml->add_nodo('id_lugar','');
			$xml->add_nodo('codigo','Todas las Ubicaciones');
			$xml->add_nodo('ubicacion','Todas las Ubicaciones');
			$xml->add_nodo('estado','');
