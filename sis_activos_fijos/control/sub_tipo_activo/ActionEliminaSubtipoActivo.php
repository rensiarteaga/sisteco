<?php
/**
 * Nombre del archivo:	ActionEliminaSubtipoActivo.php
...
$nombre_archivo = 'ActionEliminaSubtipoActivo.php';
...
if($_SESSION['autentificado']=="SI")
{
	...
	for($j = 0;$j < $cont; $j++)
	{
		...
		{
			$resp = new cls_manejo_mensajes(true, "406");
			$resp->mensaje_error = "MENSAJE ERROR = No existe el Subtipo especificado para eliminar.";
			$resp->origen = "ORIGEN = $nombre_archivo";
			$resp->proc = "PROC = $nombre_archivo";
			$resp->nivel = "NIVEL = 4";
			echo $resp->get_mensaje();
		...
		{
			//Eliminación
			$res = $Custom-> EliminarSubtipoActivo($hidden_id_sub_tipo_activo);
			if(!$res)
			{
				$resp = new cls_manejo_mensajes(true, "406");
