...
$nombre_archivo = 'ActionAprobacionProcesos.php';
...
/*		echo "id_activo_fijo_proceso->".$hidden_id_activo_fijo_proceso;
		echo "estado->".$txt_estado;
		exit;*/

		if ($hidden_id_activo_fijo_proceso != "undefined" || $hidden_id_activo_fijo_proceso != "")
		{

			$res = $CustomActivos ->AprobarAplicarAprobacionProcesos($hidden_id_activo_fijo_proceso,$txt_estado);
	...
	/*//Si la aprobación se la hizo satisfactoriamente, se ejecuta la función para ejecutar los datos del proceso a activos fijos

	$res = $CustomActivos -> AplicarAprobacionProcesos();

	if ($res)
	{
		//Arma el xml para desplegar el mensaje
		$mensaje_exito = $CustomActivos->salida[1];
	...
	$estado= "pendiente";
	if ($criterio_filtro=="") $criterio_filtro="afproc.estado = ''$estado''";
