 ...
 * Fecha:			09/04/2013  ACTIVO.origen
 */
...
if($_SESSION['autentificado']=="SI")
{
	...
	$arreglo_cabeceras = array(
			...
			"ACTIVO.monto_compra","ACTIVO.num_factura","Activo.orden_compra",
			"ACTIVO.monto_actual","ACTIVO.monto_actualiz","UBICACION.ubicacion as ubicacio","ACTIVO.vida_util_original","ACTIVO.vida_util_restante",
			"ACTIVO.flag_revaloriz AS flag_revaloriz","ACTIVO.proyecto","ACTIVO.observaciones","ACTIVO.origen",
			"EMPLEA.nombre_completo as responsable",
			"COALESCE(person.apellido_paterno, '''') || '' ''||COALESCE(person.apellido_materno, '''')|| '' '' || COALESCE(person.nombre, '''') AS custodio",
			...
			"frppa.nombre_financiador","frppa.nombre_regional","frppa.nombre_programa","frppa.nombre_proyecto","frppa.nombre_actividad","frppa.desc_epe","depto.nombre_depto","comp.id_comprobante","comp.fecha_cbte","ppto.nombre_unidad");
	...
	if($txt_chk_kbeceras == 'true'){
		...
		$posicion = array(0,1,2,5,6,7,8,9,14,21,23);
	...
	//añadido 14/07/2015
	$cabecera .= " ,frppa.codigo_programa,activo.tension";
	...
	$criterio_filtro.=" ACTIVO.estado!= ''eliminado'' AND  ";
	...
	//elseif ($ubicacion_fisica=='%'){$criterio_filtro = $criterio_filtro." and activo.id_ubicacion like ''".$ubicacion_fisica."''";}
	...
	if($tipo_reporte == '0')
	{
		...
		header("location:../../../vista/_reportes/activo_fijo_detalle_analisis/XLSReporteDetalleActivoFijo.php");
	}else{
