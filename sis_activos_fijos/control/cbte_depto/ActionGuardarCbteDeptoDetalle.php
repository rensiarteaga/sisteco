...
if($_SESSION['autentificado']=="SI")
{
	...
	for($j = 0;$j < $cont; $j++)
	{
		if ($get)
		{
			$id=$_GET["h_id_cbtedepto_det_$j"];
			$id_cbte=$_GET["h_id_cbte_depto_$j"];
			$descripcion=$_GET["txt_descripcion_$j"];
			$id_ep=$_GET["h_id_epe_$j"];
		}
		else
		{
			$id=$_POST["h_id_cbtedepto_det_$j"];
			$id_cbte=$_POST["h_id_cbte_depto_$j"];
			$descripcion=$_POST["txt_descripcion_$j"];
			$id_ep=$_POST["h_id_epe_$j"];
		...
		if ($id== "undefined" || $id== "")
		{
			...
			$res = $Custom ->InsertarCbteDeptoDetalle($id_cbte,$descripcion,$id_ep);
		...
		{
			//
			$res = $Custom->ModificarCbteDeptoDetalle($id,$id_cbte,$descripcion,$id_ep);
	...
	if($criterio_filtro == "") $criterio_filtro = '0=0 AND det.id_cbte_depto = '.$id_cbte;
