...
function pag_proc_simplif_item(idContenedor,direccion,paramConfig,maestro,idContenedorPadre){
	...
	var ds = new Ext.data.Store({
		...
		'precio_total','id_moneda_base','numeracion_periodo','num_detalle_cotizado_gral', 'num_detalle_adjudicado_gral','se_adjudica','num_detalle_adjudicado','cantidad_sol','tipo_documento','precio_total_adjudicado','cod_control_factura','num_autoriza_factura','id_empleado_adjudicacion','empleado_adjudicacion','id_caja','caja','cajero','id_cajero'
...
//		};
	...
	Atributos[25]={
		...
		var paramFunciones={
			...
			Formulario:{html_apply:'dlgInfo-'+idContenedor,height:'65%',width:'40%',minWidth:150,minHeight:'45%',	closable:true,titulo:'cotizacion',columnas:['90%'],
			grupos:[{
				tituloGrupo:'Oculto',
				columna:0,
				id_grupo:0
			...
			this.reload=function(params){
				...
				_CP.getPagina(lay_proc_simplif_item.getIdContentHijo()).pagina.limpiarStore();
            ...
            function Resultado(btn,text){
                  if(btn!='cancel'){
				    getComponente('fin').setValue(1);
                     ...
                     CM_ocultarGrupo('Datos Proveedor');
                     CM_ocultarGrupo('Oculto');
                    // getComponente('fecha_adjudicacion').setValue(new Date()); // FER-MOD-AD-07
                     var mGesI=new Date();
                     mGesI=mGesI.getFullYear();
                     if(parseFloat(mGesI)>parseFloat(getComponente('gestion').getValue())){
