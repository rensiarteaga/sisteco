...
function pag_cot_serv_dir_det(idContenedor,direccion,paramConfig,idContenedorPadre)
{
	...
	var ds = new Ext.data.Store({
		...
		,'estado','id_proceso_compra','precio_moneda_cotizada','desc_moneda','reg_cabecera','precio_cantidad','id_unidad_medidad_base','abreviatura'
			...
			function  enable(sel,row,selected){
				...
				if(selected&&record!=-1){
				    ...
				    if(maestro.estado_vigente=='cotizado'){
				        gridG.getColumnModel().setEditable(4,false);
				        gridG.getColumnModel().setEditable(5,false);
				        gridG.getColumnModel().setEditable(6,false);
				       ...
				       if(maestro.estado_vigente=='invitado'|| maestro.estado_vigente=='aperturado'){
				           if( maestro.id_moneda>0 && maestro.precio_total>0&&maestro.fecha_cotizacion!='' && parseFloat(record.reg_cabecera)>0){
				               gridG.getColumnModel().setEditable(4,true);
				               gridG.getColumnModel().setEditable(5,true);
				               gridG.getColumnModel().setEditable(6,true);
