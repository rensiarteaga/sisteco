	...
	var ds = new Ext.data.Store({
		...
		Atributos[6]= {
			validacion:{
				...
				renderer: formatDate,
				onBlur:function(e){
			     var ffinS=getComponente('fecha_fin_serv').getValue(); var afinS= ffinS.dateFormat('Y');
			     var finiS=getComponente('fecha_inicio_serv').getValue();
			     if(parseFloat(gestionS)>0){
			       if(afinS!=gestionS){
			...
			function cargar_fecha_adq(resp){
				...
				if(resp.responseXML != undefined && resp.responseXML != null && resp.responseXML.documentElement != null && resp.responseXML.documentElement != undefined){
					...
					if((root.getElementsByTagName('TotalCount')[0].firstChild.nodeValue)>0){
						...
						//txt_fecha_inicioS.maxValue=txt_lim_sup_fecha.getValue();
