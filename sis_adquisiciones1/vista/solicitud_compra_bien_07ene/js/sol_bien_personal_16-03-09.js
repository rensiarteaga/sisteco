...
function pag_sol_bien_personal(idContenedor,direccion,empleado,paramConfig){
	...
	var data_ep;
	var txt_emp=0;
	var txt_hora,txt_hora,txt_hora,txt_id_tipo_adquisicion,txt_gestion,combo_almacen,combo_almacen_logico;
	...
	var ds = new Ext.data.Store({
		...
		'id_orden_trabajo','desc_orden','id_almacen_logico','desc_almacen_logico','desc_almacen','id_almacen','id_uo_gerencia','id_empleado','id_depto','desc_depto','proveedores_propuestos','comite_calificacion','comite_recepcion'
		...
		var ds_almacen_logico = new Ext.data.Store({proxy: new Ext.data.HttpProxy({url: direccion+'../../../../sis_almacenes/control/almacen_logico/ActionListarAlmacenLogicoFisEPM.php'}),
		reader: new Ext.data.XmlReader({
			record: 'ROWS',
		...
		var ds_almacen=new Ext.data.Store({proxy:new Ext.data.HttpProxy({url:direccion+'../../../../sis_almacenes/control/almacen/ActionListarAlmacenEP.php'}),reader:new Ext.data.XmlReader({record:'ROWS',id:'idalmacen',totalRecords:'TotalCount'}, ['id_almacen','nombre','descripcion'])});
		...
		var filterCols_almacen_logico=new Array();
		
		var filterValues_almacen_logico=new Array();
		...
		var paramFunciones={
			...
			function btn_fin_ped(){
				...
				if(NumSelect!=0){
					...
					if(SR.data.estado_vigente_solicitud=='borrador'){
						var data=SR.data.id_solicitud_compra;
			...
			var onLocalidad=function(e){
				...
				Ext.Ajax.request({
					url:direccion+"../../../../sis_parametros/control/depto_ep/ActionListarDepartamentoEP.php?id_ep="+ep['id_fina_regi_prog_proy_acti'],
					method:'GET',
