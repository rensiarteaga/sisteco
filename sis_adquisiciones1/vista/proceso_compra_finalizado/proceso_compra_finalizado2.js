...
*/
function pagina_proceso_compra_finalizado(idContenedor,direccion,paramConfig){
	...
	// txt codigo_proceso
		...
		var config={titulo_maestro:'Procesos Finalizados de Bienes',grid_maestro:'grid-'+idContenedor,urlHijo:'../../../sis_adquisiciones/vista/proceso_compra_det/procesoCompraMulItemDet.php'};
		//var layout_proceso_compra_finalizado=new DocsLayoutMaestroEP(idContenedor);
		var layout_proceso_compra_finalizado=new DocsLayoutMaestroDeatalle(idContenedor);
		layout_proceso_compra_finalizado.init(config);
		...
		this.pagina(paramConfig,Atributos,ds,layout_proceso_compra_finalizado,idContenedor);
		...
		var paramFunciones={
			...
			function iniciarEventosFormularios(){
					...
					getSelectionModel().on('rowdeselect',function(){
						if(_CP.getPagina(layout_proceso_compra_finalizado.getIdContentHijo()).pagina.limpiarStore()){
							_CP.getPagina(layout_proceso_compra_finalizado.getIdContentHijo()).pagina.bloquearMenu()
						}
					})
				}
			...
			function btn_proceso_compra_det(){
				...
				if(NumSelect!=0){
					...
					var url='../../../../sis_adquisiciones/vista/proceso_compra_det/proceso_compra_mul_item_det.php?'+data;
					var ParamVentana={Ventana:{width:'90%',height:'70%'}}
					layout_proceso_compra_finalizado.loadWindows(direccion+url,'Procedimiento Detalle',ParamVentana);
					layout_proceso_compra_finalizado.getVentana().on('resize',function(){
						layout_proceso_compra_finalizado.getLayout().layout();
					})
				}
				else
				{
			...
			this.getLayout=function(){return layout_proceso_compra_finalizado.getLayout()};
			...
			layout_proceso_compra_finalizado.getLayout().addListener('layout',this.onResize);//aregla la forma en que se ve el grid dentro del layout
