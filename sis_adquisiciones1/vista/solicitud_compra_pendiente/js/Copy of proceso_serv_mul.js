...
*/
function pag_pro_serv_mul(idContenedor,direccion,paramConfig){
	...
	var ds = new Ext.data.Store({
		...
		var ds_id_comprador = new Ext.data.Store({
			...
			*/
			...
			var lay_pro_ser_mul=new DocsLayoutMaestro(idContenedor);
				...
				function btn_proceso_compra_det(){
					...
					if(NumSelect!=0){
						...
						if(SelectionsRecord.data.tipo_adq=='Bien'){
							...
							//ejecutado en mayoria de los casos
						...
						lay_pro_ser_mul.loadWindows(direccion+url,'Procedimiento Detalle',ParamVentana);
						lay_pro_ser_mul.getVentana().on('resize',function(){
							lay_pro_ser_mul.getLayout().layout();
						})
					}
