...
$this->Cell(200,13,' ',0,1); 
$this->SetFont('Arial','B',8);
//$this->Cell(180,5,'Original','T',1,'R');
$this->SetFont('Arial','BI',14);

$this->Cell(200,5,'VISTA PREVIA SOLICITUD',0,1,'C'); 
$this->SetFont('Arial','BI',6);
$this->Cell(200,5,'(DOCUMENTO NO VALIDO PARA PROCEDER CON LA COMPRA)',0,1,'C'); 
$this->Cell(200,3,' ',0,1);
...
$this->MultiCell(70,4,''.$_SESSION['PDF_nombre_solicitante'].'',1,'L',1); 

$this->SetXY(115,$y);
 $this->SetFont('Arial','B',6);
$this->Cell(25,4,'Cargo:',0,0); 
$this->SetFont('Arial','',6);
$this->MultiCell(70,4,''.$_SESSION['PDF_cargo_empleado_solicitante'].'',1,'L',1);
...
$this->SetFont('Arial','',6);
$this->MultiCell(70,4,''.$_SESSION['PDF_nombre_unidad'].'',1,'L',1); 
...
$this->SetFont('Arial','',6);
$this->MultiCell(70,4,''.$_SESSION['PDF_nombre_programa'].'',1,'L',1);
...
$this->SetFont('Arial','',6);
$this->MultiCell(70,4,''.$_SESSION['PDF_nombre_regional'].'',1,'L',1);
$this->SetXY(115,$y);

$this->SetFont('Arial','B',6);
$this->Cell(25,4,'Sub-programa/Proyecto:',0,0);
$this->SetFont('Arial','',6);
$this->MultiCell(70,4,''.$_SESSION['PDF_nombre_proyecto'].'',1,'L',1);
...
$this->SetFont('Arial','',6);
$this->MultiCell(70,4,''.$_SESSION['PDF_nombre_financiador'].'',1,'L',1);
...
$this->SetFont('Arial','',6);
$this->MultiCell(70,4,''.$_SESSION['PDF_nombre_actividad'].'',1,'L',1);
...
$pdf->Cell(25,5,'Código','LRT',0); 
	...
	$pdf->Cell(90,5,'Descripción','LRT',0);
...
$pdf->Cell(25,5,'Total','LRB',1);
	...
	$pdf->SetWidths(array(25,90,10,15,25,25));
	$pdf->SetAligns(array('L','L','L','R','R','R'));
	$pdf->SetVisibles(array(1,1,1,1,1,1,0));
	$pdf->SetFontsSizes(array(5,7,7,7,7,7,7));
...
/*$pdf->SetFont('Arial','B',8);	
$pdf->SetWidths(array(5,15,65,65,20,20));
...
//$pdf->ln(10);
 ...
 if($_SESSION['PDF_titulo2']!=''){
	...
	$pdf->SetFont('Arial','B',8);
	$pdf->Cell(10,5,'No','LRT',0);  
	$pdf->Cell(20,5,'Código','LRT',0); 

	
		$pdf->Cell(88,4,'Descripción','LRT',0);
		$pdf->Cell(10,4,'Unidad','LRT',0);
	
	$pdf->Cell(15,4,'Cantidad','LRT',0);  
	$pdf->Cell(25,4,'Precio','LRT',0); 
	$pdf->Cell(25,4,'Precio','LRT',1);
	$pdf->Cell(10,4,'','LRB',0); 
	$pdf->Cell(20,4,'','LRB',0); 
	
		$pdf->Cell(88,4,'','LRB',0);
		$pdf->Cell(10,4,'Medida','LRB',0); 
	
	$pdf->Cell(15,4,'','LRB',0);  
	$pdf->Cell(25,4,'Unitario','LRB',0);
	$pdf->Cell(25,4,'Total','LRB',1);
	
	
	
	$pdf->SetWidths(array(10,20,88,10,15,25,25));
   ...
   $aux=$refo[0];
 	...
 	array_unshift($aux,$reformulados[$i][0]);
 		$pdf->Cell(193,0,'','T',1,'C');
 		$pdf->SetFont('Arial','',8);
 		$pdf->MultiTabla($aux,1,1);
 		$pdf->MultiCell(193,4,'MOTIVO REFORMULACIÓN: '.$aux[12],'LRBT');
 		//$pdf->ln(4);
...
/*$pdf->Cell(195,4,''.$_SESSION['PDF_nombre_aprobacion'].'',0,1,'C'); 
//$pdf->Cell(90,4,''.$_SESSION['PDF_nombre_solicitante'].'',0,1,'C'); 
$pdf->SetFont('Arial','B',8);
$pdf->Cell(195,4,''.strtoupper($_SESSION['PDF_cargo_empleado_aprobador']).'',0,1,'C'); 
//$pdf->Cell(90,4,''.strtoupper($_SESSION['PDF_cargo_empleado_solicitante']),0,1,'C'); 
$pdf->Cell(195,4,'Firma Autorizada',0,0,'C'); */
