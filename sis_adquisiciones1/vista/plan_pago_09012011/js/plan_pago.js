		...
		function verificarPago(resp){
			if(resp.responseXML!=undefined && resp.responseXML!=null&& resp.responseXML.documentElement!=null &&resp.responseXML.documentElement!=undefined){
				...
				if((root.getElementsByTagName('TotalCount')[0].firstChild.nodeValue)>0){
					...
					if(SelectionsRecord.data.nro_cuota==parseFloat(getComponente('num_pago').getValue()+1) || SelectionsRecord.data.id_cotizacion==	6811){
						getComponente('monto_original').disable();
				...
				}else{if(parseFloat(SelectionsRecord.data.nro_cuota)==1 || SelectionsRecord.data.id_cotizacion==	6811 ){
			...
			if(selected&&record!=-1){
				...
				if(maestro.estado_vigente=='orden_compra'){
					...
					if(record.estado=='pendiente'){
						...
						CM_getBoton('comprobante-'+idContenedor).disable();
						CM_getBoton('comprobante_pago-'+idContenedor).disable();
						if (maestro.tipo_pago!='caja' && maestro.tipo_pago!='avance'){ //if(record.id_plan_pago==11149){alert('aa'||maestro.tipo_pago);}
							CM_getBoton('devengar-'+idContenedor).enable();}
						else{ //if(record.id_plan_pago==11149){alert('---aa'||maestro.tipo_pago);}
							CM_getBoton('devengar-'+idContenedor).disable();
						}
						CM_getBoton('pagar-'+idContenedor).disable();
						CM_getBoton('provisional-'+idContenedor).disable();
