...
$this->Cell(180,5,'VISTA PREVIA SOLICITUD',0,1,'C');
...
$fecha='22-10-2009';
	$hora='19:23:36';
	    $this->SetY(-10);
   	    ...
   	    $this->Cell(70,3,'Usuario: '.'PEREDO ECHAZU ROBERTO',0,0,'L');
		...
		$this->ln(3);
		if($_SESSION['PDF_tipo_reporte']=='1'){
			$this->Cell(70,3,sha1($_SESSION['PDF_num_solicitud'].$_SESSION['PDF_nombre_financiador'].$_SESSION['PDF_nombre_regional'].$_SESSION['PDF_nombre_programa'].$_SESSION['PDF_nombre_proyecto'].$_SESSION['PDF_nombre_actividad'].gregoriantojd('10','22','2009')/*mes,dia,año*/.$hora.$_SESSION['PDF_monto_total']),0,0,'L');
		}
}
...
for($j=0;$j<$tam_data;$j++)
 {
 	...
 	if($data_maestro[$j][3]=='0' or $data_maestro[$j][3]=='0.00'){
	    ...
	    $pdf->Cell(30,5,'   '.$data_maestro[$j][4],0,1);
	     $pdf->SetFont('Arial','',10);
	    $pdf->SetXY($x,$y1);
 	}
 	//si tiene presupuestado apra el año siguiente
		...
		$pdf->Cell(95,5,'Nombre Partida',0,0,'L'); 
		$pdf->Cell(50,5,'Total Gestión Actual',0,0,'R'); 
		$pdf->Cell(25,5,'   Disponibilidad',0,1,'L'); 
		
	 	//$pdf->SetWidths(array(25,80,30,30,25));
	    ...
	    if($data_maestro[$j][4]=='NO DISPONIBLE'){
	     ...
	     $y=$pdf->GetY();
	    $x=$pdf->GetX();
	    $pdf->Cell(25,5,$data_maestro[$j][0],0,0);
	    $pdf->MultiCell(75,5,$data_maestro[$j][1],0);
	    $y1=$pdf->GetY();
	    $pdf->SetXY($x+100,$y);
	     $pdf->SetFont('Arial','B',10);
	    $pdf->Cell(70,5,number_format($data_maestro[$j][2],2),0,0,'R');
	    
	    $pdf->Cell(25,5,'   '.$data_maestro[$j][4],0,1);
	    $pdf->SetXY($x,$y1);
	   $pdf->SetFont('Arial','',10);
	     $pdf->Cell(100,5,'Monto referencial para Presupuesto de la siguiente gestión:',0,0,'L');
	    $pdf->Cell(70,5,number_format($data_maestro[$j][3],2),0,1,'R');
