	...
	for($j = 0;$j < $cont; $j++)
	{
		...
		if ($id_cotizacion == "undefined" || $id_cotizacion == "")
		{
		    ...
		    if($fin>0){
                 ...
                 $res = $Custom -> FinalizarCotizacionDir($id_cotizacion,$retencion,$id_empresa,$num_factura,$fecha_factura,$id_caja,$id_cajero,$impuestos,$precio_total,$id_responsable_adjudicacion);
   			        if(!$res){
        				//Se produjo un error
        				$resp = new cls_manejo_mensajes(true, "406");
        				$resp->mensaje_error = $Custom->salida[1] . " (iteración $cont)";
