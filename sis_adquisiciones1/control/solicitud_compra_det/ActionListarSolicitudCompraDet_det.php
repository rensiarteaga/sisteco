...
//Parámetros del filtro
	...
	if($m_id_solicitud_compra_det>0){
		...
		$criterio_filtro=$criterio_filtro. " AND SOLDET.id_solicitud_compra_det=$m_id_solicitud_compra_det";
	}
	...
	if($vista>0){
		...
		$criterio_filtro=$criterio_filtro. " AND SOLDET.estado_reg!=''anulado'' ";
	}
	if($simplificado>0){
		
		$criterio_filtro=$criterio_filtro. " AND SOLDET.id_solicitud_compra_det not in(select id_solicitud_compra_det from compro.vad_soldet_grupo where id_grupo_sp_det is not null) ";
		
	}
	
	
	//Obtiene el criterio de orden de columnas
	$crit_sort = new cls_criterio_sort($sortcol,$sortdir,'SolicitudCompraDet_det');
	...
	if($res)
	{
		...
		foreach ($Custom->salida as $f)
		{
			...
			$xml->add_nodo('total_gestion',$f["total_gestion"]);
			$xml->add_nodo('gestion',$f["gestion"]);
			$xml->add_nodo('monto_ref_reformulado_as',$f["monto_ref_reformulado_as"]);
			$xml->fin_rama();
		}
		$xml->mostrar_xml();
	}
