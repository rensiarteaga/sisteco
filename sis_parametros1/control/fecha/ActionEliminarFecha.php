...
/**
**********************************************************
Nombre de archivo:	    ActionEliminarFecha.php
Propósito:				Permite eliminar registros de la tabla fecha
Tabla:					tfv_fecha
...
$nombre_archivo = 'ActionEliminarFecha.php';
...
if($_SESSION['autentificado']=="SI")
{
	...
	for($j = 0;$j < $cont; $j++)
	{
		...
		{
			$resp = new cls_manejo_mensajes(true, "406");
			$resp->mensaje_error = "MENSAJE ERROR = No existe la Fecha especificada para eliminar.";
			$resp->origen = "ORIGEN = $nombre_archivo";
			$resp->proc = "PROC = $nombre_archivo";
			$resp->nivel = "NIVEL = 4";
			echo $resp->get_mensaje();
		...
		{	//Eliminación
			$res = $Custom-> EliminarFecha($hidden_id_fecha);
			if(!$res)
			{
				$resp = new cls_manejo_mensajes(true, "406");
