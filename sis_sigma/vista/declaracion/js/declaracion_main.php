...
var elemento={pagina:new pagina_declaracion(idContenedor,direccion,paramConfig),idContenedor:idContenedor};
...
function pagina_declaracion(idContenedor,direccion,paramConfig){
	...
	function btn_final_sigma(){
		var sm=getSelectionModel();
		var NumSelect=sm.getCount();
		var SelectionsRecord=sm.getSelected();
	
		if(NumSelect==1){
			if(confirm('¿Esta seguro de Finalizar la Declaración de la Ejecucion Presupuestaria?')){
				Ext.MessageBox.show({
					title: 'Procesando',
					msg:"<div><img src='../../../lib/ext-yui/resources/images/default/grid/loading.gif'/> Finalizando ...</div>",
					width:200,
					height:200,
					closable:false
				});
				Ext.Ajax.request({
					url:direccion+"../../../control/declaracion/ActionFinalDeclaracion.php",
					method:'POST',
					params:{cantidad_ids:'1',id_declaracion_0:SelectionsRecord.data.id_declaracion},
					success:function(resp){
						var root = resp.responseXML.documentElement;
	...
	this.AdicionarBoton('../../../lib/imagenes/drop-yes.gif','Finalizar la Declración de la Ejecución',btn_final_sigma,true,'final_tran','Finalizar Declaración');
	this.AdicionarBoton('../../../lib/imagenes/drop-no.gif','Anular la Declración de la Ejecución',btn_anular_sigma,true,'anula_tran','Anular Declaración');
	...
	CM_getBoton('final_tran-'+idContenedor).disable();
	...
	function enable(sm,row,rec){
		...
		if(rec.data['estado'] =='Generar_archivos'){
			...
			CM_getBoton('final_tran-'+idContenedor).enable();
