...
$nombre_archivo = "ActionGuardarRevVariable.php";
...
if($_SESSION["autentificado"]=="SI")
{
	...
	for($j = 0;$j < $cont; $j++)
	{
		if ($get)
		{
			...
			$importe_rev_aa=$_GET["revertir_pg_$j"];
			$importe_rev_as=$_GET["revertir_sg_$j"];

		}
		else
		{
			$id_proceso_compra= $_POST["id_proceso_compra_$j"];
			$importe_rev_aa=$_POST["revertir_pg_$j"];
			$importe_rev_as=$_POST["revertir_sg_$j"];
	...
	if ($id_proceso_compra != "undefined" && $id_proceso_compra != ""){
		    ...
		    $res = $Custom -> RevPresVariable($id_proceso_compra,$importe_rev_aa,$importe_rev_as);
		    if(!$res)
			{
				//Se produjo un error
				$resp = new cls_manejo_mensajes(true, "406");
