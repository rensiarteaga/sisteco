...
//esta restriccion es para listar solo los procesos que tengan detalles que esten  en grupo "activo"
    ...
    if($estado!='finalizado'){
       ...
       // if($estado_proceso=='inicio'){
        ...
        //}else{
           $criterio_filtro=$criterio_filtro ." AND PROCOM.id_proceso_compra in (select id_proceso_compra
                                    from compro.tad_proceso_compra_det pcdd
                                    inner join compro.tad_grupo_sp_det gsd on
                                    gsd.id_proceso_compra_det = pcdd.id_proceso_compra_det
                                    where id_proceso_compra = procom.id_proceso_compra) ";
    //}
     }
	...
	if($res)
	{
		...
		foreach ($Custom->salida as $f)
		{
			...
			$xml->add_nodo('es_item',$f["es_item"]);//jun2015
