...
*/
	...
	$datosCabecera['valor'][1]='descripcion';
...
//Parámetros del filtro
   ...
   // echo $tipo_reporte.'---'; exit;
    /*********************/
	    ...
	    if($id_uo!='%'){
	    	...
	    	if($tipo_adq=='Bien'){
	    		...
	    		if($id_item!=''){
	    		    $criterio_filtro=$criterio_filtro." AND (COTDET1.id_item_aprobado like ''$id_item'' or COTDET1.id_servicio like ''$id_item'') ";
				}
	    	}
    ...
    //Obtiene el conjunto de datos de la consulta
     ...
     if($rango=='mayores'){
        $criterio_filtro=$criterio_filtro. ' AND (COTDET1.precio*ADJUDI.cantidad_adjudicada) >='. $valor;}
        elseif ($rango=='menores'){
        	$criterio_filtro=$criterio_filtro. ' AND (COTDET1.precio*ADJUDI.cantidad_adjudicada) <'. $valor;}
        	elseif ($rango=='rango'){
        		$criterio_filtro=$criterio_filtro. ' AND (COTDET1.precio*ADJUDI.cantidad_adjudicada) <'. $valor_fin. ' AND (COTDET1.precio*ADJUDI.cantidad_adjudicada) >'.$valor;  }
        
     	
     $_SESSION["PDF_filtro"]=$rango;
     $_SESSION["PDF_importe"]=$valor;
     $_SESSION["PDF_importe_fin"]=$valor_fin;
    if($_SESSION["ss_id_usuario"]==120){
    	//echo $criterio_filtro.'----'.$tipo_reporte; exit;
    }
     if ($tipo_reporte==2){
	    /*echo $tipo_rep;
		exit;*/
  	     if ($tipo_rep=='EXCEL')
        	{
			  //echo "entra aqui".$criterio_filtro;
		      //exit;
    		  ...
    		  if ($_SERVER['REQUEST_METHOD'] == 'POST'){
	    		...
	    		elseif ($tipo_adq=='Servicio'){
  				...
  				require_once('../../../lib/lib_modelo/ReportDriver.php');
					...
					echo('nombre_usuario'.utf8_decode($nombre_usuario));
					echo('codigo_procedimiento'.$codigo_procedimiento_cons);
					
					echo('tipo_adq'.$tipo_adq);
					echo('ep'.$desc_ep);
					echo('uo'.$unidad_organizacional);
					echo('departamento'.$nombre_departamento);
					echo('proveedor'.$nombre_proveedor);
					echo('criterio_filtro'.$criterio_filtro);
				    exit;
					*/
