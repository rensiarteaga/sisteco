...
//Parámetros del filtro
	...
	if ($m_tipo!=1){
       $criterio_filtro=$criterio_filtro." AND COTIZA.estado_vigente!=''finalizado'' AND COTIZA.estado_vigente!=''anulado''";		
		 ...
		 $criterio_filtro=$criterio_filtro." AND COTIZA.estado_vigente in (''finalizado'')";		
	}	
	if($estado_vigente!=''){
	...
	if($res)
	{
		...
		foreach ($Custom->salida as $f)
		{
			...
			//5jun2015
			$xml->add_nodo('tiene_anticipo',$f["tiene_anticipo"]);
			$xml->add_nodo('anticipo_con_ejecucion',$f["anticipo_con_ejecucion"]);
			//jun2015
			$xml->add_nodo('es_item',$f["es_item"]);
				
			//ago2015
			$xml->add_nodo('es_ampliacion',$f["es_ampliacion"]);
			$xml->add_nodo('id_cotizacion_ampliacion',$f["id_cotizacion_ampliacion"]);
			$xml->add_nodo('desc_cotizacion_ampliacion',$f["desc_cotizacion_ampliacion"]);
