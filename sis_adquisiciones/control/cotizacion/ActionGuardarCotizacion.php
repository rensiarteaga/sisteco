...
if($_SESSION["autentificado"]=="SI")
{
	...
	for($j = 0;$j < $cont; $j++)
	{
		if ($get)
		{
			...
			$id_empleado_adjudicacion=$_GET["id_empleado_adjudicacion_$j"];
			$justificacion_adjudicacion=$_GET["justificacion_adjudicacion_$j"];
			$rec=$_GET["rec_$j"];
		...
		else
		{
			...
			$id_empleado_adjudicacion=$_POST["id_empleado_adjudicacion_$j"];
			$justificacion_adjudicacion=$_POST["justificacion_adjudicacion_$j"];
			$rec=$_POST["rec_$j"];
		...
		{	///////////////////////Modificación////////////////////
			...
			if($estado_vigente =='adjudicado'){
			    
			   
				 //$res = $Custom->FinalizarCotizacionAdj($id_cotizacion,$observaciones, $id_proceso_compra,$estado_vigente,$id_empleado_adjudicacion,$justificacion_adjudicacion);
				$res = $Custom->FinalizarCotizacionAdj($id_cotizacion,$fecha_adjudicacion, $id_proceso_compra,$estado_vigente,$id_empleado_adjudicacion,$justificacion_adjudicacion);//22/04/2010
					  if(!$res)
					   {
							//Se produjo un error
							$resp = new cls_manejo_mensajes(true, "406");
			    ...
			    if($justificacion_adjudicacion!='' || $rec==1){
			      
			        $res = $Custom->ModificarRecomendacion($id_cotizacion,$justificacion_adjudicacion);
			    ...
			    if($id_moneda>0 && $fecha_cotizacion!=''){
				   ...
				   $res = $Custom->ModificarCotizacionProp($id_cotizacion,$impuestos, $garantia, $lugar_entrega, $forma_pago, $tiempo_validez_oferta, $fecha_entrega, $tipo_entrega, $observaciones, $id_proceso_compra, $id_moneda, $id_proveedor, $id_tipo_categoria_adq, $precio_total, $figura_acta, $num_cotizacion,$fecha_cotizacion,$id_empresa,$fecha_recepcion_propuestas,$factura_total,$num_autoriza_factura,$cod_control_factura,$fecha_factura);
				     ...
				     //Validación de datos (del lado del servidor)
				 	/* $res = $Custom->ValidarCotizacion("update",$id_cotizacion, $fecha_venc, $fecha_reg,  $impuestos, $garantia, $lugar_entrega, $forma_pago, $tiempo_validez_oferta, $fecha_entrega, $tipo_entrega, $observaciones, $id_proceso_compra, $id_moneda, $id_proveedor, $id_tipo_categoria_adq, $precio_total, $figura_acta, $num_factura, $num_orden_compra, $estado_vigente, $estado_reg, $nombre_pago, $siguiente_estado, $periodo, $gestion, $num_orden_compra_sis, $num_cotizacion,$fecha_orden_compra);
						if(!$res)
						{
							//Error de validación
							$resp = new cls_manejo_mensajes(true, "406");
