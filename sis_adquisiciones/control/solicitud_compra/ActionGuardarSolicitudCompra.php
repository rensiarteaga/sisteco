...
if($_SESSION["autentificado"]=="SI")
{
	...
	//mayo2016
	if($_POST["es_modificacion_0"]=='' && $_POST["correo_0"]!='0' && $id_solicitud_compra>0){
		$mensaje= '<p>
						 Se ha finalizado la solicitud <b>Nro '.$_POST["nro_fin_0"].'</b> , del tipo <b>'.
							 $_POST["tipo_adq_fin_0"].'</b>
			             por <b>'. $_POST["monto_fin_0"] .'</b>
			             <p>efectuada por el funcionario:  <b>'.$_POST["usuario_fin_0"].' </b></p>
						 <p>a nombre de :  <b>'.$_POST["solicitante_fin_0"].' </b></p>
			             ';
			
		$referencia='Notificación de Emisión de Solicitud de Compra';
			
	
		$correo_enviar=$_POST["correo_0"];
		/*if($_SESSION["ss_id_usuario"]==120){
			echo "llega aqui ".$_POST["es_modificacion_0"].'----'.$_POST["correo_0"].'---'.$correo_enviar; exit;
		}*/
		if($correo_enviar=='0'){
			$correo_enviar='mercedes.zambrana@ende.bo';
		}
			
		$var_cor = $correo ->EnviarCorreodeFlujo('endesis@ende.bo',$correo_enviar,'COMPRO: '.$referencia,$mensaje,'ENDESIS - SISTEMA DE ADQUISICIONES', 'INF. SOLICITUD DE COMPRA');
