...
class cls_DBPlanPago
{
	...
	function ListarPlanPago($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad,$id_cotizacion)
	{
		...
		$this->var->add_def_cols('motivo','text');
		...
		$this->var->add_def_cols('fk_id_plan_pago','integer');
	...
	function InsertarPlanPago($id_plan_pago,$tipo_pago,$nro_cuota,$fecha_pago,$monto,$estado,$id_cotizacion,$id_gestion,$id_cuenta_doc,$por_anticipo,$por_retgar)
	{
		...
		$this->var->add_param($id_cuenta_doc);//avance;
		$this->var->add_param($por_anticipo);//por_anticipo -- 140809
		$this->var->add_param($por_retgar);//por_retgar --140809
		$this->var->add_param("NULL");//descuento_anticipo --291009
		$this->var->add_param("NULL");//monto_no_pagar --251109
		$this->var->add_param("NULL");//fecha de devengado --09/01/2010
		$this->var->add_param("NULL");//accion (devengar o pagar) --09/01/2010
		
		
		$this->var->add_param("NULL");//descuentos --01/03/2010
		$this->var->add_param("NULL");//obs_descuentos --01/03/2010
		//Ejecuta la funci�n
	...
	function ModificarPlanPago($id_plan_pago,$tipo_pago,$nro_cuota,$fecha_pago,$monto,$estado,$id_cotizacion,$fecha_pagado,$num_factura,$observaciones,$boleta_garantia,$num_autoriza_factura,$cod_control_factura,$fecha_factura,$multas,$impuestos,$id_gestion,$id_cuenta_doc,$por_anticipo,$por_retgar,$descuento_anticipo,$monto_no_pagado,$fecha_devengado,$accion,$descuentos,$obs_descuentos)
	{
		...
		$this->nombre_funcion = 'f_tad_plan_pago_iud';
		if($accion=='pagar')
			$this->codigo_procedimiento = "'AD_PAGARPAG_UPD'";
		else 
		...
		$this->var->add_param("$descuento_anticipo");//descuento_anticipo --291009
		$this->var->add_param("$monto_no_pagado");//monto_no_pagar --251109
		$this->var->add_param("'$fecha_devengado'");//fecha de devengado --09/01/2010
		$this->var->add_param("'$accion'");//accion (devengar o pagar) --09/01/2010
		$this->var->add_param("$descuentos");//descuentos --01/03/2010
		$this->var->add_param("'$obs_descuentos'");//obs_descuentos --01/03/2010
...
$this->var->add_param("NULL");//("'$boleta_garantia'");
	...
	function RegistrarDevengado($id_cotizacion, $tipo_documento, $num_factura, $fecha_factura, $observaciones_devengado, $fecha_devengado, $importe_devengar,$id_gestion)
	{
		$this->salida = "";
		$this->nombre_funcion = 'f_tad_plan_pago_iud';
		$this->codigo_procedimiento = "'AD_DEVCOT_UPD'";
		...
		$this->var->add_param("NULL");
		$this->var->add_param("NULL");
		$this->var->add_param($importe_devengar);
		$this->var->add_param("NULL");
		$this->var->add_param($id_cotizacion);
		$this->var->add_param("NULL");
		$this->var->add_param($num_factura);
		$this->var->add_param("'$observaciones_devengado'");
		...
		$this->var->add_param("NULL");
		$this->var->add_param("NULL");
		$this->var->add_param($tipo_documento);//impuestos;
		$this->var->add_param($id_gestion);//gestion;
		$this->var->add_param("NULL");//avance;
	...
	function ListarPlanPagoCuota($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad,$id_cotizacion)
	{
		...
		$this->var->add_def_cols('por_anticipo','numeric');
		$this->var->add_def_cols('por_retgar','numeric');
		$this->var->add_def_cols('pagado_anticipo','numeric');
...
$this->var->add_param("NULL");//("'$boleta_garantia'");
	...
	function ModificarPagoAS($id_cotizacion,$total_aa,$total_as)
	{
		$this->salida = "";
		$this->nombre_funcion = 'f_tad_plan_pago_iud';
		$this->codigo_procedimiento = "'AD_PAGOAS_UPD'";

		//Instancia la clase midlle para la ejecuci�n de la funci�n de la BD
		$this->var = new cls_middle($this->nombre_funcion,$this->codigo_procedimiento,$this->decodificar);
		$this->var->add_param("NULL");
		$this->var->add_param("NULL");//("'$tipo_pago'");
		$this->var->add_param("NULL");//($nro_cuota);
		$this->var->add_param("NULL");//("'$fecha_pago'");
		$this->var->add_param("NULL");//($monto);
		...
		$this->var->add_param("NULL");//("'$fecha_factura'");
		$this->var->add_param("NULL");//($multas);
		$this->var->add_param("NULL");//(0);
		$this->var->add_param("NULL");//($impuestos);//impuestos;
		$this->var->add_param("NULL");//($id_gestion);//gestion;
		$this->var->add_param("NULL");//($id_avance);//avance;
		$this->var->add_param("$total_aa");//por_anticipo;
		$this->var->add_param("$total_as");//por_retgar;
		$this->var->add_param("NULL");//descuento_anticipo --291009
		$this->var->add_param("NULL");//monto_no_pagar --251109
		$this->var->add_param("NULL");//fecha de devengado --09/01/2010
		$this->var->add_param("NULL");//accion (devengar o pagar) --09/01/2010
...
//exit;
	...
	function UnirPagos($id_cotizacion,$id_pp)
	{
		$this->salida = "";
		$this->nombre_funcion = 'f_tad_plan_pago_iud';
		$this->codigo_procedimiento = "'AD_UNIRPAG_UPD'";
		...
		$this->var->add_param("NULL");//descuentos --01/03/2010
		$this->var->add_param("'$id_pp'");//obs_descuentos --01/03/2010
