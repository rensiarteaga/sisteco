...
var elemento={pagina:new pag_com_rap_bien(idContenedor,direccion,paramConfig,bandera),idContenedor:idContenedor};
...
function pag_com_rap_bien(idContenedor,direccion,paramConfig,bandera){
	...
	var ds = new Ext.data.Store({
		...
		'gestion',
		'categoria',
		'permite_agrupar','avance','gestion_sgte','id_depto','nro_solicitud_cadena','es_item'
		]),remoteSort:true});
...
// txt fecha_reg
		...
		Atributos[21]= {
			...
			if(bandera=='Bien'){
				config={titulo_maestro:'compra_rapida',grid_maestro:'grid-'+idContenedor,urlHijo:'../../../sis_adquisiciones/vista/detalle_solicitud_servicio/detalle_seguimiento_solicitud_det.php'};
			}
			else{
			...
			function iniciarEventos(){ 
				...
				txt_fecha_reg=getComponente('fecha_reg');
			...
			function cargar_respuesta(resp){ 
				...
				if(resp.responseXML !=undefined && resp.responseXML !=null && resp.responseXML.documentElement !=null && resp.responseXML.documentElement !=undefined)
				{
					...
					if(confirm(mensaje)){
						//FER-MOD-AD-07
						CM_mostrarComponente(txt_fecha_reg);
						CM_ocultarComponente(getComponente('fecha'));
						getComponente('fecha').setValue('01/01/'+getComponente('gestion').getValue());
						getComponente('fecha_reg').minValue=getComponente('fecha').getValue();
						var miFecha=new Date();
						var mi_gestion=miFecha.getFullYear();
						
						if(parseFloat(mi_gestion)>parseFloat(getComponente('gestion').getValue())){
							getComponente('fecha').setValue('31/12/'+getComponente('gestion').getValue());
						}else{
							getComponente('fecha').setValue(new Date());
						}
						getComponente('fecha_reg').setValue(getComponente('fecha').getValue());
						getComponente('fecha_reg').maxValue=getComponente('fecha_reg').getValue();
						CM_ocultarComponente(getComponente('gestion'));
						//--FER-MOD-AD-07
