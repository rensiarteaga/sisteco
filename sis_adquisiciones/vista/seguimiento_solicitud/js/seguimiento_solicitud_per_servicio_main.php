...
var elemento={pagina:new pagina_seguimiento_solicitud_per_servicio(idContenedor,direccion,paramConfig),idContenedor:idContenedor};
 ...
 * Fecha creación:		2008-05-15 19:39:24
 */
function pagina_seguimiento_solicitud_per_servicio(idContenedor,direccion,paramConfig){
...
// txt id_fina_regi_prog_proy_acti
	...
	var config={titulo_maestro:'seguimiento_solicitud',grid_maestro:'grid-'+idContenedor,urlHijo:'../../../sis_adquisiciones/vista/detalle_solicitud_servicio/detalle_seguimiento_solicitud_det.php'};
	var layout_seguimiento_solicitud_per_servicio=new DocsLayoutMaestroDeatalle(idContenedor);
	layout_seguimiento_solicitud_per_servicio.init(config);
	...
	this.pagina(paramConfig,Atributos,ds,layout_seguimiento_solicitud_per_servicio,idContenedor);
	...
	function btn_estado_proceso(){
		...
		if(NumSelect!=0){
			...
			layout_seguimiento_solicitud_per_servicio.loadWindows(direccion+'../../../../sis_adquisiciones/vista/historial_solicitud/historial_solicitud.php?'+data,'Historial Solicitud',ParamVentana);
layout_seguimiento_solicitud_per_servicio.getVentana().on('resize',function(){
			layout_seguimiento_solicitud_per_servicio.getLayout().layout();
				})
		}
	else
	{
	...
	this.EnableSelect=function(x,z,y){
		...
		enable(x,z,y);
		_CP.getPagina(layout_seguimiento_solicitud_per_servicio.getIdContentHijo()).pagina.reload(y.data);
		_CP.getPagina(layout_seguimiento_solicitud_per_servicio.getIdContentHijo()).pagina.desbloquearMenu();
	...
	function iniciarEventosFormularios(){
		getSelectionModel().on('rowdeselect',function(){
					if(_CP.getPagina(layout_seguimiento_solicitud_per_servicio.getIdContentHijo()).pagina.limpiarStore()){
						_CP.getPagina(layout_seguimiento_solicitud_per_servicio.getIdContentHijo()).pagina.bloquearMenu()
					}
				})
	...
	this.getLayout=function(){return layout_seguimiento_solicitud_per_servicio.getLayout()};
	...
	layout_seguimiento_solicitud_per_servicio.getLayout().addListener('layout',this.onResize);//aregla la forma en que se ve el grid dentro del layout
