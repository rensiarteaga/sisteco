...
var elemento={pagina:new pagina_seguimiento_solicitud(idContenedor,direccion,paramConfig),idContenedor:idContenedor};
 ...
 * Fecha creación:		2008-05-15 19:39:24
 */
function pagina_seguimiento_solicitud(idContenedor,direccion,paramConfig){
...
// txt id_fina_regi_prog_proy_acti
	...
	var layout_seguimiento_solicitud=new DocsLayoutMaestroEP(idContenedor);
	layout_seguimiento_solicitud.init(config);
	...
	this.pagina(paramConfig,Atributos,ds,layout_seguimiento_solicitud,idContenedor);
	...
	function btn_estado_proceso(){
		...
		if(NumSelect!=0){
			...
			layout_seguimiento_solicitud.loadWindows(direccion+'../../../../sis_adquisiciones/vista/historial_solicitud/historial_solicitud.php?'+data,'Historial Solicitud',ParamVentana);
layout_seguimiento_solicitud.getVentana().on('resize',function(){
			layout_seguimiento_solicitud.getLayout().layout();
				})
		}
	else
	{
	...
	function btn_solicitud_compra_det(){
		...
		if(NumSelect!=0){
			...
			if(SelectionsRecord.data.tipo_adq=='Bien'){
				layout_seguimiento_solicitud.loadWindows(direccion+'../../../../sis_adquisiciones/vista/detalle_seguimiento_solicitud_bien/detalle_seguimiento_solicitud_det.php?'+data,'Detalle Seguimiento Solicitud',ParamVentana);
			}
			else{
				layout_seguimiento_solicitud.loadWindows(direccion+'../../../../sis_adquisiciones/vista/detalle_seguimiento_solicitud_servicio/detalle_seguimiento_solicitud_det.php?'+data,'Detalle Seguimiento Solicitud',ParamVentana);
			}
...
layout_seguimiento_solicitud.getVentana().on('resize',function(){
	...
	this.getLayout=function(){return layout_seguimiento_solicitud.getLayout()};
	...
	//para agregar botones
		...
		this.AdicionarBoton('../../../lib/imagenes/cancel.png','Anular Solicitud',btn_anular,true,'anular_solicitud','Anulación');
			...
			var record=selected.data;
                if(selected&&record!=-1){
					   if(record.reformulacion=='1' || record.estado=='finalizado' || record.estado=='anulado'){
					       ...
					       if(record.estado!='borrador' && record.estado!='pendiente_pre_aprobacion' && record.estado!='pre_aprobado' && record.estado!='presupuesto_verificado' && record.estado!='anulado'){
					           	CM_getBoton('reporte_solicitud-'+idContenedor).enable();
					       }else{
					           	CM_getBoton('reporte_solicitud-'+idContenedor).disable();
					       }
	...
	layout_seguimiento_solicitud.getLayout().addListener('layout',this.onResize);//aregla la forma en que se ve el grid dentro del layout
