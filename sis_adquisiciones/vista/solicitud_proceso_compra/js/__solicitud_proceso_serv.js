...
*/
function pag_sol_pro_serv(idContenedor,direccion,paramConfig,maestro,idContenedorPadre)
{
	...
	var ds = new Ext.data.Store({
		...
		var config={titulo_maestro:'Procedimiento de Compra',grid_maestro:'grid-'+idContenedor,urlHijo:'../../../sis_adquisiciones/vista/solicitud_proceso_compra/solicitud_compra_mul_serv_det.php'};

		var lay_sol_proc_serv = new DocsLayoutMaestroDetalleEP(idContenedor);
		lay_sol_proc_serv.init(config);
		...
		//---------- INICIAMOS HERENCIA
		this.pagina = Pagina;
		this.pagina(paramConfig,Atributos,ds,lay_sol_proc_serv,idContenedor);
		...
		var paramFunciones={
			...
			this.reload=function(params){
				...
				_CP.getPagina(lay_sol_proc_serv.getIdContentHijo()).pagina.limpiarStore();
				this.btnActualizar();
			...
			this.EnableSelect=function(sm,row,rec){
				...
				_CP.getPagina(lay_sol_proc_serv.getIdContentHijo()).pagina.reload(rec.data);
				_CP.getPagina(lay_sol_proc_serv.getIdContentHijo()).pagina.desbloquearMenu();
			}
			...
			function btn_solicitud_compra_det(){
				...
				if(NumSelect!=0){
					...
					lay_sol_proc_serv.loadWindows(direccion+url,'Detalle de Solicitud de Compra',ParamVentana);
					lay_sol_proc_serv.getVentana().on('resize',function(){
						lay_sol_proc_serv.getLayout().layout()
					})
				}
				else{
			...
			function iniciarEventosFormularios(){
				...
				getSelectionModel().on('rowdeselect',function(){
					if(_CP.getPagina(lay_sol_proc_serv.getIdContentHijo()).pagina.limpiarStore()){
						_CP.getPagina(lay_sol_proc_serv.getIdContentHijo()).pagina.bloquearMenu()
					}
				})
			}

			//para que los hijos puedan ajustarse al tamaño
			this.getLayout=function(){return lay_sol_proc_serv.getLayout()};
			...
			lay_sol_proc_serv.getLayout().addListener('layout',this.onResize);
