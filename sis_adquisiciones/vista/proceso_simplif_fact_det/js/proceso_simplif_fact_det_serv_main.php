...
function proc_simplif_fact_det_serv(idContenedor,direccion,paramConfig,idContenedorPadre)
{
	...
	var ds = new Ext.data.Store({
		...
		,'estado','id_proceso_compra','precio_moneda_cotizada','desc_moneda','registro_adjudicado','reformular','especificaciones_tecnicas'
		//jun2015
		,'codigo_item','nombre_item','id_item'
	...
	//---------- INICIAMOS LAYOUT DETALLE
		...
		var paramFunciones={
			...
			this.reload=function(m){
				...
				Atributos[13].defecto=maestro.id_cotizacion;
			...
			function btn_detalle_adjudicacion(){
				...
				if(NumSelect!=0){
						...
						if(sw_grup){
							...
							if(maestro.es_item==0){//es servicio
				                 if(SelectionsRecord.data.id_servicio_cotizado>0){
						              item_cotiz=SelectionsRecord.data.id_servicio_cotizado;
						         }else{
							          item_cotiz=0;
						         }
								 InitDetalleAdj(SelectionsRecord.data.id_servicio,SelectionsRecord.data.id_cotizacion_det,SelectionsRecord.data.cantidad,item_cotiz,adj,maestro.es_item);
					        }else{
					        	if(SelectionsRecord.data.id_item>0){
						              item_cotiz=SelectionsRecord.data.id_item;
						         }else{
							          item_cotiz=0;
						         }
								 InitDetalleAdj(SelectionsRecord.data.id_item,SelectionsRecord.data.id_cotizacion_det,SelectionsRecord.data.cantidad,item_cotiz,adj,maestro.es_item);
					        }
						  }else{
							  if(maestro.es_item==0){
								    if(SelectionsRecord.data.id_servicio_cotizado>0){
									...
									reloadDetalleGrupo(SelectionsRecord.data.id_servicio,SelectionsRecord.data.id_cotizacion_det, SelectionsRecord.data.cantidad,item_cotiz,adj,maestro.es_item);
									 ...
									 reloadDetalleGrupo(SelectionsRecord.data.id_item,SelectionsRecord.data.id_cotizacion_det, SelectionsRecord.data.cantidad,item_cotiz,adj,maestro.es_item);
...
function btn_reformulacion(){
				...
				if(NumSelect!=0){
						...
						if(sw_grup){
							...
							if(SelectionsRecord.data.id_item>0){
								...
								reloadDetalleGrupo(SelectionsRecord.data.id_item,SelectionsRecord.data.id_cotizacion_det, SelectionsRecord.data.cantidad,item_cotiz,adj);
							}
			...
			function InitDetalleAdj(id_servicio,id_cotizacion,cantidad_cotizada,item_cotizado,adj,es_item){
...
function render_incluido(value, p, record){
							...
							if(cont>0){//cant de regis modif > 0?
								if(confirm("¿Está seguro de guardar los cambios?")){
									...
									for(i=0;i<cont;i++){
										...
										+"&motivo_ref_"+i+"="+record['motivo_ref']+"&id_item_"+i+"="+record['id_item']
	        ...
	        function reloadDetalleGrupo(id_servicio,id_cotizacion,cantidad_cotizada,item_cotizado,adj,es_item){
