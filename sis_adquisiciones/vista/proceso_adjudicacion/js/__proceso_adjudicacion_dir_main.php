...
var elemento={pagina:new pag_procAdjDir(idContenedor,direccion,paramConfig),idContenedor:idContenedor};
...
*/
function pag_procAdjDir(idContenedor,direccion,paramConfig){
	...
	var ds = new Ext.data.Store({
	 ...
	 Atributos[14]={
		...
		lay_proc_adj_dir=new DocsLayoutMaestroDeatalle(idContenedor);
		lay_proc_adj_dir.init(config);
		...
		this.pagina(paramConfig,Atributos,ds,lay_proc_adj_dir,idContenedor);
...
//						 }
                        ...
                        window.open(direccion+'../../../control/adjudicacion/ActionPDFAdjudicacion.php?id_proceso_compra='+SelectionsRecord.data.id_proceso_compra);
...
//alert((parseFloat(cont_se_adj)+parseFloat(e.record.data.cant_se_adjudica))+"---"+e.record.data.cantidad_sol+"....."+e.record.data.cant_se_adjudica);
			...
			function btn_adjudicacion(){
				...
				if(NumSelect!=0){
					...
					if(SelectionsRecord.data.tipo_adq=='Bien'){
					    lay_proc_adj_dir.loadWindows(direccion+'../../../../sis_adquisiciones/vista/proceso_adjudicacion_det/proceso_adjudicacion_dir_det.php?'+data,'Cotizacion de Proceso',ParamVentana);
					}else{
					    lay_proc_adj_dir.loadWindows(direccion+'../../../../sis_adquisiciones/vista/proceso_adjudicacion_det/proceso_adjudicacion_serv_dir_det.php?'+data,'Cotizacion de Proceso',ParamVentana);
					}

					lay_proc_adj_dir.getVentana().on('resize',function(){
			...
			function iniciarEventosFormularios(){
				...
				getSelectionModel().on('rowdeselect',function(){
					...
					if(_CP.getPagina(lay_proc_adj_dir.getIdContentHijo()).pagina.limpiarStore()){
						_CP.getPagina(lay_proc_adj_dir.getIdContentHijo()).pagina.bloquearMenu()
					}
				})
			}
			...
			this.EnableSelect=function(x,z,y){
			 enable(x,z,y);
			 _CP.getPagina(lay_proc_adj_dir.getIdContentHijo()).pagina.reload(y.data);
				_CP.getPagina(lay_proc_adj_dir.getIdContentHijo()).pagina.desbloquearMenu();
		    }

			//para que los hijos puedan ajustarse al tamaño
			this.getLayout=function(){return lay_proc_adj_dir.getLayout()};
			...
			this.AdicionarBoton('../../../lib/imagenes/copy.png','Cotizaciones Recepcionadas',btn_adjudicacion,true,'adjudicacion','Cotizaciones Recepcionadas');
			...
			lay_proc_adj_dir.getLayout().addListener('layout',this.onResize);//aregla la forma en que se ve el grid dentro del layout
