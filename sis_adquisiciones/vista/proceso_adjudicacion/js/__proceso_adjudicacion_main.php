...
var elemento={pagina:new pagina_proceso_adjudicacion(idContenedor,direccion,paramConfig),idContenedor:idContenedor};
...
*/
function pagina_proceso_adjudicacion(idContenedor,direccion,paramConfig){
	...
	var ds = new Ext.data.Store({
		...
		'desc_tipo_adq',
		'tipo_adq',
		'id_tipo_adq','id_proceso_compra_ant','num_convocatoria','id_cotizacion','id_moneda_base','numeracion_periodo_proceso','proceso_adjudicado','ejecutado','cantidad_sol','cant_se_adjudica','numeracion_periodo_cotizacion'
		]),remoteSort:true});
		...
		layout_proceso_adjudicacion=new DocsLayoutMaestro(idContenedor);
		layout_proceso_adjudicacion.init(config);
		...
		this.pagina(paramConfig,Atributos,ds,layout_proceso_adjudicacion,idContenedor);
...
//alert((parseFloat(cont_se_adj)+parseFloat(e.record.data.cant_se_adjudica))+"---"+e.record.data.cantidad_sol+"....."+e.record.data.cant_se_adjudica);
			...
			function btn_adjudicacion(){
				...
				if(NumSelect!=0){
					...
					var ParamVentana={Ventana:{width:'90%',height:'70%'}}
					   ...
					   layout_proceso_adjudicacion.loadWindows(direccion+'../../../../sis_adquisiciones/vista/proceso_adjudicacion_det/proceso_adjudicacion_dir_det.php?'+data,'Cotizacion de Proceso',ParamVentana);
					

					layout_proceso_adjudicacion.getVentana().on('resize',function(){
						layout_proceso_adjudicacion.getLayout().layout();
					});
			...
			this.getLayout=function(){return layout_proceso_adjudicacion.getLayout()};
			...
			layout_proceso_adjudicacion.getLayout().addListener('layout',this.onResize);//aregla la forma en que se ve el grid dentro del layout
