...
{	var vectorAtributos=new Array;
	...
	var ds_servicio= new Ext.data.Store({proxy: new Ext.data.HttpProxy({url: direccion+'../../../../control/servicio/ActionListarServicio_det.php?origen=filtro&tipo_vista=reporte_servicios_proveedores'}),
			reader: new Ext.data.XmlReader({
			record: 'ROWS',
			id: 'id_servicio',
			totalRecords: 'TotalCount'
	...
	vectorAtributos[0]={
		validacion:{
			...
			onSelect: function(record){ClaseMadre_getComponente('sw_servicio_clasificacion').setValue(record.data.id);
				...
				ClaseMadre_getComponente('id_tipo_adq').enable();
					//CM_mostrarGrupo('Item');
				CM_ocultarGrupo('Servicios');
				ClaseMadre_getComponente('id_servicio').allowBlank=true;
			}},		
			valueField:'id',
			displayField:'valor',
			lazyRender:true,
  ...
  var onTipoServicioSelect = function(e) {
		...
		combo_tipo_adq.on('change',funComboTipoAdq);
