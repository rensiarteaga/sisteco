...
var elemento={pagina:new ReporteOrdenCompra(idContenedor,direccion,paramConfig),idContenedor:idContenedor};
...
var tpl_id_unidad_organizacional=new Ext.Template('<div class="search-item">','<b><i>{id_unidad_organizacional} - {nombre_unidad}</b></i>','</div>');
...
vectorAtributos[0]={
		validacion:{
			...
			loadMask:true,
			triggerAction:'all',
			store:new Ext.data.SimpleStore({fields:['ID','valor'],data:[['1','Ordenes de Compra con EPs y UOs'],['2','Ordenes de Compra a detalle'],['3','Ordenes de Compra Auditoria'],['4','OC por partida/importe']]}),
			valueField:'ID',
			displayField:'valor',
			forceSelection:true,
			width:200
...
vectorAtributos[15]={
	...
	vectorAtributos[19]={
			validacion:{
				...
				loadMask:true,
				triggerAction:'all',
				store:new Ext.data.SimpleStore({fields:['valor'],data:[['todos'],['mayores'],['menores'],['rango']]}),
				valueField:'valor',
				displayField:'valor',
				forceSelection:true,
				width:150
	...
	{			
		...
		CM_ocultarComponente(CM_getComponente('importe_fin'));
	...
	function evento_tipo_adq(combo,record,index){
		...
		}else {if (componentes[0].getValue()=='3')
		{
	...
	function evento_tipo_reporte(combo,record,index){ 
		...
		/*if(componentes[0].getValue()!='2'){
			...
			if (componentes[0].getValue()!='3'){//es 1
					CM_ocultarGrupo('Reporte');
					//componentes[].allowBlank=
			}else{
				 if(componentes[0].getValue()!=4){
				 	
				 	CM_mostrarComponente(CM_getComponente('rep_fecha_ini'));
				 	CM_mostrarComponente(CM_getComponente('rep_fecha_fin'));
				 	CM_ocultarComponente(CM_getComponente('rango_monto'));
				 	CM_ocultarComponente(CM_getComponente('importe'));
				 	CM_ocultarComponente(CM_getComponente('id_partida'));
				 	CM_ocultarComponente(CM_getComponente('id_gestion'));
		...
		CM_getComponente('id_gestion').reset();
		 	...
		 	CM_getComponente('importe_fin').reset();
		/////////////////77
		if(componentes[0].getValue()=='1' || componentes[0].getValue()=='3'  ||componentes[0].getValue()=='4'){
			CM_ocultarGrupo('Servicios');
			CM_ocultarGrupo('Items');
			CM_ocultarGrupo('Reporte');
			...
			if(componentes[0].getValue()=='4'){
				 	...
				 	CM_ocultarComponente(CM_getComponente('fecha_ini'));
				 	CM_ocultarComponente(CM_getComponente('fecha_fin'));
				 	CM_mostrarComponente(CM_getComponente('rango_monto'));
				 	CM_mostrarComponente(CM_getComponente('importe'));
				 	CM_mostrarComponente(CM_getComponente('importe_fin'));
				 	...
				 	CM_getComponente('id_gestion').allowBlank=false;
				 	CM_getComponente('id_partida').allowBlank=false;
				 	
			}else{
					CM_mostrarComponente(CM_getComponente('fecha_ini'));
				 	CM_mostrarComponente(CM_getComponente('fecha_fin'));
				 	CM_ocultarComponente(CM_getComponente('rango_monto'));
				 	CM_ocultarComponente(CM_getComponente('importe'));
				 	CM_ocultarComponente(CM_getComponente('importe_fin'));
				 	CM_ocultarComponente(CM_getComponente('id_partida'));
				 	...
				 	CM_getComponente('id_gestion').allowBlank=true;
				 	CM_getComponente('id_partida').allowBlank=true;
				 	
				 	
				 	
				 	if(componentes[0].getValue()==3){
				 		CM_mostrarGrupo('Reporte');
				 	}else{
				 		CM_ocultarGrupo('Reporte');
				 	}
			...
			/*if (componentes[0].getValue()=='2'){//es 1
					CM_mostrarGrupo('Reporte');
					//componentes[].allowBlank=
			}else{
	...
	var paramFunciones={
		Formulario:{labelWidth:75,
		           // url:direccion+'../../../../../sis_adquisiciones/control/_reportes/orden_compra/ActionPDFOrdenCompraDetallado.php',
				    url:direccion+'../../../../../sis_adquisiciones/control/cotizacion/ActionPDFOrdenCompraDetallado.php',
		            abrir_pestana:true,
	...
	layout_rep_orden_compra.getLayout().addListener('layout',this.onResize);//aregla la forma en que se ve el grid dentro del layout
