<?php
/**
 * Nombre:		  	    cotizacion_serv_dir_main.php
...
/**
* Nombre:		  	    pagina_cotizacion_serv_dir.js
* Propósito: 			pagina objeto principal
* Autor:				Generado Automaticamente
* Fecha creación:		2008-05-28 17:32:05
*/
function pagina_cotizacion_serv_dir(idContenedor,direccion,paramConfig,maestro,idContenedorPadre)
{
	...
	var ds = new Ext.data.Store({
		...
		'factura_total','num_autoriza_factura','cod_control_factura',{name: 'fecha_factura',type:'date',dateFormat:'Y-m-d'},'falta_adjudicar','justificacion_adjudicacion','total_cotizado'
		//jun2015
		,'es_item'
		//ago2016 REQ16082016092652 de Ronald Coronel
		,'num_sol_por_proc'
		]),remoteSort:true});
	...
	//----------- FUNCIONES RENDER
		...
		var config={titulo_maestro:' (Maestro)',grid_maestro:'grid-'+idContenedor,urlHijo:'../../../sis_adquisiciones/vista/cotizacion_det/cotizacion_serv_dir_det.php'};
		var layout_cot_serv_dir = new DocsLayoutMaestroDeatalle(idContenedor,idContenedorPadre);
		layout_cot_serv_dir.init(config);



		//---------- INICIAMOS HERENCIA
		this.pagina = Pagina;
		this.pagina(paramConfig,Atributos,ds,layout_cot_serv_dir,idContenedor);
		...
		var paramFunciones={
			...
			this.reload=function(params){
				...
				maestro.es_item=maestro.es_item;
				...
				_CP.getPagina(layout_cot_serv_dir.getIdContentHijo()).pagina.limpiarStore();
			...
			this.btnEdit=function(){
				...
				if(NumSelect!=0){
					var SelectionsRecord=sm.getSelected();
					if(SelectionsRecord.data.estado_vigente=='pendiente' || SelectionsRecord.data.estado_vigente=='invitado' ||  SelectionsRecord.data.estado_vigente=='aperturado'){
						CM_ocultarGrupo('Oculto');
						CM_ocultarGrupo('Datos Entrega Pedido');
						CM_ocultarGrupo('Datos Oferta');
						CM_ocultarGrupo('Pago');
			...
			function btn_fin_propuestas(){
				...
				if(NumSelect!=0){
					...
					if(SelectionsRecord.data.estado_vigente=='aperturado' ||SelectionsRecord.data.estado_vigente=='invitado'|| (SelectionsRecord.data.estado_vigente=='pendiente' && SelectionsRecord.data.precio_total>0)){
						...
						//if(maestro.tipo_adq=='Bien'){  //oct2015: se cambia por maestro.tipo por el cambio en clasificacion de bienes que ahora usa servicios
						if(maestro.tipo=='Bien'){
			...
			function verificar_det(resp){
				...
				if(resp.responseXML!=undefined && resp.responseXML!=null&& resp.responseXML.documentElement!=null &&resp.responseXML.documentElement!=undefined){
					...
					var sm=getSelectionModel();
					var SelectionsRecord=sm.getSelected();
					if(confirm("Hay "+num_cotizaciones+" detalle(s) cotizado(s) por "+SelectionsRecord.data.total_cotizado+" "+SelectionsRecord.data.desc_moneda+" ¿Está seguro de terminar la Solicitud?")){
			...
			function iniciarEventosFormularios(){
				...
				var onMonedaSelect=function(g,r,c,e){
					if(c==5){
					    var sm1=getSelectionModel();var filas1=ds.getModifiedRecords();var cont1=filas1.length;var NumSelect1=sm1.getCount();
				        var SelectionsRecord1=sm1.getSelected();
				        if(NumSelect1!=0){
						if(SelectionsRecord1.data.num_detalle_cotizado>0){
							getGrid().getColumnModel().setEditable(5,false);
						}else{
							if(SelectionsRecord1.data.fecha_cotizacion==''){
				...
				var onFecha=function(g,r,c,e){
					if(c==4){
						...
						if(SelectionsRecord1.data.fecha==""){
						}else{
							if(SelectionsRecord1.data.num_detalle_cotizado>0 ){
								getGrid().getColumnModel().setEditable(5,false);
							}else{
				...
				getSelectionModel().on('rowdeselect',function(){
					...
					if(_CP.getPagina(layout_cot_serv_dir.getIdContentHijo()).pagina.limpiarStore()){

						_CP.getPagina(layout_cot_serv_dir.getIdContentHijo()).pagina.bloquearMenu();
					}
...
function btn_reporte_cotizacion(){
			...
			function btn_reporte_cotizacion_blanco(){
				...
				var data='m_id_proceso_compra='+maestro.id_proceso_compra;
...
*/
			this.EnableSelect=function(sm,row,rec){
				_CP.getPagina(layout_cot_serv_dir.getIdContentHijo()).pagina.reload(rec.data);
				_CP.getPagina(layout_cot_serv_dir.getIdContentHijo()).pagina.desbloquearMenu();
				enable(sm,row,rec);
			}


			//para que los hijos puedan ajustarse al tamaño
			this.getLayout=function(){return layout_cot_serv_dir.getLayout()};
			...
			//this.AdicionarBoton('../../../lib/imagenes/print.gif','Cotizacion en Blanco',btn_reporte_cotizacion_blanco,true,'reporte_cotizacion_blanco','Cotizacion en Blanco');
			...
			layout_cot_serv_dir.getLayout().addListener('layout',this.onResize);
			//layout_proceso_compra_serv.getVentana().addListener('beforehide',salta);
