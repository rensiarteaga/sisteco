...
, 'id_comprobante','id_comprobante_pago','id_planilla'
		...
		var ds_avance = new Ext.data.Store({proxy: new Ext.data.HttpProxy({url: direccion+'../../../../sis_tesoreria/control/cuenta_doc/ActionListarSolicitudViaticos2.php?estado=compro&tipo_cuenta_doc=solicitud_avance&estado_fa=pagado'}),reader:new Ext.data.XmlReader({record:'ROWS',id:'id_cuenta_doc',totalRecords: 'TotalCount'},['id_cuenta_doc','nro_documento','motivo','desc_empleado'])});
...
//								CM_ocultarComponente(getComponente('por_anticipo'));
							...
							if((parseFloat(root.getElementsByTagName('cuota_a_pagar')[0].firstChild.nodeValue))>(parseFloat(root.getElementsByTagName('num_pagos')[0].firstChild.nodeValue))){
								...
								if (maestro.tiene_anticipo=='si'){ 
									if (parseFloat(root.getElementsByTagName('cuota_a_pagar')[0].firstChild.nodeValue)>1){

										
										if (root.getElementsByTagName('tipo_pago')[0].firstChild.nodeValue=='normal'){
											getComponente('tipo_pago').setValue('normal');
										}else{
											getComponente('tipo_pago').setValue('adelantado');
										}
										CM_ocultarComponente(getComponente('por_anticipo'));
										CM_ocultarComponente(getComponente('por_retgar'));
									}
									...
									getComponente('tipo_pago').disable();//ago2015
...
*/
		...
		function verificarNumPago(){
			...
			_CP.loadingShow('Verificando Número de Pago');
...
//alert(SelectionsRecord.data.nro_cuota+'----'+parseFloat(getComponente('num_pago').getValue()));
		...
		function btn_revertir(){
				...
				if(NumSelect==1){
					...
					Ext.MessageBox.show({
						title: 'Procesando',
						msg:"<div><img src='../../../lib/ext-yui/resources/images/default/grid/loading.gif'/> Habilitando rendición para modificación...</div>",
						width:300,
						height:200,
						closable:false
					});
					Ext.Ajax.request({
						url:direccion+"../../../control/cotizacion/ActionRevertirRendicion.php",
						method:'POST',
						params:{cantidad_ids:'1',id_cotizacion_0:SelectionsRecord.data.id_cotizacion},
						success:esteSuccess,
						failure:Cm_conexionFailure,
						timeout:100000000
					});			
		...
		function btn_fin_def_pagos(){
			...
			if(habilita_fin_pagoI=='si'){

				var data="cantidad_ids=1&id_cotizacion="+id_cotizacionI;
				...
				if(habilita_fin_pagoI=='sino'){
					alert('Es necesario concretar el pago del anticipo definido');
				}else{
				  //	alert('Es necesario completar la definición de los '+cant_pagosI+' pagos establecidos');
				  var id_padre=_CP.getPagina(idContenedorPadre).idContenedor;
				  var data='m_id_cotizacion='+maestro.id_cotizacion;
				  data=data+'&m_precio_total_adjudicado='+total_adj;
				  data=data+'&m_total_aa='+total_aa
				  data=data+'&m_total_as='+total_as;
				  data=data+'&m_id_proceso_compra='+maestro.id_proceso_compra;
				  data=data+'&id_padre='+id_padre;
				  var ParamVentana={Ventana:{width:300,height:250}};
				  layout_plan_pago.loadWindows(direccion+'../../../../sis_adquisiciones/vista/plan_pago/pagoAS.php?'+data,'Pagos',ParamVentana);
		...
		function btn_rev_def_pagos(){
			if(maestro.estado_vigente=='formulacion_pp'){

				var data="cantidad_ids=1&id_cotizacion="+maestro.id_cotizacion+"&tipo=revertir";
				...
				alert('Solo se puede revertir cotizaciones en estado formulacion_pp');
		...
		function btn_juntar(){
			var sm=getSelectionModel();var filas=ds.getModifiedRecords();var cont=filas.length;var NumSelect=sm.getCount();
			if((parseFloat(NumSelect))>1)
			{
				var SelectionsRecord=sm.getSelections();
				var array_ids='id_cot='+maestro.id_cotizacion+'&id_pp=';
					for(var i=0;i<NumSelect;i++){
						idPago = SelectionsRecord[i].data['id_plan_pago'];
					
						if(i==0)	array_ids=array_ids+''+idPago;
						else array_ids=array_ids+','+idPago;
			...
			{ 
				Ext.MessageBox.alert('Estado', 'Debe seleccionar mas un registro.');
			}
		}
		...
		this.AdicionarBoton('../../../lib/imagenes/book_next.png','Volver Def. Pagos',btn_rev_def_pagos,true,'rev_pagos','Definición de Pagos');
		this.AdicionarBoton('../../../lib/imagenes/menu-show.gif','Devengar y Pagar',btnDevPagar,true,'dev_pagar','Devengar-Pagar');
		this.AdicionarBoton('../../../lib/imagenes/det.ico','Rev. Pago Caja',btn_revertir,true,'rev_rendicion','Rev. Pago Caja');
		...
		//ago2015
		this.AdicionarBoton('../../../lib/imagenes/menu-show.gif','Reprogramacion',btn_reprogramar,true,'reprogramar','Reprogramar');	
		this.AdicionarBoton('../../../lib/imagenes/menu-show.gif','Unir Pagos',btn_juntar,true,'juntar','Unir Pagos');	
		...
		function  enable(sel,row,selected){
			...
			if(selected&&record!=-1){
				...
				if(maestro.estado_vigente=='orden_compra'){
					...
					CM_getBoton('rev_pagos-'+idContenedor).disable();
					CM_getBoton('rev_rendicion-'+idContenedor).disable();
					CM_getBoton('comprobante-'+idContenedor).disable();
					CM_getBoton('comprobante_pago-'+idContenedor).disable();

					CM_getBoton('reprogramar-'+idContenedor).disable();
					CM_getBoton('juntar-'+idContenedor).disable();
					if(maestro.cant_pagos_def==maestro.num_pagos){
						if (maestro.por_adelanto>0){
						    if(maestro.estado_adelanto=='pagado'){
								CM_getBoton('fin_pagos-'+idContenedor).enable();
						    }else{
					...
					if(maestro.estado_vigente=='formulacion_pp'){
						...
						CM_getBoton('juntar-'+idContenedor).disable();
						CM_getBoton('rev_pagos-'+idContenedor).enable();
						if(record.estado=='devengado_conta'){
						...
						CM_getBoton('reprogramar-'+idContenedor).enable();
						CM_getBoton('juntar-'+idContenedor).enable();
					}
					...
					if(record.estado=='pendiente'){
						...
						CM_getBoton('comprobante-'+idContenedor).disable();
						CM_getBoton('comprobante_pago-'+idContenedor).disable();
						if (maestro.tipo_pago!='caja' && maestro.tipo_pago!='avance' && record.tipo_pago!='adelantado')
						...
						if (maestro.tipo_pago=='caja')
								CM_getBoton('rev_rendicion-'+idContenedor).enable();
					}
					else if(record.estado=='devengado_conta'){
						 CM_getBoton('comprobante-'+idContenedor).enable();
						 CM_getBoton('comprobante_pago-'+idContenedor).disable();
						 CM_getBoton('provisional-'+idContenedor).enable();
						 CM_getBoton('reprogramar-'+idContenedor).disable();
