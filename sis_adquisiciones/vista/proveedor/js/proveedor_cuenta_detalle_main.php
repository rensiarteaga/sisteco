<?php
/**
 * Nombre:		  	    proveedor_cuenta_detalle_main.php
...
var elemento={idContenedor:idContenedor,pagina:new pagina_proveedor_cuenta_detalle(idContenedor,direccion,paramConfig)};
...
function pagina_proveedor_cuenta_detalle(idContenedor,direccion,paramConfig)
{
    ...
    'sw_oec','sw_aux'
     ...
     var ds_cuenta_multa = new Ext.data.Store({proxy: new Ext.data.HttpProxy({url: direccion+'../../../../sis_contabilidad/control/cuenta/ActionListarCuenta.php'}),
    reader:new Ext.data.XmlReader({record:'ROWS',id:'id_cuenta',totalRecords: 'TotalCount'},
    ['id_cuenta','nro_cuenta','nombre_cuenta','desc_cta2','desc_cuenta','nivel_cuenta','tipo_cuenta',
    'sw_transaccional','id_cuenta_padre','id_parametro','id_moneda','descripcion',
    'sw_oec','sw_aux'
	...
	//FUNCIONES RENDER
		...
		function render_id_cuenta_multa(value, p, record){return String.format('{0}', record.data['desc_cta_multa']);}
		var tpl_id_cuenta_multa=new Ext.Template('<div class="search-item">','<b><i>{nombre_cuenta}</b></i><br>','<FONT COLOR="#B5A642">{nro_cuenta}</FONT>','</div>');
...
// txt id_auxiliar
	...
	Atributos[10]={
			validacion:{
 			name:'id_cuenta_multa',
			fieldLabel:'Cuenta Multa',
			allowBlank:true,			
			//emptyText:'Cuenta Anticipo...',
			desc: 'desc_cta_multa', 		
			store:ds_cuenta_multa,
			valueField: 'id_cuenta',
			displayField: 'desc_cta2',
			queryParam: 'filterValue_0',
			filterCol:'cuenta.nro_cuenta#cuenta.nombre_cuenta',
			...
			minChars:1,
			editable:true,
			renderer:render_id_cuenta_multa,
 			grid_visible:true,
 	 		grid_editable:true,
			width_grid:150,
			lazyRender:true,
		...
		form: true,
		filtro_0:true, 
		filterColValue:'cuenta.id_cuenta_multa',
		save_as:'id_cuenta_multa'
	};
	//----------- FUNCIONES RENDER
...
//DEFINICIÓN DE FUNCIONES
	...
	{	
		...
		componentes[10].store.baseParams={m_id_gestion:record.data.id_gestion,sw_transaccional:1};
	...
	this.btnEdit=function()
	{
		if(componentes[2].getValue()>0){
			...
			*/
			...
			ds_cuenta_multa.baseParams={m_id_gestion:componentes[2].getValue(),sw_transaccional:1};
			ds_cuenta_multa.modificado=true;
			componentes[10].setDisabled(false);
