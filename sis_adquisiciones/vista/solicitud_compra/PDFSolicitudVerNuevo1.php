...
$this->Cell(180,5,'VISTA PREVIA SOLICITUD DE '.$_SESSION['PDF_titulo'],0,1,'C');
	...
	$this->Cell(180,5,'(DOCUMENTO NO VÁLIDO PARA PROCEDER CON LA COMPRA)',0,1,'C'); 
...
$this->Cell(35,4,'Fecha y Hora',0,0); 
$this->Cell(20,4,'Moneda',0,0);  
$this->Cell(60,4,'Modalidad',0,0);  
...
//$this->Cell(25,4,$_SESSION['PDF_num_solicitud'].'',0,0); 
$this->Cell(45,4,''.$_SESSION['PDF_localidad'].'',0,0); 
$this->Cell(35,4,''.$fecha.' '.$_SESSION['PDF_hora_reg'],0,0); 
$this->Cell(20,4,''.$_SESSION['PDF_moneda'].'',0,0); 
$this->Cell(60,4,''.$_SESSION['PDF_modalidad'].'',0,0); 
...
//$this->MultiCell(68,4,''.'Jefe Sistema Trinidad'.'',1,'L',1);

//$this->MultiCell(68,4,''.'GERENTE ADMINISTRATIVO FINANCIERO'.'',1,'L',1);
...
//$this->MultiCell(68,4,''.'GSA -GERENCIA DE SISTEMAS AISLADOS'.'',1,'L',1); 
...
function Footer(){
  ...
  $FechaStr = str_replace(":", " ", $_SESSION['PDF_hora_reg']);
  ...
  $Minutos = ((int)$Minutos) + 10; 
	
  // Asigno la fecha modificada a una nueva variable
  $hora = date("H:i:s",mktime($Horas,$Minutos,$Segundos));
	
	$fecha1=date_create ($_SESSION['PDF_fecha_reg']); 
$fecha=date_format( $fecha1,'d-m-Y');

		//$fecha='20-01-2012';   
		//$nombre_usuario='CABALLERO VARGAS NELSON';
	//	$nombre_usuario='CASTELLON UGARTE SERGIO JUSTO';
	    //$nombre_usuario='SARAVIA HUMEREZ ZOBEIDA LENA';
	    //$nombre_usuario='JAVIER PARDO IRIARTE';
	    //$nombre_usuario='VARGAS CORDOVA JUAN CARLOS';
		//$nombre_usuario='BERAMENDI ORELLANA MARVIN VICENTE ';
		//$nombre_usuario='MONASTERIO CUELLAR FABIOLA';
	    //$nombre_usuario='RICARDO CANO CARDONA';
		//$nombre_usuario='CASTELLON UGARTE SERGIO JUSTO';
		//$nombre_usuario='MANZANEDA MAMANI JUVENAL';
		//$nombre_usuario='HINOJOSA RAMOS DANIEL WILFRAN';
		//$nombre_usuario='PAREDES JALDIN EDUARDO JOAQUIN';
		$nombre_usuario=$_SESSION['PDF_nombre_aprobacion'];
		
		//$hora=$_SESSION['PDF_hora_reg']+10 ;
		//$hora='10:45:00';
		...
		if($_SESSION['PDF_tipo_reporte']=='1'){
			...
			$this->Cell(70,3,sha1($_SESSION['PDF_num_solicitud'].$_SESSION['PDF_nombre_financiador'].$_SESSION['PDF_nombre_regional'].$_SESSION['PDF_nombre_programa'].$_SESSION['PDF_nombre_proyecto'].$_SESSION['PDF_nombre_actividad'].gregoriantojd(date('m'),date('d'),date('Y')).$hora.$_SESSION['PDF_monto_total']).date('y').'0'.date('m').'1'.date('d').'2'.date('H').date('i').date('s'),0,0,'L');
			//$this->Cell(70,3,sha1($_SESSION['PDF_num_solicitud'].$_SESSION['PDF_nombre_financiador'].$_SESSION['PDF_nombre_regional'].$_SESSION['PDF_nombre_programa'].$_SESSION['PDF_nombre_proyecto'].$_SESSION['PDF_nombre_actividad'].gregoriantojd('01','31','2011').$hora.$_SESSION['PDF_monto_total']),0,0,'L');
...
for($j=0;$j<$tam_data;$j++)
 {
 	//si no tiene presupuestado para el año siguiente
 	...
 	//si tiene presupuestado apra el año siguiente
 	else{
 		
 		$pdf->Cell(25,5,'Código Partida',0,0,'L'); 
		$pdf->Cell(90,5,'Nombre Partida',0,0,'L'); 
		$pdf->Cell(45,5,'Total Gestión Actual',0,0,'R'); 
		$pdf->Cell(35,5,'   Disponibilidad',0,1,'L');
	    ...
	    $pdf->Cell(115,5,'Monto referencial para Presupuesto de la siguiente gestión:',0,0,'L');
...
$pdf->Cell(25,5,'Código','LRT',0); 


	$pdf->Cell(88,5,'Descripción','LRT',0);
...
$pdf->Ln(2);
$pdf->Cell(36,5,'Comite de Recepción:','',0);
...
//if($_SESSION['sss_id_usuario']=='120'){
...
//$pdf->MultiCell(195,4,"\n\n\n\n____________________________"."\n".$_SESSION['PDF_nombre_aprobacion']."\nJEFE INTERINO UNIDAD TECNOLOGIAS DE INFORMACIÓN \nFirma Autorizada",'','C',0); 


//$pdf->MultiCell(195,4,"\n\n\n\n____________________________"."\n"."JAVIER PARDO IRIARTE"."\n".strtoupper("GENRENTE ADMINISTRATIVO FINANCIERO")."\nFirma Autorizada",'','C',0); 
//$pdf->MultiCell(195,4,"\n\n\n\n____________________________"."\n"."JHONNY N. CABALLERO VARGAS"."\n".strtoupper("GERENTE GENERAL INTERINO")."\nFirma Autorizada",'','C',0); 
//$pdf->MultiCell(195,4,"\n\n\n\n____________________________"."\n"."JUAN CARLOS VARGAS CORDOVA"."\n".strtoupper("GERENTE DE DISTRIBUCIÓN")."\nFirma Autorizada",'','C',0); 
//$pdf->MultiCell(195,4,"\n\n\n\n____________________________"."\n"."JUVENAL MANZANEDA MAMANI"."\n".strtoupper("GERENTE DE PLANIFICACIÓN E INGENIERIA")."\nFirma Autorizada",'','C',0); 
//$pdf->MultiCell(195,4,"\n\n\n\n____________________________"."\n"."JUVENAL MANZANEDA MAMANI"."\n".strtoupper("GERENTE DE PLANIFICACIÓN E INGENIERIA")."\nFirma Autorizada",'','C',0); 
//}
