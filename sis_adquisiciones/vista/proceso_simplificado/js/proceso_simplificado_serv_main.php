<?php 
/**
 * Nombre:		  	    proceso_adjudicacion_main.php
...
var elemento={pagina:new pag_proc_simplif_serv(idContenedor,direccion,paramConfig),idContenedor:idContenedor};
...
*/
function pag_proc_simplif_serv(idContenedor,direccion,paramConfig){
	...
	var ds = new Ext.data.Store({
		...
		var tpl_id_cajero=new Ext.Template('<div class="search-item">','<b><i>{nombre_persona} </b></i>','<b><i>{apellido_paterno_persona} </b></i>','<b><i>{apellido_materno_persona} </b></i>','<br><FONT COLOR="#B5A642">{codigo_empleado_empleado}</FONT>','</div>');
          ...
          function render_decimales(v){
		      return getComponente('monto_proceso').formatMoneda(v);
...
//		};
		...
		var config={titulo_maestro:'Caja de Pago',grid_maestro:'grid-'+idContenedor,urlHijo:'../../../sis_adquisiciones/vista/proceso_compra_det/proceso_compra_mul_serv_det.php'};
		lay_proc_simplif_serv=new DocsLayoutMaestroDeatalle(idContenedor);
		lay_proc_simplif_serv.init(config);
		...
		this.pagina(paramConfig,Atributos,ds,lay_proc_simplif_serv,idContenedor);
		...
		var paramFunciones={
			...
			Formulario:{html_apply:'dlgInfo-'+idContenedor,height:250,width:480,minWidth:150,minHeight:200,	closable:true,titulo:'Caja de Pago',
			grupos:[{	tituloGrupo:'Caja de Pago',
			        columna:0,
			        id_grupo:0
			     },
			...
			function btn_reg_factura(){
				...
				if(NumSelect!=0){
                    ...
                    if(((SelectionsRecord.data.id_caja_regis>0||SelectionsRecord.data.id_cuenta_doc>0) &&SelectionsRecord.data.tipo_recibo=='provisorio')||SelectionsRecord.data.tipo_recibo=='pago'){
                        lay_proc_simplif_serv.loadWindows(direccion+'../../../../sis_adquisiciones/vista/proceso_simplificado/proceso_simplificado_factura_serv.php?'+data,'Registro de Factura',ParamVentana);


						        lay_proc_simplif_serv.getVentana().on('resize',function(){
							         lay_proc_simplif_serv.getLayout().layout();
						        });
                    }else{
			...
			function iniciarEventosFormularios(){
				...
				getSelectionModel().on('rowdeselect',function(){
					...
					CM_getBoton('reg_factura-'+idContenedor).disable();
					CM_getBoton('fin_proceso-'+idContenedor).disable();
					if(_CP.getPagina(lay_proc_simplif_serv.getIdContentHijo()).pagina.limpiarStore()){
						_CP.getPagina(lay_proc_simplif_serv.getIdContentHijo()).pagina.bloquearMenu()
					}
				})
			}
			
			function btn_anular_proceso_serv(){
            ...
            this.EnableSelect=function(x,z,y){
			   enable(x,z,y);
			    _CP.getPagina(lay_proc_simplif_serv.getIdContentHijo()).pagina.reload(y.data);
				_CP.getPagina(lay_proc_simplif_serv.getIdContentHijo()).pagina.desbloquearMenu();
			
		    }
			//para que los hijos puedan ajustarse al tamaño
			this.getLayout=function(){return lay_proc_simplif_serv.getLayout()};
			...
			this.AdicionarBoton('../../../lib/imagenes/copy.png','Elección de Caja de Pago',btn_vale,true,'vale','Elección de Caja de Pago');
			this.AdicionarBoton('../../../lib/imagenes/copy.png','Registrar Adquisición',btn_reg_factura,true,'reg_factura','Registrar Adquisición');
            this.AdicionarBoton('../../../lib/imagenes/book_next.png','Finalizar Proceso',btn_fin_proceso,true,'fin_proceso','Finalizar Proceso');
			this.AdicionarBoton('../../../lib/imagenes/cancel.png','Anulación de Proceso',btn_anular_proceso_serv,true,'anular_proceso','Anular');
			...
			lay_proc_simplif_serv.getLayout().addListener('layout',this.onResize);//aregla la forma en que se ve el grid dentro del layout
