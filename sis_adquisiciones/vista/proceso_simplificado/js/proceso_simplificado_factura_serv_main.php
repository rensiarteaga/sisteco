<?php
/**
 * Nombre:		  	    proceso_adjudicacion_serv_dir_det_main.php
...
function pag_proc_adj_serv_dir_det(idContenedor,direccion,paramConfig,maestro,idContenedorPadre){
	...
	var ds = new Ext.data.Store({
		...
		'precio_total','id_moneda_base','numeracion_periodo','num_detalle_cotizado_gral', 'num_detalle_adjudicado_gral','se_adjudica','num_detalle_adjudicado','cantidad_sol','cod_control_factura','num_autoriza_factura','precio_total_adjudicado','id_caja','caja','id_cajero','cajero','id_empleado_adjudicacion','empleado_adjudicacion','es_item'
...
//		
	...
	Atributos[26]= {
		...
		var config={titulo_maestro:'Proceso de Adjudicacion',grid_maestro:'grid-'+idContenedor,urlHijo:'../../../sis_adquisiciones/vista/proceso_simplif_fact_det/proceso_simplif_fact_det_serv.php'};
		...
		var paramFunciones={
			...
			Formulario:{html_apply:'dlgInfo-'+idContenedor,height:'65%',width:'35%',minWidth:150,minHeight:'45%',	closable:true,titulo:'Servicios',
			grupos:[{
				tituloGrupo:'Oculto',
				columna:0,
				id_grupo:0
			...
			this.reload=function(params){
                ...
                maestro.monto_proceso=datos.m_monto_proceso;
            ...
            function Resultado(btn,text){
                  if(btn!='cancel'){
                                    ...
                                    // getComponente('fecha_adjudicacion').setValue(new Date());
                                     var mGes=new Date();
                                     mGes=mGes.getFullYear();
                                     if(parseFloat(mGes)>parseFloat(getComponente('gestion').getValue())){
                                     	 getComponente('fecha_adjudicacion').maxValue='31/12/'+getComponente('gestion').getValue();
                                     }else{
		  ...
		  function finalizado(resp){
			...
			this.btnNew=function(){
			    ...
			    if(maestro.tipo_recibo=='pago'){
			         ...
			         getComponente('fecha_adjudicacion').allowBlank=true;
    			}else{
			         CM_mostrarGrupo('Datos del Pago');
			         getComponente('tipo_documento').allowBlank=false;
			         getComponente('num_factura').allowBlank=false;
			...
			this.AdicionarBoton('../../../lib/imagenes/det.ico','Revertir Rendición',btn_revertir,true,'rev_rendicion','Revertir Rendición');
			...
			function enable(sel,row,selected){
        		var record=selected.data; 
        			if(selected&&record!=-1){
        			  ...
        			  CM_getBoton('nota-'+idContenedor).enable();
        			   if(parseFloat(record.num_detalle_adjudicado)>=0&&record.estado_vigente!='en_pago'){
        			         ...
        			         CM_getBoton('nota-'+idContenedor).disable();
        			     }else{
        			         CM_getBoton('nota-'+idContenedor).enable();
        			         CM_getBoton('finalizar_adjudicacion_dir-'+idContenedor).disable();
        			         CM_getBoton('rev_rendicion-'+idContenedor).disable();
