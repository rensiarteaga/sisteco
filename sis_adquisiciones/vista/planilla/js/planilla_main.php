...
var elemento={pagina:new pagina_planilla(idContenedor,direccion,paramConfig),idContenedor:idContenedor};
...
function pagina_planilla(idContenedor,direccion,paramConfig){
		...
		var paramFunciones={
			...
			function btn_archivo_pago_consultores(){
				...
				if(NumSelect!=0){
					...
					var data=data+'&codigo=CONSULTORES';
					nombre_a='pago_'+SelectionsRecord.data.descripcion+'-'+SelectionsRecord.data.periodo_rep+'-'+SelectionsRecord.data.gestion+'.txt';
					var data=data+'&nombre='+nombre_a;
	...
	function successGenerar(resp){ 
		  ...
		  window.open(direccion+'../../../../sis_kardex_personal/control/planilla/archivos/consultores/'+nombre_a);
	}
			

	

	function btn_reporte_abono(){
		var sm=getSelectionModel();
		var filas=ds.getModifiedRecords();
		var cont=filas.length;
		var NumSelect=sm.getCount();
		if(NumSelect!=0){
			...
			var data='m_id_planilla='+SelectionsRecord.data.id_planilla;
			    ...
			    data=data+'&m_codigo_depto_rep='+SelectionsRecord.data.cod_depto_rep;
			    data=data+'&m_depto_tesoro='+SelectionsRecord.data.depto_tesoro;
			window.open(direccion+'../../../control/cotizacion/reporte/ActionPDFAbonoPlanilla.php?'+data)
		}
		else{
			Ext.MessageBox.alert('Estado', 'Antes debe seleccionar un item.');
		}
			...
			this.AdicionarBoton('../../../lib/imagenes/print.gif','Abono en Cuentas',btn_reporte_abono,true,'abono_cuentas','Abono en Cuenta');		
