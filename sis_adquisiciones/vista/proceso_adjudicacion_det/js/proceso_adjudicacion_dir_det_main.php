...
* Fecha creación:		2008-05-28 17:32:05
*/
function pag_proc_adj_dir_det(idContenedor,direccion,paramConfig,maestro,idContenedorPadre){
	...
	function render_id_empleado(value, p, record){return String.format('{0}', record.data['empleado_adjudicacion']);}
		...
		Atributos[32]= {//==>SI
			...
			tipo:'DateField',
			filtro_0:false,
			filterColValue:'COTIZA.fecha_adjudicacio',
			dateFormat:'m-d-Y',
			defecto:'',
		...
		lay_proc_adj_dir_det=new DocsLayoutMaestroDeatalle(idContenedor,idContenedorPadre);
		lay_proc_adj_dir_det.init(config);
		...
		//---------- INICIAMOS HERENCIA
		this.pagina = Pagina;
		this.pagina(paramConfig,Atributos,ds,lay_proc_adj_dir_det,idContenedor);
		...
		var paramFunciones={
			...
			this.reload=function(params){
				...
				_CP.getPagina(lay_proc_adj_dir_det.getIdContentHijo()).pagina.limpiarStore()
				this.btnActualizar();
        ...
        function btn_recomendacion(){
				...
				if(NumSelect!=0){
                    ...
                    CM_getComponente('id_empleado_adjudicacion').allowBlank=true;
					var SelectionsRecord=sm.getSelected();
					if(SelectionsRecord.data.estado_vigente!='finalizado'){
				        CM_getComponente('rec').setValue(1);
					    CM_btnEdit();
				}}
				else{
					Ext.MessageBox.alert('Estado', 'Antes debe seleccionar un item.')
					...
					if(SelectionsRecord.data.estado_vigente!='finalizado'){
						if(SelectionsRecord.data.estado_vigente=='cotizado'){
							if(SelectionsRecord.data.num_detalle_adjudicado>0  || SelectionsRecord.data.num_detalle_adjudicado_gral>0){
								CM_getComponente('fecha_entrega').setValue('01/01/'+SelectionsRecord.data.gestion);
                                CM_getComponente('fecha_adjudicacion').minValue=CM_getComponente('fecha_entrega').getValue();
                                CM_getComponente('fecha_entrega').setValue(SelectionsRecord.data.fecha_entrega);
                                CM_getComponente('fecha_adjudicacion').maxValue=CM_getComponente('fecha_adjudicacion').getValue();
                                CM_getComponente('fecha_adjudicacion').setValue(new Date());
                                CM_getComponente('fecha_adjudicacion').maxValue=CM_getComponente('fecha_adjudicacion').getValue();
                               
                                if(SelectionsRecord.data.fecha_adjudicacion!=undefined || SelectionsRecord.data.fecha_adjudicacion!=null){
                                	CM_getComponente('fecha_adjudicacion').setValue(SelectionsRecord.data.fecha_adjudicacion);
                                }else{
                                	CM_getComponente('fecha_adjudicacion').setValue('');
                                }
                                CM_getComponente('fecha_adjudicacion').allowBlank=false;
...
//									timeout:100000000});
                                    ...
                                    CM_getComponente('id_empleado_adjudicacion').allowBlank=false;
									CM_getComponente('estado_vigente').setValue('adjudicado');
									CM_btnEdit();
				...
				if(resp.responseXML!=undefined && resp.responseXML!=null&& resp.responseXML.documentElement!=null &&resp.responseXML.documentElement!=undefined){
					...
					if(on==0){
						...
						if((root.getElementsByTagName('TotalCount')[0].firstChild.nodeValue)>0){
							...
							CM_ocultarComponente(CM_getComponente('fecha_venc'));
							CM_getComponente('fecha_venc').allowBlank=true;
							CM_getComponente('lugar_entrega').setValue(root.getElementsByTagName('lugar_entrega')[0].firstChild.nodeValue);
							CM_getComponente('lugar_entrega').disable();

						}else{
							CM_mostrarComponente(CM_getComponente('fecha_venc'));
							CM_getComponente('lugar_entrega').setValue('');
							CM_getComponente('lugar_entrega').enable();
							CM_getComponente('lugar_entrega').allowBlank=false;
						}
					}
    ...
    function btn_corregir_cotizacion(){
			...
			function iniciarEventosFormularios(){
				mensaje='';
				txt_id_moneda_base=CM_getComponente('id_moneda_base');
				//para iniciar eventos en el formulario
				txt_fecha=CM_getComponente('fecha_cotizacion');
				cmbMoneda=CM_getComponente('id_moneda');
			...
			//evento de deselecion de una linea de grid
				getSelectionModel().on('rowdeselect',function(){
					if(_CP.getPagina(lay_proc_adj_dir_det.getIdContentHijo()).pagina.limpiarStore()){
						_CP.getPagina(lay_proc_adj_dir_det.getIdContentHijo()).pagina.bloquearMenu();
					}
				})
			...
			this.EnableSelect=function(sm,row,rec){
				_CP.getPagina(lay_proc_adj_dir_det.getIdContentHijo()).pagina.reload(rec.data);
				_CP.getPagina(lay_proc_adj_dir_det.getIdContentHijo()).pagina.desbloquearMenu();
				enable(sm,row,rec);
			}
			...
			this.getLayout=function(){return lay_proc_adj_dir_det.getLayout()};
                    				        ...
                    				        if(parseFloat(record.cantidad_sol)>parseFloat(record.num_detalle_adjudicado_gral)){
                    	                         ...
                    	                         _CP.getPagina(lay_proc_adj_dir_det.getIdContentHijo()).pagina.desbloquearMenu();
                    				                 ...
                    				                 if(parseFloat(record.cantidad_sol)!=parseFloat(record.num_detalle_adjudicado)){
                    				                        alert('Ya se adjudicó el total de lo solicitado');
			...
			iniciarEventosFormularios();
			lay_proc_adj_dir_det.getLayout().addListener('layout',this.onResize);
			lay_proc_adj_dir_det.getVentana().addListener('beforehide',salta);
