...
{   
	...
	function Header()
	{
		...
		//$this->Cell(30,4,$_SESSION['PDF_codigo_depto'],0,1);
		 $this->SetX(182);
		if($_SESSION['PDF_es_item']==1 || $_SESSION['PDF_tipo_adq']=='Bien')
		 
		{
			$this->Cell(30,4,$_SESSION['PDF_codigo_depto'].'-OC'.'-'.$_SESSION['PDF_num_cotizacion'].'-'.$_SESSION['PDF_gestion'],0,1);
			
		}
			else
		{
			$this->Cell(30,4,$_SESSION['PDF_codigo_depto'].'-OS'.'-'.$_SESSION['PDF_num_cotizacion'].'-'.$_SESSION['PDF_gestion'],0,1);
		}
		...
		if($_SESSION['PDF_es_item']==1 || $_SESSION['PDF_tipo_adq']=='Bien'){
			...
			if(strlen($_SESSION['PDF_nro_contrato'])>0){
				$this->Cell(115,15,'Orden de Compra - Ctto.'.$_SESSION['PDF_nro_contrato'],0,0,'C');
			}else{
				$this->Cell(115,15,'Orden de Compra',0,0,'C');
			}
			...
			if(strlen($_SESSION['PDF_nro_contrato'])>0){
				...
				$this->Cell(115,15,'Orden de Servicio - Ctto. '.$_SESSION['PDF_nro_contrato'],0,0,'C');
			}else{
			
				$this->Cell(115,15,'Orden de Servicio',0,0,'C');
			}
		}
		...
		for($i1=0;$i1<count($cabecera);$i1++)
		{
			if($_SESSION['ss_id_usuario']==120){
				...
				$this->MultiTabla($cabecera[$i1],1,3,4.7,7,1);
		...
		$this->SetFontsSizes(array(6.5,6.5,6.5,6.5,6.5,6.5));
...
$pdf->SetFontsSizes(array(6.5,6.5,6.5,6.5,6.5,6.5));
...
$pdf->SetLineWidth(0.5);
	...
	$pdf->SetDrawColor(255,255,255);
	$pdf->SetFont('Arial','',8); 
	$pdf->MultiCell(67,5,''.$_SESSION['PDF_tipo_entrega'].'',1,'L',1);
...
if($_SESSION['PDF_titulo']=='SERVICIO'){
$pdf->Cell(35,2.5,'Nro de Servicio(s): '.$num_solicitud,0,1);
}else{
$pdf->Cell(35,2.5,'Nro de Pedido(s): '.$num_solicitud,0,1);
}
$pdf->Cell(35,2.5,'Nro de Proceso: '.$_SESSION['PDF_num_proceso'],0,1);

if ($_SESSION['PDF_partes']!=''){

	$pdf->MultiCell(197,3,'La presente Orden de '.$nombre.' tiene calidad de contrato de provisión de acuerdo a los artículos 919 al 925 del Código de Comercio.',0);
}
$pdf->SetFillColor(0,0,0);

if(strlen($_SESSION['PDF_nro_contrato'])>0){
	$pdf->Cell(197,0.1,'',1,1,'L',1);
}else{
	$pdf->Cell(197,0.3,'',1,1,'L',1);
}
$pdf->Ln(1);
...
if(strlen($_SESSION['PDF_nro_contrato'])>0){//adicionado 26.04.2016 en atencion de sast REQ26042016162424 de Mgrandillert
...
$pdf->SetFont('Arial','',5); 
	...
	if ($_SESSION['PDF_antecedentes']!='' && strlen($_SESSION['PDF_antecedentes'])>0){
		
		$pdf->SetFont('Arial','B',5);
		$pdf->Cell(23,5,'ANTECEDENTES:',0,0);
		$pdf->SetFont('Arial','',5);
		$pdf->Cell(23,5,'',0,0);
		$pdf->MultiCell(147,5,''.$_SESSION['PDF_antecedentes'].'',1);
		$pdf->Cell(102,0.5,'',0,1,'C');
	}
	if ($_SESSION['PDF_controversias']!=''){
		$pdf->Ln(1);
		$pdf->SetFont('Arial','B',5);
		$pdf->Cell(23,5,'SOLUCION DE CONTROVERSIAS:',0,0);
		$pdf->SetFont('Arial','',5);
		$pdf->Cell(23,5,'',0,0);
		$pdf->MultiCell(147,5,''.$_SESSION['PDF_controversias'].'',1);
		$pdf->Cell(102,0.5,'',0,1,'C');
	}
	
	if ($_SESSION['PDF_doc_integrantes']!=''){
		$pdf->Ln(1);
		$pdf->SetFont('Arial','B',5);
		$pdf->Cell(23,5,'DOC. INTEGRANTES:',0,0);
		$pdf->SetFont('Arial','',5);
		$pdf->Cell(23,5,'',0,0);
		$pdf->MultiCell(147,5,''.$_SESSION['PDF_doc_integrantes'].'',1);
		$pdf->Cell(102,0.5,'',0,1,'C');
	}
	
	if ($_SESSION['PDF_garantias']!=''){
		$pdf->Ln(1);
		$pdf->SetFont('Arial','B',5);
		$pdf->Cell(23,5,'GARANTIAS:',0,0);
		$pdf->SetFont('Arial','',5);
		$pdf->Cell(23,5,'',0,0);
		$pdf->MultiCell(147,5,''.$_SESSION['PDF_garantias'].'',1);
		$pdf->Cell(102,0.5,'',0,1,'C');
	}
	
	if ($_SESSION['PDF_legislacion']!=''){
		$pdf->Ln(1);
		$pdf->SetFont('Arial','B',5);
		$pdf->Cell(23,5,'LEGISLACION:',0,0);
		$pdf->SetFont('Arial','',5);
		$pdf->Cell(23,5,'',0,0);
		$pdf->MultiCell(147,5,''.$_SESSION['PDF_legislacion'].'',1);
		$pdf->Cell(102,0.5,'',0,1,'C');
	}
	
	
	if ($_SESSION['PDF_multas']!=''){
		$pdf->Ln(1);
		$pdf->SetFont('Arial','B',5);
		$pdf->Cell(23,5,'MULTAS:',0,0);
		$pdf->SetFont('Arial','',5);
		$pdf->Cell(23,5,'',0,0);
		$pdf->MultiCell(147,5,''.$_SESSION['PDF_multas'].'',1);
		$pdf->Cell(102,0.5,'',0,1,'C');
	}
	
	if ($_SESSION['PDF_obligaciones']!=''){
		$pdf->Ln(1);
		$pdf->SetFont('Arial','B',5);
		$pdf->Cell(23,5,'OBLIGACIONES:',0,0);
		$pdf->SetFont('Arial','',5);
		$pdf->Cell(23,5,'',0,0);
		$pdf->MultiCell(147,5,''.$_SESSION['PDF_obligaciones'].'',1);
		$pdf->Cell(102,0.5,'',0,1,'C');
	}
	
	
	if ($_SESSION['PDF_vigencia']!=''){
		$pdf->Ln(1);
		$pdf->SetFont('Arial','B',5);
		$pdf->Cell(23,5,'VIGENCIA:',0,0);
		$pdf->SetFont('Arial','',5);
		$pdf->Cell(23,5,'',0,0);
		$pdf->MultiCell(147,5,''.$_SESSION['PDF_vigencia'].'',1);
		$pdf->Cell(102,0.5,'',0,1,'C');
	}
	...
	/*
	$_SESSION['PDF_antecedentes'] = $f["antecedentes"];
				$_SESSION['PDF_controversias']=$f["controversias"];
				$_SESSION['PDF_doc_integrantes']=$f["doc_integrantes"];
				$_SESSION['PDF_garantias']=$f["garantias"];
				$_SESSION['PDF_legislacion']=$f["legislacion"];
				$_SESSION['PDF_multas']=$f["multas"];
				$_SESSION['PDF_obligaciones']=$f["obligaciones"];
				$_SESSION['PDF_partes']=$f["partes"];
...
if($_SESSION['PDF_justificacion_adjudicacion']!=''){

	$pdf->MultiCell(197,3,$_SESSION['PDF_justificacion_adjudicacion'],0);
}else{
...
if ($_SESSION['PDF_partes']!=''){
	$pdf->Ln(1);
	$pdf->SetFont('Arial','B',5);
	$pdf->Cell(23,5,'PARTES:',0,0);
	$pdf->SetFont('Arial','',5);
	$pdf->Cell(23,5,'',0,0);
	$pdf->MultiCell(147,5,''.$_SESSION['PDF_partes'].'',1);
	$pdf->Cell(102,0.5,'',0,1,'C');
	
	$pdf->Ln(8);
	$pdf->Cell(106,5,'____________________',0,0,'C');
	$pdf->Cell(66,5,'____________________',0,1,'C');
	$pdf->Ln(6);
}else{
