  ...
  // this.DeselectRow=function(selModel,row){
	...
	this.InitBarraMenu=function(param){
		...
		if(param.actualizar){
			...
			if(param.actualizar.text){
				text=param.actualizar.text
			}
			this.AdicionarBoton(rImagen+"actualizar.jpg",'<b>Actualizar<b>',param.actualizar.sobrecarga,param.actualizar.separador,'actualizar',text)
		}
	...
	this.InitFunciones=function(param){
		...
		if(pL){ 
			...
			if(pL.expand!=undefined){funListar.expand=pL.expand}
