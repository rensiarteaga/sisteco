...
session_start();
include(dirname(__FILE__).'/../configuracion.inc.php');
...
class ReportePDF extends MYPDF
{
	...
	function __construct($objParam){
		...
		$this->url_archivo = dirname(__FILE__)."/../../reportes_generados/".$this->objParam['nombre_archivo'];
		$this->nombre_archivo=$this->objParam['nombre_archivo'];
		$this->titulo=$this->objParam['titulo_archivo'];
		$this->orientacion=$this->objParam['orientacion'];
		$this->fecha_pie=1;
		$this->setBarcode(date('d-m-Y H:i:s').",usuario:".$_SESSION["ss_usuario"]);
		$this->tipoReporte=$this->objParam['tipoReporte'];

		//Ejecución del constructor del padre
		parent::__construct($this->orientacion, 'mm', $this->objParam['tamano'], true, 'UTF-8', false);
...
*/	
	function Footer() {
		...
		$cur_y = $this->GetY();
		//$this->Cell($ancho, 0, 'Generado por XPHS', 'T', 0, 'L');
		$this->Cell($ancho, 0, 'Usuario: '.$_SESSION["ss_usuario"], '', 0, 'L');
		$pagenumtxt = 'Página'.' '.$this->getAliasNumPage().' de '.$this->getAliasNbPages();
	...
	function Header(){
		if($this->orientacion=='P'){
			...
			$this->definirCabecera($this->objParam['codSistema'],$this->orientacion);
			$this->Ln();
			...
			throw new Exception("Reporte: Orientacion incorrecta, debe ser 'P' o 'L'");
		}
	...
	function addTabla($datas, $border=1, $fill=1,$titulos=0,$relleno=0) {
			...
			foreach ($this->tablecolumns as $col=>$single){
				...
				if($relleno){ //"#000080"
					...
					//$relleno=0;
						...
						$html.='<td width="'.$this->tablewidths[$col].'%" align="'.$align.'">'.$data[$col].'</td>';
		...
		if($this->swMaestro){
			...
			$this->maestroY=$this->maestroY+$this->objParam['filaInicioDatos'];
		}
...
//var_dump($aux);exit;
	...
	function generarReporte(){ 
        ...
        //parent::Write(0, $txt, '', 0, 'C', true, 0, false, false, 0);
       // parent::Output($this->url_archivo, 'I');
		
		//parent::Output('test.pdf', 'I');
