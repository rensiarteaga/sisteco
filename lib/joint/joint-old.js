...
/**
 * Adicion de Ariel ayaviri, Autor: David Brumann
 * 03-03-2011
 * Esta funcion adiciona o reemplaza los atributos de un obj a otro
*/
...
dummy: false // is it a dummy object?
    ...
    // connection options
    this._opt = {
//----added by Ariel Ayaviri - 12-01-2011
data: {}, //aditional Data
draggable: false,
//------------------------------
...
/// Added by Ariel Ayaviri omonte - 11-01-2011
selectJoint: function(){},    // new callback 
droppedSide: function(side){}
//-----------------------------------
    },
    /**
...
this.update();
this.callback("droppedSide",o,[capType]);
    },
    connectionWiring: function(e){
var mousePos = Joint.getMousePosition(e, this.paper.canvas);
this..vertices[this._conVerticesCurrentIndex] = mousePos;
this.update();
...
return this;
    ...
    listenAll: function(){
    	var self = this;
if (!this._opt.interactive){//adición de ariel ayaviri 11-01-2011
	this.dom.connection.mousedown(function(e){
		Joint.fixEvent(e);
		/// added by ariel ayaviri omonte - 11-01-2011
		self.callback("selectJoint",self); // tweak 
		//---------------------------------
...
if(this._opt.draggable){
		this.dom.startCap.mousedown(function(e){
			Joint.fixEvent(e);
			self.callback("selectJoint",self);
...
var key;
//----added by Ariel Ayaviri Omonte 12-01-2011
if (opt.data !== undefined) this._opt.data = opt.data; //save aditional data.
if (opt.draggable !== undefined) this._opt.draggable = opt.draggable //is the joint draggable??
//--------------------------------
...
return;
    ...
    //canvas.parentNode.removeChild(canvas);  //estas funciones no ayudan (Ariel Ayaviri)
    this._paper.clear();
...
Joint.bezierSegment = bezierSegment;
Joint.Bezier = Bezier;
//Adicion de Ariel Ayaviri Omonte para poder utilizar la funcioon deepSuplement
