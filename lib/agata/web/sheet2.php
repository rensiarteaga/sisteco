...
<td width=716 align=left valign=top>
    ...
    <td colspan=4>
        <b>&nbsp;Agata Report:: <? echo _a('Report Generation'); ?></b>
    </td>
    </tr>
    ...
    <?
        $Report = Report::OpenReport($file);
        $Breaks = Report::ExtractBreaks($Report);
        ...
        if ($Breaks)
        {
            ...
            foreach ($Breaks as $break=>$formula)
            {
                ...
                $Formulas = Report::TranslateFormulas($Report['Report']['DataSet']['Query']['Select'], $formula);
