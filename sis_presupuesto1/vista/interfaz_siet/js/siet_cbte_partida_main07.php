...
function pagina_siet_cbte_partida(idContenedor,direccion,paramConfig,maestro,idContenedorPadre)
{
	...
	ds.load({
	 ...
	 var ds_entidad_transf = new Ext.data.Store({proxy: new Ext.data.HttpProxy({url: direccion+'../../../control/interfaz_siet/ActionListarEntidadTransf.php'}),
			reader: new Ext.data.XmlReader({record:'ROWS',id:'id_siet_transf',totalRecords:'TotalCount'},['id_siet_entidad_transf','codigo','denominacion','sigla'])
			//baseParams:{sw_vista_reporte:'rep_ejecucion_partida',sid_tipo_pres:'2',$id_parametro:maestro.id_parametro}
	});
	...
	function render_id_siet_entidad_transf(value, p, record){return String.format('{0}', record.data['denominacion']);}
	var tpl_id_siet_entidad_transf=new Ext.Template('<div class="search-item">','<b><i>{codigo}</i></b>','<br><FONT COLOR="#B5A642">{denominacion}</FONT>','</div>');
	...
	vectorAtributos[5]={
			validacion:{
				...
				name:'id_siet_entidad_transf',
				desc:'codigo',
				store:ds_entidad_transf,
				valueField:'id_siet_entidad_transf',
				displayField:'codigo',
	...
	function f_evento_partida(combo, record, index){
		//comp_id_oec.setValue(record.data.id_oec)
		//mandamos parametros al componente de id_oec
		//componentes[6].store.baseParams={m_id_partida:componentes[0].getValue(),m_incluir_dobles:'no'};
