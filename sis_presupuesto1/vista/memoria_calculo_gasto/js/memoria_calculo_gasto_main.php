...
var elemento={pagina:new pagina_memoria_calculo_gasto(idContenedor,direccion,paramConfig,maestro,idContenedorPadre),idContenedor:idContenedor};
...
/**
* Nombre:		  	    pagina_memoria_calculo_gasto.js
...
function pagina_memoria_calculo_gasto(idContenedor,direccion,paramConfig,maestro,idContenedorPadre){
	...
	var monedas_for=new Ext.form.MonedaField(
	{
		...
		var ds=new Ext.data.Store({
			...
			var config={titulo_maestro:'Memoria de Calculo',grid_maestro:'grid-'+idContenedor,urlHijo:'../../../sis_presupuesto/vista/memoria_gasto/memoria_gasto.php'};
			var layout_memoria_calculo_gasto=new DocsLayoutMaestroDeatalle(idContenedor);
			layout_memoria_calculo_gasto.init(config);

			// INICIAMOS HERENCIA //
			this.pagina=Pagina;
			//-- pagina (array PARAMETROS DE CONFIGURACION, array DEFINICION DE ATRIBUTOS, SELECTION MODEL, DATA STORE, COLUM MODEL)
			this.pagina(paramConfig,Atributos,ds,layout_memoria_calculo_gasto,idContenedor);
			...
			var paramFunciones={
				...
				this.reload=function(params){
					...
					_CP.getPagina(layout_memoria_calculo_gasto.getIdContentHijo()).pagina.limpiarStore();
					_CP.getPagina(layout_memoria_calculo_gasto.getIdContentHijo()).pagina.bloquearMenu()
					//
					ds.lastOptions={
						params:{
							start:0,
				...
				function InitPaginaMemoriaCalculoGasto(){
					grid=getGrid();
					dialog=getDialog();
				...
				function iniciarEventosFormularios(){
					...
					getSelectionModel().on('rowdeselect',function(){
						if(_CP.getPagina(layout_memoria_calculo_gasto.getIdContentHijo()).pagina.limpiarStore()){
							_CP.getPagina(layout_memoria_calculo_gasto.getIdContentHijo()).pagina.bloquearMenu()
						}
					})
				}
				this.EnableSelect=function(sm,row,rec){
					var record=rec.data;
					_CP.getPagina(layout_memoria_calculo_gasto.getIdContentHijo()).pagina.reload(rec.data);
					...
					{
						_CP.getPagina(layout_memoria_calculo_gasto.getIdContentHijo()).pagina.desbloquearMenu();
					}
				...
				this.getLayout=function(){return layout_memoria_calculo_gasto.getLayout()};
				...
				InitPaginaMemoriaCalculoGasto();
				iniciarEventosFormularios();
				//Para actualizar los datos del padre al cerrar el hijo
	            layout_memoria_calculo_gasto.getVentana().addListener('beforehide',salta);
				CM_ocultarGrupo('Filtrar');
				layout_memoria_calculo_gasto.getLayout().addListener('layout',this.onResize);//aregla la forma en que se ve el grid dentro del layout
