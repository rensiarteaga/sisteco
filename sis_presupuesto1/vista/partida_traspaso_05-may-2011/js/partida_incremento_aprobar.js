/**
 * Nombre:		  	    pagina_partida_incremento_aprobar.js
 * Propósito: 			pagina objeto principal
 * Autor:				Generado Automaticamente
 * Fecha creación:		2009-02-04 19:45:09
 */
function pagina_partida_incremento_aprobar(idContenedor,direccion,paramConfig){
	...
	var ds = new Ext.data.Store({
		...
		baseParams:{filtro_aprobacion_incremento:'si'},
		remoteSort:true});
...
// txt id_partida_ejecucion_destino
	...
	Atributos[13]={
		...
		filtro_0:true,
		id_grupo:3,
		filterColValue:'UNIORG2.nombre_unidad',
...
// txt justificacion
	...
	Atributos[22] = {
		validacion: {
			name:'tipo_traspaso',			
			fieldLabel:'Tipo Incremento',
			vtype:'texto',
			//emptyText:'Tipo Increm...',
			allowBlank: false,
			typeAhead: true,			
			loadMask: true,
			triggerAction: 'all',
	...
	var config={titulo_maestro:'Aprobación de Incrementos',grid_maestro:'grid-'+idContenedor};
	var layout_partida_incremento_aprobar=new DocsLayoutMaestro(idContenedor);
	layout_partida_incremento_aprobar.init(config);
	...
	this.pagina(paramConfig,Atributos,ds,layout_partida_incremento_aprobar,idContenedor);
    ...
    function InitPaginaPartidaIncrementoAprobar()
    {						
	...
	function btn_aprobar_traspaso()
	{
		var sm=getSelectionModel();var filas=ds.getModifiedRecords();var cont=filas.length;var NumSelect=sm.getCount();
		
		if(NumSelect!=0)
		{			
			if(componentes[5].getValue()==3 || componentes[5].getValue()==5 || componentes[5].getValue()==6)	//verifica que el estado sea AUTORIZACION
			{
				var sw=false;
				if(confirm('Esta seguro de APROBAR el Incremento?'))
						{sw=true}
				if(sw)
				{
					var concluir='no';
					
					if(componentes[5].getValue()==3 || componentes[5].getValue()==6) //estados autorizacion y autorizacion_destino
					{					
						componentes[5].setValue(4);			//estado traspaso aprobado	
					}
					else if(componentes[5].getValue()==5)	//estado autorizacion origen
					{
						componentes[5].setValue(6);			//estado traspaso autorizacion destino
					}	
					ClaseMadre_save();					
				}				
			}
			...
			{
				Ext.MessageBox.alert('...', 'Solo incrementos en estado AUTORIZACION pueden ser aprobados.');
			}	
		}
		else
		{
	...
	function btn_rechazar_traspaso()
	{
		var sm=getSelectionModel();var filas=ds.getModifiedRecords();var cont=filas.length;var NumSelect=sm.getCount();
		...
		{			
			...
			{
				var sw=false;
				if(confirm('Esta seguro de RECHAZAR el Incremento?'))
						{sw=true}
				if(sw)
				{
					var concluir='no';					
					
					componentes[5].setValue(7);			//estado traspaso rechazado
			...
			{
				Ext.MessageBox.alert('...', 'Solo incrementos en estado AUTORIZACION pueden ser rechazados.');
			}	
		}
		else
		{
	...
	this.getLayout=function(){return layout_partida_incremento_aprobar.getLayout()};
	...
	//this.AdicionarBoton('../../../lib/imagenes/print.gif','Imprimir Reporte Incremento',btn_reporte_traspaso,true,'reporte_traspaso','Imprimir Reporte');
	this.AdicionarBoton('../../../lib/imagenes/ok.png','Aprobar Incremento',btn_aprobar_traspaso,true,'aprobar_traspaso','Aprobar Incremento');
	this.AdicionarBoton('../../../lib/imagenes/cross.gif','Rechazar Incremento',btn_rechazar_traspaso,true,'rechazar_traspaso','Rechazar Incremento');
	
	this.iniciaFormulario();
	InitPaginaPartidaIncrementoAprobar();
	iniciarEventosFormularios();
	layout_partida_incremento_aprobar.getLayout().addListener('layout',this.onResize);//aregla la forma en que se ve el grid dentro del layout
