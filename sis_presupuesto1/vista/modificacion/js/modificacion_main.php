...
var elemento={pagina:new pagina_modificacion(idContenedor,direccion,paramConfig),idContenedor:idContenedor};
 ...
 * Autor:				Generado Automaticamente
 * Fecha creaciï¿½n:		2010-05-10 18:01:22
 */
function pagina_modificacion(idContenedor,direccion,paramConfig)
{
	...
	var ds = new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({url: direccion+'../../../control/modificacion/ActionListarModificacion.php?tipo_modificacion=2&reformulacion=si'}),
	...
	var tpl_id_tipo_pres_gestion=new Ext.Template('<div class="search-item">','<b><i>{desc_tipo_pres}</i></b>','<br><FONT COLOR="#B5A642"><b>Gestion: </b>{desc_parametro}</FONT>','</div>');	
	...
	//Funciï¿½n para el formato de los importes
...
// txt estado_modificacion
	Atributos[6]={
		validacion:{
			...
			fieldLabel:'Estado Modificacion',
			allowBlank:true,
			maxLength:20,
			minLength:0,
			selectOnFocus:true,
...
// txt fecha_conclusion
	...
	var layout_modificacion=new DocsLayoutMaestro(idContenedor);
	layout_modificacion.init(config);
	...
	this.pagina(paramConfig,Atributos,ds,layout_modificacion,idContenedor);
	...
	var paramFunciones={
		...
		Formulario:{html_apply:'dlgInfo-'+idContenedor,height:350,width:480,minWidth:150,minHeight:200,	closable:true,titulo:'Modificacion'}};
	...
	function btnDetalleModificaciones(){
		...
		if(NumSelect!=0){
		    ...
		    layout_modificacion.loadWindows(direccion+'../../../../sis_presupuesto/vista/partida_modificacion_orig/partida_modificacion_orig.php?'+data,'Disminuciones',ParamVentana);
	...
	function btn_autorizacion_modificacion(){
		...
		if(NumSelect!=0){			
			var sw=false;
			if(confirm('Esta seguro de enviar para AUTORIZACION la modificacion?'))
			{sw=true}
			if(sw){
				Ext.MessageBox.show({
	...
	function btn_aprobar_modificacion(){
		...
		if(NumSelect!=0){			
			var sw=false;
			if(confirm('Esta seguro de APROBAR la modificacion?'))
					{sw=true}
			if(sw){
				Ext.MessageBox.show({
					title: 'Espere Por Favor...',
				...
				var SelectionsRecord=sm.getSelections(); 			
	 			...
	 			for(var i=0 ; i<NumSelect ; i++)
	 			{
					...
					url:direccion+"../../../control/modificacion/ActionEstadoModificacion.php",
					method:'POST',
					params:{cantidad_ids:NumSelect,id_modificacion:arr_id_modificacion[i],accion:'aprobar_modificacion',tipo_modificacion:2},
					success:modificacion_Success,
					failure:ClaseMadre_conexionFailure,
					timeout:100000000
					});				
	...
	function btn_rechazar_modificacion(){
		...
		if(NumSelect!=0){			
			var sw=false;
			if(confirm('Esta seguro de RECHAZAR la modificacion?'))
					{sw=true}
			if(sw){
				/*-----loading----*/
				Ext.MessageBox.show({
				...
				var SelectionsRecord=sm.getSelections(); 			
	 			...
	 			for(var i=0 ; i<NumSelect ; i++){
					...
					url:direccion+"../../../control/modificacion/ActionEstadoModificacion.php",
					method:'POST',
					params:{cantidad_ids:NumSelect,id_modificacion:arr_id_modificacion[i],accion:'rechazar_modificacion',tipo_modificacion:2},
					success:modificacion_Success,
					failure:ClaseMadre_conexionFailure,
					timeout:100000000
					});				
	...
	function btn_concluir_modificacion(){
		...
		if(NumSelect!=0){					
			var sw=false;
			if(confirm('Esta seguro de CONCLUIR la modificacion?'))
					{sw=true}
			if(sw){			
				/*-----loading----*/
				Ext.MessageBox.show({
				...
				url:direccion+"../../../control/modificacion/ActionEstadoModificacion.php",
				method:'POST',
				params:{cantidad_ids:'1',id_modificacion:SelectionsRecord.data.id_modificacion,accion:'concluir_modificacion',tipo_modificacion:2},
				success:modificacion_Success,
				failure:ClaseMadre_conexionFailure,
				timeout:100000000
				});				
	...
	function btn_corregir_modificacion(){
		var sm=getSelectionModel();
		var filas=ds.getModifiedRecords();
		var cont=filas.length;
		var NumSelect=sm.getCount();
		...
		if(NumSelect!=0){					
			var sw=false;
			if(confirm('Esta seguro de CORREGIR la modificacion?'))
			{sw=true}
			if(sw){			
				/*-----loading----*/
				Ext.MessageBox.show({
				...
				url:direccion+"../../../control/modificacion/ActionEstadoModificacion.php",
				method:'POST',
				params:{cantidad_ids:'1',id_modificacion:SelectionsRecord.data.id_modificacion,accion:'borrador_modificacion',tipo_modificacion:2},
				success:modificacion_Success,
				failure:ClaseMadre_conexionFailure,
				timeout:100000000
				});				
	...
	this.getLayout=function(){return layout_modificacion.getLayout()};
	...
	this.AdicionarBoton('../../../lib/imagenes/tucrem.png','Autorizacion de la Modificacion',btn_autorizacion_modificacion,true,'autorizacion_traspaso','Enviar a Autorizacion');
	this.AdicionarBoton('../../../lib/imagenes/book_next.png','Concluir la Modificacion',btn_concluir_modificacion,true,'concluir_traspaso','Concluir Modificacion');
	this.AdicionarBoton('../../../lib/imagenes/book_antes.png','Enviar a Correccion la Modificacion',btn_corregir_modificacion,true,'corregir_modificacion','Corregir Modificacion');
	...
	CM_getBoton('corregir_modificacion-'+idContenedor).disable();	
	CM_getBoton('concluir_traspaso-'+idContenedor).disable();
	
	function enable(sm,row,rec){		
		cm_EnableSelect(sm,row,rec);
		
		if(rec.data['estado_modificacion'] =='Borrador' || rec.data['estado_modificacion'] =='Borrador_origen' || rec.data['estado_modificacion'] =='Borrador_destino')//Borrador
		{
			//alert("llega disa");
		...
		if(rec.data['estado_modificacion']=='Autorizacion' || rec.data['estado_modificacion'] =='Autorizacion_origen' || rec.data['estado_modificacion'] =='Autorizacion_destino')//Autorizacion
		{
			//alert("llega disa");
			CM_getBoton('editar-'+idContenedor).disable();
			CM_getBoton('eliminar-'+idContenedor).disable();
		...
		if(rec.data['estado_modificacion']=='Aprobado' || rec.data['estado_modificacion'] =='Aprobado_origen' || rec.data['estado_modificacion'] =='Aprobado_destino')//Aprobado
		{
			//alert("llega disa");
			CM_getBoton('editar-'+idContenedor).disable();
			CM_getBoton('eliminar-'+idContenedor).disable();	
			CM_getBoton('detalle-'+idContenedor).enable();	
			CM_getBoton('autorizacion_traspaso-'+idContenedor).disable();	
			CM_getBoton('corregir_modificacion-'+idContenedor).enable();
			CM_getBoton('concluir_traspaso-'+idContenedor).enable();								
		}	
		if(rec.data['estado_modificacion']=='Rechazado' || rec.data['estado_modificacion'] =='Rechazado_origen' || rec.data['estado_modificacion'] =='Rechazado_destino')//Rechazado
		{
			//alert("llega disa");
			CM_getBoton('editar-'+idContenedor).enable();
			CM_getBoton('eliminar-'+idContenedor).enable();
	...
	layout_modificacion.getLayout().addListener('layout',this.onResize);//aregla la forma en que se ve el grid dentro del layout
