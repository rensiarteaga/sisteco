...
var elemento ={pagina:new DetalleEjecucionPartida(idContenedor,direccion,paramConfig,configConsolidacion),idContenedor:idContenedor};
...
function DetalleEjecucionPartida(idContenedor,direccion,paramConfig,configConsolidacion)
{
	...
	var tpl_id_presupuesto=new Ext.Template('<div class="search-item">','<b><i>{desc_unidad_organizacional}</i></b>',
																		...
																		'<br><FONT COLOR="#B50000"><b>Proyecto: </b>{nombre_proyecto}</FONT>',
																		'<br><FONT COLOR="#B50000"><b>Actividad: </b>{nombre_actividad}</FONT>',
																		'<br><FONT COLOR="#B5A642"><b>Financiamiento: </b>{denominacion}</FONT>',
	...
	vectorAtributos[9]={
			validacion:{
				...
				loadMask:true,
				triggerAction:'all',
				store:new Ext.data.SimpleStore({fields:['id','valor'],data:[['1','PDF'],['2','Excel'],['3','Excel Detallado']]}),				
				valueField:'id',
				displayField:'valor',
				lazyRender:true,								
				forceSelection:true,
				...
				*///if(fecha_fin.getValue()==""){mensaje+=" El campo Fecha Final  esta vacio";};
				...
				if(mensaje=="")
					 if (g_tipo_reporte==3){
						 ...
						 window.open(direccion+'../../../control/ejecucion/ActionPDFEjecucionPartidaDetalle.php?'+data);	
					 }
					 else{							
						 var data='start=0';
						 data+='&limit=1000';
						...
						data+='&categoria_prog='+g_categoria_prog;	//listo
						
						//alert(data);
						window.open(direccion+'../../../control/ejecucion/ActionPDFDetalleEjecucionPartida.php?'+data);					
					 }
					else{
						alert(mensaje);
					}
	...
	function evento_presupuesto( combo, record, index )	{
		...
		g_ids_actividad=record.data.id_actividad;		
	 	...
	 	g_categoria_prog=record.data.cod_categoria_prog;
	 	g_CantFiltros=1;		
		
		componentes[3].setValue(g_id_presupuesto +' - '+ g_proyecto +' '+ g_categoria_prog);
