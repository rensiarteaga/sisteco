...
function pagina_registro_comprobante(idContenedor,direccion,paramConfig , sw_vista){
	...
	var	comp_fecha_cbte;
	var comp_id_moneda_cbte;
	var comp_id_tipo_cambio;
	var comp_tipo_cambio;
	var comp_nro_cbte;

	var	comp_id_cbte_clase;
	var	comp_momento_cbte;
	var	comp_id_depto;
	var	comp_id_subsistema;

	var	comp_acreedor;
	var	comp_aprobacion;
	var	comp_conformidad;
	var	comp_pedido;
	var	comp_concepto_cbte;
	var	comp_glosa_cbte;

	var	comp_id_usuario_mod;
	var	comp_justificacion_edicion;
			
	var	comp_fk_comprobante;
	var	comp_tipo_relacion;
...
*/
	...
	var config={titulo_maestro:'registro_comprobante',grid_maestro:'grid-'+idContenedor,urlHijo:'../../../sis_presupuesto/vista/caiff/caiff_ajuste_transac.php?idSub='+decodeURI('Transaccion Detalle')+'&'};
	...
	var paramFunciones={
		...
		Formulario:{
			...
			{tituloGrupo:'Datos Relacion',columna:0,id_grupo:6}
			],
			closable:true,
	...
	function ocultar_mostrar_grupos(habilitar){
		...
		if(habilitar=='mod'){
			//Redireccionamiento del action para habilitar el comprobante para modificaciï¿½n
			...
			CM_ocultarGrupo('Preguntar:');
			CM_ocultarGrupo('Glosa:');
			CM_ocultarGrupo('Datos Relacion');
			...
			CM_mostrarGrupo('Preguntar:');
			CM_mostrarGrupo('Glosa:');
			CM_mostrarGrupo('Datos Relacion');
	...
	function resetComponentes(){ //p_grupo es el ï¿½ndice del grupo, p_allowBlank es cadena (true, false)
