function EjecucionInstitucional(idContenedor,direccion,paramConfig,configConsolidacion)
{
	...
	//render
		...
		{						
			...
			if(value == 3)
			{return "Inversión"}
			if (value=='2,3'){return "Gasto - Inversion"}
			return '';
		}	
	...
	baseParams:{vista:'ejecucion_institucional'}
	});*/
...
/*vectorAtributos[2]={
	...
	vectorAtributos[2]={
			...
			queryParam: 'filterValue_0',
			filterCol:'UNIORG.nombre_unidad#FUNFIN.denominacion#FINANC.nombre_financiador#REGION.nombre_regional#PROGRA.nombre_programa#PROYEC.nombre_proyecto#ACTIVI.nombre_actividad',			
			typeAhead:false,			
			tpl:tpl_id_presupuesto,
			forceSelection:true,
			mode:'remote',
...
/*	vectorAtributos[8]={
		validacion:{
			name              :  'estado',
			fieldLabel        :  'Estados',
			dataFields        :  ['id_presupuesto', 'desc_presupuesto'],
			              :  ds_presupuesto,
			valueField        :  'id_presupuesto',
			displayField      :  'desc_presupuesto',
			tpl               : tpl_id_presupuesto,
			//rende r            : render_id_presupuesto,
			width             :  150,
			height            :  150,
			allowBlank        :  false
		},
				...
				*///if(fecha_fin.getValue()==""){mensaje+=" El campo Fecha Final  esta vacio";};
				...
				{							
					...
					data+='&fecha_desde='+formatDate(dteFechaDesde.getValue());
					 ...
					 data+='&m_id_presupuesto='+g_id_presupuesto;//listo
					...
					data+='&desc_presupuesto='+g_desc_presupuesto;
					//data+='&fecha_desde='+g_fecha_desde;
					//alert(data);
					window.open(direccion+'../../../control/ejecucion/ActionPDFEjecucionInstitucional.php?'+data);					
