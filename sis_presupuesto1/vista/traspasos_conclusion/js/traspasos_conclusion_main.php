...
var elemento={pagina:new pagina_traspasos_conclusion(idContenedor,direccion,paramConfig),idContenedor:idContenedor};
 ...
 * Fecha creación:		2010-05-10 18:01:22
 */
function pagina_traspasos_conclusion(idContenedor,direccion,paramConfig)
{
	...
	var ds = new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({url: direccion+'../../../control/modificacion/ActionListarModificacion.php?tipo_modificacion=1&conclusion=si'}),
...
// txt fecha_conclusion
	...
	var layout_traspasos_conclusion=new DocsLayoutMaestro(idContenedor);
	layout_traspasos_conclusion.init(config);
	...
	this.pagina(paramConfig,Atributos,ds,layout_traspasos_conclusion,idContenedor);
	...
	function btnDetalleModificaciones()
	{
		...
		if(NumSelect!=0)
		{
			...
			layout_traspasos_conclusion.loadWindows(direccion+'../../../../sis_presupuesto/vista/partida_modificacion_orig/partida_modificacion_orig_conclusion.php?'+data,'Disminuciones',ParamVentana);
			layout_traspasos_conclusion.getVentana().on('resize',function(){
						layout_traspasos_conclusion.getLayout().layout();
					});
	...
	this.getLayout=function(){return layout_traspasos_conclusion.getLayout()};
	...
	layout_traspasos_conclusion.getLayout().addListener('layout',this.onResize);//aregla la forma en que se ve el grid dentro del layout
