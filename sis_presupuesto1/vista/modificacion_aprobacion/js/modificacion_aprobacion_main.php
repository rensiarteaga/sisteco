...
var elemento={pagina:new pagina_modificacion_aprobacion(idContenedor,direccion,paramConfig),idContenedor:idContenedor};
...
function pagina_modificacion_aprobacion(idContenedor,direccion,paramConfig)
{
	...
	var ds = new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({url: direccion+'../../../control/modificacion/ActionListarModificacion.php?tipo_modificacion=2&aprobacion=si'}),
...
// txt fecha_conclusion
	...
	function btnDetalleModificaciones(){
		...
		if(NumSelect!=0){
			...
			layout_modificacion_aprobar.loadWindows(direccion+'../../../../sis_presupuesto/vista/partida_modificacion_orig/partida_modificacion_orig_aprobacion.php?'+data,'Disminuciones',ParamVentana);
	...
	function btn_aprobar_modificacion(){
		...
		filas=ds.getModifiedRecords();
		cont=filas.length;
		NumSelect=sm.getCount();
		SelectionsRecord=sm.getSelected();
