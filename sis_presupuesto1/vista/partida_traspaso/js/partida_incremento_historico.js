 ...
 */
function pagina_partida_incremento_historico(idContenedor,direccion,paramConfig){
	...
	var ds = new Ext.data.Store({
		...
		baseParams:{filtro_incremento_concluidos:'si'},
		remoteSort:true});
...
// txt id_partida_ejecucion_destino
	...
	Atributos[8]={
			...
			queryParam: 'filterValue_0',
			filterCol:'UNIORG.nombre_unidad#PROYEC.nombre_proyecto',
...
// txt justificacion
	...
	var layout_partida_incremento_historico=new DocsLayoutMaestro(idContenedor);
	layout_partida_incremento_historico.init(config);
	...
	this.pagina(paramConfig,Atributos,ds,layout_partida_incremento_historico,idContenedor);
	...
	var paramFunciones={
		...
		Save:{url:direccion+'../../../control/partida_traspaso/ActionGuardarPartidaTraspasoConcluidos.php'},
		ConfirmSave:{url:direccion+'../../../control/partida_traspaso/ActionGuardarPartidaTraspasoConcluidos.php'},
		Formulario:{
	...
	/*function btn_autorizacion_incremento()
	{
		...
		{			
			...
			{
				var sw=false;
				if(confirm('Esta seguro de enviar para AUTORIZACIÓN el Incremento?'))
						{sw=true}
				if(sw)
				{
					var concluir='no';
					...
					{	
						componentes[5].setValue(6);	//estado traspaso es autorización	
					}		
					else
					{
						componentes[5].setValue(6); //estado autorizacion origen
					}
					ClaseMadre_save();					
				}				
			}
			...
			{
				Ext.MessageBox.alert('...', 'Solo incrementos en estado VERIFICACION o RECHAZADO pueden ser enviados para autorización.');
			}	
		}
		else
		{
	...
	this.getLayout=function(){return layout_partida_incremento_historico.getLayout()};
	...
	layout_partida_incremento_historico.getLayout().addListener('layout',this.onResize);//aregla la forma en que se ve el grid dentro del layout
