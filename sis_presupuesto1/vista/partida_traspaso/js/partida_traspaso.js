/**
 * Nombre:		  	    pagina_partida_traspaso.js
 * Propósito: 			pagina objeto principal
 * Autor:				Generado Automaticamente
 * Fecha creación:		2009-02-04 19:45:09
 */
function pagina_partida_traspaso(idContenedor,direccion,paramConfig){
	...
	var ds = new Ext.data.Store({
		...
		baseParams:{filtro_reformulacion:'no'},
		remoteSort:true});
...
// txt justificacion
	...
	Atributos[19] = {
		validacion: {
			...
			store: new Ext.data.SimpleStore({
				fields: ['ID', 'valor'],
				data :Ext.partida_traspaso_combo.tipo_pres_sin_inv // from states.js
			}),
			valueField:'ID',
			displayField:'valor',
	...
	var config={titulo_maestro:'Traspasos',grid_maestro:'grid-'+idContenedor};
	var layout_partida_traspaso=new DocsLayoutMaestro(idContenedor);
	layout_partida_traspaso.init(config);
	...
	this.pagina(paramConfig,Atributos,ds,layout_partida_traspaso,idContenedor);
    ...
    {	
		...
		{			
			Ext.MessageBox.alert('...', 'Solo traspasos en estado VERIFICACION o RECHAZADO pueden ser modificados.');
		}
	}
    ...
    {		
		...
		{				
			Ext.MessageBox.alert('...', 'Solo traspasos en estado VERIFICACION o RECHAZADO pueden ser eliminados.');
		}
	}
    ...
    function InitPaginaPartidaTraspaso()
    {						
	...
	/*function btn_autorizacion_traspaso()
	{
		...
		{			
			...
			{
				var sw=false;
				if(confirm('Esta seguro de enviar para AUTORIZACIÓN el Traspaso?'))
						{sw=true}
				if(sw)
				{
					var concluir='no';
			...
			{
				Ext.MessageBox.alert('...', 'Solo traspasos en estado VERIFICACION o RECHAZADO pueden ser enviados para autorización.');
			}	
		}
		else
		{
	...
	function btn_autorizacion_traspaso()
	{
		...
		{			
			...
			{*/
				var sw=false;
				if(confirm('Esta seguro de enviar para AUTORIZACIÓN los registros seleccionados?'))
						{sw=true}
				if(sw)
				{					
	...
	/*function btn_concluir_traspaso()
	{
		...
		{	
			...
			{
				var sw=false;
				if(confirm('Esta seguro de CONCLUIR el Traspaso?'))
						{sw=true}
				if(sw)
				{
					var concluir='si';
					...
					var SelectionsRecord=sm.getSelections(); 			
		 			...
		 			var data='id_partida_traspaso='+arr_id_partida_traspasos;				
			...
			{
				Ext.MessageBox.alert('...', 'Solo traspasos en estado APROBADO pueden ser concluidos.');
			}	
		}
		else
		{
	...
	/*function btn_reporte_traspaso()
	{
		...
		if(NumSelect!=0)
		{
			...
			{
				Ext.MessageBox.alert('...', 'Solo traspasos en estado CONCLUIDO pueden ser impresos.');
			}			
		}
		else
		{
	...
	this.getLayout=function(){return layout_partida_traspaso.getLayout()};
	...
	this.AdicionarBoton('../../../lib/imagenes/tucrem.png','Autorización Traspaso',btn_autorizacion_traspaso,true,'autorizacion_traspaso','Enviar Para Autorización');
	this.AdicionarBoton('../../../lib/imagenes/book_next.png','Concluir Traspaso',btn_concluir_traspaso,true,'concluir_traspaso','Concluir Traspaso');
	//this.AdicionarBoton('../../../lib/imagenes/print.gif','Imprimir Reporte Traspaso',btn_reporte_traspaso,true,'reporte_traspaso','Imprimir Reporte');
	...
	layout_partida_traspaso.getLayout().addListener('layout',this.onResize);//aregla la forma en que se ve el grid dentro del layout
