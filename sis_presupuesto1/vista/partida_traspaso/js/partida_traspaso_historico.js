 ...
 */
function pagina_partida_traspaso_historico(idContenedor,direccion,paramConfig){
	...
	var ds = new Ext.data.Store({
		...
		'fecha_conclusion',
		{name: 'fecha_conclusion_edit',type:'date',dateFormat:'Y-m-d'},
		...
		baseParams:{filtro_reformulacion_concluidos:'no'},  //mostramos solo los traspasos concluidos
		remoteSort:true});
...
// txt id_partida_ejecucion_destino
	...
	Atributos[13]={
			...
			queryParam: 'filterValue_0',
			filterCol:'UNIORG2.nombre_unidad#PROYEC2.nombre_proyecto',
			typeAhead:true,
			tpl:tpl_id_presupuesto,
			forceSelection:true,
			mode:'remote',
...
// txt justificacion
	...
	var config={titulo_maestro:'Historico de Traspasos y Reformulaciones',grid_maestro:'grid-'+idContenedor};
	var layout_partida_traspaso_historico=new DocsLayoutMaestro(idContenedor);
	layout_partida_traspaso_historico.init(config);
	...
	this.pagina(paramConfig,Atributos,ds,layout_partida_traspaso_historico,idContenedor);
	...
	this.getLayout=function(){return layout_partida_traspaso_historico.getLayout()};
	...
	layout_partida_traspaso_historico.getLayout().addListener('layout',this.onResize);//aregla la forma en que se ve el grid dentro del layout
