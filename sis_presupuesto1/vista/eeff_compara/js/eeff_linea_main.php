...
function main(){
	...
	var elemento={idContenedor:idContenedor,pagina:new pagina_eeff_linea(idContenedor,direccion,paramConfig,idContenedorPadre)};
...
{        
	...
	*/
	...
	function render_dato(value, p, record){	
		...
		if(value=='operacion'){return 'OPERACION';}
		if(value=='partida'){return 'PARTIDA';}
		}
	
	function render_borde(value, p, record){	
		if(value=='ninguno'){return 'NINGUNO';}
		if(value=='simple'){return 'SIMPLE';}
		if(value=='doble'){return 'DOBLE';}
	}
	
	function render_letra(value, p, record){	
		if(value=='mayuscula'){return 'MAYUSCULA';}
		if(value=='minuscula'){return 'Minúscula';}		
	}
	...
	*/
	...
	Atributos[3]={
		validacion:{
			...
			minLength:0,
			triggerAction:'all',
			store:new Ext.data.SimpleStore({fields:['ID','valor','filtro'],data:[['partida','PARTIDA'],['total','TOTAL'],['operacion','OPERACION']]}),
			valueField:'ID',
			displayField:'valor',
			mode:'local',
			lazyRender:true,			
	...
	Atributos[7]={
		validacion:{
			...
			minLength:0,
			triggerAction:'all',
			store:new Ext.data.SimpleStore({fields:['ID','valor','filtro'],data:[['mayuscula','MAYUSCULA'],['minuscula','Minúscula']]}),
			valueField:'ID',
			displayField:'valor',
			mode:'local',
			lazyRender:true,			
	...
	Atributos[11]={
		validacion:{
			...
			minLength:0,
			triggerAction:'all',
			store:new Ext.data.SimpleStore({fields:['ID','valor','filtro'],data:[['ninguno','NINGUNO'],['simple','SIMPLE'],['doble','DOBLE']]}),
			valueField:'ID',
			displayField:'valor',
			mode:'local',
			lazyRender:true,			
	...
	function btn_opera(){
		...
		if(NumSelect!=0){			
			...
			if (SelectionsRecord.data.linea_dato == 'operacion'){
				...
				layout_eeff.loadWindows(direccion+'../../../../sis_presupuesto/vista/eeff_compara/eeff_opera.php?'+data,'eeff_opera',ParamVentana);
			}else{	
