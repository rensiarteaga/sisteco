function FormulacionReporte(idContenedor,direccion,paramConfig,configConsolidacion)
{
	...
	var ds_presupuesto = new Ext.data.Store({proxy: new Ext.data.HttpProxy({url: direccion+'../../../control/presupuesto/ActionListarPresupuesto.php?oc=si'}),
			reader:new Ext.data.XmlReader({record:'ROWS',id:'id_presupuesto',totalRecords: 'TotalCount'},['id_presupuesto','tipo_pres','estado_pres','id_fina_regi_prog_proy_acti','desc_fina_regi_prog_proy_acti','id_unidad_organizacional','desc_unidad_organizacional',
						...
						'id_categoria_prog','cod_categoria_prog',   'cp_cod_programa','cp_cod_proyecto','cp_cod_actividad','cp_cod_organismo_fin','cp_cod_fuente_financiamiento','codigo_sisin' ])
	});	
	...
	vectorAtributos[2]={
			validacion:{
				...
				loadMask:true,
				triggerAction:'all',
				store:new Ext.data.SimpleStore({fields:['id','valor'],data:[['1','Presupuesto'],['2','Categoría Programática']]}),
				valueField:'id',
				displayField:'valor',
				lazyRender:true,								
				forceSelection:true,
	...
	vectorAtributos[7]=
	{
		...
		vectorAtributos[8]=
		{
			validacion:{
				...
				loadMask:true,
				triggerAction:'all',
				store:new Ext.data.SimpleStore({fields:['id','valor'],data:[['1','Todos los Niveles'],['2','Solo Grupos y Partidas de Movimiento']]}),				
				valueField:'id',
				displayField:'valor',
				lazyRender:true,								
				forceSelection:true,
	...
	var paramFunciones = {
		...
		Formulario:{
			...
			{					
				...
				if(id_tipo_pres.getValue()==""){mensaje+=" El campo Tipo de Presupuesto esta vacio";};
	...
	{			
		...
		//cmb_filtro_niveles= ClaseMadre_getComponente('filtro_niveles');
		...
		componentes[7].on('select',evento_reporte);		//tipo reporte
		componentes[8].on('select',evento_filtro_niveles);		//tipo reporte
	}
	...
	function evento_tipo_presupuesto( combo, record, index )
	{
		...
		componentes[4].store.baseParams={vista:'ejecucion_institucional',id_parametro_ei:componentes[0].getValue(),m_id_tipo_pres:componentes[1].getValue()};  //,m_id_unidad_organizacional:record.data.id_unidad_organizacional
