...
var elemento ={pagina:new MatrizProyectos(idContenedor,direccion,paramConfig,configConsolidacion),idContenedor:idContenedor};
...
function MatrizProyectos(idContenedor,direccion,paramConfig,configConsolidacion)
{   
	...
	vectorAtributos[4]=
	{
			validacion:{
				...
				loadMask:true,
				triggerAction:'all',
				store:new Ext.data.SimpleStore({fields:['id','valor'],data:[ ['3','Mensual'] ]}),		//['1','Anual'],['2','Trimestral']	,['4','Mensual Acumulado']	
				valueField:'id',
				displayField:'valor',
				lazyRender:true,								
				forceSelection:true,
	...
	vectorAtributos[5]={
		validacion:{
			...
			loadMask:true,
			triggerAction:'all',
			store:new Ext.data.SimpleStore({fields:['ID','valor'],data:[['Proyecto','Proyecto'] ]}),	//,['Presupuesto','Presupuesto'],['Unidad Organizacional','Unidad Organizacional'],['Categoria Programatica','Categoria Programatica'],['Categoria Programatica Detallada','Categoria Programatica Detallada']	
			valueField:'ID',
			displayField:'valor',
			forceSelection:true,
			width:250
	...
	vectorAtributos[9]=
	{
		validacion:{
			...
			loadMask:true,
			triggerAction:'all',
			store:new Ext.data.SimpleStore({fields:['id','valor'],data:[ ['2','Excel'] ]}),	   //, ['1','PDF']  ['2','Excel']		solo el reportes trimestral existe en formato Excel.	
			valueField:'id',
			displayField:'valor',
			lazyRender:true,								
			forceSelection:true,				
	...
	var paramFunciones = {
		...
		Formulario:{
			...
			titulo_pestana:'Ejecución Presupuestaria Trimestral',
			fileUpload:false,
			columnas:[405,405],			
			grupos:[
			{
			...
			{
				tituloGrupo:'Categoria Programatica',
				columna:1,
				id_grupo:6
			}
			...
			{					
				...
				{	//alert (g_id_partida);					
					var data='tipo_pres='+id_tipo_pres.getValue();	//listo
					 ...
					 data+='&id_categoria_prog='+id_categoria_prog.getValue();
					
					if(g_desc_mes=="")
					{ 
						window.open(direccion+'../../../control/_reportes/ejecucion_trimestral/ActionPDFEjecucionTrimestral.php?'+data);
					}
					else
					{
						window.open(direccion+'../../../control/_reportes/matriz_proyectos/ActionPDFMatrizProyectosProgramacion.php?'+data);						
					}
				}
				else
				{
	...
	{		
		...
		ClaseMadre_getComponente('id_categoria_prog').store.baseParams={tipo_vista:'formulacion',m_id_parametro:componentes[0].getValue(),m_tipo_pres:componentes[1].getValue()}; 
		ClaseMadre_getComponente('id_categoria_prog').modificado=true;
	...
	{
		g_num_mes=record.data.id;
		g_desc_mes=record.data.valor;
	}
	...
	{	  	//alert(ClaseMadre_getComponente('reporte').getValue());
	  	...
	  	if (record.data.ID=='Presupuesto')
	  	{
			...
			ClaseMadre_getComponente('id_categoria_prog').reset();
			ClaseMadre_getComponente('id_categoria_prog').allowBlank=true;
		    CM_ocultarGrupo('Proyecto');
		...
		else if (record.data.ID=='Categoria Programatica' || record.data.ID=='Categoria Programatica Detallada' )
		{
			ClaseMadre_getComponente('id_uo').reset();
			ClaseMadre_getComponente('id_uo').allowBlank=true;
			ClaseMadre_getComponente('id_proyecto').reset();
