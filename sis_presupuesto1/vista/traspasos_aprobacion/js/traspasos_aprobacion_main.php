...
var elemento={pagina:new pagina_traspasos_aprobacion(idContenedor,direccion,paramConfig),idContenedor:idContenedor};
 ...
 * Fecha creación:		2010-05-10 18:01:22
 */
function pagina_traspasos_aprobacion(idContenedor,direccion,paramConfig)
{
	...
	var ds = new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({url: direccion+'../../../control/modificacion/ActionListarModificacion.php?tipo_modificacion=1&aprobacion=si'}),
...
// txt fecha_conclusion
	...
	var layout_traspaso_aprobar=new DocsLayoutMaestro(idContenedor);
	layout_traspaso_aprobar.init(config);
	...
	this.pagina(paramConfig,Atributos,ds,layout_traspaso_aprobar,idContenedor);
	...
	function btnDetalleModificaciones()
	{
		...
		if(NumSelect!=0)
		{
			...
			layout_traspaso_aprobar.loadWindows(direccion+'../../../../sis_presupuesto/vista/partida_modificacion_orig/partida_modificacion_orig_aprobacion.php?'+data,'Disminuciones',ParamVentana);
			layout_traspaso_aprobar.getVentana().on('resize',function(){
						layout_traspaso_aprobar.getLayout().layout();
					});
	...
	this.getLayout=function(){return layout_traspaso_aprobar.getLayout()};
	...
	layout_traspaso_aprobar.getLayout().addListener('layout',this.onResize);//aregla la forma en que se ve el grid dentro del layout
