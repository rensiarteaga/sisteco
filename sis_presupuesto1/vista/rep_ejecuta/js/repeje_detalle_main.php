...
function main(){
	...
	var elemento={pagina:new pagina_repeje_detalle(idContenedor,direccion,paramConfig,sw_vista),idContenedor:idContenedor};
...
function pagina_repeje_detalle(idContenedor, direccion, paramConfig, sw_vista) {
	...
	vectorAtributos[5]={
		validacion:{
			...
			loadMask:true,
			triggerAction:'all',
			store:new Ext.data.SimpleStore({fields:['ID','valor'],data:[['CD','PDF - c/Detalle Consulta'],['xls','XLS - Excel']]}),
			valueField:'ID',
			displayField:'valor',
			forceSelection:true,
			width:200
