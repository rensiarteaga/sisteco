...
class PDF extends FPDF
{
	...
	$this->Cell(65,4,'DESCRIPCIÓN','R',0,'C');
	$this->Cell(29,4,'IMPORTE','R',0,'C');
	$this->Cell(29,4,'PERIODO','R',0,'C');
	$this->Cell(57,4,'JUSTIFICACIÓN','R',0,'C');
	$this->Ln(4);
	$this->SetFont('Arial','B',4);
	$this->Cell(10,3,'','LBR',0,'C');
	$this->Cell(65,3,'','BR',0,'C');
	$this->Cell(29,3,'('.$_SESSION['rep_mem_cal_simbolo'].')','BR',0,'C');
	$this->Cell(29,3,'','BR',0,'C');
	...
	function LoadData($id_partida_presupuesto,$id_presupuesto,$tipo_memoria,$id_partida,$id_moneda,$tipo_pres){
	    ...
	    //$criterio_filtroC='PARPRE.id_partida_presupuesto='.$id_partida_presupuesto.' AND MEMCAL.tipo_detalle='.$tipo_memoria;
	    //$CustomC= new cls_CustomDBPresupuesto();
		//$CustomC->ContarRepMemoriaCalculoOtros(15,0,'','',$criterio_filtroC,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);		
		//$cant=$CustomC->salida;
	    
		$Custom = new cls_CustomDBPresupuesto();
		$Custom->ListarRepMemoriaCalculoOtros($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);	
	...
	function FancyTable($data,$id_partida_presupuesto,$id_presupuesto,$tipo_memoria,$id_partida,$id_moneda,$tipo_pres){
	    ...
	    //Leer las líneas del fichero
	    $CustomC= new cls_CustomDBPresupuesto();
		$CustomC->ContarRepMemoriaCalculoOtros(15,0,'','',$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);		
	    $cantidad=sizeof($CustomC->salida);
	    $CustomS= new cls_CustomDBPresupuesto();
		$CustomS->SumaCostoMemoriaCalculoOtros(15,0,'','',$criterio_suma,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);		
		...
		$w=array(10,65,29,29,57);			
			...
			//Datos					
				...
				$this->SetAligns(array('C','L','C','R','J'));
		       ...
		       /*foreach($data as $row){       
					...
					$this->Cell($w[2],4,$this->imprimirLinea($row[2]),'R',0,'R');
					$this->Cell($w[3],4,$row[1],'R',0,'C');															
					//$this->Cell($w[4],4,$row[3],'R',0,'L');
					$this->MultiCell($w[4],4,$row[3],'RL','J',0);
					//$this->Ln(4);
					$numero=$numero+1;
			    }
			   for($i=1;$i<=$restante;$i++){
					...
					//$this->Cell($w[4],4,'','R',0,'L');
					$this->MultiCell($w[4],4,'','RL','J',0);
					//$this->Ln(4);
					$numero=$numero+1;
			   }*/
