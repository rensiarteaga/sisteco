...
class PDF extends FPDF
{
	...
	$this->Cell(58,4,'','TR',0,'L');
	$this->Cell(35,4,'','TR',0,'L');
	$this->Cell(22,4,'','TR',0,'L');
	$this->Cell(22,4,'','TR',0,'C');
	$this->Cell(43,4,'','TR',0,'L');
	$this->Ln(4);
	$this->Cell(10,4,'Nro.','LR',0,'C');
	$this->Cell(58,4,'DESCRIPCIÓN','R',0,'C');
	$this->Cell(35,4,'DESTINO','R',0,'C');
	$this->Cell(22,4,'IMPORTE','R',0,'C');
	$this->Cell(22,4,'PERIODO','R',0,'C');
	$this->Cell(43,4,'JUSTIFICACIÓN','R',0,'C');
	$this->Ln(4);
	$this->SetFont('Arial','B',4);
	$this->Cell(10,3,'','LBR',0,'C');
	$this->Cell(58,3,'','BR',0,'C');
	$this->Cell(35,3,'','BR',0,'C');
	$this->Cell(22,3,'('.$_SESSION['rep_mem_cal_simbolo'].')','BR',0,'C');
	$this->Cell(22,3,'','BR',0,'C');
	$this->Cell(43,3,'','BR',0,'C');
	$this->Ln(3);
	$this->Cell(10,4,'','LR',0,'L');
	$this->Cell(58,4,'','R',0,'L');
	$this->Cell(35,4,'','R',0,'L');
	$this->Cell(22,4,'','R',0,'L');
	$this->Cell(22,4,'','R',0,'C');
	$this->Cell(43,4,'','R',0,'L');
	$this->Ln(3);
	}
	//Pie de página
	function Footer(){
	...
	function LoadData($id_partida_presupuesto,$id_presupuesto,$tipo_memoria,$id_partida,$id_moneda,$tipo_pres){
		$cant=1000;
	    $puntero=0;
	    //$sortcol='MEMVIA.total_general';
	    $sortcol='MEMVIA.periodo_pres';
	    $sortdir='asc';
		...
		$Custom->ListarRepMemoriaCalculoViaje($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);		
	    $data=$Custom->salida;
	    return $data;
	}
	//Tabla coloreada
	function FancyTable($data,$id_partida_presupuesto,$id_presupuesto,$tipo_memoria,$id_partida,$id_moneda,$tipo_pres){
		...
		$CustomC->ContarRepMemoriaCalculoViaje(15,0,'','',$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);		
	    $cantidad=sizeof($CustomC->salida);
	    $CustomS= new cls_CustomDBPresupuesto();
		$CustomS->SumaCostoMemoriaCalculoViaje(15,0,'','',$criterio_suma,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);		
		$suma_total=$CustomS->salida;
	  	//Cabecera
		$w=array(10,58,35,22,22,43);			
			$this->SetFont('Arial','',7);
			...
			//Datos					
				...
				$this->SetWidths(array(10,58,35,22,22,43));
				$this->SetFills(array(0,0,0,0,0,0));
				$this->SetAligns(array('C','L','C','R','J','J'));
		       ...
		       /*foreach($data as $row){       
					...
					$this->Cell($w[2],4,$row[4],'R',0,'L');
					$this->Cell($w[3],4,$this->imprimirLinea($row[2]),'R',0,'R');
					$this->Cell($w[4],4,$row[1],'R',0,'C');															
					//$this->Cell($w[5],4,$row[3],'R',0,'L');
					$this->MultiCell($w[5],4,$row[3],'RL','J',0);
					//$this->Ln(4);
					$numero=$numero+1;
			    }
