...
class PDF extends FPDF
{
	...
	$this->Ln(8);
	$this->Multicell(190,5,'PARTIDA '.$_SESSION['rep_mem_cal_codigo_partida'].' "'.$_SESSION['rep_mem_cal_nombre_partida'].'"',0,'C');
	$this->Ln(1);
	...
	function LoadData($id_partida_presupuesto,$id_presupuesto,$tipo_memoria,$id_partida,$id_moneda,$tipo_pres){
	   ...
	   $criterio_filtro="PARPRE.id_partida=$id_partida AND PRESUP.id_presupuesto like ''$id_presupuesto'' AND MEMCAL.tipo_detalle=$tipo_memoria AND MONEDA.id_moneda=$id_moneda AND PRESUP.tipo_pres =$tipo_pres";
		...
		$Custom->ListarRepGralMemoriaCalculoOtros($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);		
	    $data=$Custom->salida;
	    return $data;
	}
	//Tabla coloreada
	function FancyTable($data,$id_partida_presupuesto,$id_presupuesto,$tipo_memoria,$id_partida,$id_moneda,$tipo_pres){
		...
		$CustomC->ContarRepGralMemoriaCalculoOtros(15,0,'','',$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);		
		...
		$w=array(10,65,29,29,57);			
			...
			//Datos					
		       ...
		       /*foreach($data as $row){       
					...
					$this->Cell($w[2],4,$this->imprimirLinea($row[2]/12),'R',0,'R');
