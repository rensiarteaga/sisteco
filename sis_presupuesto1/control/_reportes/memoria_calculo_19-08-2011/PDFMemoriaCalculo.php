...
class PDF extends FPDF
{
	...
	$this->Cell(40,4,'JUSTIFICACIÓN','R',0,'C');
	$this->Ln(4);
	...
	$this->Cell(19,3,'(Bs)','BR',0,'C');
	$this->Cell(19,3,'','BR',0,'C');
	...
	function LoadData($id_partida_presupuesto,$id_presupuesto,$tipo_memoria,$id_partida){
		$cant=15;
	    $puntero=0;
		...
		$Custom->ListarRepMemoriaCalculo($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);		
	    $data=$Custom->salida;
	    return $data;
	}
	//Tabla coloreada
	function FancyTable($data,$id_partida_presupuesto,$id_presupuesto,$tipo_memoria,$id_partida){
	...
	/*	$this->SetFillColor(255,255,255);
		...
		$criterio_filtro='PARPRE.id_partida_presupuesto='.$id_partida_presupuesto.' AND MEMCAL.tipo_detalle='.$tipo_memoria;
		$criterio_suma='PARPRE.id_partida_presupuesto='.$id_partida_presupuesto.' AND PRESUP.id_presupuesto='.$id_presupuesto.' AND MEMCAL.tipo_detalle='.$tipo_memoria;
	    //Leer las líneas del fichero
	    $CustomC= new cls_CustomDBPresupuesto();
		$CustomC->ContarRepMemoriaCalculo(15,0,'','',$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);		
	    $cantidad=$CustomC->salida;
	    $CustomS= new cls_CustomDBPresupuesto();
		$CustomS->SumaCostoMemoriaCalculo(15,0,'','',$criterio_suma,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);		
		...
		$w=array(10,50,20,15,19,17,40);
			...
			$this->SetTextColor(0);*/
			$this->SetFont('Arial','',7);
			$numero_paginas=ceil($cantidad/37);
			if($numero_paginas==0){
				$numero_paginas=1;
			}
			$cantidad_estimada=$numero_paginas*37;
...
$pdf->AliasNbPages();
//Carga de datos
$data=$pdf->LoadData($id_partida_presupuesto,$id_presupuesto,$tipo_memoria,$id_partida);
...
$pdf->AddPage();
$pdf->FancyTable($data,$id_partida_presupuesto,$id_presupuesto,$tipo_memoria,$id_partida);
$pdf->Output();
?>