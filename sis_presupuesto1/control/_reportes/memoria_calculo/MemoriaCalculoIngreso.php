	        ...
	        if($res)
	        {
		        if(sizeof($Custom->salida) != 0)	
	    		{
		        	...
		        	if($tipo_reporte=="Periodo"){
		         		header("location:PDFMemoriaCalculoIngreso.php?id_partida_presupuesto=$id_partida_presupuesto&id_presupuesto=$id_presupuesto&tipo_memoria=$tipo_memoria&id_partida=$id_partida&id_moneda=$id_moneda&tipo_pres=$tipo_pres&cod_prog=$cod_prog&cod_proy=$cod_proy&cod_act=$cod_act&cod_fuente_fin=$cod_fuente_fin&cod_organismo=$cod_organismo");	        			        	              
		        	}
		        	else{
						if($_GET['formato_reporte'] == 1) 	
						{
							header("location:PDFMemoriaCalculoIngresoGeneral.php?id_partida_presupuesto=$id_partida_presupuesto&id_presupuesto=$id_presupuesto&tipo_memoria=$tipo_memoria&id_partida=$id_partida&id_moneda=$id_moneda&tipo_pres=$tipo_pres&cod_prog=$cod_prog&cod_proy=$cod_proy&cod_act=$cod_act&cod_fuente_fin=$cod_fuente_fin&cod_organismo=$cod_organismo");	        			        	              
						}
						else
						{
							header("location:XLSMemoriaCalculoIngresoGeneral.php?id_partida_presupuesto=$id_partida_presupuesto&id_presupuesto=$id_presupuesto&tipo_memoria=$tipo_memoria&id_partida=$id_partida&id_moneda=$id_moneda&tipo_pres=$tipo_pres&cod_prog=$cod_prog&cod_proy=$cod_proy&cod_act=$cod_act&cod_fuente_fin=$cod_fuente_fin&cod_organismo=$cod_organismo");	        			        	              
						}	        			        	              
		        	}	
	    		}   
	    		else 
