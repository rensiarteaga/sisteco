...
class PDF extends FPDF
{
	...
	function Header()
	{
	    ...
	    $this->Ln();
	    $this->Cell(0,4,'Departamento Contable:'.$_SESSION['g_depto'],0,0,'C'); //dibuja una celad con contenido y orientacion  x, y
	    $this->Ln();
	    $this->Cell(0,4,'Presupuesto:'.$_SESSION['g_presupuesto'],0,0,'C'); //dibuja una celad con contenido y orientacion  x, y  
	    $this->SetFont('Arial','I',7);
	    $this->Ln();
	    $this->Cell(0,4,'Del '.$_SESSION['m_fecha_inicio_rep'].' Al '.$_SESSION['m_fecha_fin_rep']." ",0,0,'C'); //dibuja una celad con contenido y orientacion  x, y 
	    $this->Ln(3);
	    $this->Cell(0,4,'(Expresado en '.$_SESSION['id_moneda_desc'].")",0,0,'C'); //dibuja una celad con contenido y orientacion  x, y 
	    $this->Ln(7);//10
	    $this->SetFont('Arial','I',7);
  	...
  	function maestro(){
		...
		$res = $Custom->ListarConsistenciaEjecucion($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad ,$_SESSION['m_fecha_fin'],$_SESSION['m_fecha_inicio'],$_SESSION['m_ids_presupuesto'],$_SESSION['m_ids_depto'], $_SESSION['id_moneda']);
