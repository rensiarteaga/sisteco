...
/**
**********************************************************
Nombre de archivo:	    ActionGenerarExtractoBancario.php
Propósito:				Permite insertar partidas a los comprobantes que ingresaran a la declaración
Tabla:					tpr_siet_partidas
...
$nombre_archivo = "ActionGenerarExtractoBancario.php";
	...
	for($j = 0;$j < $cont; $j++)
	{
		...
		//Validación satisfactoria, se ejecuta la inserción en la tabla tpr_categoria<a
			if ($accion=='generar_backup'){
				$res = $Custom ->GenerarBackupEB($id_periodo);
				
			}else{
				$res = $Custom ->SubirBackupEB($id_periodo);
			}
