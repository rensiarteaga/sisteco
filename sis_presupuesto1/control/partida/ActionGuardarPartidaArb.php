...
if($_SESSION["autentificado"]=="SI")
{
	...
	elseif($proc==='add'){
		if($nodo['id_p']=='' || $nodo['id_p']=='undefined' || $nodo['id_p']=='null'){
			...
			$res = $Custom -> InsertarPartidaArb($nodo['id'],$nodo['codigo_partida'],$nodo['nombre_partida'],$nodo['nivel_partida'],$nodo['sw_transaccional'],$nodo['tipo_partida'],$nodo['id_parametro'],$nodo['id_p'],$nodo['tipo_memoria'],$nodo['desc_partida'],$nodo['sw_movimiento'],$nodo['id_concepto_colectivo'],$nodo['id_oec_sigma'],$nodo['ent_trf'],$nodo['cod_ascii'],$nodo['cod_excel'],$nodo['cod_trans']);
			if(!$res){
				$resp = new cls_manejo_mensajes(true, "406");
				$resp->mensaje_error = $Custom->salida[1] ;
				$resp->origen = $Custom->salida[2];
	...
	elseif($proc==='upd'){
		...
		$res=$Custom->ModificarPartidaArb($nodo['id'],$nodo['codigo_partida'],$nodo['nombre_partida'],$nodo['nivel_partida'],$nodo['sw_transaccional'],$nodo['tipo_partida'],$nodo['id_parametro'],$nodo['id_p'],$nodo['tipo_memoria'],$nodo['desc_partida'],$nodo['sw_movimiento'],$nodo['id_concepto_colectivo'],$nodo['id_oec_sigma'],$nodo['ent_trf'],$nodo['cod_ascii'],$nodo['cod_excel'],$nodo['cod_trans']);
			if(!$res){
				$resp = new cls_manejo_mensajes(true, "406");
				$resp->mensaje_error = $Custom->salida[1] ;
				$resp->origen = $Custom->salida[2];
