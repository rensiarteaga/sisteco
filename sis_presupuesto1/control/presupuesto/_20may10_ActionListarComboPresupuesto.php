...
//Parámetros del filtro
	...
	if($sw_inv_gasto=='si')	{$criterio_filtro= $criterio_filtro." and presup.tipo_press  in (2,3,5,6)";}
	
	if($m_id_concepto_gasto!=''){$criterio_filtro= $criterio_filtro." and presup.id_presupuesto  in (	select id_presupuesto 
	...
	/*and id_parametro in (select id_parametro
																																		from presto.tpr_partida 
																																		where id_partida in (	select id_partida 
																																								from presto.tpr_concepto_ingas 
																																              				    where id_concepto_ingas =$m_id_concepto_gasto)
	...
	if ($vista=='libro_mayor_partida'){
		$criterio_filtro= $criterio_filtro." AND presup.id_parametro in (
																		...
																		where gestio.id_gestion=$id_gestion_rlm) AND presup.id_fina_regi_prog_proy_acti  in (select id_ep from param.tpm_depto_ep where id_depto=$id_depto and estado=''activo'') and presup.id_fina_regi_prog_proy_acti in (select aep.id_fina_regi_prog_proy_acti from sss.tsg_usuario_asignacion usa
