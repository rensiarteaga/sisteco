...
//Parámetros del filtro
	...
	//$cond->add_criterio_extra("PARAMP.estado_gral",$estado_gral);
		...
		{		 	
		 	...
		 	//solo cuando se envia el valor tipo_pres se filtra por este
		 	if($m_tipo_pres){
		 		$cond->add_criterio_extra("PRESUP.tipo_pres",$m_tipo_pres);	
		 	}	 		 	
		}	
		...
		{		/*echo 'muestra algo dfkshd?'.$m_id_tipo_pres;
		 	...
		 	$cond->add_criterio_extra("PARAMP.id_parametro",$m_id_parametro);
		 	$cond->add_criterio_extra("PRESUP.id_unidad_organizacional",$m_id_unidad_organizacional);
		 	$cond->add_criterio_extra("PRESUP.tipo_pres",$m_id_tipo_pres);		 		 	
		}	
/**** fin de REP **/
	...
	else
	{
		...
		if ($sw_usuario==1){$criterio_filtro=$criterio_filtro." and UNIORG.id_unidad_organizacional in (
	...
	if ($vista=='ejecucion_institucional'){
		$criterio_filtro= $criterio_filtro." AND presup.tipo_pres  in ($m_id_tipo_pres) AND presup.id_parametro=$id_parametro_ei and presup.id_fina_regi_prog_proy_acti in (select aep.id_fina_regi_prog_proy_acti from sss.tsg_usuario_asignacion usa
