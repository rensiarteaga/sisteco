...
//Parámetros del filtro
	...
	{		/*echo 'muestra algo dfkshd?'.$m_id_tipo_pres;
	        exit;*/ 	
	 	$cond->add_criterio_extra("PRESUP.id_parametro",$id_parametro_ei);
	 	$cond->add_criterio_extra("PRESUP.id_unidad_organizacional",$m_id_unidad_organizacional);
	 	//$cond->add_criterio_extra("PRESUP.tipo_press",$m_id_tipo_pres);	
	 	$criterio_filtro=$criterio_filtro."  and PRESUP.tipo_press in (".$m_id_tipo_pres.") ";	 		 	
	}	
	/**** fin de REP **/
	...
	if($vista=='rep_formulacion_ejecucion'){
     $criterio_filtro= $criterio_filtro."  and PRESUP.id_parametro=$m_id_parametro_g and tipo_press=$m_tipo_pres_g
                                 and (SELECT
                                    sum(mes_01)as Enero
