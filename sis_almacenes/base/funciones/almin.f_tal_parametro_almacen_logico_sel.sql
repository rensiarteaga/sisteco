...
CREATE OR REPLACE FUNCTION almin.f_tal_parametro_almacen_logico_sel (
...
***************************************************************************
 SCRIPT: 		almin.f_tal_parametro_almacen_logico_sel
 DESCRIPCIÓN: 	Devuelve las consultas a la tabla almin.tal_kardex_logico
 AUTOR: 		rac 
 FECHA:			03/12/2016
 COMENTARIOS:	
***************************************************************************
...
BEGIN
    ...
    g_nombre_funcion := 'almin.f_tal_parametro_almacen_logico_sel';
    ...
    IF pm_codigo_procedimiento  = 'AL_PALOG_SEL' THEN
        BEGIN
            g_consulta := '
                          SELECT 
                            ...
                            INNER JOIN almin.tal_parametro_almacen PARALM  ON PARALM.id_parametro_almacen =  PAL.id_parametro_almacen
							WHERE ';
    ...
    ELSIF pm_codigo_procedimiento  = 'AL_PALOG_COUNT' THEN

        BEGIN
        --Cuenta todos los registros
            g_consulta := 	'SELECT
                          	COUNT(PAL.id_parametro_almacen_logico) AS total
                          	FROM almin.tal_parametro_almacen_logico PAL
                            INNER JOIN almin.tal_parametro_almacen PARALM  ON PARALM.id_parametro_almacen =  PAL.id_parametro_almacen
							WHERE ';
            g_consulta := g_consulta || pm_criterio_filtro;
