...
CREATE OR REPLACE FUNCTION almin.f_tal_unidad_constructiva_sel (
...
***************************************************************************
 SCRIPT: 		almin.f_tal_unidad_constructiva
 DESCRIPCIÓN: 	Devuelve las consultas a la tabla almin.tal_unidad_constructiva
 AUTOR: 		(Generado Automaticamente)
 FECHA:			2008-03-31 11:16:22
 COMENTARIOS:	
***************************************************************************
...
BEGIN
    ...
    g_nombre_funcion := 'almin.f_tal_unidad_constructiva_sel';
    ...
    IF pm_codigo_procedimiento  = 'AL_UNICON_SEL' THEN
        BEGIN
            g_consulta := 'SELECT
							UNICON.id_unidad_constructiva,
							UNICON.codigo,
							UNICON.fecha_reg,
							UNICON.id_tipo_unidad_constructiva,
							TIPOUC.codigo as desc_tipo_unidad_constructiva,
							UNICON.id_prog_proy_acti,
							PROGRA.nombre_programa ||''   -   ''||PROYEC.nombre_proyecto||''   -   ''||ACTIVI.nombre_actividad as desc_programa_proyecto_actividad
							FROM almin.tal_unidad_constructiva UNICON
						  	INNER JOIN almin.tal_tipo_unidad_constructiva TIPOUC ON TIPOUC.id_tipo_unidad_constructiva=UNICON.id_tipo_unidad_constructiva
							INNER JOIN param.tpm_programa_proyecto_actividad PGPYAC ON PGPYAC.id_prog_proy_acti=UNICON.id_prog_proy_acti
    ...
    ELSIF pm_codigo_procedimiento  = 'AL_UNICON_COUNT' THEN

        BEGIN
        --Cuenta todos los registros
            g_consulta := 	'SELECT
                          	COUNT(UNICON.id_unidad_constructiva) AS total
                          	FROM almin.tal_unidad_constructiva UNICON
						  	INNER JOIN almin.tal_tipo_unidad_constructiva TIPOUC ON TIPOUC.id_tipo_unidad_constructiva=UNICON.id_tipo_unidad_constructiva
							INNER JOIN param.tpm_programa_proyecto_actividad PGPYAC ON PGPYAC.id_prog_proy_acti=UNICON.id_prog_proy_acti
             				INNER JOIN param.tpm_programa PROGRA ON PROGRA.id_programa = PGPYAC.id_programa
