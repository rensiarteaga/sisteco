...
CREATE OR REPLACE FUNCTION almin.f_tal_motivo_salida_sel (
...
***************************************************************************
 SCRIPT: 		almin.f_tal_motivo_salida
 DESCRIPCIÓN: 	Devuelve las consultas a la tabla almin.tal_motivo_salida
 AUTOR: 		(Generado Automaticamente)
 FECHA:			2007-10-18 10:10:18
 COMENTARIOS:	
***************************************************************************
 HISTORIA DE MODIFICACIONES:

 DESCRIPCIÓN: Se Agrego el atributo de tipo a listar MOTSAL
...
BEGIN
    ...
    g_nombre_funcion := 'almin.f_tal_motivo_salida_sel';
    g_rol_adm := false;
    ...
    IF pm_codigo_procedimiento  = 'AL_MOTSAL_SEL' THEN
        BEGIN
            g_consulta := 'SELECT
							MOTSAL.id_motivo_salida,
							MOTSAL.nombre,
							MOTSAL.descripcion,
							MOTSAL.estado_registro,
							MOTSAL.fecha_reg,
            				MOTSAL.codigo,
               				MOTSAL.tipo
							FROM almin.tal_motivo_salida MOTSAL
						  							WHERE ';
    ...
    ELSIF pm_codigo_procedimiento  = 'AL_MOTSAL_SELEP' THEN
        BEGIN
            g_consulta := 'SELECT
				           MOTSAL.id_motivo_salida,
						   MOTSAL.nombre,
            ...
            /*  RAC 26/11/2016 ...aparentemente este filtro no funciona bien
            
            IF NOT g_rol_adm THEN
                -- SI EL USUARIO NO TIENE ROL ADMINISTRADOR SE AUMENTA LA RESTRICCIÓN DE LA ESTRUCTURA PROGRAMÁTICA
                g_consulta := g_consulta || ' AND MOTSAL.id_motivo_salida IN
                                                       (SELECT
                                                        MOSACU.id_motivo_salida
                                                        FROM almin.tal_motivo_salida_cuenta MOSACU
                                                        WHERE MOSACU.id_fina_regi_prog_proy_acti IN
                                                                        (SELECT DISTINCT
  ...
  raise notice 'consulta.... %',g_consulta;
    ...
    ELSIF pm_codigo_procedimiento  = 'AL_MOTSAL_COUNT' THEN

        BEGIN
        --Cuenta todos los registros
            g_consulta := 	'SELECT
                          	COUNT(MOTSAL.id_motivo_salida) AS total
                          	FROM almin.tal_motivo_salida MOTSAL
						  							WHERE ';
            g_consulta := g_consulta || pm_criterio_filtro;
    ...
    ELSIF pm_codigo_procedimiento  = 'AL_MOTSAL_COUNTEP' THEN
