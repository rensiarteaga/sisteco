...
$nombre_archivo = "ActionGuardarInventario.php";
...
{	//Verifica si los datos vienen por POST o GET
	...
	for($j = 0;$j < $cont; $j++)
	{
		...
		{	////////////////////Inserción/////////////////////
			...
			$res = $Custom->ValidarInventario("insert",$hidden_id_inventario, $txt_observaciones,$txt_fecha_inicio,$txt_fecha_fin,$txt_fecha_reg,$txt_tipo_inventario,$txt_id_almacen,$txt_id_responsable_almacen,$txt_id_almacen_ep,$txt_id_almacen_logico,$txt_estado,$txt_id_almacenero);
			if(!$res)
			{	//Error de validación
				$resp = new cls_manejo_mensajes(true, "406");
			...
			//Validación satisfactoria, se ejecuta la inserción en la tabla tal_inventario
			$res = $Custom -> InsertarInventario($hidden_id_inventario, $txt_observaciones,$txt_fecha_inicio, $txt_fecha_fin, $txt_fecha_reg,$txt_tipo_inventario, $txt_id_almacen, $txt_id_responsable_almacen, $txt_id_almacen_ep, $txt_id_almacen_logico,$txt_estado,$txt_id_almacenero);
			if(!$res)
			{	//Se produjo un error
				$resp = new cls_manejo_mensajes(true, "406");
		...
		{	///////////////////////Modificación////////////////////
			//Validación de datos (del lado del servidor)
			$res = $Custom->ValidarInventario("update",$hidden_id_inventario, $txt_observaciones, $txt_fecha_inicio, $txt_fecha_fin, $txt_fecha_reg,$txt_tipo_inventario, $txt_id_almacen, $txt_id_responsable_almacen, $txt_id_almacen_ep, $txt_id_almacen_logico,$txt_estado,$txt_id_almacenero);
			if(!$res)
			{	//Error de validación
				$resp = new cls_manejo_mensajes(true, "406");
				$resp->mensaje_error = $Custom->salida[1];
			...
			$res = $Custom->ModificarInventario($hidden_id_inventario, $txt_observaciones, $txt_fecha_inicio, $txt_fecha_fin, $txt_fecha_reg,$txt_tipo_inventario, $txt_id_almacen, $txt_id_responsable_almacen, $txt_id_almacen_ep, $txt_id_almacen_logico,$txt_estado,$txt_id_almacenero);
			if(!$res)
			{	//Se produjo un error
				$resp = new cls_manejo_mensajes(true, "406");
				$resp->mensaje_error = $Custom->salida[1];
	...
	$res = $Custom->ContarInventario($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);
