...
Propósito:				Permite eliminar registros de la tabla tal_caracteristica
Tabla:					tal_caracteristica
Parámetros:				$hidden_id_caracteristica
...
if($_SESSION['autentificado']=="SI")
{
	...
	for($j = 0;$j < $cont; $j++)
	{
		...
		{
			$resp = new cls_manejo_mensajes(true, "406");
			$resp->mensaje_error = "MENSAJE ERROR = No existe la Característica.";
			$resp->origen = "ORIGEN = $nombre_archivo";
			$resp->proc = "PROC = $nombre_archivo";
			$resp->nivel = "NIVEL = 4";
			echo $resp->get_mensaje();
		...
		{	
			...
			$res = $Custom->ValidarCaracteristica("delete",$hidden_id_caracteristica,$txt_nombre,$txt_tipo_dato,$txt_descripcion,$txt_id_tipo_unidad_medida,$txt_id_tipo_caracteristica,$txt_fecha_reg);
			if(!$res)
			{
				//Error de validación
				$resp = new cls_manejo_mensajes(true, "406");
			...
			//Eliminación
			$res = $Custom-> EliminarCaracteristica($hidden_id_caracteristica);
			if(!$res)
			{
				$resp = new cls_manejo_mensajes(true, "406");
				$resp->mensaje_error = $Custom->salida[1] ;
