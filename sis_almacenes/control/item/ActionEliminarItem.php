...
/**
**********************************************************
Nombre de archivo:	    ActionEliminarItem.php
Propósito:				Permite eliminar registros de la tabla tal_item
Tabla:					tal_item
Parámetros:				$hidden_id_item
...
$nombre_archivo = 'ActionEliminarItem.php';
...
if($_SESSION['autentificado']=="SI")
{
	...
	for($j = 0;$j < $cont; $j++)
	{
		...
		{
			$resp = new cls_manejo_mensajes(true, "406");
			$resp->mensaje_error = "MENSAJE ERROR = No existe el Item.";
			$resp->origen = "ORIGEN = $nombre_archivo";
			$resp->proc = "PROC = $nombre_archivo";
			$resp->nivel = "NIVEL = 4";
			echo $resp->get_mensaje();
		...
		{	
			...
			//Eliminación
			$res = $Custom-> EliminarItem($hidden_id_item);
			if(!$res)
			{
				$resp = new cls_manejo_mensajes(true, "406");
				$resp->mensaje_error = $Custom->salida[1] ;
