...
/**
**********************************************************
Nombre de archivo:	    ActionEliminarCaracteristicaItem.php
Propósito:				Permite eliminar registros de la tabla tal_caracteristica_item
...
$nombre_archivo = 'ActionEliminarCaracteristicaItem.php';
...
if($_SESSION['autentificado']=="SI")
{
	...
	for($j = 0;$j < $cont; $j++)
	{
		...
		{
			$resp = new cls_manejo_mensajes(true, "406");
			$resp->mensaje_error = "MENSAJE ERROR = No existe la Característica del Item.";
			$resp->origen = "ORIGEN = $nombre_archivo";
			$resp->proc = "PROC = $nombre_archivo";
			$resp->nivel = "NIVEL = 4";
			echo $resp->get_mensaje();
		...
		{	
			...
			$res = $Custom->ValidarCaracteristicaItem("delete",$hidden_id_caracteristica_item,$txt_valor,$txt_observaciones,$txt_fecha_reg,$txt_id_item,$txt_id_caracteristica,$txt_id_unidad_medida_base);
			if(!$res)
			{
				//Error de validación
				$resp = new cls_manejo_mensajes(true, "406");
			...
			//Eliminación
			$res = $Custom-> EliminarCaracteristicaItem($hidden_id_caracteristica_item);
			if(!$res)
			{
				$resp = new cls_manejo_mensajes(true, "406");
				$resp->mensaje_error = $Custom->salida[1] ;
