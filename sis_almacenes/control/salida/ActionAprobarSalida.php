...
$nombre_archivo = 'ActionAprobarSalida.php';
...
if($_SESSION['autentificado']=="SI")
{
	...
	for($j = 0;$j < $cont; $j++)
	{
		if ($get)
		{
			...
			$estado=$_GET["txt_estado_salida_$j"];
		...
		else
		{
			...
			$estado=$_POST["txt_estado_salida_$j"];
		...
		else
		{
			...
			if($estado=="Aprobado"){
				$res = $Custom->AprobarSalida($hidden_id_salida,$observaciones);
			}
			if($estado=="Borrador"){
				$res = $Custom->CorreccionSalida($hidden_id_salida,$observaciones);
			}
			if($estado=="Rechazado"){
				$res = $Custom->RechazarSalida($hidden_id_salida,$observaciones);
			}
			if(!$res){
				//Se produjo un error
				$resp = new cls_manejo_mensajes(true, "406");
	...
	$res = $Custom->ContarSalidaAprobacion($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);
