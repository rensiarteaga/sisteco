...
/**
**********************************************************
Nombre de archivo:	    ActionListarSalidaProy.php
...
Fecha de Creación:		02-04-2008 12:33
Versión:				1.0.0
...
if($_SESSION['autentificado']=='SI')
{
	...
	//Se aumenta el criterio para filtrar solo por el tipo de motivo ingreso
	if($tipo!='General' && $tipo!='')
	{
		$cond->add_criterio_extra("MOTSAL.tipo","''$tipo''");
	}
	
	//Aumenta la restricción del usuario
//	$cond->add_criterio_extra("SALIDA.id_usuario","$id_usuario");
	
	$criterio_filtro = $cond -> obtener_criterio_filtro();
	
	//$criterio_filtro = $criterio_filtro ." AND SALIDA.id_usuario=$id_usuario AND LOWER(SALIDA.tipo_pedido)=LOWER(''$tipo_pedido'' )";
	$criterio_filtro = $criterio_filtro ." AND LOWER(SALIDA.tipo_pedido)=LOWER(''$tipo_pedido'' )";
	...
	$res = $Custom -> ContarSalidaProy($cant ,$puntero,$sortcol,$sortdir,$criterio_filtro,$hidden_ep_id_financiador,$hidden_ep_id_regional,$hidden_ep_id_programa,$hidden_ep_id_proyecto,$hidden_ep_id_actividad);

	if($res) $total_registros= $Custom->salida;

	//Obtiene el conjunto de datos de la consulta
	$res = $Custom->ListarSalidaProy($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$hidden_ep_id_financiador,$hidden_ep_id_regional,$hidden_ep_id_programa,$hidden_ep_id_proyecto,$hidden_ep_id_actividad);
	...
	if($res)
	{
		...
		foreach ($Custom->salida as $f)
		{
			...
			$xml->add_nodo('desc_almacen_logico',$f["desc_almacen_logico"]);//13
