...
//Parámetros del filtro
	...
	if($res)
	{
		...
		if($origen == 'filtro'){
			$xml->add_rama('ROWS');
			$xml->add_nodo('id_almacen','%');
			$xml->add_nodo('codigo','Todos los Almacenes');
			$xml->add_nodo('nombre','Todos los Almacenes');
			$xml->add_nodo('descripcion','Todos los Almacenes');
			$xml->add_nodo('direccion','Todos los Almacenes');
			$xml->add_nodo('via_fil_max','Todos los Almacenes');
			$xml->add_nodo('via_col_max','Todos los Almacenes');
			$xml->add_nodo('bloqueado','Todos los Almacenes');
			$xml->add_nodo('cerrado','Todos los Almacenes');
			$xml->add_nodo('nro_prest_pendientes','Todos los Almacenes');
			$xml->add_nodo('nro_ing_no_finalizados','Todos los Almacenes');
			$xml->add_nodo('nro_sal_no_finalizadas','Todos los Almacenes');
			$xml->add_nodo('observaciones','Todos los Almacenes');
			$xml->add_nodo('fecha_ultimo_inventario','Todos los Almacenes');
			$xml->add_nodo('fecha_reg','Todos los Almacenes');
			$xml->add_nodo('id_regional','Todos los Almacenes');
			$xml->add_nodo('desc_regional','Todos los Almacenes');
			$xml->add_nodo('fecha_apertura','Todos los Almacenes');
			$xml->add_nodo('fecha_cierre','Todos los Almacenes');
			$xml->add_nodo('superficie_m2','Todos los Almacenes');
			$xml->fin_rama();
		}
		...
		foreach ($Custom->salida as $f)
		{
			...
			$xml->add_nodo('superficie_m2',$f["superficie_m2"]);
			$xml->fin_rama();
		}
		$xml->mostrar_xml();
	}
