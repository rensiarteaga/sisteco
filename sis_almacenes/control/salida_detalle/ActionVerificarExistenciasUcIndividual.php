...
if($_SESSION["autentificado"]=="SI"){
	...
	$resp = $Custom_tuc -> EliminarOrdenSalidaUCDetalleTmp($id_salida);
	if($resp){


		if($nodo["id"]!='id'){
			$mensaje = insertaMaterialesArb ($nodo["id"],$nodo["id_reg"],$nodo['cantidad'],$id_salida,$id_almacen_logico,$repeticion);

			$resp0 = $Custom_tuc ->  verificarPedidoTucTmp($id_salida,$id_almacen_logico);
			if($resp0){

				$tmp['success'] = 'true';
				$tmp['mensaje'] = $Custom_tuc->salida[2];
			...
			if($llave){
				$resp0 = $Custom_tuc ->  VerificarReservarPedidoTucInt($id_salida,$id_almacen_logico);
				if($resp0){
...
function insertaMaterialesArb ($id_tuc,$id_reg,$cantidad,$id_salida,$id_almacen_logico,$repeticion){
	...
	if($resp){
		...
		$resp2 = $Custom ->insertarTucTpmPedido($id_reg,$id_tuc,$id_salida,$cantidad,$repeticion,$id_almacen_logico);

		if(!$resp2){

			$resp3 = $Custom -> EliminarOrdenSalidaUCDetalleTmp($id_salida);
