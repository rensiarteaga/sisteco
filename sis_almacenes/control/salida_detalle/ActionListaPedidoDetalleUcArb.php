		...
		if($res)
		{
			foreach ($Custom->salida as $f)
			{
				if($f["cantidad_padre"]<>'')
				{
					...
					$cant_tot = $f["cantidad_padre"] * $f["cant_tot"];
					$cantidad=" [".$cant_tot."]";
				}
				else
				{
					//$cantidad=" [".utf8_encode($f["cantidad"]."]");
					$cantidad=" [".utf8_encode($f["cant_tot"]."]");
				}

				//$tmp['text'] = utf8_encode($f["codigo_item"])." - ".utf8_encode($f["nombre_item"] . "<b>$cantidad</b>");
				if($f["cant_demasia"]>0){
					$dem="<b><FONT COLOR=blue>+ ".$f["demasia_porc"]."%</FONT></b>";
					$tmp['text'] = utf8_encode($f["codigo_item"]). " - ".utf8_encode($f["nombre_item"]." <b>".$cantidad."</b>")." $dem";
				}
				else{
					$tmp['text'] = utf8_encode($f["codigo_item"]). " - ".utf8_encode($f["nombre_item"]." <b>".$cantidad."</b>");
				}
				...
				$tmp['qtip'] = "Nombre: ".$tmp['nombre']." <br \/>Descripcion: ".$tmp["descripcion"]." <br \/>Cantidad por 1 Unidad Constructiva: ".$f["cantidad"]." <br \/>Demasia: ".$f["cant_demasia"];
				$tmp['qtipTitle']="Codigo: ".$tmp["codigo"];
				$tmp['allowDrag']	= false;
