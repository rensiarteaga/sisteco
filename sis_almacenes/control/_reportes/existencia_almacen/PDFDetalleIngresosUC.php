...
class PDF extends FPDF
{
	...
	function FancyTable($header,$data,$tipo)
	{
		...
		$header_items=array('id_item','Item','Cantidad por UC','Existencia','Exist_UC');
		$witem=array(15,20,20,15,15);

		foreach($this->items as $row)
		{
			$this->Cell($w[0],4,$row['nombre'],'LTRB',0,'L',$fill);
			$this->Cell($w[1],4,$row['existencia'],'LTRB',1,'R',$fill);

			//Títulos de las Columnas
			for($i=0;$i<count($header_items);$i++)
			$this->Cell($witem[$i],4,$header_items[$i],1,0,'C',1);
			$this->Ln();
			//Items
			foreach ($row['items'] as $items)
			{
				if($items['cant_repartida']==0)
				{
					$this->SetFillColor(255,0,0);
					$this->Cell($witem[0],3,$items['id_item'],'LTRB',0,'L',1);
					$this->Cell($witem[1],3,$items['nombre_item'],'LTRB',0,'L',1);
					$this->Cell($witem[2],3,$items['cantidad'],'LTRB',0,'L',1);
					$this->Cell($witem[3],3,$items['cant_repartida'],'LTRB',0,'R',1);
					$this->Cell($witem[4],3,$items['exist_uc'],'LTRB',1,'R',1);
					$this->SetFillColor(224,235,255);
				}
				else
				{
					$this->Cell($witem[0],3,$items['id_item'],'LTRB',0,'L',$fill);
					$this->Cell($witem[1],3,$items['nombre_item'],'LTRB',0,'L',$fill);
					$this->Cell($witem[2],3,$items['cantidad'],'LTRB',0,'L',$fill);
					$this->Cell($witem[3],3,$items['cant_repartida'],'LTRB',0,'R',$fill);
					$this->Cell($witem[4],3,$items['exist_uc'],'LTRB',1,'R',1);
				}
			}
			$this->Ln();
