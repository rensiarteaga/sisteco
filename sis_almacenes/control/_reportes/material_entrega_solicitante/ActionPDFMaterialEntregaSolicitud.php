...
{   
   ...
   $fecha_desde1=date_create ($txt_fecha_desde); 
	...
	$_SESSION['PDF_motivo_salida']=$txt_id_motivo_salida;
	$_SESSION['PDF_nombre_motivo_salida']=utf8_decode($txt_desc_motivo_salida);
...
//Se obtiene el criterio del filtro con formato sql para mandar a la BD
	...
	if( $nombre_institucion!=''){
		$criterio_filtro= $criterio_filtro ." AND SALIDA.estado_salida=''Finalizado'' AND SALIDA.tipo_pedido=''Item'' AND PARALM.id_parametro_almacen=$txt_id_parametro_almacen and ALMEP.id_fina_regi_prog_proy_acti = $id_fina_regi_prog_proy_acti AND
...
SALIDA.id_institucion=$txt_id_institucion and MOTSAL.id_motivo_salida=$txt_id_motivo_salida	GROUP BY ITEM.codigo,ITEM.nombre,ITEM.descripcion,ITEM.peso_kg, UNIDAD.abreviatura
";
...
ALMLOG.id_almacen_logico=$txt_id_almacen_logico and ALMACE.id_almacen=$txt_id_almacen AND
SALIDA.fecha_finalizado_cancelado >=''$txt_fecha_desde'' and SALIDA.fecha_finalizado_cancelado<=''$txt_fecha_hasta'' and
SALIDA.id_empleado=$txt_id_empleado and MOTSAL.id_motivo_salida=$txt_id_motivo_salida	GROUP BY ITEM.codigo,ITEM.nombre,ITEM.descripcion,ITEM.peso_kg, UNIDAD.abreviatura
";
...
ALMLOG.id_almacen_logico=$txt_id_almacen_logico and ALMACE.id_almacen=$txt_id_almacen AND
SALIDA.fecha_finalizado_cancelado >=''$txt_fecha_desde'' and SALIDA.fecha_finalizado_cancelado<=''$txt_fecha_hasta'' and
SALIDA.id_contratista=$txt_id_contratista and MOTSAL.id_motivo_salida=$txt_id_motivo_salida	GROUP BY ITEM.codigo,ITEM.nombre,ITEM.descripcion,ITEM.peso_kg, UNIDAD.abreviatura
";
...
(SALIDA.id_empleado=$txt_id_empleado or SALIDA.id_contratista=$txt_id_contratista or SALIDA.id_institucion=$txt_id_institucion)	";
	...
	*/
	    $salida_det = array();
		$salida_det = $Custom-> ReporteMaterialEntregadoDetalle($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$hidden_ep_id_financiador,$hidden_ep_id_regional,$hidden_ep_id_programa,$hidden_ep_id_proyecto,$hidden_ep_id_actividad);
	...
	//	http://cincel.ende.bo/endesis_desarrollo/sis_almacenes/vista/material_entregado_solicitante/PDFMaterialEntregadoSolicitante.php
			header("location: ../../../vista/_reportes/material_entregado_solicitante/PDFMaterialEntregadoSolicitante.php");
	}
else
	{
