...
if($_SESSION['autentificado']=='SI')
{
	...
	$cond->add_criterio_extra("TIPOUC.estado","''Terminado''");
	...
	$crit_sort = new cls_criterio_sort($sortcol,$sortdir,'TucTerminadoArb');
	...
	*/
	...
	if($node=='id'){
		...
		$res = $Custom->ListarTipoUnidadConstructivaAgrupador($cant,$puntero,'id_tipo_unidad_constructiva desc','desc','0=0',$hidden_ep_id_financiador,$hidden_ep_id_regional,$hidden_ep_id_programa,$hidden_ep_id_proyecto,$hidden_ep_id_actividad);
	...
	elseif($tipo=='raiz')
	{
		...
		//Obtiene el conjunto de datos de la consulta
		$res = $Custom->ListarTipoUnidadConstructivaItem($cant,$puntero,'ITEM.codigo asc',$sortdir,'0=0',$hidden_ep_id_financiador,$hidden_ep_id_regional,$hidden_ep_id_programa,$hidden_ep_id_proyecto,$hidden_ep_id_actividad,$node);
		...
		if($res)
		{
			foreach ($Custom->salida as $f){
				...
				//$tmp['text'] = utf8_encode($f["nombre_item"]." <b>[".$f["cantidad"]."]</b>");
				if($f["cant_demasia"]>0){
	...
	elseif($tipo=='rama')
	{
		...
		if($res)
		{
			foreach ($Custom->salida as $f)
			{
				...
				$tmp['text'] = utf8_encode($f["codigo"]." <b>[".$f["cantidad"]."]</b>");
				$tmp['id'] = utf8_encode($f["id_tipo_unidad_constructiva"]);
				$tmp['leaf'] = false;
				$tmp['terminado'] = utf8_encode($f["estado"])=='Terminado' ? 'true':'false';
				if($tmp['terminado']=='true'){
