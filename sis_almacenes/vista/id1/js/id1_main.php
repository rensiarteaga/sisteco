...
function main(){
	...
	var paramConfig={TamanoPagina:_CP.getConfig().ss_tam_pag,TiempoEspera:_CP.getConfig().ss_tiempo_espera,CantFiltros:1,FiltroEstructura:false,FiltroAvanzado:fa,registro:reg};
	var elemento={pagina:new PaginaId1(idContenedor,direccion,paramConfig),idContenedor:idContenedor};
	_CP.setPagina(elemento);
}
Ext.onReady(main,main);

function PaginaId1(idContenedor,direccion,paramConfig){
	var vectorAtributos=new Array;
	var ds,h_txt_codigo,h_txt_cod,layout_id1;
	...
	var sw=0;
	var combo_supergrupo,combo_grupo,combo_subgrupo,txt_codigo,txt_nombre,txt_descripcion,txt_convertido,txt_fecha_reg,txt_nivel_convertido;
	//  DATA STORE //
	ds=new Ext.data.Store({
		proxy:new Ext.data.HttpProxy({url:direccion+'../../../control/id1/ActionListarId1.php'}),
		reader:new Ext.data.XmlReader({record:'ROWS',id:'id_id1',totalRecords:'TotalCount'},
	...
	vectorAtributos[6]={
		...
		tipo:'TextField',
		filtro_0:true,
		filterColValue:'id1.descripcion',
	...
	vectorAtributos[7]={
		...
		tipo:'TextArea',
		filtro_0:true,
		filterColValue:'id1.observaciones',
	...
	vectorAtributos[8]={
		validacion:{
			...
			loadMask:true,
			triggerAction:'all',
		//	store:new Ext.data.SimpleStore({fields:['id', 'valor'],data:Ext.id1Combo.estado}),
		...
		tipo:'ComboBox',
		filtro_0:true,
		filterColValue:'id1.estado_registro',
	...
	// ---------- Inicia Layout ---------------//
	var config={titulo_maestro:"Identificador 1",grid_maestro:"grid-"+idContenedor};
	layout_id1=new DocsLayoutMaestro(idContenedor);
	layout_id1.init(config);
	// INICIAMOS HERENCIA //
	this.pagina=Pagina;
	this.pagina(paramConfig,vectorAtributos,ds,layout_id1,idContenedor);
	...
	var paramFunciones={
		btnEliminar:{url:direccion+"../../../control/id1/ActionEliminarId1.php"},
		Save:{url:direccion+"../../../control/id1/ActionGuardarId1.php"},
		ConfirmSave:{url:direccion+"../../../control/id1/ActionGuardarId1.php"},
		Formulario:{
			guardar:overloadSave,
			html_apply:"dlgInfo"+idContenedor,width:'40%',height:'47%',minWidth:150,minHeight:200,labelWidth:75,closable:true,titulo:'Identificador 1'}
	};
	...
	function crearItem(){
		if(txt_convertido.getValue()=='no'){
			...
			var data="cantidad_ids=1&hidden_id_id1_0="+SelectionsRecord.data.id_id1;
			...
			Ext.Ajax.request({url:direccion+"../../../control/id1/ActionCrearItemId1.php",
			params:data,
			method:'POST',
	...
	this.btnEdit=function(){
		if(txt_convertido.getValue()=='si'){
			...
			//h_txt_cod=h_txt_codigo.substr(11,3);
			var editar = h_txt_codigo.split('.');
			txt_codigo.setValue(editar[3]);
			combo_grupo.disable();
			combo_subgrupo.disable();
			ClaseMadre_btnEdit()
		}
	...
	layout_id1.getLayout().addListener('layout',this.onResize);
