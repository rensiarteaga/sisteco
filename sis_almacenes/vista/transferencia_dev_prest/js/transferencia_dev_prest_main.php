...
function main(){
	...
	var elemento={pagina:new pagina_transferencia_dev_prest(idContenedor,direccion,paramConfig),idContenedor:idContenedor};
...
* Fecha creación:		2007-11-21 08:59:18
*/
function pagina_transferencia_dev_prest(idContenedor,direccion,paramConfig)
{
	...
	ds = new Ext.data.Store({
		...
		proxy: new Ext.data.HttpProxy({url: direccion+'../../../control/transferencia/ActionListarTransfPrestamoDev.php'}),
	...
	vectorAtributos[1]= {
		validacion:{
			...
			fieldLabel:'Almacén Físico Origen',
			grid_visible:true,
			grid_editable:false,
			width_grid:100,
			grid_indice:1
	...
	function btn_fin()
	{
		...
		if(NumSelect!=0)
		{
			if(confirm("¿Está seguro de realizar la Devolución del Material?"))
			{
				var SelectionsRecord=sm.getSelected();
				var data=SelectionsRecord.data.id_transferencia
				Ext.Ajax.request({
					url:direccion+"../../../control/transferencia/ActionFinalizarTransfPrestamoDev.php?hidden_id_transferencia_0="+data,
					method:'GET',
					success:fin,
	...
	{
		Ext.MessageBox.hide();
		Ext.MessageBox.alert('Estado', '<br>Devolución realizada.<br>');
	...
	this.AdicionarBoton('../../../lib/imagenes/arrow_rotate_clockwise.png','Devolver Préstamo',btn_fin,true,'dev_prest','');
