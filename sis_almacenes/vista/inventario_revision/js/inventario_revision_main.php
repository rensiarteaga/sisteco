...
var elemento={pagina:new pagina_inventario_revision(idContenedor,direccion,paramConfig),idContenedor:idContenedor};
 ...
 * Fecha creación:		2007-10-30 18:41:53
 */
function pagina_inventario_revision(idContenedor,direccion,paramConfig)
{	var vectorAtributos=new Array;
	var ds,combo_almacen,combo_almacen_ep,combo_almacen_logico,combo_responsable_almacen,combo_almacenero,txt_estado,txt_fecha_reg;
	...
	ds = new Ext.data.Store({
		...
		proxy: new Ext.data.HttpProxy({url: direccion+'../../../control/inventario/ActionListarInventarioRevision.php'}),
...
function btn_inventario_det(){
		...
		if(NumSelect!=0){
			...
			layout_inventario.loadWindows(direccion+'../../../vista/inventario_revision_detalle/inventario_revision_detalle.php?'+data,'Inventario Detalle',ParamVentana);
	...
	/////////////////////////////
	function btn_ajuste_inventario(){
		var sm=getSelectionModel();var filas=ds.getModifiedRecords();var cont=filas.length;var NumSelect=sm.getCount();
		if(NumSelect!=0){
			var SelectionsRecord=sm.getSelected();
			var data='m_id_inventario='+SelectionsRecord.data.id_inventario;
			...
			var ParamVentana={Ventana:{width:'85%',height:'65%'}}
			layout_inventario.loadWindows(direccion+'../../../vista/ajuste_inv/ajuste_inv.php?'+data,'Ajuste',ParamVentana);
            layout_inventario.getVentana().on('resize',function(){
			layout_inventario.getLayout().layout();
				})	}
	else
	...
	/////////////////////////////
function btn_reconteo_inventario(){
		var sm=getSelectionModel();var filas=ds.getModifiedRecords();var cont=filas.length;var NumSelect=sm.getCount();
		if(NumSelect!=0){
			if(txt_estado.getValue()=='Revision'){
			 if(confirm("¿Está seguro de Realizar un Reconteo del Inventario?"))
			  {	var SelectionsRecord  = sm.getSelected(); //es el primer registro selecionado
				...
				Ext.Ajax.request({url:direccion+"../../../control/inventario/ActionReconteoInventario.php",
				params:data,
				method:'POST',
				success:iniciado,
				...
				timeout:100000})
			     ...
			     {	Ext.MessageBox.alert('Estado', 'El Inventario no tiene estado RECONTEO completo le falta llenar los detalles')	}
	    	}
	    else
	      {	   Ext.MessageBox.alert('Estado', 'Antes debe seleccionar un item.') }
	  }
	 function iniciado(resp)
			{	Ext.MessageBox.hide();
				Ext.MessageBox.alert('Estado', '<br>Inicio El inicio para el Reconteo del Inventario fue Realizado Satisfactoriaente.<br>');
				ClaseMadre_btnActualizar()}
//////////////////////////////				
	function btn_conclusion_inventario(){
		var sm=getSelectionModel();var filas=ds.getModifiedRecords();var cont=filas.length;var NumSelect=sm.getCount();
		if(NumSelect!=0){
			if(txt_estado.getValue()=='Revision')
			{ if(confirm("¿Está seguro de concluir Inventario?"))
			  {	var SelectionsRecord  = sm.getSelected(); //es el primer registro selecionado
				var data = "cantidad_ids=1&hidden_id_inventario_0=" + SelectionsRecord.data.id_inventario;			       			        			
				Ext.Ajax.request({url:direccion+"../../../control/inventario/ActionConcluirInventario.php",
				params:data,
				method:'POST',
				success:revision,
				failure:ClaseMadre_conexionFailure,
				timeout:100000})
			     ...
			     {	Ext.MessageBox.alert('Estado', 'El Inventario no tiene estado Revision') }
	    	}
	    else
	      {   Ext.MessageBox.alert('Estado', 'Antes debe seleccionar un item.')}
	  }
	  function revision(resp)
			{	Ext.MessageBox.hide();
				Ext.MessageBox.alert('Estado', '<br>La Coclusion de Inventario Realizado Satisfactoriaente.<br>');
				ClaseMadre_btnActualizar()	}
	////////////////		
  ...
  this.btnEdit = function()	{
	...
	this.setPagina=function(elemento){elementos.push(elemento);};
		//-------------- FIN DEFINICIÓN DE FUNCIONES PROPIAS --------------//
		        ...
		        this.AdicionarBoton('../../../lib/imagenes/menu-show.gif','',btn_reconteo_inventario,true,'inventario_det','Reconteo de Inventario');
		        this.AdicionarBoton('../../../lib/imagenes/menu-show.gif','',btn_ajuste_inventario,true,'ajuste','Ajuste por Inventario');
		        this.AdicionarBoton('../../../lib/imagenes/menu-show.gif','',btn_conclusion_inventario,true,'conclusion','Concluir Inventario');
