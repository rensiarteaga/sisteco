...
function main(){
	...
	var maestro={
		id_transferencia:<?php echo $m_id_transferencia;?>,id_almacen_logico:<?php echo $m_id_almacen_logico;?>,desc_almacen_logico:'<?php echo $m_almacen_logico;?>'};
		var elemento={idContenedor:idContenedor,pagina:new pagina_baja_det_det(idContenedor,direccion,paramConfig,maestro,'<?php echo $idContenedorPadre;?>')};
		ContenedorPrincipal.setPagina(elemento);
}
Ext.onReady(main,main);

/**
* Nombre:		  	pagina_baja_gen_det.js
* Propósito: 			pagina objeto principal
* Autor:			José abraham Mita Huanca
* Fecha creación:		2007-12-13
*/
function pagina_baja_det_det(idContenedor,direccion,paramConfig,maestro,idContenedorPadre)
{
	...
	var gridMaestro=new Ext.grid.Grid(div_grid_detalle,{ds:new Ext.data.SimpleStore({fields:['atributo','valor'],data:[['Baja',maestro.id_transferencia],['Almacen Logico Id',maestro.id_almacen_logico],['Almacen Logico',maestro.desc_almacen_logico]]}),cm:cmMaestro});
	gridMaestro.render();
	//DATA STORE COMBOS
	...
	vectorAtributos[1]= {
		validacion: {
			...
			loadMask: true,
			triggerAction: 'all',
			//store: new Ext.data.SimpleStore({fields: ['ID','valor'],data : Ext.baja_gen_det_combo.estado_item}),
	...
	var config={
		...
		titulo_detalle:'Detalle de Bajas (Detalle)',
		grid_maestro:'grid-'+idContenedor
	};
	layout_baja_det = new DocsLayoutDetalle(idContenedor,idContenedorPadre);
	layout_baja_det.init(config);
	//---------- INICIAMOS HERENCIA
	this.pagina = Pagina;
	this.pagina(paramConfig,vectorAtributos,ds,layout_baja_det,idContenedor);
	...
	var paramFunciones={
		...
		width:'80%',
		minWidth:150,minHeight:200,closable:true,titulo: 'Detalle de Bajas',
		columnas:['47%','47%'],
		grupos:[
		{
			tituloGrupo:'Oculto',
	...
	this.reload=function(params){
		...
		maestro.id_almacen_fisico=datos.m_id_almacen_fisico;
		maestro.almacen_logico= datos.m_desc_almacen_logico;		
		ds.load({
			params:{
				start:0,
				limit: paramConfig.TamanoPagina,
		...
		gridMaestro.getDataSource().loadData([['Baja',maestro.id_transferencia],['Almacen Logico Id',maestro.id_almacen_logico],['Almacen Lógico',maestro.desc_almacen_logico]]);
		...
		var paramFunciones={
			...
			Formulario:{html_apply:'dlgInfo-'+idContenedor,height:340,width:480,minWidth:150,minHeight:200,closable:true,titulo: 'Baja Detalle'}
		};
	...
	this.getLayout=function(){
		return layout_baja_det.getLayout();
	};
	function InitPaginaBajaDet()
	{
	...
	InitPaginaBajaDet();
	layout_baja_det.getLayout().addListener('layout',this.onResize);
