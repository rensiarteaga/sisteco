...
function main(){
	...
	var elemento={pagina:new pagina_salida_pedido_emergencia(idContenedor,direccion,paramConfig),idContenedor:idContenedor};
...
* Fecha creación:		2007-10-25 15:08:17
*/ 
function pagina_salida_pedido_emergencia(idContenedor,direccion,paramConfig)
{
	...
	var sw=0;
	var combo_almacen,combo_almacen_logico,combo_solicitante,combo_contratista,combo_empleado,combo_motivo_salida,combo_motivo_salida_cuenta,cmb_ep;
	...
	ds=new Ext.data.Store({
		...
		proxy:new Ext.data.HttpProxy({url:direccion+'../../../control/salida/ActionListarSalidaPedidoEmergencia.php'}),
	...
	var resultTplContratista = new Ext.Template('<div class="search-item">','<b><i>{codigo}</i></b>','<br><FONT COLOR="#B5A642">{nombre_contratista}</FONT>','</div>');
	...
	vectorAtributos[4]={
		validacion: {
			...
			loadMask: true,
			triggerAction: 'all',
			//store: new Ext.data.SimpleStore({fields: ['ID','valor'],data : Ext.salida_pedido_combo.solic}),
			store: new Ext.data.SimpleStore({fields: ['ID','valor'],data : [['contratista','Contratista'],['empleado','Empleado']]}),
			valueField:'ID',
			displayField:'valor',
			lazyRender:true,
			forceSelection:true,
	...
	var paramFunciones={
		btnEliminar:{url:direccion+'../../../control/salida/ActionEliminarSalida.php',parametros:'&tipo=Emergencia&tipo_pedido=Item'},
		Save:{url:direccion+'../../../control/salida/ActionGuardarSalidaPedido.php',parametros:'&tipo=Emergencia&tipo_pedido=Item'},
		ConfirmSave:{url:direccion+'../../../control/salida/ActionGuardarSalidaPedido.php',parametros:'&tipo=Emergencia&tipo_pedido=Item'},
	...
	function btn_salida_detalle(){
		...
		if(NumSelect!=0){
			...
			layout_salida.loadWindows(direccion+'../../salida_pedido_detalle/salida_pedido_detalle_det_emergencia.php?'+data,'Detalles Material Solicitud',ParamVentana);
	...
	function iniciarEventosFormularios(){
		...
		var onSolicitanteChange = function(e){
			...
			if(valor == 'contratista'){
				...
				combo_contratista.modificado=true
	...
	this.btnNew=function(){
		...
		CM_ocultarComponente(componentes[5]);//empleado
		CM_mostrarComponente(componentes[6]);//contratista
		CM_ocultarComponente(componentes[11]);//emergencia
		CM_ocultarComponente(componentes[12]);//tipo_pedido

		componentes[2].disable();//almacen fisico
		componentes[3].disable();//almacen logico
		componentes[8].disable();//motivo salida
		componentes[9].disable();//motivo salida cuenta
	...
	this.btnEdit=function(){
		...
		componentes[8].enable();//motivo salida
		componentes[9].enable();//motivo salida cuenta
		componentes[4].enable();//solicitante
