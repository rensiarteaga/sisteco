...
function main(){
	...
	var elemento={pagina:new pagina_ingreso_pend(idContenedor,direccion,paramConfig),idContenedor:idContenedor};
...
function pagina_ingreso_pend(idContenedor,direccion,paramConfig)
{
	...
	//Inicia Layout
	var config = {
		titulo_maestro:'Aprobación de Orden de Ingreso',
	...
	var paramFunciones = {
		...
		columnas:['97%'],grupos:[{tituloGrupo:'Almacén',columna:0,id_grupo:0}],width:'40%',
	...
	function btn_confirm_ing()
	{
		...
		if(NumSelect!=0)
		{
			if(confirm("¿Está seguro de Confirmar el Ingreso?"))
			{
				var SelectionsRecord=sm.getSelected();
	...
	{
		Ext.MessageBox.hide();
		Ext.MessageBox.alert('Estado', '<br>Confirmación existosa.<br>');
	...
	function btn_anular()
	{
		...
		if(NumSelect!=0)
		{
			if(confirm("¿Está seguro de Anular el Ingreso?"))
			{
				var SelectionsRecord=sm.getSelected();
