...
function main(){
	...
	var elemento={pagina:new pagina_transferencia_aprob(idContenedor,direccion,paramConfig),idContenedor:idContenedor};
...
* Fecha creación:		2007-11-21 08:59:18
*/
function pagina_transferencia_aprob(idContenedor,direccion,paramConfig)
{
	...
	ds = new Ext.data.Store({
		...
		proxy: new Ext.data.HttpProxy({url: direccion+'../../../control/transferencia/ActionListarTransfPendiente.php'}),
		...
		vectorAtributos[7]= {
			...
			tipo: 'Field',
			filtro_0:true,
			filterColValue:'TRASUB.descripcion',
		...
		vectorAtributos[8]= {
			...
			tipo: 'Field',
			filtro_0:true,
			filterColValue:'TRASUB.observaciones',
		...
		function btn_aprob()
		{
			...
			if(NumSelect!=0)
			{
				if(confirm("¿Está seguro de aprobar la transferencia?"))
				{
					var SelectionsRecord=sm.getSelected();
					var data=SelectionsRecord.data.id_transferencia
					Ext.Ajax.request({
						url:direccion+"../../../control/transferencia/ActionAprobarTransfPendiente.php?hidden_id_transferencia_0="+data,
						method:'GET',
						success:aprobado,
		...
		function btn_rech()
		{
			...
			if(NumSelect!=0)
			{
				if(confirm("¿Está seguro de rechazar la transferencia?"))
				{
					var SelectionsRecord=sm.getSelected();
					var data=SelectionsRecord.data.id_transferencia
					Ext.Ajax.request({
		...
		this.AdicionarBoton('../../../lib/imagenes/detalle.png','Detalle de Transferencia',btn_transferencia_detalle,true,'transferencia_detalle','');
		this.AdicionarBoton('../../../lib/imagenes/ok.png','Aprobar Transferencia',btn_aprob,true,'aprobar_transf','');
		this.AdicionarBoton('../../../lib/imagenes/pedido_delete.png','Rechazar Transferencia',btn_rech,true,'rechazar_transf','');
