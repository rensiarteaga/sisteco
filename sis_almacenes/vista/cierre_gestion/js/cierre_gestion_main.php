...
var elemento={pagina:new RepCierreGestionAlmacen(idContenedor,direccion,paramConfig),idContenedor:idContenedor};
ContenedorPrincipal.setPagina(elemento);
}
Ext.onReady(main,main);

function RepCierreGestionAlmacen(idContenedor,direccion,paramConfig){
	var vectorAtributos=new Array();
	var data_ep,combo_almacen,txt_fecha_cierre,layout_rep_cierre_gestion;
	//DATA STORE
	...
	var config={titulo_maestro:"Cierre de Gestion de Almacenes"};
	layout_rep_cierre_gestion=new DocsLayoutProceso(idContenedor);
	layout_rep_cierre_gestion.init(config);
	//---------         INICIAMOS HERENCIA           -----------//
	this.pagina=BaseParametrosReporte;
	this.pagina(paramConfig,vectorAtributos,layout_rep_cierre_gestion,idContenedor);
   ...
   //-------------- DEFINICIÓN DE FUNCIONES PROPIAS --------------//
	...
	function ter(resp){
		...
		var d_fecha_cierre=new Date();
			...
			CerrarAlmacen(d_id_almacen,d_codigo,d_nombre,d_direccion,d_descripcion,d_observaciones,d_gestion,d_fecha_apertura,d_fecha_cierre);
			mostrarFrm()
			}
	function CerrarAlmacen(d_id_almacen,d_codigo,d_nombre,d_direccion,d_descripcion,d_observaciones,d_gestion,d_fecha_apertura,d_fecha_cierre){
	marcas_html="<div class='x-dlg-hd'>"+'Cierre de Gestion de Almacen'+"</div><div class='x-dlg-bd'><div id='form-ct2_"+idContenedor+"'></div></div>";
	...
	Formulario.fieldset({legend:'Cierre de Gestion'},id_almacen,codigo,nombre,direcciones,descripcion,observaciones,gestion,fecha_apertura,fecha_cierre);
   ...
   function ingresosAlm(d_id_almacen,d_fecha_apertura,d_fecha_cierre){	
	...
	var paramFunciones={ 
		Formulario:{ 
			labelWidth:75,
			url:direccion+'../../../../sis_almacenes/control/cierre_almacen/ActionCierreGestionAlmacen.php',
			abrir_pestana:false,
			fileUpload:false,
			success:retorno,
			columnas:[305,305],
			grupos:[{tituloGrupo:'Selección de Almacen',columna:0,id_grupo:0}]
			,parametros:''}
	};
	this.Init();
