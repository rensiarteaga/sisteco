...
function main(){
	...
	var elemento={pagina:new pagina_orden_ingreso_sol_import(idContenedor,direccion,paramConfig),idContenedor:'<?php echo $idContenedor;?>'};
...
* Fecha creación:		2007-10-17 12:31:23
*/
function pagina_orden_ingreso_sol_import(idContenedor,direccion,paramConfig)
{
	...
	var sw=0;
	var tipo_ord='Importacion';

	var combo_almacen,combo_almacen_logico,combo_solicitante,combo_proveedor,combo_empleado,combo_motivo_ingreso;
	...
	vectorAtributos[8]= {
		validacion: {
			...
			loadMask: true,
			triggerAction: 'all',
			//store: new Ext.data.SimpleStore({fields: ['ID','valor'],data : Ext.orden_ingreso_sol_combo.solicitante_comp_loc}),
			store: new Ext.data.SimpleStore({fields: ['ID','valor'],data : [['empleado','Funcionario'],['proveedor','Proveedor']]}),
			valueField:'ID',
			displayField:'valor',
			lazyRender:true,
			forceSelection:true,
	...
	//Inicia Layout
	var config = {
		titulo_maestro:'Orden Ingreso - Importación',
	...
	var paramFunciones = {
		...
		],minWidth:150,minHeight:200,	closable:true,titulo:'Orden Ingreso - Importación'}
	};
	...
	this.btnNew = function()
	{
		...
		CM_mostrarComponente(componentes[10]);//empleado
	...
	function iniciarEventosFormularios(){
		...
		var onSolicitanteSelect = function(e) {
			...
			if(valor == 'empleado'){
				...
				CM_ocultarComponente(componentes[10])//empleado
			}
		};
		...
		var onEpSelect = function(e){
			...
			//inicializa el filtro de motivo ingreso cuenta
			filterValues_motivo_ingreso_cuenta[0]='x'
		};
