...
function main(){
	...
	var mtr={
		id_salida:'<?php echo $m_id_salida;?>',
		descripcion:'<?php echo $m_descripcion;?>',
		id_almacen_logico:'<?php echo $m_id_almacen_logico;?>'
	};

	var elemento={pagina:new pagina_pedido_uc_arb(idContenedor,direccion,paramConfig,mtr),idContenedor:'<?php echo $idContenedor;?>'};
	ContenedorPrincipal.setPagina(elemento);

}
Ext.onReady(main,main);

function pagina_pedido_uc_arb(idContenedor,direccion,paramConfig,maestro)
{
	var Atributos=[];
	var sw=0;//variable para reiniciar el array de la variable de sesión la primera vez
	...
	*******************/
	...
	var paramFunciones={
		Basicas:{
			url:direccion+'../../../../sis_almacenes/control/existencias_uc_verif/ActionGuardarPedidoDetalleUcArb.php',
		...
		Listar:{
			url:direccion+'../../../../sis_almacenes/control/existencias_uc_verif/ActionListaPedidoDetalleUcArb.php',
			//url:direccion+'../../../../sis_almacenes/control/existencias_uc_verif/ActionDatos.php',
	...
	this.btnActualizar = function(){
		if(confirm("¿Está de seguro de limpiar la lista de Estructuras?")){
			btnActualizar()
		}
	}
	...
	function iniciarEventos(){
		...
		Dialog=getDialog();
		Dialog.addButton("Reemplazar",Remplazar);
		...
		getTreePanel().on('beforenodedrop', function(e){
			...
			if(!e.target.parentNode){
				...
				if(copy.attributes.tipo=='item'){
					...
					copy.setText(copy.attributes.codigo+' - '+copy.attributes.nombre+" <b>[1.00]</b>")
				}
				e.dropNode=copy;
				var nodo={};
	...
	function sEdit(){
		...
		if(n&&n.parentNode&&(n.parentNode.id=='id'||(n.parentNode.id!='id'&&n.attributes.opcional=='true'))){
	...
	function gSuccess(r){
		...
		if(n.attributes.tipo=='item'){
			...
			//n.setText(n.attributes.codigo+" <b>["+aux+"]</b>")
	...
	function validarSuccess(resp){
		...
		if(regreso.mensaje=='true'){
			...
			Ext.MessageBox.alert('Status', 'Todos los materiales necesarios para la construccion de esta unidad constructiva están disponibles.');
		}
		else{
	...
	function btn_verificar_exist(){
		getCtxMenu().hide();

		if(confirm("¿Esta Seguro de continuar con la validacion? \n¡Este proceso puede tardar varios minutos!")){
			var postData=sw;//'datos='+encodeURIComponent(Ext.encode(nodo))+'&proc=validar&id_salida='+maestro.id_salida+'&id_almacen_logico='+maestro.id_almacen_logico;
			...
			Ext.Ajax.request({
				url:direccion+'../../../../sis_almacenes/control/existencias_uc_verif/ActionVerificarExistencias.php',
				params: postData,
				method:'POST',
				success:VerifExist,
				//argument:{nodo:n},
	...
	iniciarEventos();
	this.AdicionarBoton('../../../lib/imagenes/logo_pdf2.bmp','Verificar Existencias de Unidades Constructivas',btn_verif_exist_sal,true,'verexist','',false);
	...
	//layout_tuc.getLayout().addListener('layout',this.onResize);
	//ContenedorPrincipal.getPagina(idContenedorPadre).pagina.getLayout().addListener('layout',this.onResizePrimario);


	layout_tuc.getLayout().addListener('layout',this.onResize);//aregla la forma en que se ve el grid dentro del layout
