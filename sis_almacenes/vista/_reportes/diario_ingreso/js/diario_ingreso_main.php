...
var elemento={pagina:new pagina_diario_ingreso(idContenedor,direccion,paramConfig),idContenedor:idContenedor};
...
*/
function pagina_diario_ingreso(idContenedor,direccion,paramConfig)
{
...
/*	ds_supergrupo=new Ext.data.Store({
	...
	ds_proveedor = new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({url: direccion+'../../../../../sis_adquisiciones/control/proveedor/ActionListarProveedor.php'}),
		reader: new Ext.data.XmlReader({
			record: 'ROWS',
			id: 'id_proveedor',
			totalRecords: 'TotalCount'
...
vectorAtributos[10]={
	...
	vectorAtributos[12]= {
		validacion: {
			...
			loadMask: true,
			triggerAction: 'all',
			//store: new Ext.data.SimpleStore({fields: ['ID','valor'],data : Ext.diario_ingresoCombo.origen}),
	...
	vectorAtributos[13]= {
		validacion: {
			...
			queryParam: 'filterValue_0',
			filterCol:'PROVEE.nombre_proveedor',//codigo#PROVEE.observaciones',
		...
		tipo:'ComboBox',
		filtro_0:true,
		filterColValue:'PROVEE.nombre_proveedor',
		defecto: '',
	...
	var paramFunciones = {
		...
		height:'70%',
		url:direccion+'../../../../control/_reportes/diario_ingreso/ActionResumenDiarioIngresos.php',
	...
	function iniciarEventosFormularios(){
		...
		*/
		...
		nombre_proveedor=ClaseMadre_getComponente('nombre_proveedor');
		...
		var onProveedorSelect = function(e) {
			var id = combo_proveedor.getValue();
			if(combo_proveedor.store.getById(id)!=undefined){
				nombre_proveedor.setValue(combo_proveedor.store.getById(id).data.nombre_proveedor);
		...
		var onSolicitanteSelect=function(e){
			...
			componentes[13].enable();//proveedor
			componentes[14].enable();//empleado
			componentes[15].enable();//contratista
			componentes[16].enable();//institucion
			if(valor == 'proveedor'){

				CM_mostrarComponente(componentes[13]);//proveedor
                ...
                CM_ocultarComponente(componentes[13]);//proveedor  
		...
		var onSolicitanteChange = function(e) {
			...
			if(valor == 'proveedor'){
		        ...
		        combo_proveedor.on('select',onProveedorSelect);
