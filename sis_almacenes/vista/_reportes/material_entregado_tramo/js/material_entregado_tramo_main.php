...
var elemento={pagina:new pagina_material_entregado_tramo(idContenedor,direccion,paramConfig),idContenedor:idContenedor};
...
/**
* Nombre:		  	    pagina_material_entregado_tramo_main.js
* Propósito: 			pagina objeto principal
* Autor:				RCM
* Fecha creación:		25/08/2008
*/ 
function pagina_material_entregado_tramo(idContenedor,direccion,paramConfig)
{
	...
	//DATA STORE's
	var ds_tramo = new Ext.data.Store({proxy: new Ext.data.HttpProxy({url: direccion+'../../../../control/tramo/ActionListarTramo.php'}),reader: new Ext.data.XmlReader({record: 'ROWS',id: 'id_tramo',totalRecords: 'TotalCount'}, ['id_tramo','codigo','descripcion','observaciones','fecha_reg'])});
    ...
    function render_id_tramo(value, p, record){return String.format('{0}', record.data['nombre'])}
    ...
    var resultTplTramo = new Ext.Template('<div class="search-item">','<b><i>{codigo}</i></b>','<FONT COLOR="#B5A642"><br>Descripcion: {descripcion}</FONT>','</div>');
...
/*vectorAtributos[13]= {
		validacion: {
			...
			store: new Ext.data.SimpleStore({
				fields: ['ID','valor'],
				data : Ext.proc_existenciasCombo.solicitante
			}),
			valueField:'ID',
			displayField:'valor',
			lazyRender:true,
...
*/
	vectorAtributos[13]= {
		...
		filtro_0:true,
		filtro_1:true,
		//filterColValue:'TRAMO.id_tramo#PERSON.nombre#PERSOn.apellido_paterno#PERSON.apellido_materno',
		defecto: '',
...
/*	vectorAtributos[19]={
	...
	//Inicia Layout
	var config={
		titulo_maestro:'Entradas por Tramo'
	...
	var paramFunciones = {
		...
		height:'70%',
		url:direccion+'../../../../control/_reportes/material_entrega_solicitante/ActionPDFMaterialEntregaTramo.php',
		abrir_pestana:true, //abrir pestana
		titulo_pestana:obtenerTitulo,
		fileUpload:false,
		width:'70%',
		columnas:['47%'],
		minWidth:150,minHeight:200,	closable:true,titulo:'Entrega Material Tramo',
		grupos:[
		{tituloGrupo:'Fechas',columna:0,id_grupo:0},
		{tituloGrupo:'Almacén',columna:0,id_grupo:1},
		{tituloGrupo:'Tramo',columna:0,id_grupo:2}
		]}
	};
	...
	function iniciarEventosFormularios(){
		...
		*/
		combo_tramo = ClaseMadre_getComponente('id_tramo');
		...
		*/
		...
		desc_tramo=ClaseMadre_getComponente('desc_tramo');
       ...
       */
		...
		var onTramoSelect = function(e) {
			var id = combo_tramo.getValue();
			if(combo_tramo.store.getById(id)!=undefined){
			desc_tramo.setValue(combo_tramo.store.getById(id).data.descripcion);
				}
			};
