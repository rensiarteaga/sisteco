...
var elemento={pagina:new pagina_existencia_ep(idContenedor,direccion,paramConfig),idContenedor:idContenedor};
...
/**
 * Nombre:		  	    pagina_existencia_ep_main.js
...
/*
		...
		vectorAtributos[6] = param_id_almacen_ep;
	*/
/*// txt fecha_reg15
	var paramMes = {
		validacion: {
			...
			store: new Ext.data.SimpleStore({
				fields: ['mes', 'nombre'],
				data : Ext.proc_existencia_epCombo.meses
			}),
			valueField:'mes',
			displayField:'nombre',
			width: 120,
	...
	var paramGestion = {
		validacion:{
			...
			store: new Ext.data.SimpleStore({
				fields: ['v1'],
				data : Ext.proc_existencia_epCombo.anos
			}),
			valueField:'v1',
			displayField:'v1',
			lazyRender:true,
...
/*var param_fecha= {
			validacion:{
				name:'fecha_existencia_ep',
				fieldLabel:'Fecha de existencia_ep',
				allowBlank:true,
			...
			dateFormat:'m-d-Y',
			defecto:'',
			save_as:'txt_fecha_existencia_ep',
			id_grupo:2
		};
	...
	//Inicia Layout
	var config={
		titulo_maestro:'existencia_ep de Valor'
		
	};
	layout_existencia_ep=new DocsLayoutProceso(idContenedor);
	...
	this.pagina(paramConfig,vectorAtributos,layout_existencia_ep,idContenedor);
	...
	var paramFunciones = {
		...
		height:'70%',
		url:direccion+'../../../../../control/_reportes/existencia_ep/ActionReporteExistenciaEp.php',
...
//			combo_almacen_ep.setValue('%');
   ...
   function verificar_ep()
	...
	this.getLayout=function(){return layout_existencia_ep.getLayout();};
