...
function main(){
	...
	var elemento={pagina:new pagina_ingreso_val(idContenedor,direccion,paramConfig),idContenedor:idContenedor};
...
* Fecha creación:		2007-10-17 12:31:23
*/
function pagina_ingreso_val(idContenedor,direccion,paramConfig)
{
	...
	ds = new Ext.data.Store({
		...
		proxy: new Ext.data.HttpProxy({url: direccion+'../../../control/ingreso/ActionListarIngresoVal.php'}),
	...
	layout_ingreso_val=new DocsLayoutMaestro(idContenedor);
	layout_ingreso_val.init(config);
	...
	this.pagina(paramConfig,vectorAtributos,ds,layout_ingreso_val,idContenedor);
	...
	function btn_ingreso_val_det(){
		...
		if(NumSelect!=0){
			...
			layout_ingreso_val.loadWindows(direccion+'../../../vista/orden_ingreso_aprob_det/orden_ingreso_aprob_det.php?'+data,'Detalle Ingreso',ParamVentana);
			layout_ingreso_val.getVentana().on('resize',function(){
				layout_ingreso_val.getLayout().layout()
			})
		}
		else
		{
	...
	function btn_fin_val()
	{
		...
		if(NumSelect!=0)
		{
			if(confirm("¿Está seguro de Finalizar la Valoración?"))
			{
				var SelectionsRecord=sm.getSelected();
				var data=SelectionsRecord.data.id_ingreso
				Ext.Ajax.request({
					url:direccion+"../../../control/ingreso/ActionFinalizarIngresoVal.php?hidden_id_ingreso_0="+data,
					method:'GET',
					success:val_terminado,
	...
	function val_terminado(resp)
	{
		Ext.MessageBox.hide();
		Ext.MessageBox.alert('Estado', '<br>Valoración finalizada con éxito.<br>');
	...
	this.getLayout=function(){
		return layout_ingreso_val.getLayout()
	};
	...
	this.AdicionarBoton('../../../lib/imagenes/detalle.png','Detalle del Ingreso',btn_ingreso_val_det,true,'ingreso_val_det','');
	this.AdicionarBoton('../../../lib/imagenes/ok.png','Finalizar Valoración del Ingreso',btn_fin_val,true,'Finalizar Valoración','');
	this.AdicionarBoton('../../../lib/imagenes/cancel.png','Anular el Ingreso',btn_frm_anular,false,'Anular Ingreso','');

	this.iniciaFormulario();

	layout_ingreso_val.getLayout().addListener('layout',this.onResize);//aregla la forma en que se ve el grid dentro del layout
