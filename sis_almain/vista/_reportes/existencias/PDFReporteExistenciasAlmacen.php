...
$pdf->SetFont('Arial','B',14);
	...
	$pdf->Cell(220,3,'INFORMACIÓN DE EXISTENCIAS',0,1,'C');
	$pdf->Ln();
	...
	$pdf->SetX(87);
	$pdf->SetFont('Arial','B',6);
	$pdf->Cell(55,3,'  AL :  '.$dia_fin.' DE '.strtoupper($mes_fin_literal).' DE '.$anio_fin,0,0,'C');
	...
	$detalle=$_SESSION['pdf_existencias_almacen'];
	$sum=0;
	...
	for ($i=0; $i<count($detalle); $i++)
	{
		...
		$pdf->Cell(18,3,number_format($detalle[$i]["precio_unitario"] * $detalle[$i]["existencias"],6),'RL',0,'R');//1
