...
{	 
	...
	var tpl_item=new Ext.Template('<div class="search-item">','<i><b>{nombre}</b></i></br>','<b>C&oacute;digo </b>: <FONT COLOR="#B5A642">{codigo}</FONT></br>','<b>Descripci&oacute;n </b>: <FONT COLOR="#B5A642">{descripcion}</FONT>','</div>');
	...
	Atributos[2]={
		...
		fieldLabel: 'Item',
				...
				filterCol:'al.codigo#al.descripcion#al.nombre',
				typeAhead: false,
				forceSelection : true,
				mode: 'remote',
				queryDelay: 50,
	...
	var paramFunciones={ 
		...
		Formulario:{html_apply:'dlgInfo-'+idContenedor,height:246,columnas:['90%'],
			grupos:[
			...
			minHeight:50,	
			//closable:true,
			titulo:' Formulario Registro : Stock Items - Almacen'
	...
	this.AdicionarBoton("../../../lib/imagenes/script_gear.png",'<b>Definir Ubicacion<b>',btnUbicacion,true,'Define ubicación','Ubicaci\u00F3n Item');
	this.AdicionarBoton("../../../lib/imagenes/script_gear.png",'<b>Definir Orden<b>',btnOrdenUbicacion,true,' Definir Orden Ubicaciones','Orden Ubicaci\u00F3n');
	...
	function btnOrdenUbicacion()
	{
		...
		if(NumSelect == 1)
		{
			...
			data = data+"&m_desc_item="+SelectionsRecord.data.desc_item;
			data = data+"&m_desc_almacen="+SelectionsRecord.data.desc_almacen;

			var ParamVentana={Ventana:{width:'60%',height:'70%'}};
			layout_stock_item.loadWindows(direccion+'../../../../sis_almain/vista/ubicacion_item/orden_ubicacion_item.php?'+data,'orden ubicacion items',ParamVentana);
		}
		else
		{
	...
	function btnUbicacion()
	{
		...
		if(NumSelect == 1)
		{
			...
			var ParamVentana={Ventana:{width:'60%',height:'70%'}};
			layout_stock_item.loadWindows(direccion+'../../../../sis_almain/vista/ubicacion_item/ubicacion_item.php?'+data,'ubicacion Items',ParamVentana);
		}
		else
		{
			Ext.MessageBox.alert('Estado', 'Debe tener seleccionado solo un registro.');
