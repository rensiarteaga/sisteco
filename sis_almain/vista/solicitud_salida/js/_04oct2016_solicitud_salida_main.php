	...
	function cargar_fecha_bd(resp)
	{
		...
		if(resp.responseXML != undefined && resp.responseXML != null && resp.responseXML.documentElement != null && resp.responseXML.documentElement != undefined)
		{
			...
			if((root.getElementsByTagName('TotalCount')[0].firstChild.nodeValue)>0)
			{
				...
				/*var min_date = (new Date(fecha)).clearTime();
				var formato = min_date.getDate()+'/'+(min_date.getMonth()+1)+'/'+min_date.getFullYear();
				cm_getComponente('fecha_solicitud').minValue = formato;
				alert(formato.to_date);*/
			}
		}
	}
