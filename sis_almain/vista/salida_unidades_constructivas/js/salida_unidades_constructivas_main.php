...
var elemento={pagina:new PaginaSalidaUnidadesConstructivas(idContenedor,direccion,paramConfig),idContenedor:'<?echo $idContenedor;?>'};
...
*/
function PaginaSalidaUnidadesConstructivas(idContenedor,direccion,paramConfig)
{
	var vectorAtributos=new Array;
	var componentes=new Array();
	var idAlmacen;
	var combo_solicitante,combo_proveedor,combo_contratista,combo_empleado;
	var combo_institucion,combo_motivo_ingreso;
	...
	var ds = new Ext.data.Store({
		...
		reader: new Ext.data.XmlReader({
		    ...
		    'id_salida_uc','id_almacen','desc_almacen','id_contratista','desc_contratista'
		    ,'id_proveedor','desc_proveedor','id_empleado','desc_empleado','id_institucion'
		    ,'desc_institucion','id_fase','desc_fase','id_tramo','desc_tramo','id_unidad_constructiva','desc_uc'
		    ,'nro_contrato', {
				name : 'fecha_salida',
				type : 'date',
				dateFormat : 'd-m-Y'
			}	,'concepto_salida','observaciones','supervisor','ci_supervisor'
		    ,'receptor','ci_receptor','solicitante','ci_solicitante','usuario_reg','origen_salida','estado','codigo',
		    {
				name : 'fecha_reg',
				type : 'date',
				dateFormat : 'd-m-Y'
	...
	vectorAtributos = [
		...
		},
		{
			...
			tipo:'ComboBox',
			filtro_0:true,
			filterColValue:'fas.codigo#fas.descripcion',
			defecto: '',
			save_as:'txt_id_fase',
			id_grupo:2
		},
		{
			validacion: {
				...
				queryParam: 'filterValue_0',
				filterCol:'tra.codigo#tra.descripcion',
				tpl: tpl_tramo,
				typeAhead:true,
				forceSelection:true,
				mode:'remote',
			...
			tipo:'ComboBox',
			filtro_0:true,
			filterColValue:'tra.codigo#tra.descripcion',
			defecto: '',
		...
		},
		{
			...
			tipo : 'TextField',
			filtro_0 : false,
			filterColValue : 'suc.usuario_reg',
			form : false
		},
		{
			validacion : {
	...
	//Inicia Layout
	var config = { 
		titulo_maestro:'Salida Unidades Constructivas',
		grid_maestro:'grid-'+idContenedor,
		urlHijo : '../../../sis_almain/vista/salida_unidades_constructivas_det/salida_uc_det.php'
	};
	...
	this.AdicionarBoton('../../../lib/imagenes/menu-show.gif','Finalizar',btn_finalizar_salida,true,'finalizar','Finalizar');
	
	function btn_finalizar_salida()
	{
		var sm=getSelectionModel();
		var NumSelect=sm.getCount();
		if(NumSelect==1)
		{
			var SelectionsRecord=sm.getSelected();
			if(confirm("Esta seguro de finalizar la salida del proyecto?"))
			{
				if(SelectionsRecord.data.estado == 'borrador')
					cm_getComponente('accion').setValue('finalizar_mov');
				else if(SelectionsRecord.data.estado == 'finalizado')
					cm_getComponente('accion').setValue('corregir_mov');
				
				cm_Save();
			}							
		}
		else{
			Ext.MessageBox.alert('Estado', 'Antes debe seleccionar un solo registro.');
		}		
	}
