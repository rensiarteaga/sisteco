...
function PaginaMovimiento(idContenedor, direccion, paramConfig,mov_estado) {
	...
	ds = new Ext.data.Store({
		...
		reader : new Ext.data.XmlReader({
			...
			totalRecords : 'TotalCount'
				...
				'observaciones', 'estado', 'valorizado','usuario_reg','id_almacen_trans','desc_almacen', 'id_movimiento_fk','almacen_destino',
				{
					name : 'fecha_reg',
					type : 'date',
					dateFormat : 'd-m-Y'
	...
	this.EnableSelect = function(selEvent, rowIndex, selectedRow) 
	{
		...
		if (mov_estado == "borrador") 
		{
			...
			//añadido 27062014 control boton de finalizacion de entrega
	...
	{
		this.AdicionarBoton('../../../lib/imagenes/det.ico','Solicitar Correccion',btn_correccion,false,'corregir_movimiento','Correccion');
	...
	else if(mov_estado == "finalizado")
	{
		...
		this.AdicionarBoton('../../../lib/imagenes/wrench.png','Valorizacion',btn_valorizar,true,'valorizacion','Valorar Movimiento');
		this.AdicionarBoton('../../../lib/imagenes/det.ico','Corregir Movimiento',btn_corregir_movimiento,false,'corregir_movimiento','Corregir Movimiento');
		cm_getBoton('reportePDF-' + idContenedor).disable(); 
		cm_getBoton('valorizacion-' + idContenedor).disable();
		cm_getBoton('corregir_movimiento-' + idContenedor).disable();
	}
