...
function PaginaMovimiento(idContenedor, direccion, paramConfig,mov_estado) {
	...
	var ds_transferencia = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : direccion
					+ "../../../control/almacen/ActionListarAlmacen.php"		}),
		reader : new Ext.data.XmlReader({
	...
	function iniciarEventosFormularios()
	{
		...
		var onComboTipoMovimientoSelect=function(e)
		{
			...
			{ 
				...
				//filtro almacenes distintos al almacen donde se realiza el movimiento
				ds_transferencia.baseParams = {
						filtro_trans : 'si',
						id_almacen : cbxSisAlma.getValue()
					};
	...
	function cargar_fecha_bd(resp)
	{
		...
		if(resp.responseXML != undefined && resp.responseXML != null && resp.responseXML.documentElement != null && resp.responseXML.documentElement != undefined)
		{
			...
			if((root.getElementsByTagName('TotalCount')[0].firstChild.nodeValue)>0)
			{
				...
				cm_getComponente('fecha_movimiento').maxValue = new Date();			
			}
		}
	}
