 ...
 * Propósito:				Permite eliminar registros de la tabla tal_almacen
 * Tabla:					tal_almacen
 * Parámetros:				$hidden_id_almacen	--> id de almacen
...
$nombre_archivo = 'ActionEliminaIngresoProyecto.php';
...
if ($_SESSION['autentificado'] == "SI") {
	...
	for($j = 0; $j < $cont; $j ++) {
		...
		if ($get) {
			$hidden_id_ingresos = $_GET["hidden_id_ingresos_$j"];
		} else {
			$hidden_id_ingresos = $_POST["hidden_id_ingresos_$j"];
		}
		
		if ($hidden_id_ingresos == "undefined" || $hidden_id_ingresos == "") {
			$resp = new cls_manejo_mensajes(true, "406");
		...
		{ // Eliminación
			$res = $Custom->EliminarIngresosFinalizados($hidden_id_ingresos);
			if (! $res)
			{
				$resp = new cls_manejo_mensajes(true, "406");
	...
	if ($sortdir == "")
		$sortdir = 'asc';
	if ($criterio_filtro == "")
		$criterio_filtro = "0=0 AND ing.id_costeo LIKE($id_costeo)";
