 ...
 * Propósito:				Permite desplegar almacenes
 * Tabla:					tal_almacen
 * Parámetros:				$cant
 * $puntero
 * $sortcol
...
if ($_SESSION['autentificado'] == 'SI') {
	...
	/*$crit_sort = new cls_criterio_sort($sortcol, $sortdir, 'Alma');
	$sortcol = $crit_sort->get_criterio_sort();*/
	
	if (isset($filtro_trans) && $filtro_trans == 'si')
	{
		$criterio_filtro = $criterio_filtro." AND al.id_almacen NOT IN ($id_almacen)";
	}
	...
	if(isset($filtro_solic) && $filtro_solic == 'si')
	{
		if(isset($id_uo))
		{
			$criterio_filtro = $criterio_filtro." AND al.id_almacen IN (SELECT  DISTINCT a.id_almacen
																			FROM alma.tai_almacen_unidad_org a
																			WHERE a.id_unidad_organizacional = $id_uo AND a.estado = ''activo'')";
		}
	}
	...
	$res = $Custom->ListarAlmacen($cant, $puntero, $sortcol, $sortdir, $criterio_filtro, $id_financiador, $id_regional, $id_programa, $id_proyecto, $id_actividad);
	...
	if ($res) {
		...
		foreach ( $Custom->salida as $f ) {
			...
			$xml->add_nodo('tipo_control', $f["tipo_control"]);
