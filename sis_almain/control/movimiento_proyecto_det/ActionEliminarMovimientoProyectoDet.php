...
$nombre_archivo = 'ActionEliminarMovimientoProyectoDet.php';
...
if ($_SESSION['autentificado'] == "SI") {
	...
	for($j = 0; $j < $cont; $j ++) 
	{
		...
		if ($id_mov_proy_det == "undefined" || $id_mov_proy_det == "") {
			$resp = new cls_manejo_mensajes(true, "406");
			$resp->mensaje_error = "MENSAJE ERROR = No existe el detalle del movimiento proyecto especificado para eliminar.";
			$resp->origen = "ORIGEN = $nombre_archivo";
			$resp->proc = "PROC = $nombre_archivo";
			$resp->nivel = "NIVEL = 4";
			echo $resp->get_mensaje();
			...
			$res = $Custom->EliminarMovimientoProyectoDet($id_mov_proy_det);
			if (! $res) {
				$resp = new cls_manejo_mensajes(true, "406");
