...
CREATE OR REPLACE FUNCTION alma.f_tai_unidad_constructiva_sel (
...
***************************************************************************
 SCRIPT: 		 alma.f_tai_unidad_constructiva_sel
 DESCRIPCION: 	 Devuelve las consultas a la tabla alma.tai_unidad_constructiva
 AUTOR: 		 UNKNOW
 FECHA:			 11-08-2014
***************************************************************************/
...
BEGIN
    ...
    IF pm_codigo_procedimiento  = 'AL_UNICONST_SEL' THEN
        BEGIN
            g_consulta := 'SELECT 
            					uni.usuario_reg,
            					COALESCE(to_char(uni.fecha_reg,''dd-mm-yyyy''),''0'') AS fecha_reg,
                                uni.id_unidad_constructiva,
                                uni.codigo,
                                uni.nombre,
                                uni.descripcion,
                                uni.nombre,
                                uni.estado,
                                uni.codigo||'' - ''||uni.nombre as desc_unidad_const,
                                uni.id_unidad_constructiva_fk
                               ,'''':: varchar as tipo_rama
                               ,uni.orden
                               ,uni.cod_tramo
							FROM alma.tai_unidad_constructiva uni
                            WHERE ';
            ...
            --modificacion del tipo de rama en el arbol de unidad constructiva
   
            IF (g_registros.id_unidad_constructiva >=0 AND g_registros.id_unidad_constructiva_fk IS NULL)
            THEN
            	g_registros.tipo_rama='padre';
                
            ELSIF( g_registros.id_unidad_constructiva_fk IS NOT NULL)
            THEN
            	IF EXISTS(SELECT 1 FROM alma.tai_unidad_constructiva uc WHERE uc.id_unidad_constructiva_fk=g_registros.id_unidad_constructiva)
                THEN
    	...
    	ELSIF pm_codigo_procedimiento  = 'AL_UNICONSTCOUNT_SEL' THEN

        BEGIN
        --Cuenta todos los registros de tasa_categoria sin condiciones
            g_consulta := 'SELECT count(uni.id_unidad_constructiva)
							FROM alma.tai_unidad_constructiva uni
                            WHERE  ';
        ...
        ELSIF pm_codigo_procedimiento  = 'AL_UNICONSTRAM_COUNT' THEN

        BEGIN
        --Cuenta todos los registros de tasa_categoria sin condiciones
            g_consulta := 'SELECT COUNT(uc.id_unidad_constructiva) as total
                          FROM alma.tai_unidad_constructiva uc
                          WHERE uc.estado=''activo''  AND  ';
            ...
            g_descripcion_log_error := 'Consulta de cantidad registrada';
        END;
        ELSIF pm_codigo_procedimiento  = 'AL_UNICONSTRAM_SEL' THEN

        BEGIN
        --Cuenta todos los registros de tasa_categoria sin condiciones
            g_consulta := '
            				SELECT 	uc.id_unidad_constructiva,uc.id_unidad_constructiva_fk,uc.codigo||'' - ''||uc.nombre as desc_unidad_constructiva
                                   ,uc.estado,uc.descripcion,uc.observaciones,
                                   COALESCE(to_char(uc.fecha_reg,''dd-mm-yyyy''),''0'') AS fecha_reg, uc.usuario_reg
                                   ,uc.codigo,uc.nombre
                          	FROM alma.tai_unidad_constructiva uc
                          	WHERE uc.estado=''activo''  AND  ';
