...
CREATE OR REPLACE FUNCTION alma.f_ai_generar_cod_solicitud (
...
THEN
		...
		--raise notice '%',v_cad;
          ...
          if EXISTS(select 1 from alma.tai_solicitud_salida sal where sal.codigo like(v_res))
          then
          		--codigo de movimiento generado -> error
                raise exception '%','Error, El codigo generado:'||v_res||' ya existe, verificar : param.tpm_correlativo_general y alma.tai_solicitud_salida';
          end if;
