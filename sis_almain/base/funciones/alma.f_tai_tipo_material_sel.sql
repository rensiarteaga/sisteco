...
CREATE OR REPLACE FUNCTION alma.f_tai_tipo_material_sel (
...
***************************************************************************
 SCRIPT: 		 alma.f_tai_tipo_material_sel
...
BEGIN
    ...
    IF pm_codigo_procedimiento  = 'AL_TIPMAT_SEL_NEW' THEN
        BEGIN
            g_consulta := ' SELECT 	tip.id_tipo_material,tip.cod_tipo_material,tip.nombre_tipo_material,tip.desc_tipo_material,
                                     COALESCE(to_char(tip.fecha_reg,''dd-mm-yyyy''),''0'') AS fecha_reg, tip.usuario_reg
                            FROM alma.tai_tipo_material tip
                            WHERE ';
    ...
    ELSIF pm_codigo_procedimiento  = 'AL_COUNTTIPMAT_SEL' THEN

        BEGIN
        --Cuenta todos los registros de tasa_categoria sin condiciones
            g_consulta := 'SELECT count(tip.id_tipo_material) as total
							FROM alma.tai_tipo_material tip
                            WHERE  ';
