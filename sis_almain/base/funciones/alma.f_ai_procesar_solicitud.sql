...
CREATE OR REPLACE FUNCTION alma.f_ai_procesar_solicitud (
        ...
        BEGIN
            ...
            where tm.tipo = 'solicitud';
              ...
              --id_tipo_movimiento=2 manejado por defecto 
 			  v_res = "alma"."f_tai_movimiento_iud"(pm_id_usuario,pm_ip_origen,''::text,'AL_MOVI_INS'::varchar,null
              									     ,null,v_id_tipo_mov,v_id_almacen,al_id_solicitud_salida,v_codigo
                                                     ,now()::TIMESTAMP,v_descripcion,'',null,null,null);
              
              select max(alma.tai_movimiento.id_movimiento) into v_max_id_mov
      		...
      		for v_det_solicitud in (  select det.id_item,det.cantidad as cantidad_solicitada
                                      ...
                                      left join alma.tai_item it on it.id_item=det.id_item
                                      where sol.id_solicitud_salida=al_id_solicitud_salida
                                      order by det.fecha_reg ASC
                                      )
      		loop
            	v_res = (alma.f_tai_detalle_movimiento_iud(pm_id_usuario,pm_ip_origen,''::text,'AL_DETMOV_INS',null
        ...
        END;
      	...
      	begin
                ...
                */
                  ...
                  v_codigo_sol = (alma.f_ai_cod_correlativo(v_proc_cod.codigo)); 
                                                              
                                                            
                -- raise exception '%',v_cod_res;
                 select alma.f_tai_solicitud_salida_iud(	pm_id_usuario,pm_ip_origen,''::text,'AL_SOLSAL_INS2',null,null
                	...
                	select max(alma.tai_solicitud_salida.id_solicitud_salida) into v_max_id_sol
                  ...
                  LOOP
                  			/*v_res = "alma"."f_tai_detalle_solicitud_iud"(	pm_id_usuario,pm_ip_origen,''::text,'AL_DETSOL_INS2',null,null
                            											,v_max_id_sol
                                                                        ,v_det_solicitud.id_item
                                                                        ,v_det_solicitud.cantidad_solicitada
                            										);*/
                            select alma.f_tai_detalle_solicitud_iud(	pm_id_usuario,pm_ip_origen,''::text,'AL_DETSOL_INS2',null,null
                            											,v_max_id_sol
                                                                        ,v_det_solicitud.id_item
