...
CREATE OR REPLACE FUNCTION alma.f_tai_ubicacion_sel (
...
***************************************************************************
 SCRIPT:          alma.f_tai_ubicacion_sel
 DESCRIPCI√?N:     Devuelve las consultas a la tabla tal_ubicacion
...
BEGIN
        ...
        BEGIN
            ...
            LOOP
        		if EXISTS (	select 1 
                            ...
                            where s.id_almacen = pm_id_financiador and s.id_item in(select DISTINCT a.id_item
                            														from alma.tai_ubicacion_item_detalle a
                                                                                    where a.id_almacen = pm_id_financiador
                                                                                    	and a.id_ubicacion = pm_id_regional
                            														) 
                            )
                then
    ...
    -- PARA LA CONSULTA DE CONTEO  
        ...
        --Cuenta todos los registros
            g_consulta := 	'SELECT COUNT(ub.id_ubicacion) as total
        					FROM alma.tai_ubicacion ub
                            INNER JOIN alma.tai_almacen al on ub.id_almacen=al.id_almacen AND al.estado=''activo''
                            WHERE ';
