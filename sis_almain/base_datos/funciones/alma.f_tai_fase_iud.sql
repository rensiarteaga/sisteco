...
CREATE OR REPLACE FUNCTION alma.f_tai_fase_iud (
...
---*** SI NO SE TIENE PERMISOS DE EJECUCIO?N SE RETORNA EL MENSAJE DE ERROR
    ...
    BEGIN
        ...
        IF EXISTS(SELECT 1 FROM alma.tai_fase f WHERE f.codigo=al_codigo AND f.id_almacen=al_id_almacen)
        THEN
		...
		INSERT INTO alma.tai_fase (
            ...
            g_descripcion_log_error := 'Registro exitoso de un nuevo registro en la tabla alma.tai_fase';
  ...
  --procedimiento de modificacion      
	...
	BEGIN
            IF NOT EXISTS (SELECT 1 FROM alma.tai_fase f WHERE f.id_fase=al_id_fase) THEN
                              
                g_descripcion_log_error := 'Modificacion no realizada: no existe el registro' || al_id_fase || 'en la tabla alma.tai_fase';
                g_nivel_error := '4';
                g_respuesta := param.f_pm_mensaje_error(g_descripcion_log_error, g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
                RETURN 'f'||g_separador||g_respuesta;      
            END IF;

                UPDATE alma.tai_fase SET 
		...
		-- DESCRIPCION DE EXITO PARA GUARDAR EN EL LOG
            g_descripcion_log_error := 'Modificacion exitosa en alma.tai_fase del registro '||  al_id_fase;
            g_respuesta := 't'||g_separador||g_descripcion_log_error;
    	...
    	BEGIN
            ...
            IF NOT EXISTS(SELECT 1 FROM  alma.tai_fase f WHERE f.id_fase=al_id_fase) THEN
                ...
                g_descripcion_log_error := 'Eliminacion no realizada: no existe el registro ' || al_id_fase || ' en la tabla alma.tai_fase';
                g_nivel_error := '4';
                g_respuesta := param.f_pm_mensaje_error(g_descripcion_log_error, g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
                RETURN 'f'||g_separador||g_respuesta;
        	...
        	IF EXISTS(SELECT 1 FROM alma.tai_fase_tramo ft WHERE ft.id_fase=al_id_fase)
            THEN
            ...
            -- DESCRIPCI√??N DE √??XITO PARA GUARDAR EN EL LOG
            g_descripcion_log_error := 'Eliminacion exitosa del registro '||al_id_fase||' en alma.tai_fase';
            g_respuesta := 't'||g_separador||g_descripcion_log_error;
        END;
