...
CREATE OR REPLACE FUNCTION alma.f_tai_costeo_sel (
...
***************************************************************************
 SCRIPT: 		 alma.f_tai_costeo_sel
 DESCRIPCION: 	 Devuelve las consultas a la tabla alma.tal_costeo
 AUTOR: 		 UNKNOW
 FECHA:			 05-05-2015
***************************************************************************/
...
BEGIN
    ...
    g_nombre_funcion := 'f_tai_costeo_sel';
    ...
    IF pm_codigo_procedimiento  = 'AL_COSTEO_SEL' THEN
        BEGIN
            g_consulta := ' SELECT 	cos.id_costeo,
            						COALESCE(to_char(cos.fecha_ingreso,''dd-mm-yyyy'')) as fecha_ingreso,
                                    COALESCE(to_char(cos.fecha_salida,''dd-mm-yyyy'')) as fecha_salida,
                                    cos.estado,
                                    cos.descripcion,cos.usuario_reg,cos.fecha_reg,
                                   -- alm.id_almacen,alm.codigo||'' - ''||alm.nombre as desc_almacen,
                                    pro.id_movimiento_proyecto,pro.codigo||'' - ''||pro.concepto_ingreso as desc_mov_proy
                                    ,cos.tipo_costeo
                            FROM alma.tai_costeo cos
                            --LEFT JOIN alma.tal_almacen alm on alm.id_almacen=cos.id_almacen AND alm.estado=''activo''
                            INNER JOIN alma.tai_movimiento_proyecto pro on pro.id_movimiento_proyecto=cos.id_movimiento_proyecto
                            WHERE  ';
    ...
    ELSIF pm_codigo_procedimiento  = 'AL_COSTEO_COUNT' THEN

        BEGIN
        	--Cuenta todos los registros de tasa_categoria sin condiciones
            g_consulta := '	SELECT count(cos.id_costeo) as total
                            FROM alma.tai_costeo cos
                            --INNER JOIN alma.tai_almacen alm on alm.id_almacen=cos.id_almacen AND alm.estado=''activo''
                             INNER JOIN alma.tai_movimiento_proyecto pro on pro.id_movimiento_proyecto=cos.id_movimiento_proyecto
                            WHERE   ';
