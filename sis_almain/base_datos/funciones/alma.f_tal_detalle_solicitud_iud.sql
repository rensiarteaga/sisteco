...
CREATE OR REPLACE FUNCTION alma.f_tal_detalle_solicitud_iud (
...
FECHA:           18-09-2013
DESCRIPCION:     Realiza modificaciones en la tabla alma.f_tal_detalle_solicitud
***************************************************************************/
...
BEGIN
  ...
  g_nombre_funcion :='f_tal_detalle_solicitud_iud';
  ...
  IF pm_codigo_procedimiento = 'AL_DETSOL_INS' THEN
    BEGIN
        ...
        if EXISTS (select 1 from alma.tal_detalle_solicitud det where det.id_solicitud_salida=al_id_solicitud_salida AND det.id_item=al_id_item)
        then
      ...
      INSERT INTO alma.tal_detalle_solicitud(
    		id_solicitud_salida, 
            id_item, 
            cantidad
            )
    ...
    BEGIN   
      ...
      from alma.tal_detalle_solicitud
      where id_detalle_solicitud = al_id_detalle_solicitud;

      IF NOT EXISTS(select 1 from alma.tal_detalle_solicitud det where det.id_detalle_solicitud=al_id_detalle_solicitud) THEN
              g_descripcion_log_error :='Modificacion no realizada: no existe el registro '||al_id_detalle_solicitud || ' en la tabla alma.tal_detalle_solicitud';
              g_nivel_error := '4';
              g_respuesta := f_pm_mensaje_error(g_descripcion_log_error,g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
        RETURN 'f'||g_separador||g_respuesta;
      END IF;
      
      UPDATE alma.tal_detalle_solicitud
      SET 
      ...
      WHERE alma.tal_detalle_solicitud.id_detalle_solicitud = al_id_detalle_solicitud;

      -- DESCRIPCION DE EXITO PARA GUARDAR EN EL LOG
      g_descripcion_log_error :=
        'Modificacion exitosa en alma.tal_detalle_solicitud del registro '||
    ...
    BEGIN
      ...
      IF g_id_detalle_solicitud is null THEN
        g_nivel_error := '4';
        g_descripcion_log_error := 'Eliminacion no realizada: registro '||al_id_detalle_solicitud || ' en detalle alma.tal_detalle_solicitud inexistente';
        g_respuesta := f_pm_mensaje_error(g_descripcion_log_error,g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
        RETURN 'f'||g_separador||g_respuesta;
      END IF;
      DELETE
      ...
      g_descripcion_log_error := 'Eliminacion exitosa del registro '||
        al_id_detalle_solicitud||' alma.tal_detalle_solicitud';
      g_respuesta := 't'||g_separador||g_descripcion_log_error;
    END;
    ELSE
