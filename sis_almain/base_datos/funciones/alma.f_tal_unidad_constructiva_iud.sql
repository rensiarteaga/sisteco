...
CREATE OR REPLACE FUNCTION alma.f_tal_unidad_constructiva_iud (
...
/**************************************************************************
 AUTOR:           UNKNOW
 FECHA:           11-08-2014	
 DESCRIPCIÃ?N:     Realiza modificaciones en la tabla alma.f_tal_unidad_constructiva
***************************************************************************/
...
BEGIN
    ...
    g_nombre_funcion :='f_tal_unidad_constructiva_iud';
...
---*** SI NO SE TIENE PERMISOS DE EJECUCIÃ??N SE RETORNA EL MENSAJE DE ERROR
    ...
    BEGIN
        ...
        */
        ...
        /*if EXISTS(	select 1 
        			from alma.tal_unidad_constructiva uc 
                    inner join alma.tal_detalle_unidad_constructiva det on uc.id_unidad_constructiva=det.id_unidad_constructiva
                    where det.id_unidad_constructiva=al_id)*/
         --verificacion del codigo de tramo (unico), en la unidad constructiva
        IF EXISTS(SELECT 1 FROM alma.tal_unidad_constructiva uc WHERE uc.cod_tramo=al_cod_tramo)
        THEN
        ...
        THEN
        	INSERT INTO alma.tal_unidad_constructiva (
        ...
        ELSE
         ...
         IF EXISTS(SELECT 1 FROM alma.tal_unidad_constructiva c WHERE c.id_unidad_constructiva_fk = al_id_unidad_constructiva_fk)
            THEN
            	IF NOT EXISTS(SELECT 1 FROM alma.tal_unidad_constructiva cl WHERE cl.id_unidad_constructiva_fk = al_id_unidad_constructiva_fk
            ...
            END IF;
       		...
       		UPDATE alma.tal_unidad_constructiva
        	SET orden = orden + 1
    ...
    BEGIN
        ...
        IF NOT EXISTS(SELECT 1 FROM alma.tal_unidad_constructiva uni WHERE uni.id_unidad_constructiva = al_id_unidad_constructiva) THEN
        	g_descripcion_log_error := 'Modificacion no realizada: no existe el registro' || al_id_unidad_constructiva || 'en la tabla alma.tal_unidad_constructiva';
            g_nivel_error := '4';
            g_respuesta := f_pm_mensaje_error(g_descripcion_log_error, g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
            RETURN 'f'||g_separador||g_respuesta;
        END IF;
        
         IF EXISTS(SELECT 1 FROM alma.tal_unidad_constructiva uc WHERE uc.cod_tramo=al_cod_tramo AND uc.id_unidad_constructiva != al_id_unidad_constructiva)
        THEN
        ...
        FROM alma.tal_unidad_constructiva c
        ...
        THEN
        	...
        	UPDATE alma.tal_unidad_constructiva SET orden = orden - 1
            WHERE alma.tal_unidad_constructiva.id_unidad_constructiva_fk = al_id_unidad_constructiva_fk AND  orden >= g_nro_linea;
            --actualizacion nros_linea > nro_linea_actual
            UPDATE alma.tal_unidad_constructiva SET
            orden = orden + 1
            WHERE alma.tal_unidad_constructiva.id_unidad_constructiva_fk = al_id_unidad_constructiva_fk AND orden > al_orden AND id_unidad_constructiva <> al_id_unidad_constructiva;
        ...
        THEN
            ...
            UPDATE alma.tal_unidad_constructiva SET orden = orden + 1
            WHERE alma.tal_unidad_constructiva.id_unidad_constructiva_fk = al_id_unidad_constructiva_fk AND orden >= al_orden;
        END IF;
		...
		WHERE alma.tal_unidad_constructiva.id_unidad_constructiva = al_id_unidad_constructiva;
		
		-- DESCRIPCIÃ??N DE Ã??XITO PARA GUARDAR EN EL LOG
        g_descripcion_log_error := 'Modificacion exitosa en alma.tal_unidad_constructiva del registro '||al_id_unidad_constructiva;
        g_respuesta := 't'||g_separador||g_descripcion_log_error;
	...
	BEGIN
            ...
            IF NOT EXISTS (select 1 from alma.tal_unidad_constructiva uni where uni.id_unidad_constructiva = al_id_unidad_constructiva) THEN
                g_nivel_error := '4';
                g_descripcion_log_error := 'Eliminacion no realizada: registro '|| al_id_unidad_constructiva || ' en alma.tal_unidad_constructiva inexistente';
                g_respuesta := f_pm_mensaje_error(g_descripcion_log_error, g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
                RETURN 'f'||g_separador||g_respuesta;
            END IF;
            ...
            */
            ...
            DELETE FROM alma.tal_unidad_constructiva
            WHERE id_unidad_constructiva = al_id_unidad_constructiva;
            
            UPDATE alma.tal_unidad_constructiva SET orden = orden - 1
            WHERE alma.tal_unidad_constructiva.id_unidad_constructiva_fk = g_id_uc AND orden >= g_nro_linea;  
            
            -- DESCRIPCIÃ??N DE Ã??XITO PARA GUARDAR EN EL LOG
            g_descripcion_log_error := 'EliminaciÃ³n exitosa del registro '||al_id_unidad_constructiva||' alma.tal_unidad_constructiva';
            g_respuesta := 't'||g_separador||g_descripcion_log_error;
    END;
	ELSE
