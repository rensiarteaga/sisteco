...
CREATE OR REPLACE FUNCTION alma.f_tal_detalle_solicitud_sel (
...
$body$
/************************************************************************** SISTEMA ENDESIS - SISTEMA ALMACENES*************************************************************************** SCRIPT:         alma.f_tal_detalle_solicitud_sel
 DESCRIPCION:      Devuelve las consultas a la tabla alma.tal_detalle_solicitud
...
DECLARE
  ...
  g_consulta text;
  -- VARIABLE QUE CONTENDRÃ???Ã??Ã?Â LA CONSULTA DINAÂMICA PARA EL FILTRO
  g_nivel_error varchar; -- VARIABLE QUE CONTIENE EL NIVEL DE ERROR
  --      ERROR DE SINTAXIS (cuando llega a exception) = 1
  --      ERROR LOGICO (CRAMATICO) = 2
  --      ERROR LOGICO (INTERMEDIO) = 3
  --      ERROR LOGICO (ADVERTENCIA) = 4
  g_nombre_funcion varchar; --NOMBRE FIÂSICO DE LA FUNCION
...
BEGIN
  ...
  g_nombre_funcion := 'f_tal_detalle_solicitud_sel';
  ...
  IF pm_codigo_procedimiento  = 'AL_DETSOL_SEL' THEN
    BEGIN
      g_consulta := 'SELECT 
							...
							FROM alma.tal_detalle_solicitud des
                            INNER JOIN alma.tal_solicitud_salida sol ON sol.id_solicitud_salida = des.id_solicitud_salida
                            INNER JOIN alma.tal_item itm ON itm.id_item = des.id_item
    ...
    BEGIN
      ...
      -- DESCRIPCION DE Ã????XITO PARA GUARDAR EN EL LOG
