...
DECLARE
  ...
  g_respuesta varchar; -- VARIABLE QUE CONTENDRÃ???Ã??Ã?Â LOS MENSAJES DE ERROR
  g_consulta text; -- VARIABLE QUE CONTENDRÃ???Ã??Ã?Â LA CONSULTA DINÃ???Ã??Ã?ÂMICA PARA EL FILTRO
...
BEGIN
  ...
  g_separador = '#@@@#'; --Separador para mensajes devueltos por la funciÃ???Ã??Ã?Â³n
  ...
  IF NOT (g_privilegio_procedimiento OR
    ...
    g_descripcion_log_error :=
      'El usuario no tiene permisos de ejecuciÃ???Ã??Ã?Â³n del procedimiento';
    g_respuesta := param.f_pm_mensaje_error(g_descripcion_log_error,
      g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
  ...
  IF pm_codigo_procedimiento  = 'AL_SOLSAL_SEL' THEN
    BEGIN
      ...
      -- SE AUMENTA EL ORDEN Y LOS PARÃ???Ã??Ã?ÂMETROS DE LA CANTIDAD DE REGISTROS A DESPLEGAR
