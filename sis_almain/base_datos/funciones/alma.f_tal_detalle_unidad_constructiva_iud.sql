...
CREATE OR REPLACE FUNCTION alma.f_tal_detalle_unidad_constructiva_iud (
...
***************************************************************************
 SCRIPT:          	alma.f_tal_detalle_unidad_constructiva_iud
...
---*** INICIACIÃ???N DE VARIABLES 
    ...
    g_nombre_funcion :='f_tal_detalle_unidad_constructiva_iud';
...
---*** SI NO SE TIENE PERMISOS DE EJECUCIÃ???N SE RETORNA EL MENSAJE DE ERROR
    ...
    BEGIN
        ...
        if EXISTS (select 1 from alma.tal_unidad_constructiva uc where uc.id_unidad_constructiva_fk = al_id_unidad_constructiva)
        then
        ...
        --verificacion de duplicidad del item
        	if EXISTS (select 1 from alma.tal_detalle_unidad_constructiva det where det.id_item=al_id_item and det.id_unidad_constructiva=al_id_unidad_constructiva)then
            	raise exception '%','Error: el item seleccionado ('||al_id_item||') ya fue registrado en el nodo.';
            end if;
        end if;
        
        IF EXISTS(select 1 from alma.tal_detalle_unidad_constructiva where alma.tal_detalle_unidad_constructiva.id_unidad_constructiva = al_id_unidad_constructiva)
        THEN
        	IF NOT EXISTS(select 1 from alma.tal_detalle_unidad_constructiva where alma.tal_detalle_unidad_constructiva.id_unidad_constructiva = al_id_unidad_constructiva 
            AND alma.tal_detalle_unidad_constructiva.orden = al_orden AND al_orden > 0)
            THEN
        ...
        UPDATE alma.tal_detalle_unidad_constructiva
        SET orden = orden + 1
        WHERE id_unidad_constructiva=al_id_unidad_constructiva AND orden >= al_orden + 1;
        
		INSERT INTO alma.tal_detalle_unidad_constructiva(
        ...
        VALUES (
            ...
            g_descripcion_log_error := 'Registro exitoso de un nuevo registro en la tabla alma.tal_detalle_unidad_constructiva';
    ...
    THEN
      BEGIN
            --añadir control para seleccionar el orden maximo, de acuerdo a una unidad constructiva      
      		IF EXISTS(	select 1 
            			from alma.tal_detalle_unidad_constructiva 
  ...
  --procedimiento de modificacion      
	...
	BEGIN
            ...
            IF NOT EXISTS (select 1 from alma.tal_detalle_unidad_constructiva det where det.id_unidad_constructiva=al_id_unidad_constructiva) THEN
                              
                g_descripcion_log_error := 'Modificacion no realizada: no existe el registro' || al_id_detalle_unidad_constructiva || 'en la tabla alma.tal_detalle_unidad_constructiva';
                g_nivel_error := '4';
                g_respuesta := param.f_pm_mensaje_error(g_descripcion_log_error, g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
                RETURN 'f'||g_separador||g_respuesta;
            END IF;
            ...
            THEN
            		...
            		UPDATE alma.tal_detalle_unidad_constructiva SET
                    orden = orden - 1
                    WHERE alma.tal_detalle_unidad_constructiva.id_unidad_constructiva=al_id_unidad_constructiva
                    	  AND  orden >= nro_linea;
            ...
            --fin condicionordenamiento   
                ...
                UPDATE alma.tal_detalle_unidad_constructiva SET 
                    ...
                    ,orden = nro_linea_act
				WHERE alma.tal_detalle_unidad_constructiva.id_detalle_unidad_constructiva = al_id_detalle_unidad_constructiva;
			...
			--DESCRIPCION DE EXITO PARA GUARDAR EN EL LOG
            g_descripcion_log_error := 'Modificacion exitosa en alma.tal_almacen del registro '||  al_id_detalle_unidad_constructiva;
            g_respuesta := 't'||g_separador||g_descripcion_log_error;
    	...
    	BEGIN
            ...
            IF NOT EXISTS(select 1 from alma.tal_detalle_unidad_constructiva det where det.id_detalle_unidad_constructiva=al_id_detalle_unidad_constructiva) THEN
                              
                g_descripcion_log_error := 'Eliminacion no realizada: no existe el registro' || al_id_unidad_constructiva || 'en la tabla alma.tal_detalle_unidad_constructiva';
                g_nivel_error := '4';
                g_respuesta := param.f_pm_mensaje_error(g_descripcion_log_error, g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
                RETURN 'f'||g_separador||g_respuesta;
            ...
            DELETE FROM alma.tal_detalle_unidad_constructiva  
			WHERE alma.tal_detalle_unidad_constructiva.id_detalle_unidad_constructiva = al_id_detalle_unidad_constructiva;
            
            UPDATE alma.tal_detalle_unidad_constructiva SET
            orden = orden -1
            WHERE alma.tal_detalle_unidad_constructiva.id_unidad_constructiva = id_uc AND orden >= nro_linea;               
 			
            -- DESCRIPCION DE EXITO PARA GUARDAR EN EL LOG
            g_descripcion_log_error := 'Eliminacion exitosa del registro '||al_id_detalle_unidad_constructiva||' en alma.tal_detalle_unidad_constructiva';
            g_respuesta := 't'||g_separador||g_descripcion_log_error;
        END;
