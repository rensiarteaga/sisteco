...
CREATE OR REPLACE FUNCTION alma.f_tai_detalle_movimiento_sel (
...
/************************************************************************** SISTEMA ENDESIS - SISTEMA ALMACENES*************************************************************************** 
 SCRIPT:         alma.f_tai_detalle_movimiento_sel
 DESCRIPCI√????N:      Devuelve las consultas a la tabla alma.tai_detalle_movimiento
...
DECLARE
  ...
  g_consulta text;
  -- VARIABLE QUE CONTENDRA¬Å LA CONSULTA DINAMICA PARA EL FILTRO
...
BEGIN
  ...
  g_nombre_funcion := 'f_tai_detalle_movimiento_sel';
  ...
  IF pm_codigo_procedimiento  = 'AL_DETMOV_SEL' THEN
    BEGIN
      g_consulta := 'SELECT 
            					COALESCE(to_char(dem.fecha_reg,''dd-mm-yyyy HH:MI:SS am''),''0'') AS fecha_reg,
                               	dem.usuario_reg,
                                dem.id_detalle_movimiento,
                                dem.id_movimiento,
                                dem.id_item,
                                dem.cantidad,
                                dem.cantidad_solicitada,
                                COALESCE(dem.tipo_saldo,'' '')AS tipo_saldo,
                                ...
                                ,dem.costo_unitario
                                ,dem.costo_valorado
                                --,(dem.costo_unitario*dem.cantidad) as costo_total
                                ,round(dem.costo_total,2) as costo_total
                                
                                
							FROM alma.tai_detalle_movimiento dem
                            INNER JOIN alma.tai_movimiento mov ON mov.id_movimiento = dem.id_movimiento
                            INNER JOIN alma.tai_item itm ON itm.id_item = dem.id_item
    ...
    ELSIF pm_codigo_procedimiento  = 'AL_DETMOV_COUNT' THEN

    BEGIN
      --Cuenta todos los registros de tasa_categoria sin condiciones
      g_consulta := 'SELECT count(dem.id_detalle_movimiento)
                            FROM alma.tai_detalle_movimiento dem
                            INNER JOIN alma.tai_movimiento mov ON mov.id_movimiento = dem.id_movimiento
                            INNER JOIN alma.tai_item itm ON itm.id_item = dem.id_item
                            INNER JOIN param.tpm_unidad_medida_base um on um.id_unidad_medida_base=itm.id_unidad_medida and um.estado_registro=''activo''
  ...
  ---*** REGISTRO EN EL LOG EL √????XITO DE LA EJECUCION DEL PROCEDIMIENTO
  g_reg_evento:= sss.f_tsg_registro_evento(pm_id_usuario             ,
    g_id_subsistema           ,g_id_lugar         ,g_descripcion_log_error,
    pm_ip_origen              ,pm_mac_maquina            ,'log'              ,
    NULL,
