...
CREATE OR REPLACE FUNCTION alma.f_tai_movimiento_sel (
...
/************************************************************************** SISTEMA ENDESIS - SISTEMA DE ALMACEN***************************************************************************
 SCRIPT:          alma.f_tai_movimiento_sel
...
DECLARE
  ...
  g_consulta text; -- VARIABLE QUE CONTENDRA LA CONSULTA DINAMICA PARA EL FILTRO
...
BEGIN
  ...
  g_nombre_funcion :='f_tai_movimiento_sel';
  g_rol_adm:= false;
  ...
  IF pm_codigo_procedimiento  = 'AL_MOVI_SEL' THEN
    BEGIN
      ...
      g_consulta := 'SELECT
                                ...
                                ,al.nro_compra
							FROM alma.tai_movimiento al
                            INNER JOIN alma.tai_tipo_movimiento tip ON tip.id_tipo_movimiento = al.id_tipo_movimiento
                            INNER JOIN param.tpm_documento doc ON doc.id_documento = tip.id_documento
                            LEFT JOIN alma.tai_almacen alm on alm.id_almacen=al.id_almacen
                            LEFT JOIN alma.tai_almacen alm2 on alm2.id_almacen=al.id_almacen_trans
                            WHERE ';
