...
---*** SI NO SE TIENE PERMISOS DE EJECUCIÃ??N SE RETORNA EL MENSAJE DE ERROR
    ...
    BEGIN
    	...
    	ELSE
            ...
            IF EXISTS(SELECT 1 FROM alma.tal_clasificacion c WHERE c.id_clasificacion_fk = al_id_clasificacion_fk)
            THEN
            	IF NOT EXISTS(SELECT 1 FROM alma.tal_clasificacion cl WHERE cl.id_clasificacion_fk = al_id_clasificacion_fk
    ...
    BEGIN
        ...
        THEN
        	...
        	UPDATE alma.tal_clasificacion SET orden = orden - 1
            WHERE alma.tal_clasificacion.id_clasificacion_fk = al_id_clasificacion_fk AND  orden >= g_nro_linea;
            --actualizacion nros_linea > nro_linea_actual
            UPDATE alma.tal_clasificacion SET
            orden = orden + 1
            WHERE alma.tal_clasificacion.id_clasificacion_fk = al_id_clasificacion_fk AND orden > al_orden AND id_clasificacion <> al_id_clasificacion;
        ...
        THEN
            ...
            UPDATE alma.tal_clasificacion SET orden = orden + 1
            WHERE alma.tal_clasificacion.id_clasificacion_fk = al_id_clasificacion_fk AND orden >= al_orden;
        END IF;
	...
	BEGIN
            ...
            IF g_id_clasificacion is null THEN
                g_nivel_error := '4';
                g_descripcion_log_error := 'EliminaciÃ³n no realizada: registro '|| al_id_clasificacion || ' en alma.tal_clasificacion inexistente';
                g_respuesta := f_pm_mensaje_error(g_descripcion_log_error, g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
                RETURN 'f'||g_separador||g_respuesta;
            END IF;
            ...
            UPDATE alma.tal_clasificacion SET orden = orden -1
            WHERE alma.tal_clasificacion.id_clasificacion_fk = g_id_clasificacion AND orden >= g_nro_linea;               
 			
            
            -- DESCRIPCIÃ??N DE Ã??XITO PARA GUARDAR EN EL LOG
            g_descripcion_log_error := 'EliminaciÃ³n exitosa del registro '||al_id_clasificacion||' alma.tal_clasificacion';
            g_respuesta := 't'||g_separador||g_descripcion_log_error;
    END;
	ELSE
