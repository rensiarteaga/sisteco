...
CREATE OR REPLACE FUNCTION alma.f_tal_fase_tramo_iud (
...
***************************************************************************
 SCRIPT:          alma.f_tal_fase_tramo_iud
 DESCRIPCION:    Realiza modificaciones en la tabla alma.tal_fase_tramo
 AUTOR:           UNKNOW
 FECHA:           09-12-2014
 COMENTARIOS:    
***************************************************************************
...
---*** INICIACIÃ??N DE VARIABLES 
    ...
    g_nombre_funcion :='f_tal_fase_tramo_iud';
...
---*** SI NO SE TIENE PERMISOS DE EJECUCIO?N SE RETORNA EL MENSAJE DE ERROR
    ...
    BEGIN
        ...
        IF EXISTS(SELECT 1 FROM alma.tal_fase_tramo ft WHERE ft.id_tramo=al_id_tramo AND ft.id_fase= al_id_fase)
        THEN
		...
		INSERT INTO alma.tal_fase_tramo (
            id_fase,
            id_tramo, 
            estado_registro
        ) VALUES (
            ...
            g_descripcion_log_error := 'Registro exitoso de un nuevo registro en la tabla alma.tal_fase_tramo';
  ...
  --procedimiento de modificacion      
	...
	BEGIN
            IF NOT EXISTS (SELECT 1 FROM alma.tal_fase_tramo f WHERE f.id_fase_tramo=al_id_fase_tramo) THEN
                              
                g_descripcion_log_error := 'Modificacion no realizada: no existe el registro' || al_id_fase_tramo || 'en la tabla alma.tal_fase_tramo';
                g_nivel_error := '4';
                g_respuesta := param.f_pm_mensaje_error(g_descripcion_log_error, g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
                RETURN 'f'||g_separador||g_respuesta;      
            END IF;

                UPDATE alma.tal_fase_tramo SET 
                	usuario_reg = "current_user"(),
                	fecha_reg = now(),
                	id_fase = al_id_fase,
                    id_tramo = al_id_tramo,
                    estado_registro = al_estado
				WHERE alma.tal_fase_tramo.id_fase_tramo = al_id_fase_tramo;
                
			
		-- DESCRIPCION DE EXITO PARA GUARDAR EN EL LOG
            g_descripcion_log_error := 'Modificacion exitosa en alma.tal_fase_tramo del registro '||  al_id_fase_tramo;
            g_respuesta := 't'||g_separador||g_descripcion_log_error;
    	...
    	BEGIN
            ...
            IF NOT EXISTS(SELECT 1 FROM  alma.tal_fase_tramo f WHERE f.id_fase_tramo=al_id_fase_tramo) THEN
                ...
                g_descripcion_log_error := 'Eliminacion no realizada: no existe el registro' || al_id_fase_tramo || 'en la tabla alma.tal_fase_tramo';
                g_nivel_error := '4';
                g_respuesta := param.f_pm_mensaje_error(g_descripcion_log_error, g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
                RETURN 'f'||g_separador||g_respuesta;
            ...
            g_descripcion_log_error := 'Eliminacion exitosa del registro '||al_id_fase_tramo||' en alma.tal_fase_tramo';
            g_respuesta := 't'||g_separador||g_descripcion_log_error;
        END;
