...
CREATE OR REPLACE FUNCTION alma.f_tai_stock_item_sel (
    ...
    IF pm_codigo_procedimiento  = 'AL_STOCKITEM_SEL' THEN
        BEGIN
            g_consulta := 'SELECT  stock.id_stock_item,
                                   ...
                                   med.nombre
                                  -- ,ubic.id_ubicacion,ubic.codigo||'' - ''||ubic.nombre as desc_ubicacion
                                   
                            FROM alma.tai_stock_item stock
                                 INNER JOIN alma.tai_almacen alm on alm.id_almacen = stock.id_almacen and alm.estado = ''activo''
                                 INNER JOIN alma.tai_item it on it.id_item = stock.id_item and it.estado =''activo''
                                ...
                                -- LEFT JOIN alma.tai_ubicacion ubic on ubic.id_ubicacion=stock.id_ubicacion
                            WHERE  ';
