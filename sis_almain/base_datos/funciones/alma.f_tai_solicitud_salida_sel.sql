...
CREATE OR REPLACE FUNCTION alma.f_tai_solicitud_salida_sel (
...
***************************************************************************
 SCRIPT:          alma.f_tai_solicitud_salida_sel
 DESCRIPCI√????N:     Devuelve las consultas a la tabla train.tal_solicitud_salida
...
BEGIN
  ...
  g_nombre_funcion :='f_tai_solicitud_salida_sel';
  g_rol_adm:= false;
  ...
  IF pm_codigo_procedimiento  = 'AL_SOLSAL_SEL' THEN
    ...
    ELSIF pm_codigo_procedimiento = 'AL_LASTSOL_SEL' THEN 
    BEGIN
    	IF NOT EXISTS (select 1 from alma.tai_solicitud_salida s WHERE s.id_almacen = pm_cant AND s.estado='entregado') 
        THEN
        		g_consulta:='select now()::date';
        ELSE
          g_consulta:=	' SELECT max(s.fecha_solicitud)::date
                          FROM alma.tai_solicitud_salida s
                          WHERE s.estado=''entregado'' AND s.id_almacen= ';           
          g_consulta := g_consulta || pm_cant;
        END IF;
