...
CREATE OR REPLACE FUNCTION alma.f_tai_almacen_unidad_org_sel (
...
***************************************************************************
 SCRIPT: 		 alma.f_tai_almacen_unidad_org_sel
 DESCRIPCION: 	 Devuelve las consultas a la tabla alma.tai_almacen_unidad_org
 AUTOR: 		 UNKNOW
 FECHA:			 01-12-2014
***************************************************************************/
...
BEGIN
    ...
    g_nombre_funcion := 'f_tai_almacen_unidad_org_sel';
    ...
    IF pm_codigo_procedimiento  = 'AL_ALMUO_SEL' THEN
        BEGIN
        
            g_consulta := 'select COALESCE(to_char(auo.fecha_reg,''dd-mm-yyyy''),''0'') AS fecha_reg, auo.usuario_reg,auo.estado,
                            alm.id_almacen,alm.codigo||'' - ''||alm.nombre as desc_almacen,
                            uo.id_unidad_organizacional,uo.nombre_unidad,uo.descripcion,auo.id_almacen_unidad_org,
                            COALESCE(to_char(auo.fecha_desde,''dd-mm-yyyy''),''0'') AS fecha_desde,
                            COALESCE(to_char(auo.fecha_hasta,''dd-mm-yyyy''),''0'') AS fecha_hasta
                            from alma.tai_almacen_unidad_org auo
                            inner join alma.tai_almacen alm on alm.id_almacen = auo.id_almacen AND alm.estado=''activo''
                            inner join kard.tkp_unidad_organizacional uo on uo.id_unidad_organizacional=auo.id_unidad_organizacional AND uo.estado_reg = ''activo''
                            where  ';
    ...
    ELSIF pm_codigo_procedimiento  = 'AL_ALMUO_COUNT' THEN

        BEGIN
        --Cuenta todos los registros de tasa_categoria sin condiciones
            g_consulta := '	SELECT count(auo.id_almacen_unidad_org) as total
							from alma.tai_almacen_unidad_org auo
                            inner join alma.tai_almacen alm on alm.id_almacen = auo.id_almacen AND alm.estado=''activo''
                            inner join kard.tkp_unidad_organizacional uo on uo.id_unidad_organizacional=auo.id_unidad_organizacional AND uo.estado_reg = ''activo''
                            where ';  
