...
CREATE OR REPLACE FUNCTION alma.f_tai_costeo_detalle_sel (
...
***************************************************************************
 SCRIPT: 		 alma.f_tai_costeo_detalle_sel
 DESCRIPCION: 	 Devuelve las consultas a la tabla alma.tai_costeo_detalle
 AUTOR: 		 UNKNOW
 FECHA:			 12-05-2015
***************************************************************************/
...
BEGIN
    ...
    g_nombre_funcion := 'f_tai_costeo_detalle_sel';
    ...
    IF pm_codigo_procedimiento  = 'AL_COSTDET_SEL' THEN
        BEGIN
            g_consulta := 'SELECT 	det.id_costeo_detalle,det.valor_costo
                                    ,cos.id_costeo,cos.id_costeo||'' - ''||cos.descripcion as desc_costeo
                                    ,c.id_costo,c.codigo_costo||'' - ''||c.desc_costo as desc_costo
                                    ,det.estado
                                    ,det.usuario_reg
                                    ,COALESCE(to_char(det.fecha_reg,''dd-mm-yyyy''),''0'') AS fecha_reg
            						
                            FROM alma.tai_costeo_detalle det
                            INNER JOIN alma.tai_costeo cos ON cos.id_costeo=det.id_costeo
                            INNER JOIN alma.tai_costo c ON c.id_costo=det.id_costo AND c.estado=''activo'' 
                            WHERE  ';
    ...
    ELSIF pm_codigo_procedimiento  = 'AL_COSTDET_COUNT' THEN

        BEGIN
        --Cuenta todos los registros de tasa_categoria sin condiciones
            g_consulta := '	SELECT count(det.id_costeo_detalle) as total
							FROM alma.tai_costeo_detalle det
                            INNER JOIN alma.tai_costeo cos ON cos.id_costeo=det.id_costeo
                            INNER JOIN alma.tai_costo c ON c.id_costo=det.id_costo AND c.estado=''activo'' 
                            WHERE  ';  
