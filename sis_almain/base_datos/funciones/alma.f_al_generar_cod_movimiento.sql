...
CREATE OR REPLACE FUNCTION alma.f_al_generar_cod_movimiento (
...
THEN
          ...
          if EXISTS(select 1 from alma.tal_movimiento mov where mov.codigo like(v_res))
          then
          		--codigo de movimiento generado -> error
                raise exception '%','Error, El codigo generado :'||v_res||' ya existe, verificar : param.tpm_correlativo_general y alma.tal_movimiento';
          end if;
