...
CREATE OR REPLACE FUNCTION alma.f_al_valorizar_movimiento (
...
then
    ...
    if (v_movimiento.tipo = 'ingreso' OR v_movimiento.tipo = 'transpaso_ingreso')
    then
    	...
    	v_consulta:='		SELECT det.id_item,det.cantidad,det.costo_unitario,det.costo_valorado,mov.id_almacen,it.metodo_valoracion,det.id_detalle_movimiento,tip.tipo
                            ...
                            INNER JOIN alma.tal_tipo_movimiento tip on tip.id_tipo_movimiento=mov.id_tipo_movimiento
                            INNER JOIN alma.tal_item it on it.id_item=det.id_item
                            WHERE mov.id_movimiento='||al_id_movimiento||'
            ...
            */
            ...
            THEN 
            	BEGIN
                    ...
                    if v_rec_ing.costo_unitario < 1 then
                    	raise exception '%','Error: Verfique el costo unitario de ingreso del item :'||v_rec_ing.id_item||chr(10)||'Precio Unitario:'||v_rec_ing.costo_unitario;
                         ...
                         if NOT EXISTS (	select 1 from alma.tal_item_valoracion_pp val
                          ...
                          then
                             BEGIN
                              ...
                              INSERT INTO alma.tal_item_valoracion_pp( id_item	,cantidad	,costo_unitario
                             ...
                             UPDATE alma.tal_item_valoracion_pp
                             SET 	cantidad = cant_actualiz
                					...
                					,fecha_reg = now()
                             WHERE alma.tal_item_valoracion_pp.id_item=v_rec_ing.id_item AND alma.tal_item_valoracion_pp.id_almacen=v_rec_ing.id_almacen;
                             	 
                             --se actualizan los datos de los costos valorados en la tabla alma.tal_detalle movimiento
                             UPDATE alma.tal_detalle_movimiento
    	...
    	BEGIN
            ...
            loop 
                ...
                BEGIN
                    ...
                    then 
                            raise exception '%','El Item:'||v_rec_sal.id_item||' no fue registrado nunca como ingreso, verifique la tabla alma.tal_item_valoracion_pp';
                    else
                            select val.* into v_datos_valorados
                            from alma.tal_item_valoracion_pp val
                            where val.id_item=v_rec_sal.id_item and val.id_almacen=v_rec_sal.id_almacen;
                            ...
                            --calculos
                             ...
                             SET 	cantidad = cant_actualiz
                                    ...
                                    ,usuario_reg = "current_user"() 
                					,fecha_reg = now()
                             WHERE alma.tal_item_valoracion_pp.id_item=v_rec_sal.id_item AND alma.tal_item_valoracion_pp.id_almacen=v_rec_sal.id_almacen;
                             	 
                             --se actualizan los datos de los costos valorados en la tabla alma.tal_detalle movimiento
                             UPDATE alma.tal_detalle_movimiento
                             SET costo_valorado = costo_actualiz
                             WHERE alma.tal_detalle_movimiento.id_detalle_movimiento=v_rec_sal.id_detalle_movimiento;
          ...
          loop
                ...
                then
                	BEGIN
                      ...
                      then 
                      		raise exception '%','El Item:'||v_rec_sal.id_item||'  no fue registrado nunca como ingreso, verifique la tabla alma.tal_item_valoracion';
                      else
                            select val.* into v_datos_valorados
                            from alma.tal_item_valoracion_pp val
                            where val.id_item=v_rec_sal.id_item and val.id_almacen=v_rec_sal.id_almacen;
