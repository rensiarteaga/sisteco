...
CREATE OR REPLACE FUNCTION alma.f_tai_detalle_solicitud_sel (
...
$body$
/************************************************************************** SISTEMA ENDESIS - SISTEMA ALMACENES*************************************************************************** SCRIPT:         alma.f_tai_detalle_solicitud_sel
 DESCRIPCION:      Devuelve las consultas a la tabla alma.tai_detalle_solicitud
...
DECLARE
  ...
  g_respuesta varchar; -- VARIABLE QUE CONTENDRA¬Å LOS MENSAJES DE ERROR
  g_consulta text;
  -- VARIABLE QUE CONTENDR√???√??√?¬Å LA CONSULTA DINA¬ÅMICA PARA EL FILTRO
  g_nivel_error varchar; -- VARIABLE QUE CONTIENE EL NIVEL DE ERROR
  --      ERROR DE SINTAXIS (cuando llega a exception) = 1
  --      ERROR LOGICO (CRAMATICO) = 2
  --      ERROR LOGICO (INTERMEDIO) = 3
  --      ERROR LOGICO (ADVERTENCIA) = 4
  g_nombre_funcion varchar; --NOMBRE FI¬çSICO DE LA FUNCION
...
BEGIN
  ...
  g_nombre_funcion := 'f_tai_detalle_solicitud_sel';
  ...
  IF pm_codigo_procedimiento  = 'AL_DETSOL_SEL' THEN
    BEGIN
      g_consulta := 'SELECT 
            					COALESCE(to_char(des.fecha_reg,''dd-mm-yyyy HH:MI:SS am''),''0'') AS fecha_reg,
                                des.usuario_reg,
                                des.id_detalle_solicitud,
                                des.id_solicitud_salida,
                                des.id_item,
                                des.cantidad,
                                itm.nombre as nombre_item
                                ,itm.codigo||'' - ''||itm.nombre as desc_item
                                ,um.id_unidad_medida_base,um.nombre as nombre_medida
                                
							FROM alma.tai_detalle_solicitud des
                            INNER JOIN alma.tai_solicitud_salida sol ON sol.id_solicitud_salida = des.id_solicitud_salida
                            INNER JOIN alma.tai_item itm ON itm.id_item = des.id_item
                            INNER JOIN param.tpm_unidad_medida_base um on um.id_unidad_medida_base=itm.id_unidad_medida and um.estado_registro=''activo''
                            WHERE ';

      g_consulta := g_consulta || pm_criterio_filtro;

      -- SE AUMENTA EL ORDEN Y LOS PARA¬ÅMETROS DE LA CANTIDAD DE REGISTROS A DESPLEGAR
    ...
    ELSIF pm_codigo_procedimiento  = 'AL_DETSOL_COUNT' THEN

    BEGIN
      --Cuenta todos los registros de tasa_categoria sin condiciones
      g_consulta := 'SELECT count(des.id_detalle_solicitud) as total
                            FROM alma.tai_detalle_solicitud des
                            INNER JOIN alma.tai_solicitud_salida sol ON sol.id_solicitud_salida = des.id_solicitud_salida
                            INNER JOIN alma.tai_item itm ON itm.id_item = des.id_item
                            INNER JOIN param.tpm_unidad_medida_base um on um.id_unidad_medida_base=itm.id_unidad_medida and um.estado_registro=''activo''
      ...
      -- DESCRIPCION DE √????XITO PARA GUARDAR EN EL LOG
