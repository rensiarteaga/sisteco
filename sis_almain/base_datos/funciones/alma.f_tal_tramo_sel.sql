...
CREATE OR REPLACE FUNCTION alma.f_tal_tramo_sel (
...
***************************************************************************
 SCRIPT: 		 alma.f_tal_tramo_sel
 DESCRIPCION: 	 Devuelve las consultas a la tabla alma.tal_tramo
 AUTOR: 		 UNKNOW
 FECHA:			 01-12-2014
***************************************************************************/
...
BEGIN
    ...
    g_nombre_funcion := 'f_tal_tramo_sel';
    ...
    IF pm_codigo_procedimiento  = 'AL_TRAM_SEL' THEN
        BEGIN
            g_consulta := 'SELECT 	  tram.id_tramo,tram.codigo,tram.descripcion,tram.observaciones,tram.estado
                                      ,COALESCE(to_char(tram.fecha_reg,''dd-mm-yyyy''),''0'') AS fecha_reg,
                                      tram.usuario_reg
                            FROM alma.tal_tramo tram
                            WHERE tram.estado=''activo'' AND ';
    ...
    ELSIF pm_codigo_procedimiento  = 'AL_TRAM_COUNT' THEN

        BEGIN
        --Cuenta todos los registros de tasa_categoria sin condiciones
            g_consulta := '	SELECT 	COUNT(tram.id_tramo) AS total
                            FROM alma.tal_tramo tram
                            WHERE tram.estado=''activo'' AND ';  
    ...
    ---*** REGISTRO EN EL LOG EL Ã??XITO DE LA EJECUION DEL PROCEDIMIENTO
    g_reg_evento:= sss.f_tsg_registro_evento(pm_id_usuario             ,g_id_subsistema           ,g_id_lugar         ,g_descripcion_log_error,
                                         pm_ip_origen              ,pm_mac_maquina            ,'log'              ,NULL,
                                         pm_codigo_procedimiento   ,pm_proc_almacenado);
