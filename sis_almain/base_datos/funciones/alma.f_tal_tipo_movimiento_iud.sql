...
DECLARE
    ...
    g_id_fina_regi_prog_proy_acti 	integer;     -- VARIABLE DE LA ESTRUCTURA PROGRAMÃ????Ã???Ã??Ã?ÂTICA
...
---*** SI NO SE TIENE PERMISOS DE EJECUCIÃ?????N SE RETORNA EL MENSAJE DE ERROR
    IF NOT g_privilegio_procedimiento THEN
        g_nivel_error := '3';
        g_descripcion_log_error := 'El usuario no tiene permisos de ejecuciÃ????Ã???Ã??Ã?Â³n del procedimiento';
        g_respuesta := param.f_pm_mensaje_error(g_descripcion_log_error, g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
    ...
    --*** EJECUCIONN DEL PROCEDIMIENTO ESPECIÂFICO
