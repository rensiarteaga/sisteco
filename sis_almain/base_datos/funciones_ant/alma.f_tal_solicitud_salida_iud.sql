...
CREATE OR REPLACE FUNCTION alma.f_tal_solicitud_salida_iud (
...
***************************************************************************
 SCRIPT:          alma.f_tal_solicitud_salida
 DESCRIPCIÃ?????N:     Realiza modificaciones en la tabla alma.tal_solicitud_salida
...
DECLARE
    ...
    g_nivel_error                 varchar;     -- VARIABLE QUE CONTIENE EL NIVEL DE ERROR
                                               ...
                                               --      ERROR LÃ?????GICO (CRÃ????Ã???Ã??Ã?ÃTICO) = 2
                                               --      ERROR LÃ?????GICO (INTERMEDIO) = 3
                                               --      ERROR LÃ?????GICO (ADVERTENCIA) = 4
    
    g_nombre_funcion              varchar;     -- NOMBRE FÃ????Ã???Ã??Ã?ÃSICO DE LA FUNCIÃ?????N
...
---*** INICIACION DE VARIABLES 
    g_separador = '#@@@#'; --Separador para mensajes devueltos por la funciÃ????Ã???Ã??Ã?Ã³n	
    g_nombre_funcion :='f_tal_solicitud_salida_iud';
...
---*** SI NO SE TIENE PERMISOS DE EJECUCIÃ?????N SE RETORNA EL MENSAJE DE ERROR
    IF NOT g_privilegio_procedimiento THEN
        g_nivel_error := '3';
        g_descripcion_log_error := 'El usuario no tiene permisos de ejecuciÃ????Ã???Ã??Ã?Ã³n del procedimiento';
        g_respuesta := param.f_pm_mensaje_error(g_descripcion_log_error, g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
    ...
    END IF;
      ...
      --*** EJECUCIÃ?????N DEL PROCEDIMIENTO ESPECÃ????Ã???Ã??Ã?ÃFICO
    IF pm_codigo_procedimiento = 'AL_SOLSAL_INS' THEN
    BEGIN

		INSERT INTO alma.tal_solicitud_salida(
		    --usuario_reg,
            --fecha_reg,
            id_almacen,
        ...
        VALUES (
            ...
            g_descripcion_log_error := 'Registro exitoso de un nuevo registro en la tabla alma.tal_solicitud_salida';
  ...
  BEGIN
		SELECT nextval('alma.tal_solicitud_salida_id_solicitud_salida_seq') INTO g_id_solicitud_salida;
  ...
  --procedimiento de modificacion      
	...
	BEGIN
            ...
            where alma.tal_solicitud_salida.id_solicitud_salida = al_id_solicitud_salida;
            
            IF (g_id_solicitud_salida is null) THEN
                              
                g_descripcion_log_error := 'Modificacion no realizada: no existe el registro' || al_id_solicitud_salida || 'en la tabla alma.tal_solicitud_salida';
                g_nivel_error := '4';
                g_respuesta := param.f_pm_mensaje_error(g_descripcion_log_error, g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
            ...
            END IF;
                ...
                UPDATE alma.tal_solicitud_salida SET 
                	...
                	cargo_empleado = 'cargo de prueba',
                	id_aprobador = al_id_aprobador,
                    cargo_aprobador = 'cargo de prueba',
                    fecha_solicitud = al_fecha_solicitud,
                    descripcion = al_descripcion
                WHERE alma.tal_solicitud_salida.id_solicitud_salida = al_id_solicitud_salida;
			
		-- DESCRIPCIÃ?????N DE Ã?????XITO PARA GUARDAR EN EL LOG
        
            g_descripcion_log_error := 'Modificacion exitosa en alma.tal_solicitud_salida del registro '||  al_id_solicitud_salida;
            g_respuesta := 't'||g_separador||g_descripcion_log_error;
    ...
    ELSIF pm_codigo_procedimiento = 'AL_SOLSAL_DEL' THEN
    	...
    	BEGIN
            ...
            IF (g_id_solicitud_salida is null) THEN
                ...
                g_descripcion_log_error := 'Eliminacion no realizada: no existe el registro' || al_id_solicitud_salida || 'en la tabla alma.tal_solicitud_salida';
                g_nivel_error := '4';
                g_respuesta := param.f_pm_mensaje_error(g_descripcion_log_error, g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
                RETURN 'f'||g_separador||g_respuesta;
            ...
            DELETE FROM alma.tal_solicitud_salida 
				WHERE alma.tal_solicitud_salida.id_solicitud_salida = al_id_solicitud_salida;
                
 			
            -- DESCRIPCION DE EXITO PARA GUARDAR EN EL LOG
            g_descripcion_log_error := 'Eliminacion exitosa del registro '||al_id_solicitud_salida||' en alma.tal_solicitud_salida';
            g_respuesta := 't'||g_separador||g_descripcion_log_error;
        END;
    ...
    ELSIF pm_codigo_procedimiento = 'AL_ALACTIVE_UPD' THEN
    	...
    	BEGIN
            ...
            from alma.tal_solicitud_salida alma
	...
	ELSIF pm_codigo_procedimiento = 'AL_PROCSOL_UPD' THEN
    	BEGIN
            ...
            for g_solicitud_salida in select
                    ...
                    sol.descripcion,
                    sol.id_almacen
                from alma.tal_solicitud_salida sol where sol.id_solicitud_salida = al_id_solicitud_salida
            LOOP
                ...
                select max(id_movimiento) into g_id_movimiento from alma.tal_movimiento;
                
                -- falta obtener el id_movimiento recientemente insertado.
            	FOR g_detalle_solicitud in select * from alma.tal_detalle_solicitud detsol where detsol.id_solicitud_salida = al_id_solicitud_salida
                LOOP
    ...
    ELSIF pm_codigo_procedimiento = 'AL_SOLBORFIN_UPD' THEN
        BEGIN
            --VERIFICACION DE EXISTENCIA DEL REGISTRO
           IF NOT EXISTS(select 1 from alma.tal_solicitud_salida where alma.tal_solicitud_salida.id_solicitud_salida=al_id_solicitud_salida) 
            THEN       
                g_descripcion_log_error := 'Modificacion no realizada: no existe el registro' || al_id_solicitud_salida || 'en la tabla alma.tal_solicitud_salida';
                g_nivel_error := '4';
                g_respuesta := param.f_pm_mensaje_error(g_descripcion_log_error, g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
            ...
            UPDATE alma.tal_solicitud_salida
            SET estado = 'pendiente_aprobacion'
            WHERE id_solicitud_salida = al_id_solicitud_salida;
 			     
            -- DESCRIPCION DE EXITO PARA GUARDAR EN EL LOG
            g_descripcion_log_error := 'Modificacion exitosa del registro '||al_id_solicitud_salida||' en alma.tal_solicitud_salida';
            g_respuesta := 't'||g_separador||g_descripcion_log_error;
     ...
     ELSIF pm_codigo_procedimiento = 'AL_SOLPENREV_UPD' THEN
        BEGIN
           ...
           IF NOT EXISTS(select 1 from alma.tal_solicitud_salida where alma.tal_solicitud_salida.id_solicitud_salida=al_id_solicitud_salida) 
            THEN       
                g_descripcion_log_error := 'Modificacion no realizada: no existe el registro' || al_id_movimiento || 'en la tabla alma.tal_solicitud_salida';
                g_nivel_error := '4';
                g_respuesta := param.f_pm_mensaje_error(g_descripcion_log_error, g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
                RETURN 'f'||g_separador||g_respuesta;
            ELSE
        ...
        BEGIN
           ...
           IF NOT EXISTS(select 1 from alma.tal_solicitud_salida where alma.tal_solicitud_salida.id_solicitud_salida=al_id_solicitud_salida) 
            ...
            FOR g_solicitud_salida IN EXECUTE  ('
            									...
            									')
            LOOP
            		g_detalle_solicitud	= alma.f_al_calculo_saldo(NULL,al_id_solicitud_salida,g_solicitud_salida.id_almacen,g_solicitud_salida.id_item,'SOLIC');
                    g_saldo_movimiento = alma.f_al_calculo_saldo(NULL,NULL,g_solicitud_salida.id_almacen,g_solicitud_salida.id_item,'MOVIM');
                    ...
                    then
                        ...
                        from alma.tal_solicitud_salida  sol
                        inner join alma.tal_detalle_solicitud det on det.id_solicitud_salida=sol.id_solicitud_salida
                        left join alma.tal_item it on it.id_item=det.id_item and it.estado='activo'
            ...
            g_cod_solicitud = alma.f_al_generar_cod_solicitud(al_id_solicitud_salida);
    ...
    ELSIF pm_codigo_procedimiento = 'AL_SOLPROC_UPD' THEN
        BEGIN
           ...
           IF NOT EXISTS(select 1 from alma.tal_solicitud_salida where alma.tal_solicitud_salida.id_solicitud_salida=al_id_solicitud_salida) 
            ...
            select "alma"."f_al_procesar_solicitud"(pm_id_usuario,pm_ip_origen,al_id_solicitud_salida,'PROCSOL') into g_res;
