...
if v_mov.id_solicitud_salida is NULL 	THEN
  BEGIN
      if (v_mov.tipo='ingreso' OR v_mov.tipo='transpaso_ingreso')  then
        BEGIN
            ...
            then
                ...
                from alma.tal_movimiento mov
                inner join alma.tal_tipo_movimiento tip on tip.id_tipo_movimiento=mov.id_tipo_movimiento
                where mov.estado in ('pendiente_aprobacion') AND (tip.tipo='ingreso' OR tip.tipo='transpaso_ingreso' ) AND mov.id_almacen=v_mov.id_almacen;
                
                if (v_max_fecha IS NOT NULL) AND (v_max_fecha > v_mov.fecha_movimiento)
                then
                	--error se quiere valorar un ingreso  teniendo otros que estan pendientes de aprobacion
                ...
                then
                	...
                	raise exception '%','Verifique la fecha de finalizacion del movimiento, el ultimo ingreso fue registrado en fecha: '||v_max_fecha;
                end if;
                res:=false;
            end if;
            ...
            then
                ...
                then
                	...
                	raise exception '%','Verifique la fecha de finalizacion de la salida. La ultima  salida en el almacen fue registrado en fecha: '||v_max_fecha||chr(10)||
                    				'Fecha del Movimiento: '||v_mov.fecha_movimiento;
                end if;
                res:=false;
