...
CREATE OR REPLACE FUNCTION alma.f_tal_tipo_movimiento_iud (
...
/**************************************************************************
 AUTOR:           Ariel Ayaviri Omonte
				Ruddy LujÃ?Ã¡n Bravo
 FECHA:           2013-07-26
 DESCRIPCIÃ???N:     Realiza modificaciones en la tabla alma.f_tal_tipo_movimiento
***************************************************************************/
...
BEGIN
    ...
    g_separador = '#@@@#'; --Separador para mensajes devueltos por la funciÃ???Ã??Ã?Ã³n
    g_nombre_funcion :='f_tal_tipo_movimiento_iud';
...
---*** SI NO SE TIENE PERMISOS DE EJECUCIÃ????N SE RETORNA EL MENSAJE DE ERROR
    IF NOT g_privilegio_procedimiento THEN
        g_nivel_error := '3';
        g_descripcion_log_error := 'El usuario no tiene permisos de ejecuciÃ???Ã??Ã?Ã³n del procedimiento';
        g_respuesta := param.f_pm_mensaje_error(g_descripcion_log_error, g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
    ...
    --*** EJECUCIONN DEL PROCEDIMIENTO ESPECIFICO
    ...
    BEGIN
        ...
        if exists(select 1 from alma.tal_tipo_movimiento where alma.tal_tipo_movimiento.id_documento=al_id_documento)
        then 
        ...
        then
              if EXISTS(	select 1 from alma.tal_tipo_movimiento tm where tm.tipo = al_tipo	  )
              then
            ...
            then
                     ...
                     WHERE tm.tipo = al_tipo;
                      ...
                      g_descripcion_log_error := 'La devolucion, ya fue registrado anteriormente'||chr(10)||
                                              'Fecha Registro: '||g_registros.fecha_reg||chr(10)||
                                              'Usuario Registro: '||g_registros.usuario_reg||chr(10)||
                                              'Tipo Movimiento: '||g_registros.tipo;
                      g_nivel_error := '1';
		...
		INSERT INTO alma.tal_tipo_movimiento (
            id_documento,
            tipo,
            requiere_aprobacion
        ) VALUES (
        ...
        g_descripcion_log_error := 'Registro exitoso de un nuevo registro en la tabla alma.tal_tipo_movimiento';
    ...
    BEGIN
        ...
        from alma.tal_tipo_movimiento where id_tipo_movimiento = al_id_tipo_movimiento;
        
        IF g_id_tipo_movimiento is null THEN
        	g_descripcion_log_error := 'Modificacion no realizada: no existe el registro' || al_id_tipo_movimiento || 'en la tabla alma.tal_tipo_movimiento';
            g_nivel_error := '4';
            g_respuesta := f_pm_mensaje_error(g_descripcion_log_error, g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
            RETURN 'f'||g_separador||g_respuesta;
        END IF;
    	...
    	if EXISTS(select 1 from alma.tal_movimiento mov where mov.id_tipo_movimiento=al_id_tipo_movimiento)
        then
        			  --detalle del tipo movimiento existente en un movimiento
                       ...
                       FROM alma.tal_tipo_movimiento tm
                       INNER JOIN alma.tal_movimiento mov on mov.id_tipo_movimiento=tm.id_tipo_movimiento
		...
		UPDATE alma.tal_tipo_movimiento SET
            usuario_reg = "current_user"(),
            fecha_reg = now(),
            id_documento = al_id_documento,
           	tipo = al_tipo,
            requiere_aprobacion = al_requiere_aprobacion
		WHERE alma.tal_tipo_movimiento.id_tipo_movimiento = al_id_tipo_movimiento;
        
		-- DESCRIPCION DE EXITO PARA GUARDAR EN EL LOG
        g_descripcion_log_error := 'Modificacion exitosa en alma.tal_tipo_movimiento del registro '||  al_id_tipo_movimiento   ;
        g_respuesta := 't'||g_separador||g_descripcion_log_error;
	END;
	...
	BEGIN
            ...
            IF g_id_tipo_movimiento is null THEN
                g_nivel_error := '4';
                g_descripcion_log_error := 'Eliminacion no realizada: registro '|| al_id_tipo_movimiento || ' en alma.tal_tipo_movimiento inexistente';
                g_respuesta := f_pm_mensaje_error(g_descripcion_log_error, g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
                RETURN 'f'||g_separador||g_respuesta;
            END IF;
            ...
            DELETE FROM alma.tal_tipo_movimiento
            WHERE id_tipo_movimiento = al_id_tipo_movimiento;
            
            -- DESCRIPCION DE EXITO PARA GUARDAR EN EL LOG
            g_descripcion_log_error := 'Eliminacion exitosa del registro '||al_id_tipo_movimiento||' alma.tal_tipo_movimiento';
            g_respuesta := 't'||g_separador||g_descripcion_log_error;
    END;
	ELSE
...
EXCEPTION
	--control de unicidad y manejo del error en la tabla alma.tal_tipo_movimiento.id_documento
