...
CREATE OR REPLACE FUNCTION alma.f_tal_stock_item_iud (
...
***************************************************************************
 SCRIPT:          alma.f_tal_stock_item_iud
...
---*** SI NO SE TIENE PERMISOS DE EJECUCIO?N SE RETORNA EL MENSAJE DE ERROR
    ...
    BEGIN
        ...
        then
                ...
                from alma.tal_stock_item st
                	 inner join alma.tal_almacen al on al.id_almacen=st.id_almacen
                     inner join alma.tal_item it on it.id_item=st.id_item
		...
		INSERT INTO alma.tal_stock_item (
            id_almacen,
            id_item,
            minimo, 
            maximo
            ...
            g_descripcion_log_error := 'Registro exitoso de un nuevo registro en la tabla alma.tal_stock_item';
  ...
  --procedimiento de modificacion      
	...
	BEGIN
            ...
            THEN
                raise EXCEPTION 'Verifique que el stock maximo sea mayor al stock minimo';
            END IF;
			--se verifica que id_stock item no pueda ser añadido mas de una vez en un mismo almacen
            ...
            if(g_cant_repeticiones = 1)
       		then 
             	UPDATE alma.tal_stock_item SET 
                	...
                	minimo = al_minimo,
                	maximo = al_maximo
				WHERE alma.tal_stock_item.id_stock_item = al_id_stock_item;
            else 
            	select st.id_stock_item,st.fecha_reg,al.codigo||'-'||al.nombre as desc_almacen,it.codigo||'-'||it.nombre as desc_item into g_registros
                from alma.tal_stock_item st
                	 inner join alma.tal_almacen al on al.id_almacen=st.id_almacen
		...
		-- DESCRIPCIÃ?N DE Ã?XITO PARA GUARDAR EN EL LOG
            g_descripcion_log_error := 'Modificacion exitosa en alma.tal_stock_item del registro '||al_id_stock_item;
            g_respuesta := 't'||g_separador||g_descripcion_log_error;
    	...
    	BEGIN
            ...
            IF (g_id_stock_item is null) THEN
                ...
                g_descripcion_log_error := 'Eliminacion no realizada: no existe el registro' || al_id_stock_item || 'en la tabla alma.tal_stock_item';
                g_nivel_error := '4';
                g_respuesta := param.f_pm_mensaje_error(g_descripcion_log_error, g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
                RETURN 'f'||g_separador||g_respuesta;
            ...
            DELETE FROM alma.tal_stock_item
			WHERE alma.tal_stock_item.id_stock_item=al_id_stock_item;
                
 			
            -- DESCRIPCIÃ?N DE Ã?XITO PARA GUARDAR EN EL LOG
            g_descripcion_log_error := 'Eliminacion exitosa del registro '||al_id_stock_item||' en alma.tal_stock_item';
            g_respuesta := 't'||g_separador||g_descripcion_log_error;
        END;
