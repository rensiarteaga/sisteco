...
DECLARE
    ...
    g_respuesta                varchar; -- VARIABLE QUE CONTENDRÃ???Ã??Ã?Ã LOS MENSAJES DE ERROR
    g_consulta                 text;    -- VARIABLE QUE CONTENDRÃ???Ã??Ã?Ã LA CONSULTA DINÃ???Ã??Ã?ÃMICA PARA EL FILTRO
    g_nivel_error              varchar; -- VARIABLE QUE CONTIENE EL NIVEL DE ERROR
                                        --      ERROR DE SINTAXIS (cuando llega a exception) = 1
                                        --      ERROR LÃ????GICO (CRÃ???Ã??Ã?ÃTICO) = 2
                                        --      ERROR LÃ????GICO (INTERMEDIO) = 3
                                        --      ERROR LÃ????GICO (ADVERTENCIA) = 4
    g_nombre_funcion           varchar; --NOMBRE FÃ???Ã??Ã?ÃSICO DE LA FUNCIÃ????N
    g_separador                varchar(10); --Caracteres que servirÃ???Ã??Ã?Ã¡n para separar el mensaje, nivel y origen del error
