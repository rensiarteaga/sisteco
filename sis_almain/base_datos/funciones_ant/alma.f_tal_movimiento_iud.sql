...
CREATE OR REPLACE FUNCTION alma.f_tal_movimiento_iud (
...
***************************************************************************
 ...
 DESCRIPCION:     Realiza modificaciones en la tabla alma.tal_movimiento
...
---*** SI NO SE TIENE PERMISOS DE EJECUCION SE RETORNA EL MENSAJE DE ERROR
  ...
  --procedimiento de modificacion      
	...
	BEGIN
		...
		-- DESCRIPCION DE EXITO PARA GUARDAR EN EL LOG
            g_descripcion_log_error := 'Modificacion exitosa en alma.tal_movimiento del registro '||  al_id_movimiento;
            g_respuesta := 't'||g_separador||g_descripcion_log_error;
        END;
    	...
    	BEGIN
            ...
            -- VERIFICACIÓ DE EXISTENCIA DE HIJOS
            IF EXISTS(SELECT 1 FROM alma.tal_movimiento
                     INNER JOIN alma.tal_detalle_movimiento ON alma.tal_detalle_movimiento.id_movimiento=alma.tal_movimiento.id_movimiento
                     WHERE alma.tal_movimiento.id_movimiento = al_id_movimiento) THEN
                g_nivel_error := '4';
                g_descripcion_log_error := 'Eliminacióo realizada: El movimiento tiene Conceptos asociados en el Detalle';
                g_respuesta := param.f_pm_mensaje_error(g_descripcion_log_error, g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
                RETURN 'f'||g_separador||g_respuesta;
            END IF; 
            ...
            -- DESCRIPCION DE EXITO PARA GUARDAR EN EL LOG
            g_descripcion_log_error := 'Eliminacion exitosa del registro '||al_id_movimiento||' en alma.tal_movimiento';
            g_respuesta := 't'||g_separador||g_descripcion_log_error;
        END;
        ...
        BEGIN
            --VERIFICACION DE EXISTENCIA DEL REGISTRO
            IF NOT EXISTS(select 1 from alma.tal_movimiento where alma.tal_movimiento.id_movimiento=al_id_movimiento) 
            THEN       
                g_descripcion_log_error := 'Modificacion no realizada: no existe el registro' || al_id_movimiento || 'en la tabla alma.tal_movimiento';
                g_nivel_error := '4';
                g_respuesta := param.f_pm_mensaje_error(g_descripcion_log_error, g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
            ...
            */
            ...
            then
                ...
                from alma.tal_movimiento mov
                inner join alma.tal_tipo_movimiento tm on tm.id_tipo_movimiento = mov.id_tipo_movimiento
                where mov.id_movimiento = al_id_movimiento;
              
                    if g_det_movimiento.codigo is NULL
                    then 
                        --generacion del codigo del movmiento
                        g_cod_movimento = alma.f_al_generar_cod_movimiento(al_id_movimiento);
            ...
            -- DESCRIPCION DE EXITO PARA GUARDAR EN EL LOG
            g_descripcion_log_error := 'Modificacion exitosa del registro '||al_id_movimiento||' en alma.tal_movimiento';
            g_respuesta := 't'||g_separador||g_descripcion_log_error;
        END;
        ...
        BEGIN
            ...
            --control costos
            if EXISTS (select 1 from alma.tal_movimiento mov 
            			inner join alma.tal_detalle_movimiento det on det.id_movimiento=mov.id_movimiento
                        where det.id_movimiento=al_id_movimiento and det.costo_total <= 0
                        )
            then
            		raise exception '%','Los costos totales del transpaso no pueden ser "0" '||chr(10)||'Verifique los costos totales de los items asociados al movimiento :'||al_id_movimiento;
            end if;
                	UPDATE alma.tal_movimiento SET 
        ...
        BEGIN
            ...
            */
            if ( g_fecha = 'f') THEN
                    ...
                    */
                        FOR g_det_movimiento in execute('  	select mov.id_almacen,det.id_item,sum (det.cantidad) as cantidad,tip.tipo
                                                        ...
                                                        ')
                        LOOP
                        --g_saldo := alma.f_al_calcular_saldo_movimento(al_id_movimiento,g_det_movimiento.id_almacen,g_det_movimiento.id_item);
                        g_saldo_movimientos	= alma.f_al_calculo_saldo(NULL,NULL,g_det_movimiento.id_almacen,g_det_movimiento.id_item,'MOVIM');
                        g_saldo_reservados = alma.f_al_calculo_saldo(NULL,NULL,g_det_movimiento.id_almacen,g_det_movimiento.id_item,'SOLIC');
            ...
            -- DESCRIPCION DE EXITO PARA GUARDAR EN EL LOG
            g_descripcion_log_error := 'Modificacion exitosa  '||al_id_movimiento||' en alma.tal_movimiento';
            g_respuesta := 't'||g_separador||g_descripcion_log_error;
        END;
        ...
        BEGIN
           ...
           --       raise exception 'llega';
            	--se verifica que el movimiento a finalizar tenga en detalle_movimiento 1 item con tipo_saldo='por_entregar'
                  ...
                  if EXISTS(	select 1 
                              from alma.tal_movimiento mov inner join alma.tal_detalle_movimiento det on det.id_movimiento=mov.id_movimiento 
                  ...
                  then
                      ...
                      select alma.f_al_procesar_solicitud(pm_id_usuario,pm_ip_origen,al_id_movimiento,'FINSOL') into g_res;
                      
                       -- DESCRIPCIÓ DE ÉITO PARA GUARDAR EN EL LOG
                      g_descripcion_log_error := 'Registro exitoso en alma.tal_solicitud_salida ' || al_id_solicitud_salida;
                      g_respuesta := 't'||g_separador||g_descripcion_log_error;
                 ...
                 -- codigo = g_cod_movimento,
                  ...
                  estado = 'entregado'
                  WHERE alma.tal_solicitud_salida.id_solicitud_salida = g_id_sol_salida;
    ...
    --transpaso de items de un almacen origen a un almacen destino
     ELSIF pm_codigo_procedimiento = 'AL_TRANSMOVFIN_UPD'
        ...
        BEGIN
            ...
            select alma.f_al_procesar_transpaso(al_id_movimiento,pm_id_usuario,pm_ip_origen) into g_res;  
