...
CREATE OR REPLACE FUNCTION alma.f_tal_ubicacion_sel (
...
***************************************************************************
 SCRIPT:          alma.f_tal_ubicacion_sel
 DESCRIPCIÃN:     Devuelve las consultas a la tabla tal_ubicacion
 AUTOR:           Ruddy Luján Bravo
 FECHA:           08-08-2013 17:10:00
 COMENTARIOS:    

***************************************************************************/
--------------------------
-- CUERPO DE LA FUNCIÃN --
--------------------------
...
BEGIN
    ...
    g_nombre_funcion :='f_tal_ubicacion_sel';
    g_rol_adm:= false;
    ...
    IF pm_codigo_procedimiento  = 'AL_ALUB_SEL' THEN
        BEGIN
        	
              g_consulta := 'SELECT al.usuario_reg,
		--COALESCE(per.nombre||'' ''||per.apellido_paterno||'' ''||per.apellido_materno,'' '') as usuario_reg,
            					COALESCE(to_char(al.fecha_reg,''dd-mm-yyyy''),''0'') AS fecha_reg,
                               -- COALESCE(per2.nombre||'' ''||per2.apellido_paterno||'' ''||per2.apellido_materno,'' '') as usuario_mod,
                               -- COALESCE(to_char(al.fecha_mod,''dd-mm-yyyy''),''0'') AS fecha_mod,
                            al.id_ubicacion,
                            al.id_ubicacion_fk,
                            al.id_almacen,
    						al.codigo,
                			al.nombre,
                			al.estado
							FROM alma.tal_ubicacion al
                           -- INNER JOIN sss.tsg_usuario u ON (al.id_usuario_reg=u.id_usuario)
                          --  LEFT JOIN sss.tsg_usuario u2 ON (al.id_usuario_mod=u2.id_usuario)
    ...
    -- PARA LA CONSULTA DE CONTEO  
    	ELSIF pm_codigo_procedimiento  = 'AL_ALUB_COUNT' THEN

        BEGIN
        --Cuenta todos los registros
            g_consulta := 	'SELECT
                           COUNT(al.id_ubicacion)
        				FROM alma.tal_ubicacion al
                            INNER JOIN sss.tsg_usuario u ON (al.id_usuario_reg=u.id_usuario)
                            LEFT JOIN sss.tsg_usuario u2 ON (al.id_usuario_mod=u2.id_usuario)
                            INNER JOIN sss.tsg_persona per ON (u.id_persona = per.id_persona)
