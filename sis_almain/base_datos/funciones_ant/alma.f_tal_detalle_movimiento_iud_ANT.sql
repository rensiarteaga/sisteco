...
/************************************************************************** AUTOR:           Ruddy Limbert Lujan Bravo
 FECHA:           09-09-2013
 DESCRIPCIÃÆ?N:     Realiza modificaciones en la tabla alma.f_tal_detalle_movimiento
***************************************************************************/

-- PARÃÆÃÃâÃMETROS FIJOS
/*
...
DECLARE
  ...
  --PARAÃâÃMETROS FIJOS
  g_id_subsistema integer; -- ID SUBSISTEMA
  g_id_lugar integer; -- ID LUGAR
  g_numero_error varchar; -- ALMACENA EL NÃÆ??MERO DE ERROR
  g_mensaje_error varchar; -- ALMACENA MENSAJE DEL ERROR
  g_privilegio_procedimiento boolean;
  -- BANDERA PARA VERIFICAR LLAMADA DE LA FUNCIÃÆ??N
  g_descripcion_log_error text; -- PARA ALMACENAR EL MENSAJE DE ERROR O LOG
  g_reg_evento varchar;
  g_reg_error varchar;
  g_respuesta varchar; -- VARIABLE QUE CONTIENE LA RESPUESTA DE LA FUNCIÃÆ??N
  g_nivel_error varchar; -- VARIABLE QUE CONTIENE EL NIVEL DE ERROR
  --      ERROR DE SINTAXIS (cuando llega a exception) = 1
  --      ERROR LÃÆ??GICO (CRÃÆ?ÃâÃTICO) = 2
  --      ERROR LÃÆ??GICO (INTERMEDIO) = 3
  --      ERROR LÃÆ??GICO (ADVERTENCIA) = 4
  g_nombre_funcion varchar;
  g_separador varchar(10); -- SEPARADOR DE CADENAS
  g_id_fina_regi_prog_proy_acti integer;
  -- VARIABLE DE LA ESTRUCTURA PROGRAMÃÆ?ÃâÃTICA
...
BEGIN
  --- INICIACIÃÆ??N DE VARIABLES
  g_separador = '#@@@#'; --Separador para mensajes devueltos por la funciÃÆ?ÃâÃ³n
  g_nombre_funcion :='f_tal_detalle_movimiento_iud';
  g_privilegio_procedimiento := FALSE;
  g_respuesta := FALSE;

  ---*** OBTENCIÃÆ??N DEL ID DEL SUBSISTEMA
  ...
  ---*** OBTENCIÃÆ??N DEL ID DEL LUGAR ASIGNADO AL USUARIO
  ...
  ---*** VALIDACIÃÆ??N DE LLAMADA POR USUARIO O FUNCIÃÆ??N
  ...
  ---*** VERIFICACIÃÆ??N DE PERMISOS DEL USUARIO
  IF NOT g_privilegio_procedimiento THEN
    g_privilegio_procedimiento := sss.f_sg_validacion_procedimiento(
      pm_id_usuario,pm_codigo_procedimiento,NULL);
  END IF;

  ---*** SI NO SE TIENE PERMISOS DE EJECUCIÃÆ??N SE RETORNA EL MENSAJE DE ERROR
  IF NOT g_privilegio_procedimiento THEN
    g_nivel_error := '3';
    g_descripcion_log_error :=
      'El usuario no tiene permisos de ejecuciÃÆ?ÃâÃ³n del procedimiento';
    g_respuesta := param.f_pm_mensaje_error(g_descripcion_log_error,
      g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
  ...
  --*** EJECUCIÃÆ??N DEL PROCEDIMIENTO ESPECÃÆ?ÃâÃFICO
  IF pm_codigo_procedimiento = 'AL_DETMOV_INS' THEN
    BEGIN
      ...
      -- DESCRIPCIÃÆ??N DE ÃÆ??XITO PARA GUARDAR EN EL LOG
      g_descripcion_log_error :=
    ...
    select estado into g_estado_padre
      ...
      IF g_id_detalle_movimiento is null THEN
        g_descripcion_log_error :=
          'ModificaciÃÆÃ³n no realizada: no existe el registro' ||
    ...
    select estado into g_estado_padre
      ...
      IF g_id_detalle_movimiento is null THEN
        g_nivel_error := '4';
        g_descripcion_log_error := 'EliminaciÃÆÃ³n no realizada: registro '||
      ...
      -- DESCRIPCIÃÆ??N DE ÃÆ??XITO PARA GUARDAR EN EL LOG
      g_descripcion_log_error := 'EliminaciÃÆÃ³n exitosa del registro '||
  ...
  ---*** REGISTRO EN EL LOG EL ÃÆ??XITO DE LA EJECUIÃÆ??N DEL PROCEDIMIENTO
  g_reg_evento:= sss.f_tsg_registro_evento(pm_id_usuario             ,
    g_id_subsistema           ,g_id_lugar         ,g_descripcion_log_error,
  ...
  WHEN others THEN
  BEGIN
    --SE OBTIENE EL MENSAJE Y EL NÃÆ??MERO DEL ERROR LANZADO POR EL GESTOR DE BASE DE DATOS
