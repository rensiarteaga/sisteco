...
// Ariel Ayaviri Omonte - 12-01-2011 - Changed the value from (1,1) to (0,0)
dia._currentDrag.translate(0,0);
//-------------------------------------
...
this.translateToolbox(dx, dy);
   ...
   //// ADICIÓN DE ARIEL AYAVIRI PARA MOVER LOS VÉRTICES DE LOS
   //// NODOS RECURSIVOS. 01-03-2011
   var auxJoints = this.joints();
   for(var i=0;i<auxJoints.length;i++){
		if(auxJoints[i]._start.shape.id==auxJoints[i]._end.shape.id){
			for(var k = 0;k<auxJoints[i]._opt.vertices.length; k++){
				auxJoints[i]._opt.vertices[k].x+= dx;
				auxJoints[i]._opt.vertices[k].y+= dy;
			}
		}
		else{/*
			FALTA IMPLEMENTAR UN CODIGO QUE RECORRA BIEN LOS VERTICES DE LAS 
			ARISTAS.
			for(var k = 0;k<auxJoints[i]._opt.vertices.length; k++){
				auxJoints[i]._opt.vertices[k].x+= dy;
				auxJoints[i]._opt.vertices[k].y+= dx;
			}*/
		}
   }
   //------------------------------------------------------
...
//---- Ariel Ayaviri - 12-01-2011 - Changed the values of the translation from (1,1) to (0,0) for no moving when clicking
dia._currentDrag.translate(0,0);
//------------------
    }
