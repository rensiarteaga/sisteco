...
$nombre_archivo = "ActionGuardarRendiciones.php";
	...
	for($j = 0;$j < $cont; $j++)
	{
		if ($get)
		{
			...
			$id_financiador= $_GET["txt_id_financiador_$j"];
				...
				$fk_id_caja_regis = $_GET["fk_id_caja_regis_$j"];
		...
		else
		{
			...
			$codigo_control=$_POST["codigo_control_$j"];
				...
				$fk_id_caja_regis = $_POST["fk_id_caja_regis_$j"];
				$id_presupuesto = $_POST["id_presupuesto_$j"];
				$concepto_regis = $_POST["concepto_regis_$j"];
		}
 		if($id_presupuesto){$id_financiador=$id_presupuesto;$id_regional=$id_presupuesto;$id_programa=$id_presupuesto; $id_actividad=$id_presupuesto;$id_proyecto=$id_presupuesto;}
		if ($id_caja_regis == "undefined" || $id_caja_regis == "")
		{
			////////////////////Inserción/////////////////////
		
			//Validación de datos (del lado del servidor)
			$res = $Custom->ValidarRendiciones("insert",$id_caja_regis,$id_cajero,$id_presupuesto,$id_unidad_organizacional,$id_concepto_ingas,$importe_regis,$tipo_documento,$fecha_documento,$nro_documento,$razon_social,$nro_nit,$nro_autorizacion,$codigo_control,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);
			...
			$res = $Custom -> InsertarRendiciones($id_caja_regis,$id_cajero,$id_presupuesto,$id_unidad_organizacional,$id_concepto_ingas,$importe_regis,$tipo_documento,$fecha_documento,$nro_documento,$razon_social,$nro_nit,$nro_autorizacion,$codigo_control,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad,$fk_id_caja_regis,$concepto_regis);
		...
		{	///////////////////////Modificación////////////////////
			...
			//Validación de datos (del lado del servidor)
			$res = $Custom->ValidarRendiciones("update",$id_caja_regis,$id_cajero,$id_presupuesto,$id_unidad_organizacional,$id_concepto_ingas,$importe_regis,$tipo_documento,$fecha_documento,$nro_documento,$razon_social,$nro_nit,$nro_autorizacion,$codigo_control,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);
