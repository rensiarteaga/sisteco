...
//Parámetros del filtro
	...
	if($estado_repo){//solo cajas activas==> estado_caja=1 en TESORO
	...
	if($estado=='1'){//solo cajas activas==> estado_caja=1 (filtro enviado desde proceso_simplificado_factura en COMPRO)
	    $criterio_filtro.=" AND CAJA.estado_caja=1 and  (CAJA.id_caja in (select * from compro.f_tad_obtener_cajas($id_proceso_compra,''proceso'',''caja'') as (id_caja integer)))";
	}
	if($estado=='2'){//solo cajas activas==> estado_caja=1 (filtro enviado desde proceso_simplificado_factura en COMPRO)
	    $criterio_filtro.=" AND CAJA.estado_caja=1 and (CAJA.id_caja in (select * from compro.f_tad_obtener_cajas($id_cotizacion,''cotizacion'',''caja'') as (id_caja integer)))";
	}
	
	if($cajero=='1'){
		$criterio_filtro.=" AND (CAJA.id_caja in (select * from tesoro.f_ts_obtener_cajas(".$_SESSION["ss_id_usuario"].",''CAJERO'') as (id_caja integer)))";
	}else if($cajero=='2'){
		$criterio_filtro.=" AND (CAJA.id_caja in (select * from tesoro.f_ts_obtener_cajas(".$_SESSION["ss_id_usuario"].",''RESPON'') as (id_caja integer)))";
	}
