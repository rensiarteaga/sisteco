...
/**
**********************************************************
Nombre de archivo:	    ActionFinalizarCuentaDoc.php
...
$nombre_archivo = "ActionFinalizarCuentaDoc.php";
...
if($_SESSION["autentificado"]=="SI")
{
	...
	{
		if ($get){ 
			$id_cuenta_doc = $_GET["id_cuenta_doc"];
		} else{
			$id_cuenta_doc = $_POST["id_cuenta_doc"];
		}

		if ($id_cuenta_doc == "undefined" || $id_cuenta_doc == ""){
			$resp = new cls_manejo_mensajes(true, "406");
			$resp->mensaje_error = "MENSAJE ERROR = No existe el registro especificado para la finalización.";
			$resp->origen = "ORIGEN = $nombre_archivo";
			$resp->proc = "PROC = $nombre_archivo";
			$resp->nivel = "NIVEL = 4";
			echo $resp->get_mensaje();
			...
			$res = $Custom-> FinalizarCuentaDoc($id_cuenta_doc);
			if(!$res){
				$resp = new cls_manejo_mensajes(true, "406");
