...
$nombre_archivo='ActionListarSolicitudAvance.php';
...
//Parámetros del filtro
	...
	if($sort=='') $sortcol='periodo,num_solicitud';
	else $sortcol=$sort;
...
//	exit;
	...
	$res = $Custom -> ContarSolicitudADQAvance($cant ,$puntero,$sortcol,$sortdir,$criterio_filtro,$hidden_ep_id_financiador,$hidden_ep_id_regional,$hidden_ep_id_programa,$hidden_ep_id_proyecto,$hidden_ep_id_actividad);

	if($res) $total_registros= $Custom->salida;

	//Obtiene el conjunto de datos de la consulta
	$res = $Custom->ListarSolicitudADQAvance($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$hidden_ep_id_financiador,$hidden_ep_id_regional,$hidden_ep_id_programa,$hidden_ep_id_proyecto,$hidden_ep_id_actividad);
	...
	if($res)
	{
		...
		foreach ($Custom->salida as $f)
		{
			...
			$xml->add_nodo('id_empleado_solicitante',$f["id_empleado_solicitante"]);
			...
			$xml->add_nodo('desc_ep',$f["codigo_financiador"]."-".$f["codigo_regional"]."-".$f["codigo_programa"]."-".$f["codigo_proyecto"]."-".$f["codigo_actividad"]);
			...
			$xml->add_nodo('numeracion_solicitud',$f["periodo"]."/".$f["num_solicitud"]);
			$xml->add_nodo('desc_solicitud',$f["periodo"]."/".$f["num_solicitud"]." - ".$f["solicitante"]);
			$xml->fin_rama();								
		}
		$xml->mostrar_xml();
	}
