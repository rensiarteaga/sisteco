...
$pdf->Cell(180,5,'RENDICIÓN DE CAJA',0,1,'C');
$pdf->Cell(170,5,'De: '.$_SESSION['PDF_fecha_regis'].'   A:'.$_SESSION['PDF_fecha_actual'],0,0,'C');

$pdf->Cell(200,5,' ',0,1); 
$pdf->Cell(90,5,'Cajero: '.$_SESSION['PDF_nombre_completo'],0,0,'C');
if ($_SESSION['PDF_tipo_caja']==1) {$pdf->Cell(90,5,'Tipo de Caja: Caja',0,1,'C');} 
if ($_SESSION['PDF_tipo_caja']==2) {$pdf->Cell(90,5,'Tipo de Caja: Caja Chica',0,1,'C');} 
if ($_SESSION['PDF_tipo_caja']==3) {$pdf->Cell(90,5,'Tipo de Caja: Fondo Rotatorio',0,1,'C');} 
$pdf->Cell(115,5,'Unidad Organizacional: '.$_SESSION['PDF_nombre_unidad'],0,0,'C'); 
$pdf->Cell(75,5,'Moneda: '.$_SESSION['PDF_nombre_moneda'],0,1,'C');
$pdf->Cell(200,2,' ',0,1); 
//$pdf->ln(6);
$pdf->SetFont('Arial','B',8);
...
foreach ($data as $linea){
	...
	$pdf->SetFont('Arial','B',8);
	$pdf->Cell(25,6,'SUB-PROBRAMA:','',0,'L');
	$pdf->SetFont('Arial','',8); 
	...
	if($cont==0){
		...
		if ($_SESSION['PDF_sw_cheque']==2) {
			...
			$arreglo[0]='';
			$arreglo[1]='';
			$arreglo[2]='No se obtuvo el tipo de pago';
			$arreglo[3]='';
			$arreglo[4]='';
		}
