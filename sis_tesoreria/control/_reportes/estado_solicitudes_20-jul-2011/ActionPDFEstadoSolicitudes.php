...
if($_SESSION['autentificado']=="SI")
{
	...
	if($id_usuario == "undefined" || $id_usuario == "" || $id_usuario == 'null')
	{
		if($id_unidad_organizacional == "undefined" || $id_unidad_organizacional == "" || $id_unidad_organizacional == 'null')
		{
			...
			{
				$criterio_fil=' and CUEDOC.id_empleado='.$id_empleado;
			}
			else 
			{
				$criterio_fil=' and CUEDOC.id_depto='.$id_depto;
			}	
		}
		else 
		{
	...
	{		
		$criterio_fil=' and CUEDOC.id_usuario_reg='.$id_usuario;	
	}
	...
	$estado_solicitudes = $Custom-> ListarReporteEstadoSolicitudes(1500,$puntero,$sortcol,$sortdir,$criterio_fil,$hidden_ep_id_financiador,$hidden_ep_id_regional,$hidden_ep_id_programa,$hidden_ep_id_proyecto,$hidden_ep_id_actividad,$id_depto,$estado_solicitud,$fecha_ini,$fecha_fin);
