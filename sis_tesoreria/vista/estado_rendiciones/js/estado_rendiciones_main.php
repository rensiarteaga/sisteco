...
var elemento ={pagina:new EstadoRendiciones(idContenedor,direccion,paramConfig,configConsolidacion),idContenedor:idContenedor};
...
function EstadoRendiciones(idContenedor,direccion,paramConfig,configConsolidacion)
{
   ...
   /* function renderEstadoRendicion(value, p, record)
	{						
	...
	vectorAtributos[0] = { 
		validacion: {
			...
			loadMask: true,
			triggerAction: 'all',
			//store: new Ext.data.SimpleStore({fields: ['ID', 'valor'],data : Ext.estado_rendicion_combo.tipo_reporte}),
	...
	// txt tipo_solicitud 
	 vectorAtributos[1]  = {
		validacion: {
			...
			loadMask: true,
			triggerAction: 'all',
			//store: new Ext.data.SimpleStore({fields: ['ID', 'valor'],data :Ext.estado_rendicion_combo.tipo_solicitud }),
			store: new Ext.data.SimpleStore({fields: ['ID', 'valor'],data :[
	...
	// txt tipo_pres  
	 vectorAtributos[2]  = {
		validacion: {
			...
			loadMask: true,
			triggerAction: 'all',
			//store: new Ext.data.SimpleStore({fields: ['ID', 'valor'],data :Ext.estado_rendicion_combo.estado_rendicion }),
			store: new Ext.data.SimpleStore({fields: ['ID', 'valor'],data : [       
			...
			valueField:'ID',
			displayField:'valor',
			//renderer: renderEstadoRendicion,
			forceSelection:true,
			grid_visible:true, // se muestra en el grid
			grid_editable:true, //es editable en el grid,
			width_grid:100, // ancho de columna en el gris
	...
	vectorAtributos[6]={
			validacion:{
			name:'id_usuario',
			fieldLabel:'Responsable del Registro',
			allowBlank:false,			
	...
	var config=
	{
		titulo_maestro:"Detalle del Estado de Rendiciones"
	};
			...
			{	
				...
				{							
					var data='start=0';
					 ...
					 data+='&estado_rendicion='+g_estado_rendicion;	//listo
					 				 
					 data+='&fecha_desde='+formatDate(dteFechaDesde.getValue());
					 data+='&fecha_hasta='+formatDate(dteFechaHasta.getValue());	
					
					 //alert(data);
					 window.open(direccion+'../../../control/_reportes/estado_rendiciones/ActionPDFEstadoRendiciones.php?'+data);					
				}
				else
				{
					alert(mensaje);
	...
	{			
		...
		estado_rendicion = ClaseMadre_getComponente('estado_rendicion');		
		...
		componentes[2].on('select',evento_estado_rendicion);
	...
	function evento_estado_rendicion( combo, record, index )
	{
		g_estado_rendicion=componentes[1].getValue();
	}*/	
	...
	{
		g_tipo_reporte=componentes[0].getValue();
		g_estado_rendicion=componentes[2].getValue();		
	}
