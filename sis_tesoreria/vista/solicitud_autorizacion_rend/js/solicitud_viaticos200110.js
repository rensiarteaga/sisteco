...
// txt id_cajero
	...
	// txt id_moneda
		...
		Atributos[25]={
				validacion:{
					...
					disabled:false,
					onSelect:function(record){
								componentes[25].setValue(record.data.ID);
								componentes[25].collapse();
								if(record.data.ID=='efectivo'){
									...
									NoBlancosGrupo(8);
									//Oculta y fija el valor opcional al nro_deposito
									CM_ocultarComp(componentes[27]);
									componentes[27].allowBlank=true;
									//Nombre cheque permite en blanco
									componentes[30].allowBlank=false;
		...
		Atributos[28]={
				...
				onSelect:function(record){
					...
					componentes[28].setValue(record.data.id_caja);
					componentes[28].collapse();
					componentes[29].reset();
					ds_cajero.baseParams.m_id_caja=record.data.id_caja;
					componentes[29].modificado=true							
				},
	...
	function btnRendicion(){
				...
				if(NumSelect!=0){
					...
					layout_solicitud_viaticos2.loadWindows(direccion+'../../../../sis_tesoreria/vista/cuenta_doc_rendicion_cab/cuenta_doc_rendicion_cab.php?'+data,'Rendicion de Viático',ParamVentana);
	...
	*/
	function btn_fin_rend(){
		...
		if(NumSelect==1){
			if(confirm('¿Está seguro de Terminar todas las Rendiciones?')){
				Ext.MessageBox.show({
					title: 'Procesando',
					msg:"<div><img src='../../../lib/ext-yui/resources/images/default/grid/loading.gif'/> Finalizando la Solicitud...</div>",
					width:300,
	...
	function btn_cue_dep(){
		...
		if(NumSelect==1){
			...
			// Despliega el grupo de los datos de la cuenta bancaria
			CM_mostrarGrupo('Tipo Pago Finalización');
