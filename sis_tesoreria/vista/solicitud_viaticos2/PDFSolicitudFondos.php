...
function Footer()
{
 $fecha=date("d-m-Y");
	$hora=date("h:i:s");
		...
		/*if($_SESSION['PDF_nro_sol']='FA-GOP-03007')
		{
			//$this->Cell(18,3,'Fecha: '.$_SESSION['PDF_fecha_solicitud'],0,0,'L');
			$this->Cell(18,3,'Fecha: '.'30/03/2012',0,0,'L');
		}*/
		/*else
		{*/
...
$pdf->Cell(100,5,'Lugar',1,0,'C');
$pdf->Cell(38,5,'Fecha Iniciación',1,0,'C');
$pdf->Cell(39,5,'Tipo Pago',1,1,'C');
//$pdf->Cell(30,5,'Fecha Conclusión',1,0,'C');
$pdf->SetX(35);
$pdf->SetFont('Arial','',8);
//$y=$pdf->GetY();

$pdf->Cell(100,5,$_SESSION['PDF_lugar'],1,0,'C');
$pdf->Cell(38,5,$_SESSION['PDF_fecha_ini'],1,0,'C');
$pdf->Cell(39,5,$_SESSION['PDF_tipo_pago'],1,1,'C');
$pdf->SetFont('Arial','B',8);
//$pdf->Cell(30,5,'',1,0,'C');
//$pdf->MultiCell(87,15,number_format($_SESSION['PDF_monto'],2),1,'C');
$pdf->Cell(60,5,'DESCRIPCIÓN ',1,0,'C');  
 $pdf->Cell(18,5,'IMPORTE ',1,0,'C');  
 $pdf->Cell(64,5,'PRESUPUESTO ',1,0,'C');  
 $pdf->Cell(45,5,'OBSERVACIONES',1,0,'C');  
 $pdf->Cell(20,5,'DISPONIBLE',1,1,'C');  
 		
		$pdf->SetWidths(array(0,60,0,18,0,64,45,20));
		$pdf->SetFills(array(0,0,0,0,0,0,0,0));
 		$pdf->SetAligns(array('L','L','R','L','L','L','L','R'));
 		$pdf->SetVisibles(array(0,1,0,1,0,1,1,1));
		...
		for ($i=0;$i<sizeof($solicitud_detalle);$i++){
	  		...
	  		$total_importe=$total_importe+$solicitud_detalle[$i][3];
   		}
...
$data=array();
$data=$_SESSION['PDF_solicitud_fondo'];

	
			
			$pdf->SetFont('Arial','B',6);
			$pdf->Cell(68.8,5,'SOLICITUD','LTBR',0,'C',true);		
			$pdf->Cell(68.8,5,'AUTORIZACIÓN','LTBR',0,'C',true);
			$pdf->Cell(68.8,5,'Vo. Bo.','LTBR',1,'C',true);
			$pdf->SetFont('Arial','B',8);
			
			$pdf->Cell(68.8,15,'','LTBR',0,'C');		
			$pdf->Cell(68.8,15,'','LTBR',0,'C');
			$pdf->Cell(68.8,15,'','LTBR',1,'C');
				
			$pdf->SetFont('Arial','',6);
			$pdf->Cell(68.8,3,$data[0]['person_nombre'],'LTBR',0,'C');		
			$pdf->Cell(68.8,3,$data[0]['firma_autorizador'],'LTBR',0,'C');
			
			/*if($_SESSION['PDF_nro_sol']='FA-GPI-09010')
			{
				$pdf->Cell(68.8,3,'PARDO IRIARTE JAVIER ROLANDO','LTBR',1,'C');
			}
			else
			{*/				
				$pdf->Cell(68.8,3,$data[0]['gerente_financiero'],'LTBR',1,'C');
			/*}*/
				
			$pdf->SetFont('Arial','B',6);
			$pdf->Cell(68.8,3,'Solicitante','LTBR',0,'C',true);		
			$pdf->Cell(68.8,3,$data[0]['cargo_autorizador'],'LTBR',0,'C',true);	

		    /*if($_SESSION['PDF_nro_sol']=='FA-DEPT-04008')
			{
				$pdf->Cell(68.8,3,'JEFE DEPTO. GESTION FINANCIERA Y NEGOCIOS INTERINO','LTBR',1,'C',true);
			}
			else
			{	*/
				$pdf->Cell(68.8,3,$data[0]['cargo_vo_bo'],'LTBR',1,'C',true);
			//}
