<?php 
/**
 * Nombre:		  	    solicitud_viaticos2_main.php
...
function main(){
    ...
    echo "var vista='$vista';";
     echo "var id_cuenta_doc=$id_cuenta_doc;";
	?>
var fa=false;
<?php if($_SESSION["ss_filtro_avanzado"]!=''){echo 'fa='.$_SESSION["ss_filtro_avanzado"].';';}?>
var paramConfig={TamanoPagina:_CP.getConfig().ss_tam_pag,TiempoEspera:_CP.getConfig().ss_tiempo_espera,CantFiltros:1,FiltroEstructura:false,FiltroAvanzado:fa};
var elemento={pagina:new pagina_solicitud_viaticos2(idContenedor,direccion,paramConfig,vista,id_cuenta_doc),idContenedor:idContenedor};
...
function pagina_solicitud_viaticos2(idContenedor,direccion,paramConfig,vista,id_cuenta_doc){
	...
	var ds_moneda = new Ext.data.Store({proxy: new Ext.data.HttpProxy({url: direccion+'../../../../sis_parametros/control/moneda/ActionListarMoneda.php?estado=activo&prioridad=1'}),
	...
	/********** abril2016 ***********/
	function render_id_caja_fin(value, p, record){return String.format('{0}', record.data['desc_caja_fin']+'<br>'+record.data['desc_cajero_fin']);}
	/******** fin abril2016 *********/
	...
	//FUNCIONES RENDER
		...
		function render_id_categoria(value, p, record){return String.format('{0}', record.data['desc_categoria']+'<br>'+'Moneda: '+record.data['desc_moneda']);}
		...
		function render_id_depto(value, p, record){return String.format('{0}', record.data['desc_depto']+'<br>'+'Gestión: '+record.data['desc_parametro']);}
		...
		validacion:{
			...
			onSelect:function(record){
								...
								if(record.data.ID=='efectivo'){
									...
									CM_ocultarComp(componentes[31]);
									CM_mostrarGrupo('Datos Caja');
									componentes[18].store.baseParams={estado_repo:1};
...
// txt tipo_contrato
	Atributos[9]={
		validacion: {
			...
			store:new Ext.data.SimpleStore({fields:['ID','valor'],data:[['planta','Planta - Sin retención'],['contrato_cr','Contrato'],['comision','Comisión - Con retención'],['miembro','Miembros del Directorio - Con retención']]}),  //,['comision','Comisión - Con retención']
			valueField:'ID',
			displayField:'valor',
			lazyRender:true,
			forceSelection:true,
...
// txt observaciones
	...
	if(vista!='solicitud_efectivo'){	
		...
		Atributos[27]={
			validacion:{
				...
				disabled:false,
				onSelect:function(record){
							componentes[27].setValue(record.data.ID);
							...
							if(record.data.ID=='efectivo'){
								...
								NoBlancosGrupo(9);
								//Nombre cheque permite en blanco
								componentes[32].allowBlank=true;
							} else if(record.data.ID=='cheque'){
								...
								//Nombre cheque permite en blanco
								componentes[32].allowBlank=false;
		...
		Atributos[30]={
			validacion:{
				...
				onSelect:function(record){
					...
					componentes[30].setValue(record.data.id_caja);
	...
	if(vista=='solicitud_viatico' || vista=='ampliacion_viatico'){
		var config={titulo_maestro:titulo,grid_maestro:'grid-'+idContenedor,urlHijo:'../../../sis_tesoreria/vista/detalle_viatico/detalle_viatico_ana.php'};
	...
	function evento_parametro( combo, record, index ){		
		...
		componentes[3].store.baseParams={m_sw_rendicion:'si',sw_inv_gasto:'si',id_parametro:record.data.id_parametro};
	...
	function btnRendicion(){
		...
		if(NumSelect!=0){
			...
			data=data+'&tipo_contrato='+SelectionsRecord.data.tipo_contrato;	
			data=data+'&vista='+vista;
			...
			if(vista=='solicitud_viatico'){
				...
				layout_solicitud_viaticos2.loadWindows(direccion+'../../../../sis_tesoreria/vista/cuenta_doc_rendicion_cab/cuenta_doc_rendicion_cab_fa.php?'+data,'Rendiciones',ParamVentana);	
			}		
		}else{
	...
	function btnAmpliacion(){
		...
		if(NumSelect!=0){
			...
			var data='id_cuenta_doc='+SelectionsRecord.data.id_cuenta_doc;
			var ParamVentana={Ventana:{width:'90%',height:'70%'}}
			if(vista=='solicitud_viatico' || vista =='fin_viatico' ){
	...
	if(vista=='solicitud_viatico'||vista=='solicitud_avance'||vista=='fin_avance'||vista=='fin_viatico'){
	...
	function enable(sm,row,rec){
		...
		if(maestro.id_cuenta_doc==0){//es una solicitud
			if(rec.data['estado']=='pagado'){
				...
				if(vista=='solicitud_viatico' ){
					...
					//habilitamos correccion para solicitudes con importe 0 cero
					if(rec.data['saldo_solicitante']==0){
						CM_getBoton('cor_sol-'+idContenedor).enable();
					}
				}
		...
		//habilitamos correccion solo en el caso de ampliacion_viatico 
		if(vista=='ampliacion_viatico' ){
			//habilitamos correccion para solicitudes con importe 0 cero
			if(rec.data['saldo_solicitante']==0){
				CM_getBoton('cor_sol-'+idContenedor).enable();
