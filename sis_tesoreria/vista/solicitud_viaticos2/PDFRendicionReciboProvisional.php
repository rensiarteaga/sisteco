...
{   
	...
	function Footer()
	{
		...
		//$this->Cell(18,3,'Fecha: 08-03-2012',0,0,'L');
		if($_SESSION['ss_id_usuario']==100){
			$this->Cell(18,3,'Fecha: '.$_SESSION['PDF_fecha'],0,0,'L');
		}else{
...
$pdf->SetFont('Arial','B',8);
$pdf->Line(15,$pdf->GetY(),15,$pdf->GetY()+20);
$pdf->Cell(25,5,'CONCEPTO:','L',0,'L');
$pdf->SetFont('Arial','',8); 
$pdf->MultiCell(165,5,$_SESSION['PDF_motivo'],'RB','L'); 
...
//exit; 
 ...
 $pdf->Line(205,$pos_ini,205,$pos_fin);
	...
	if($total_cargo>=$total_descargo){
		...
		$saldo_empresa=$total_cargo-$total_descargo;
		$pdf->Cell(150,4,"Saldo a Favor de la Empresa",'L',0,'L');  
		$pdf->Cell(20,4,'','',0,'R');
		$pdf->Cell(20,4,number_format($saldo_empresa,2),'R',1,'R');
		if($total_cargo==$total_descargo){
		...
		$saldo_funcionario=$total_descargo-$total_cargo;
		$pdf->Cell(150,4,"Saldo a Favor del Funcionario",'L',0,'L');  
		$pdf->Cell(20,4,number_format($saldo_funcionario,2),'',0,'R');
	    $pdf->Cell(20,4,'','R',1,'R');  
	    $data=array("APROBADO POR:\n\n\n__________________________\n".$_SESSION['PDF_aprobador'],
			"RECIBI CONFORME:\n\n\n__________________________\n".$_SESSION['PDF_empleado'],
			"ENTREGUE CONFORME:\n\n\n__________________________\n".$_SESSION['PDF_cajero']);
