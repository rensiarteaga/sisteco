...
// 
	...
	Atributos[10]={
			...
			onSelect:function(record){
				...
				componentes[10].setValue(record.data.id_caja);
...
// txt id_cajero
	...
	this.EnableSelect=function(sm,row,rec)
	{
		...
		rec.data.rendicion='rendicion_rendido';
	...
	function btn_pagar()
	{
		...
		if(NumSelect==1){
		    ...
		    componentes[12].setValue(SelectionsRecord.data.importe);		    
			SiBlancosGrupo(0);
			SiBlancosGrupo(1);
			SiBlancosGrupo(2);
			NoBlancosGrupo(3);
	...
	function enable(sm,row,rec){
		...
		if(vista=='recibo_caja'){
			...
			if(rec.data.tipo_recibo=='provisorio'&&rec.data.estado=='solicitud_efectivo'
				&&rec.data.id_subsistema!='4'&&(rec.data.fk_id_cuenta_doc==''||rec.data.fk_id_cuenta_doc==undefined)){
				CM_getBoton('corsol-'+idContenedor).enable();
			}
