...
// txt nombre_cheque
	...
	Atributos[13]={
		validacion:{
			name:'motivo_viaje',
			fieldLabel:'Motivo de Ampliación',
			allowBlank:false,
			maxLength:400,
			minLength:0,
			selectOnFocus:true,
...
// txt importe_regis
	...
	var config={titulo_maestro:'Viático (Maestro)',titulo_detalle:'Ampliación de Viático (Detalle)',grid_maestro:'grid-'+idContenedor};
...
// 	}
	...
	this.btnEdit = function()
	{
		...
		if(NumSelect!=0)
		{
			...
			{
				 Ext.MessageBox.alert('Estado','Solo ampliaciones en estado VERIFICACIÓN pueden ser editados')
			}	
		}
		else
		{
	...
	this.btnEliminar = function()
	{
		...
		if(NumSelect!=0)
		{
			...
			{
				Ext.MessageBox.alert('Estado','Solo ampliaciones en estado VERIFICACIÓN pueden ser eliminados');
			}
		}
		else
		{
	...
	function btn_calcular()
	{
		...
		{	
			...
			{		
				...
				layout_ampliacion_viaticos.loadWindows(direccion+'../../../../sis_tesoreria/vista/viatico_calculo/viatico_calculo.php?'+data,'Cálculo de Viáticos',ParamVentana);
			...
			{
				Ext.MessageBox.alert('Estado','Solo ampliaciones en estado VERIFICACIÓN o CÁLCULO pueden realizar calculos.');	
			}
		}
		else
		{
	...
	function btn_pago()
	{
		...
		{	
			...
			{
				Ext.MessageBox.alert('Estado','Solo ampliacines en estado CÁLCULO pueden emitir pagos.');	
			}	
		}
		else
		{
	...
	*/
	...
	function btn_sin_pago(sm,filas,cont,NumSelect,SelectionsRecord)
	{
		...
		{			
			...
			{
				var sw=false;
				if(confirm('La ampliación no requiere generar pago y será finalizada. \nEsta seguro de Concluir la solicitud?'))
				{
					sw=true
				}
	...
	function btn_sin_pago_con_rendicion(sm,filas,cont,NumSelect,SelectionsRecord)
	{
		...
		{			
			...
			{
				var sw=false;
				if(confirm('La ampliación cambiará a estado Pendiente de Rendición. \nEsta seguro de continuar?'))
				{
					sw=true
				}
				if(sw)	//confirma la finalizacion del viatico
	...
	{		
		...
		if(NumSelect!=0)
		{
			...
			{			
				...
				{	
					var sw=false;
					if(confirm('Esta seguro de contabilizar la ampliación?'))
							{sw=true}
					if(sw){				
						g_sw_contabilizar.setValue('1');
						//componentes[9].setValue(4);	//Estado viatico = Ampliación Contabilizada por la base
						ClaseMadre_save();
						//CM_btnEdit();
		...
		{
			Ext.MessageBox.alert('Atención','Antes debe seleccionar una ampliacion.')
	    }	
	}*/
	...
	this.AdicionarBoton('../../../lib/imagenes/print.gif','Imprime la Ampliacion de Viáticos',btn_solicitud,true,'imp_ejecucion','Solicitud');
	this.AdicionarBoton('../../../lib/imagenes/menu-show.gif','Generar Pago de Ampliación de Viáticos',btn_pago,true,'emitir_pago','Generar de Pago');
