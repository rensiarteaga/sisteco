<?php 
/**
 * Nombre:		  	    carta_conclusion_main.php
...
var elemento={pagina:new pagina_carta_conclusion(idContenedor,direccion,paramConfig),idContenedor:idContenedor};
...
/**
 * Nombre:		  	    pagina_carta_conclusion.js
 * Propósito: 			pagina objeto principal
 * Autor:				Generado Automaticamente
 * Fecha creación:		2008-11-18 20:39:05
 */
function pagina_carta_conclusion(idContenedor,direccion,paramConfig){
	var Atributos=new Array,sw=0;
	var componentes=new Array();
	
	//---DATA STORE
	var ds = new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({url: direccion+'../../../control/carta/ActionListarCartaRegistro.php?sw_estado=3'}),
  ...
  var ds_unidad_organizacional = new Ext.data.Store({proxy: new Ext.data.HttpProxy({url: direccion+'../../../../sis_kardex_personal/control/unidad_organizacional/ActionListarUnidadOrganizacional.php'}),
	...
	Atributos[0]={
		...
		validacion:{
			...
			loadMask:true,
			triggerAction:'all',
			//store:new Ext.data.SimpleStore({fields:['id','valor'],data:Ext.carta_conclusion_combo.clase_carta}),
...
// txt fecha_vence
 ...
 Atributos[14]={
		validacion:{
			...
			loadMask:true,
			triggerAction:'all',
			//store:new Ext.data.SimpleStore({fields:['id','valor'],data:Ext.carta_conclusion_combo.estado_carta}),
...
// txt tipo_carta
	...
	Atributos[15]={
		validacion:{
			...
			loadMask:true,
			triggerAction:'all',
			//store:new Ext.data.SimpleStore({fields:['id','valor'],data:Ext.carta_conclusion_combo.tipo_carta}),
	...
	var config={titulo_maestro:'carta_conclusion',grid_maestro:'grid-'+idContenedor};
	var layout_carta_conclusion=new DocsLayoutMaestroEP(idContenedor);
	layout_carta_conclusion.init(config);

	// INICIAMOS HERENCIA //
	
	this.pagina=Pagina;
	//-- pagina (array PARAMETROS DE CONFIGURACION, array DEFINICION DE ATRIBUTOS, SELECTION MODEL, DATA STORE, COLUM MODEL)
	this.pagina(paramConfig,Atributos,ds,layout_carta_conclusion,idContenedor);
...
/*function btn_carta_docs(){
		...
		if(NumSelect!=0){
			...
			layout_carta_conclusion.loadWindows(direccion+'../../../../sis_tesoreria/vista/carta_docs/carta_docs.php?'+data,'Documentos por Carta',ParamVentana);
	...
	txt_ep=ClaseMadre_getComponente('id_fina_regi_prog_proy_acti');
	//alert('hh '+txt_ep);
...
function InitPaginaCartaRegistro()
	...
	this.getLayout=function(){return layout_carta_conclusion.getLayout()};
	...
	layout_carta_conclusion.getLayout().addListener('layout',this.onResize);//aregla la forma en que se ve el grid dentro del layout
