function EstadoSolicitudes(idContenedor,direccion,paramConfig,configConsolidacion)
{
	...
	var ds_usuario = new Ext.data.Store({proxy: new Ext.data.HttpProxy({url: direccion+'../../../../sis_seguridad/control/usuario/ActionListarUsuarioVista.php'}),
	...
	vectorAtributos[0] = { 
		validacion: {
			...
			store: new Ext.data.SimpleStore({
				fields: ['ID', 'valor'],
				data : Ext.estado_solicitud_combo.tipo_reporte
			}),
	...
	// txt tipo_pres  
	 vectorAtributos[1]  = {
		validacion: {
			...
			store: new Ext.data.SimpleStore({
				fields: ['ID', 'valor'],
				data :Ext.estado_solicitud_combo.estado_solicitud // from states.js
			}),
			valueField:'ID',
			displayField:'valor',
	...
	{			
		...
		componentes[1].on('select',evento_estado_solicitud);

		componentes[4].on('select',evento_departamento);		//departamento	
		componentes[5].on('select',evento_id_usuario);	
		componentes[6].on('select',evento_id_unidad_organizacional);
		componentes[7].on('select',evento_id_empleado);
	...
	{
		g_tipo_reporte=componentes[0].getValue();
		g_estado_solicitud=componentes[1].getValue();		
	}	

	function evento_id_empleado( combo, record, index )
	{
		g_id_empleado=componentes[7].getValue();
