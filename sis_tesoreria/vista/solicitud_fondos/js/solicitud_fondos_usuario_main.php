	...
	function btn_finalizar_solicitud(){
		...
		if(NumSelect!=0){
			...
			if(SelectionsRecord.data.saldo!=''){
		        ...
		        Ext.MessageBox.alert('Estado','No se pudo finalizar la solicitud debido a que no se emitió ningun cheque o no se tiene ningún descargo validado')		
			}			
		}
	   else{
	...
	function btn_ampliacion_fondos(){
		...
		if(NumSelect!=0){	
			if(SelectionsRecord.data.estado_avance==3 || SelectionsRecord.data.id_caja >= 1){
				Ext.MessageBox.alert('Estado', 'Solo se pueden ampliar fondos que se entregarán por cheque y no por efectivo.');
				...
				layout_solicitud_fondos_usuario.loadWindows(direccion+'../../../../sis_tesoreria/vista/ampliacion_fondos_avance/ampliacion_fondos_avance.php?'+data,'Ampliación de Fondos en Avance',ParamVentana);				
			}
		}
		else{
...
function btn_descargo(){
	...
	this.AdicionarBoton('../../../lib/imagenes/list-items.gif','Ampliación de Fondos',btn_ampliacion_fondos,true,'ampliacion_fondos','Ampliación de Fondos');			
