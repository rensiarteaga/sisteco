...
function pagina_devengar_pagar_det(idContenedor,direccion,paramConfig,idContenedorPadre,idUsuario){
	...
	var tpl_cuenta_bancaria=new Ext.Template('<div class="search-item">','<b><i>{desc_institucion}</i></b>','<br><FONT COLOR="#B5A642"><b>Nro. Cuenta: </b></FONT>{nro_cuenta_banco}','<br><FONT COLOR="B5A642">Gesti�n: </FONT>{gestion}','</div>');
	...
	this.reload=function(params){
		...
		// Verifica el t�tulo del bot�n de Desembolso
		...
		// Deshabilita en cualquier caso el bot�n de Documentos
	...
	function f_imprimir_cheque(){
		...
		if(sm.getCount()!=0){
			...
			if(SelectionsRecord.data.id_cheque!=''){
				...
				Ext.MessageBox.alert('Informaci�n','Cheque no generado');
			}
		}
		else{
	...
	function f_exito_impresion_cheque(resp){
		...
		// alert('Impresi�n del cheque: '+data);
	...
	function cont_pago(resp){
		...
		if(v_error==0){
			...
			Ext.MessageBox.alert('Informaci�n', 'No se pudo imprimir el Cheque, comun�quese con personal de Sistemas.');
		}
	}
			...
			Ext.Ajax.request({
				...
				success: function(resp){
					...
					if(v_tipo_pago=='pago'){
						...
						//Llamar a funci�n para Generaci�n del comprobante
						//alert('PAGO: Registro del tipo de documento y Generaci�n del comprobante');
						...
						if(v_tipo_doc=='proforma'){
							...
							//Llamar a funci�n para Generaci�n del comprobante
							//alert('DEVENGADO: Registro del tipo de documento y Generaci�n del comprobante');
						} else if(v_tipo_doc=='proforma_reg'){
							//Desplegar formulario para registrar el tipo de documento para regularizaci�n de la proforma
							...
							//Llamar a funci�n para Generaci�n del comprobante
							//alert('Generaci�n directamente del comprobante');
						}
		...
		if(v_existe_doc == 1){
			...
			if(NumSelect!=0){
				...
				if(SelectionsRecord.data.id_comprobante!=''){
					Ext.MessageBox.alert('Informaci�n','Pago ya contabilizado')
				}
				else{
	 ...
	 * NumSelect=sm.getCount(); if(NumSelect!=0){ alert('Esta opci�n est�
	 ...
	 * registrarlas por el total del pago realizado.'); if(confirm('�Est� seguro
	 * de continuar?')){ var SelectionsRecord=sm.getSelected();
	 * if(SelectionsRecord.data.id_comprobante!=''){
	 * Ext.MessageBox.alert('Informaci�n','Pago ya contabilizado') } else{
	...
	function btn_ajustar(){
		if(confirm("�Est� seguro de realizar el Ajuste? \n\nNota: El Ajuste se lo realiza sobre el �ltimo registro.")){
	...
	function btn_devengado_dcto(){
		...
		if(NumSelect!=0){
			...
			// Verifica si se podr� o no modificar el Tipo de Documento
	...
	function btn_rec_pag(){
		...
		if(NumSelect!=0){
			...
			if(SelectionsRecord.data.estado_devengado==11||SelectionsRecord.data.estado_devengado==17||SelectionsRecord.data.estado_devengado==12){
				...
				Ext.MessageBox.alert('Informaci�n','El pago debe estar contabilizado');
			}
		} else{
			Ext.MessageBox.alert('Informaci�n','Debe seleccionar un registro');
		}
	}
	...
	this.AdicionarBoton('../../../lib/imagenes/print.gif','Impresi�n del Cheque',f_imprimir_cheque,false,'btn_cheque','Imprimir Cheque');
	...
	// Define el t�tulo de Bot�n de Desembolso
