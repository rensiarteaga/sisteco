...
function pagina_devengar_servicios(idContenedor,direccion,paramConfig,tipoFormDev){
	...
	var paramFunciones={
		...
		grupos:[{tituloGrupo:'Datos',columna:0,id_grupo:0},{tituloGrupo:'Tipo Pago',columna:0,id_grupo:1},{tituloGrupo:'Datos Caja',columna:0,id_grupo:2},{tituloGrupo:'Cuenta Bancaria',columna:0,id_grupo:3}]}
	};
		...
		if(selected&&record!=-1){ //reg_desem, fin_sol
			if(tipoFormDev=='dev'){
				...
				if(record.estado_devengado==7){
					CM_getBoton('pag_ef-'+idContenedor).disable();
					CM_getBoton('docs-'+idContenedor).hide();
					//CM_getBoton('fin_dev_pag-'+idContenedor).disable();
				}else{
	...
	function evento_parametro( combo, record, index ){		
		...
		cmbConceptoIngas.store.baseParams={id_gestion:record.data.id_gestion};
	...
	function btn_fin_sol(){
		...
		if(sm.getCount()!=0) {
			...
			if(confirm('¿Está seguro de Finalizar la Solicitud?')){
					Ext.Ajax.request({
						url:direccion+'../../../../sis_tesoreria/control/devengado/ActionFinalizarSolicitudDevengado.php',
						method:'POST',
						params:{cantidad_ids:'1',id_devengado:id_devengado},
