<?php
/**
 * Nombre:		  	    devengar_pagar_det_main.php
 * Propósito: 			pagina que arranca la configuracion de la vista del Hijo
 * Autor:				RCM
 * Fecha creación:		28/10/2008
 *
 */
session_start();
?>
...
function main(){
	...
	echo 'idUsuario='.$_SESSION["ss_id_usuario"].';';
	?>
	var paramConfig={TamanoPagina:_CP.getConfig().ss_tam_pag,TiempoEspera:_CP.getConfig().ss_tiempo_espera,CantFiltros:1,FiltroEstructura:false,FiltroAvanzado:fa};
	idContenedorPadre='<?php echo $idContenedorPadre;?>';
	var elemento={idContenedor:idContenedor,pagina:new pagina_devengar_pagar_det(idContenedor,direccion,paramConfig,idContenedorPadre,idUsuario)};
...
function pagina_devengar_pagar_det(idContenedor,direccion,paramConfig,idContenedorPadre,idUsuario){
	...
	var ds_cuenta_bancaria = new Ext.data.Store({proxy: new Ext.data.HttpProxy({url: direccion+'../../../../sis_tesoreria/control/cuenta_bancaria/ActionListarCuentaBancaria.php'}),
    	reader:new Ext.data.XmlReader({record:'ROWS',id:'id_cuenta_bancaria',totalRecords: 'TotalCount'},['id_cuenta_bancaria','id_institucion','desc_institucion','id_cuenta','desc_cuenta','nro_cheque','estado_cuenta','nro_cuenta_banco','gestion']),baseParams:{m_sw_combo:'combo'}
    });
	...
	var tpl_cuenta_bancaria=new Ext.Template('<div class="search-item">','<b><i>{desc_institucion}</i></b>','<br><FONT COLOR="#B5A642"><b>Nro. Cuenta: </b></FONT>{nro_cuenta_banco}','<br><FONT COLOR="B5A642">Gestiï¿½n: </FONT>{gestion}','</div>');
	...
	Atributos[25]={
			validacion:{
				...
				queryParam:'filterValue_0',
				filterCol:'CUEBAN.nro_cuenta_banco#INSTIT.nombre',
				typeAhead:false,
	...
	Atributos[28]={
			validacion:{
				...
				fieldLabel:'Importe No Valido C.F.',
				align:'right',
				grid_visible:true,
				grid_indice:4,
				width_grid:130
	...
	this.reload=function(params){
		...
		// Verifica el tï¿½tulo del botï¿½n de Desembolso
		...
		// Deshabilita en cualquier caso el botï¿½n de Documentos
			...
			if(maestro.tipoFormDev=='fin'){
				...
				if(record.estado_devengado==9){ 
					...
					if(record.sw_solo_devengado=='si'){
						...
						//MZM 24/05/2012: pagos proenientes de planilla no generan cbte de pago
					if(parseFloat(record.id_planilla)>0 &&parseFloat(record.id_cotizacion)>0){
						CM_getBoton('btn_cont_pago-'+idContenedor).disable();
					} else{
					...
					CM_getBoton('btn_docs-'+idContenedor).hide();
					v_dcto_solo_lectura=1;
					if(record.id_comprobante!=''&& record.tipo_plantilla==16&&record.id_comprobante_reg==''){
						CM_getBoton('btn_cont_pago-'+idContenedor).enable();
					} else{
						CM_getBoton('btn_cont_pago-'+idContenedor).disable();
					}
	...
	function f_imprimir_cheque(){
		...
		if(sm.getCount()!=0){
			...
			if(SelectionsRecord.data.id_cheque!=''){
				...
				Ext.MessageBox.alert('Informaciï¿½n','Cheque no generado');
			}
		}
		else{
	...
	function f_exito_impresion_cheque(resp){
		...
		// alert('Impresiï¿½n del cheque: '+data);
	...
	function cont_pago(resp){
		...
		if(v_error==0){
			...
			Ext.MessageBox.alert('Informaciï¿½n', 'No se pudo imprimir el Cheque, comunï¿½quese con personal de Sistemas.');
		}
	}
			...
			Ext.Ajax.request({
				...
				success: function(resp){
					...
					if(v_tipo_pago=='pago'){
						...
						//Llamar a funciï¿½n para Generaciï¿½n del comprobante
						//alert('PAGO: Registro del tipo de documento y Generaciï¿½n del comprobante');
						...
						if(v_tipo_doc=='proforma'){
							...
							//Llamar a funciï¿½n para Generaciï¿½n del comprobante
							//alert('DEVENGADO: Registro del tipo de documento y Generaciï¿½n del comprobante');
						} else if(v_tipo_doc=='proforma_reg'){
							//Desplegar formulario para registrar el tipo de documento para regularizaciï¿½n de la proforma
							RegistrarTipoDocumento('prof_reg');
						}
						else{
							RegistrarTipoDocumento('dev_doc');
							
							//alert("no se muestra formulario v_tipo_pago="+v_tipo_pago)
							//Llamar a funciï¿½n para Generaciï¿½n del comprobante
							//alert('Generaciï¿½n directamente del comprobante');
						}
		...
		if(v_existe_doc == 1){
			...
			if(NumSelect!=0){
				...
				if(SelectionsRecord.data.id_comprobante!=''){
					Ext.MessageBox.alert('Informaciï¿½n','Pago ya contabilizado')
				}
				else{
	 ...
	 * NumSelect=sm.getCount(); if(NumSelect!=0){ alert('Esta opciï¿½n estï¿½
	 ...
	 * registrarlas por el total del pago realizado.'); if(confirm('ï¿½Estï¿½ seguro
	 * de continuar?')){ var SelectionsRecord=sm.getSelected();
	 * if(SelectionsRecord.data.id_comprobante!=''){
	 * Ext.MessageBox.alert('Informaciï¿½n','Pago ya contabilizado') } else{
	...
	function btn_ajustar(){
		if(confirm("ï¿½Estï¿½ seguro de realizar el Ajuste? \n\nNota: El Ajuste se lo realiza sobre el ï¿½ltimo registro.")){
	...
	function btn_rec_pag(){
		...
		if(NumSelect!=0){
			...
			if(SelectionsRecord.data.estado_devengado==11||SelectionsRecord.data.estado_devengado==17||SelectionsRecord.data.estado_devengado==12){
				...
				Ext.MessageBox.alert('Informaciï¿½n','El pago debe estar contabilizado');
			}
		} else{
			Ext.MessageBox.alert('Informaciï¿½n','Debe seleccionar un registro');
		}
	}
	...
	this.AdicionarBoton('../../../lib/imagenes/print.gif','Impresiï¿½n del Cheque',f_imprimir_cheque,false,'btn_cheque','Imprimir Cheque');
	...
	// Define el tï¿½tulo de Botï¿½n de Desembolso
