<?php 
/**
 * Nombre:		  	    devengar_servicios_main.php
 * Propósito: 			pagina que arranca la configuracion de la vista
 * Autor:				Generado Automaticamente
 * Fecha creación:		2008-10-21 15:43:27
 *
 */
session_start();
?>
...
function main(){
	...
	echo "var tipoFormDev='$tipoFormDev';"
	?>
	var fa=false;
	var paramConfig={TamanoPagina:_CP.getConfig().ss_tam_pag,TiempoEspera:_CP.getConfig().ss_tiempo_espera,CantFiltros:1,FiltroEstructura:false,FiltroAvanzado:fa};
	var elemento={pagina:new pagina_devengar_servicios(idContenedor,direccion,paramConfig,tipoFormDev),idContenedor:idContenedor};
...
function pagina_devengar_servicios(idContenedor,direccion,paramConfig,tipoFormDev){
	...
	var tpl_id_periodo_subsistema=new Ext.Template('<div class="search-item">','<b>Gestión: </b><FONT COLOR="#B5A642">{gestion}</FONT><br>','<b>Periodo: </b><FONT COLOR="#B5A642">{desc_periodo_subsistema}</FONT>','<b> - </b><FONT COLOR="#B5A642">{estado_periodo}</FONT>','</div>');
	...
	var paramFunciones={
		...
		Formulario:{html_apply:'dlgInfo-'+idContenedor,height:'60%',width:500,minWidth:150,minHeight:200,	closable:true,titulo:'Devengar Servicios',
	...
	this.EnableSelect = function(sm,row,rec){
		...
		/**************************/ //04.11.2014 MZM: adicion de validacion para evitar error de js
		if(tipoFormDev=='pag'||tipoFormDev=='fin'||tipoFormDev=='aprob'||tipoFormDev=='desc'){
		}else{
				if(rec.data['estado_devengado'] > 3){
		...
		/**************************/ //04.11.2014 MZM:fin adicion de validacion para evitar error de js
		enable(sm,row,rec);
	}
	...
	function exito_edit(resp){
		...
		var regParam = cmbParametro.store.getById(cmbParametro.getValue());
		//alert('tot: '+v_tot;
		if(v_tot>0){
			if(confirm("El Prorrateo ya fue registrado. Si modifica el Importe, todo el prorrateo será eliminado.\n\n ¿Está seguro de continuar?")){
				...
				if(SelectionsRecord.data.tipo_desembolso==2){
					...
					getComponente('id_caja').store.baseParams={estado_repo:1};
					getComponente('id_caja').modificado=true;
		...
		cmbConceptoIngas.store.baseParams={sw_tesoro:6,id_gestion:regParam.data.id_gestion};
		cmbConceptoIngas.modificado=true;									
		
		cmbProveedor.store.baseParams={id_gestion:regParam.data.id_gestion};
		cmbProveedor.modificado=true;			
		
		cmbPeriodoSubsis.store.baseParams={tesoro:1,m_id_subsistema:12,m_estado_periodo:'abierto',id_gestion:regParam.data.id_gestion};
	...
	function iniciarEventosFormularios(){
		...
		cmbParametro.on('blur',evento_parametro);
