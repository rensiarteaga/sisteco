...
function pagina_usuario(idContenedor,direccion,paramConfig)
{
	...
	ds = new Ext.data.Store({
		...
		ds_nivel_seguridad = new Ext.data.Store({proxy: new Ext.data.HttpProxy({url: direccion+'../../../sis_seguridad/control/nivel_seguridad/ActionListarNivelSeguridad.php'}),
		reader: new Ext.data.XmlReader({
			record: 'ROWS',
...
/*
		...
		*/
		...
		var paramFunciones = {
			btnEliminar:{url:direccion+'../../../sis_seguridad/control/usuario/ActionEliminarUsuario.php'},
			Save:{url:direccion+'../../../sis_seguridad/control/usuario/ActionGuardarUsuario.php',
			success:miFuncionSuccess},
			ConfirmSave:{url:direccion+'../../../sis_seguridad/control/usuario/ActionGuardarUsuario.php'},
			...
			function btn_usuario_rol(){
				...
				if(NumSelect!=0){
					...
					var data='id_usuario='+SelectionsRecord.data.id_usuario;
					data=data+'&id_persona='+SelectionsRecord.data.id_persona;
					data=data+'&apellido_paterno='+SelectionsRecord.data.desc_persona;
					data=data+'&apellido_materno='+SelectionsRecord.data.login;
					data=data+'&nombre='+SelectionsRecord.data.fecha_registro;

					var ParamVentana={ventana:{width:'90%',height:'70%'}}
					
					layout_usuario.loadWindows(direccion+'../../../sis_seguridad/vista/usuario_rol/usuario_rol_det.php?'+data,'Usuario Rol',ParamVentana,'pagina_usuario_rol_det');
			...
			function btn_usuario_lugar(){
				...
				if(NumSelect!=0){
					...
					var ParamVentana={ventana:{width:'90%',height:'70%'}}
					layout_usuario.loadWindows(direccion+'../../../sis_seguridad/vista/usuario_lugar/usuario_lugar_det.php?txt_usuario=0&'+data,'Usuario Lugar',ParamVentana);
					layout_usuario.getVentana().on('resize',function(){
						layout_usuario.getLayout().layout()
					})
				}
			...
			function btn_usuario_asignacion(){
				...
				if(NumSelect!=0){
					...
					layout_usuario.loadWindows(direccion+'../../../sis_seguridad/vista/usuario_asignacion/usuario_asignacion_det.php?'+data,'Usuario Asignación',ParamVentana);
					layout_usuario.getVentana().on('resize',function(){
						layout_usuario.getLayout().layout()
					})
				}
			...
			function btn_usuario_sincronizar(){
				...
				if(NumSelect!=0){
					...
					Ext.Ajax.request({
						url:direccion+"../../../sis_seguridad/control/usuario/ActionSincronizarEPUsuarioEmpleado.php",
			...
			function btn_hist_clave(){
				...
				if(NumSelect!=0){
					...
					data=data+'&m_nombre='+SelectionsRecord.data.fecha_registro;
					var ParamVentana={ventana:{width:'90%',height:'70%'}}
					layout_usuario.loadWindows(direccion+'../../../sis_seguridad/vista/hist_clave/hist_clave_det.php?'+data,'Historico Contrasenia',ParamVentana);
					layout_usuario.getVentana().on('resize',function(){
						layout_usuario.getLayout().layout()
					})
				}
			...
			/*function btn_preferencia_usuario(){
			var sm=getSelectionModel();var filas=ds.getModifiedRecords();var cont=filas.length;var NumSelect=sm.getCount();
			if(NumSelect!=0){
			var SelectionsRecord=sm.getSelected();
			var data='m_id_usuario='+SelectionsRecord.data.id_usuario;
			...
			var ParamVentana={ventana:{width:'90%',height:'70%'}}
			layout_usuario.loadWindows(direccion+'../../../vista/preferencia_usuario/preferencia_usuario_det.php?'+data,'Preferencias',ParamVentana);
			layout_usuario.getVentana().on('resize',function(){
			layout_usuario.getLayout().layout()
			})
			}
			...
			this.AdicionarBoton('../../lib/imagenes/menu-show.gif','Rol del Usuario',btn_usuario_rol,true,'usuario_rol','Usuario Rol');
			this.AdicionarBoton('../../lib/imagenes/menu-show.gif','Lugar del Usuario',btn_usuario_lugar,true,'usuario_lugar','Usuario Lugar');
			this.AdicionarBoton('../../lib/imagenes/menu-show.gif','Asignación de Estructura',btn_usuario_asignacion,true,'usuario_asignacion','Usuario Asignación');
			...
			this.AdicionarBoton('../../lib/imagenes/menu-show.gif','Historico Contraseñas',btn_hist_clave,true,'hist_clave','Historico Contrasenia');

			this.AdicionarBoton('../../lib/imagenes/menu-show.gif','Inactivar',btn_inactivar,true,'inactivar','Inactivar Cuenta');

			
			//this.AdicionarBoton('../../lib/imagenes/menu-show.gif','Preferencias',btn_preferencia_usuario,true,'preferencia_usuario','Preferencia de Usuario');
