...
//			
//			treeL.baseParams.id=n.attributes.id;
...
//		}, this);
		...
		/*getTreePanel().on('beforenodedrop', function(e){
			...
			if(n.getOwnerTree()==treLoader){//proviene del mismo arbol?
				//e.target.expand();//expandimos el nodo antes de buscar elementos repetidos
				if(!e.target.findChild('id',n.id)&&e.point!='append'){
					...
					var  nodo={};
					nodo['id_lugar']=copy.attributes.id;
					nodo['id_nuevo_lugar']= e.target.id;
					nodo['padre_nuevo']=e.target.attributes.fk_id_lugar
					copy.attributes.id_p=e.target.id;
					nodo['tipo']='hoja';
					nodo['codigo']=cmpCodigo.getValue();
					
					alert('nodo a mover'+copy.attributes.id);
					alert("encima/abajo de"+e.target.id);
					alert('padre'+e.target.attributes.fk_id_lugar);
					
					var postData='datos='+encodeURIComponent(Ext.encode(nodo))+'&proc=dd';
...
//		getTreePanel().on('dblclick',function(n,e){
//			alert("fafsfsd");
//			var nodo=getSm().getSelectedNode();
//			alert(nodo.attributes.id);
//			btnEdit();
//			
//		});
