...
/**
**********************************************************
Nombre de archivo:	    ActionEliminarSubsistema.php
Propósito:				Permite eliminar registros de la tabla tsg_subsistema
...
$nombre_archivo = "ActionEliminarProcedimiento.php";
...
if($_SESSION["autentificado"]=="SI")
{
	...
	for($j = 0;$j < $cont; $j++)
	{
		...
		if ($codigo_procedimiento == "undefined" || $codigo_procedimiento== "")
		{
			$resp = new cls_manejo_mensajes(true, "406");
			$resp->mensaje_error = "MENSAJE ERROR = No existe el registro especificado para eliminar $codigo_procedimiento.";
			$resp->origen = "ORIGEN = $nombre_archivo";
			$resp->proc = "PROC = $nombre_archivo";
			$resp->nivel = "NIVEL = 4";
			echo $resp->get_mensaje();
		...
		{	//Eliminación
			$res = $Custom->  EliminarProcedimiento_db($codigo_procedimiento);
			if(!$res)
			{
				$resp = new cls_manejo_mensajes(true, "406");
