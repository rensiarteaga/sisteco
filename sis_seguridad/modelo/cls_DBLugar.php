...
class cls_DBLugar
{
	...
	function ListarLugar($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad)
	{
		...
		$this->var->add_def_cols('sigla_sigma','varchar');
...
/*	    echo $this->query;
	...
	function InsertarLugar($id_lugar,$fk_id_lugar,$nivel,$codigo,$nombre,$ubicacion,$telefono1,$telefono2,$fax,$observacion,$sw_municipio,$sw_impuesto,$prioridad_kard,$sigla_sigma)
	{
		...
		$this->var->add_param("'$prioridad_kard'");
		$this->var->add_param("'$sigla_sigma'");
		//Ejecuta la función
...
//		exit;
	...
	function DropLugarHoja($id_lugar,$fk_id_lugar,$tipo)
	{
		...
		$this->var->add_param("NULL");//sw_municipio
		$this->var->add_param("NULL");//prioridad_kard
		$this->var->add_param("NULL");//sigla_sigma
		//Ejecuta la función
		$res = $this->var->exec_non_query();
	 ...
	 * Fecha de creación:		2007-10-25 16:40:30
	 */
	function ModificarLugar($id_lugar,$fk_id_lugar,$nivel,$codigo,$nombre,$ubicacion,$telefono1,$telefono2,$fax,$observacion,$sw_municipio,$sw_impuesto,$prioridad_kard,$sigla_sigma)
	{
		$this->salida = "";
		$this->nombre_funcion = 'f_tsg_lugar_iud';
		...
		$this->var->add_param("'$observacion'");
		$this->var->add_param("'$sw_municipio'");
		$this->var->add_param("'$sw_impuesto'");//sw_municipio
		$this->var->add_param("'$prioridad_kard'");//prioridad_kard
		$this->var->add_param("'$sigla_sigma'");//sigla_sigma
		//Ejecuta la función
		$res = $this->var->exec_non_query();
