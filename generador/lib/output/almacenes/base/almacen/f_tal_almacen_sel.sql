	...
	CREATE OR REPLACE FUNCTION "public"."f_tal_almacen_sel" (
...
***************************************************************************
 SCRIPT: 		f_tal_almacen
 DESCRIPCIÓN: 	Devuelve las consultas a la tabla tal_almacen
...
BEGIN
    ...
    IF pm_codigo_procedimiento  = 'AL_ALMACE_SEL' THEN
        BEGIN
            g_consulta := 'SELECT 
							ALMACE.id_almacen,
							ALMACE.codigo,
							...
							ALMACE.bloquear,
							ALMACE.cerrado,
							ALMACE.nro_prest_pendientes,
							ALMACE.nro_ing_no_finalizados,
							ALMACE.nro_sal_no_finalizadas,
							ALMACE.observaciones,
							ALMACE.fecha_ultimo_inventario,
							ALMACE.fecha_reg,
							ALMACE.id_regional,
							REGION.codigo_regional as desc_regional

						  FROM tal_almacen ALMACE
						  
	  		 INNER JOIN tpm_regional REGION
             ON REGION.id_regional=ALMACE.id_regional
...
/*            -- SE AUMENTA LAS RESTRICCIONES DE LA ESTRUCTURA PROGRAMÁTICA
    ...
    ELSIF pm_codigo_procedimiento  = 'AL_ALMACE_COUNT' THEN

        BEGIN
        --Cuenta todos los registros
            g_consulta := 'SELECT
                          COUNT(ALMACE.id_almacen) AS total
                          FROM tal_almacen ALMACE
