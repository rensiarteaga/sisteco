	...
	CREATE OR REPLACE FUNCTION "public"."f_tal_almacen_sector_sel" (
...
***************************************************************************
 SCRIPT: 		f_tal_almacen_sector
 DESCRIPCIÓN: 	Devuelve las consultas a la tabla tal_almacen_sector
 AUTOR: 		(Generado Automaticamente)
 FECHA:			2007-10-11 16:17:00
 COMENTARIOS:	
***************************************************************************
...
BEGIN
    ...
    g_nombre_funcion := 'f_tal_almacen_sector_sel';
    ...
    IF pm_codigo_procedimiento  = 'AL_ALMSEC_SEL' THEN
        BEGIN
            g_consulta := 'SELECT 
							ALMSEC.id_almacen_sector,
							ALMSEC.superficie,
							ALMSEC.altura,
							ALMSEC.via_fil,
							ALMSEC.via_col,
							...
							ALMSEC.aire_acond,
							ALMSEC.fecha_reg,
							ALMSEC.id_tipo_sector,
							TIPSEC.codigo as desc_tipo_sector,
							ALMSEC.id_almacen,
							ALMACE.codigo as desc_almacen

						  FROM tal_almacen_sector ALMSEC
						  
	  		 INNER JOIN tal_tipo_sector TIPSEC
             ON TIPSEC.id_tipo_sector=ALMSEC.id_tipo_sector

	  		 INNER JOIN tal_almacen ALMACE
             ON ALMACE.id_almacen=ALMSEC.id_almacen
...
/*            -- SE AUMENTA LAS RESTRICCIONES DE LA ESTRUCTURA PROGRAMÁTICA
    ...
    ELSIF pm_codigo_procedimiento  = 'AL_ALMSEC_COUNT' THEN

        BEGIN
        --Cuenta todos los registros
            g_consulta := 'SELECT
                          COUNT(ALMSEC.id_almacen_sector) AS total
                          FROM tal_almacen_sector ALMSEC
