	...
	CREATE OR REPLACE FUNCTION "public"."f_tal_item_ubicacion_iud" (
		pm_id_usuario integer,
		pm_ip_origen varchar,
		pm_mac_maquina macaddr,
		pm_codigo_procedimiento varchar,
		pm_proc_almacenado varchar,
		al_id_item_ubicacion  int4,
		al_nivel  int4,
...
***************************************************************************
 SCRIPT: 		f_tal_item_ubicacion_iud
 DESCRIPCIÓN: 	Permite registrar en la tabla tal_item_ubicacion
 AUTOR: 		(generado automaticamente)
 FECHA:			2007-10-11 16:18:27
 COMENTARIOS:	
***************************************************************************
...
---*** INICIACIÓN DE VARIABLES 
    ...
    IF pm_codigo_procedimiento = 'AL_ITEUBI_INS' THEN

        BEGIN
        	
            INSERT INTO tal_item_ubicacion(
		nivel,
		descripcion,
		observaciones,
		fecha_reg,
		...
		al_id_estante
            ...
            g_descripcion_log_error := 'Registro exitoso en tal_item_ubicacion';
  ...
  --procedimiento de modificacion      
   ...
   ELSIF pm_codigo_procedimiento = 'AL_ITEUBI_UPD' THEN

        BEGIN
            --VERIFICA EXISTENCIA DEL REGISTRO
            IF NOT EXISTS(SELECT 1 FROM tal_item_ubicacion
                          WHERE tal_item_ubicacion.id_item_ubicacion=al_id_item_ubicacion) THEN
                              
                g_descripcion_log_error := 'Modificación no realizada: no existe el registro de tal_item_ubicacionno existente';
                g_nivel_error := '4';
                g_respuesta := f_pm_mensaje_error(g_descripcion_log_error, g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
            ...
            UPDATE tal_item_ubicacion SET 
		nivel=al_nivel,
		descripcion=al_descripcion,
		observaciones=al_observaciones,
		fecha_reg=al_fecha_reg,
		id_item=al_id_item,
		id_estante=al_id_estante

				WHERE tal_item_ubicacion.id_item_ubicacion= al_id_item_ubicacion;

            -- DESCRIPCIÓN DE ÉXITO PARA GUARDAR EN EL LOG
            g_descripcion_log_error := 'Modificación exitosa en tal_item_ubicacion';
            g_respuesta := 't'||g_separador||g_descripcion_log_error;

        END;
        
        ELSIF pm_codigo_procedimiento = 'AL_ITEUBI_DEL' THEN
    ...
    BEGIN
            ...
            IF NOT EXISTS(SELECT 1 FROM tal_item_ubicacion
                ...
                g_nivel_error := '4';
                g_descripcion_log_error := 'Eliminación no realizada: registro en tal_item_ubicacion inexistente';
                g_respuesta := f_pm_mensaje_error(g_descripcion_log_error, g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
                RETURN 'f'||g_separador||g_respuesta;
         ...
         --   IF EXISTS(SELECT 1 FROM tal_item_ubicacion
         --            INNER JOIN tal_id1 ON tal_item_ubicacion.id_subgrupo = tal_id1.id_subgrupo
         --            WHERE tal_item_ubicacion.id_item_ubicacion = al_id_item_ubicacion) THEN
         --            
         --       g_nivel_error := '4';
         --       g_descripcion_log_error := 'Eliminación no realizada: El registro en tal_item_ubicacion tiene regitros asociados en XXX';
         --       g_respuesta := f_pm_mensaje_error(g_descripcion_log_error, g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
         --      RETURN 'f'||g_separador||g_respuesta;
         ...
         -- BORRADO DE DATO
            DELETE FROM tal_item_ubicacion WHERE tal_item_ubicacion.id_item_ubicacion = al_id_item_ubicacion;

            -- DESCRIPCIÓN DE ÉXITO PARA GUARDAR EN EL LOG
            g_descripcion_log_error := 'Eliminación exitosa del registro en tal_item_ubicacion';
            g_respuesta := 't'||g_separador||g_descripcion_log_error;
        END;
