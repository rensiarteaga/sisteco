	...
	CREATE OR REPLACE FUNCTION "public"."f_tal_tipo_sector_sg_iud" (
		pm_id_usuario integer,
		pm_ip_origen varchar,
		pm_mac_maquina macaddr,
		pm_codigo_procedimiento varchar,
		pm_proc_almacenado varchar,
		al_id_tal_tipo_sector_sg  int4,
		al_id_tipo_sector  int4,
		al_id_supergrupo  int4
)
		RETURNS varchar AS
$body$
...
***************************************************************************
 SCRIPT: 		f_tal_tipo_sector_sg_iud
 DESCRIPCIÓN: 	Permite registrar en la tabla tal_tipo_sector_sg
 AUTOR: 		(generado automaticamente)
 FECHA:			2007-10-11 16:16:51
 COMENTARIOS:	
***************************************************************************
...
---*** INICIACIÓN DE VARIABLES 
    ...
    IF pm_codigo_procedimiento = 'AL_TISESG_INS' THEN

        BEGIN
        	
            INSERT INTO tal_tipo_sector_sg(
		id_tipo_sector,
		id_supergrupo
		        ) VALUES (
		 al_id_tipo_sector,
		al_id_supergrupo
            ...
            g_descripcion_log_error := 'Registro exitoso en tal_tipo_sector_sg';
  ...
  --procedimiento de modificacion      
   ...
   ELSIF pm_codigo_procedimiento = 'AL_TISESG_UPD' THEN

        BEGIN
            --VERIFICA EXISTENCIA DEL REGISTRO
            IF NOT EXISTS(SELECT 1 FROM tal_tipo_sector_sg
                          WHERE tal_tipo_sector_sg.id_tal_tipo_sector_sg=al_id_tal_tipo_sector_sg) THEN
                              
                g_descripcion_log_error := 'Modificación no realizada: no existe el registro de tal_tipo_sector_sgno existente';
                g_nivel_error := '4';
                g_respuesta := f_pm_mensaje_error(g_descripcion_log_error, g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
            ...
            UPDATE tal_tipo_sector_sg SET 
		id_tipo_sector=al_id_tipo_sector,
		id_supergrupo=al_id_supergrupo

				WHERE tal_tipo_sector_sg.id_tal_tipo_sector_sg= al_id_tal_tipo_sector_sg;

            -- DESCRIPCIÓN DE ÉXITO PARA GUARDAR EN EL LOG
            g_descripcion_log_error := 'Modificación exitosa en tal_tipo_sector_sg';
            g_respuesta := 't'||g_separador||g_descripcion_log_error;

        END;
        
        ELSIF pm_codigo_procedimiento = 'AL_TISESG_DEL' THEN
    ...
    BEGIN
            ...
            IF NOT EXISTS(SELECT 1 FROM tal_tipo_sector_sg
                ...
                g_nivel_error := '4';
                g_descripcion_log_error := 'Eliminación no realizada: registro en tal_tipo_sector_sg inexistente';
                g_respuesta := f_pm_mensaje_error(g_descripcion_log_error, g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
                RETURN 'f'||g_separador||g_respuesta;
         ...
         --   IF EXISTS(SELECT 1 FROM tal_tipo_sector_sg
         --            INNER JOIN tal_id1 ON tal_tipo_sector_sg.id_subgrupo = tal_id1.id_subgrupo
         --            WHERE tal_tipo_sector_sg.id_tal_tipo_sector_sg = al_id_tal_tipo_sector_sg) THEN
         --            
         --       g_nivel_error := '4';
         --       g_descripcion_log_error := 'Eliminación no realizada: El registro en tal_tipo_sector_sg tiene regitros asociados en XXX';
         --       g_respuesta := f_pm_mensaje_error(g_descripcion_log_error, g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
         --      RETURN 'f'||g_separador||g_respuesta;
         ...
         -- BORRADO DE DATO
            DELETE FROM tal_tipo_sector_sg WHERE tal_tipo_sector_sg.id_tal_tipo_sector_sg = al_id_tal_tipo_sector_sg;

            -- DESCRIPCIÓN DE ÉXITO PARA GUARDAR EN EL LOG
            g_descripcion_log_error := 'Eliminación exitosa del registro en tal_tipo_sector_sg';
            g_respuesta := 't'||g_separador||g_descripcion_log_error;
        END;
