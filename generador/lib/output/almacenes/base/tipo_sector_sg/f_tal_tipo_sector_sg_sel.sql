	...
	CREATE OR REPLACE FUNCTION "public"."f_tal_tipo_sector_sg_sel" (
...
***************************************************************************
 SCRIPT: 		f_tal_tipo_sector_sg
 DESCRIPCIÓN: 	Devuelve las consultas a la tabla tal_tipo_sector_sg
...
BEGIN
    ...
    g_nombre_funcion := 'f_tal_tipo_sector_sg_sel';
    ...
    IF pm_codigo_procedimiento  = 'AL_TISESG_SEL' THEN
        BEGIN
            g_consulta := 'SELECT 
							TISESG.id_tal_tipo_sector_sg,
							TISESG.id_tipo_sector,
							TIPSEC.codigo as desc_tipo_sector,
							TISESG.id_supergrupo,
							SUPGRU.codigo as desc_supergrupo

						  FROM tal_tipo_sector_sg TISESG
						  
	  		 INNER JOIN tal_tipo_sector TIPSEC
             ON TIPSEC.id_tipo_sector=TISESG.id_tipo_sector

	  		 INNER JOIN tal_supergrupo SUPGRU
             ON SUPGRU.id_supergrupo=TISESG.id_supergrupo
...
/*            -- SE AUMENTA LAS RESTRICCIONES DE LA ESTRUCTURA PROGRAMÁTICA
    ...
    ELSIF pm_codigo_procedimiento  = 'AL_TISESG_COUNT' THEN

        BEGIN
        --Cuenta todos los registros
            g_consulta := 'SELECT
                          COUNT(TISESG.id_tal_tipo_sector_sg) AS total
                          FROM tal_tipo_sector_sg TISESG
