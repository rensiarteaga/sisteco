	...
	CREATE OR REPLACE FUNCTION "public"."f_tal_estante_sel" (
...
***************************************************************************
 SCRIPT: 		f_tal_estante
 DESCRIPCIÓN: 	Devuelve las consultas a la tabla tal_estante
 AUTOR: 		(Generado Automaticamente)
 FECHA:			2007-10-11 16:17:20
 COMENTARIOS:	
***************************************************************************
...
BEGIN
    ...
    g_nombre_funcion := 'f_tal_estante_sel';
    ...
    IF pm_codigo_procedimiento  = 'AL_ESTANT_SEL' THEN
        BEGIN
            g_consulta := 'SELECT 
							ESTANT.id_estante,
							ESTANT.codigo,
							...
							ESTANT.via_fil,
							ESTANT.via_col,
							ESTANT.estado_registro,
							ESTANT.fecha_reg,
							ESTANT.id_almacen_sector,
							ALMSEC.superficie as desc_almacen_sector

						  FROM tal_estante ESTANT
						  
	  		 INNER JOIN tal_almacen_sector ALMSEC
             ON ALMSEC.id_almacen_sector=ESTANT.id_almacen_sector
...
/*            -- SE AUMENTA LAS RESTRICCIONES DE LA ESTRUCTURA PROGRAMÁTICA
    ...
    ELSIF pm_codigo_procedimiento  = 'AL_ESTANT_COUNT' THEN

        BEGIN
        --Cuenta todos los registros
            g_consulta := 'SELECT
                          COUNT(ESTANT.id_estante) AS total
                          FROM tal_estante ESTANT
