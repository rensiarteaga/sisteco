	...
	CREATE OR REPLACE FUNCTION "public"."f_tal_motivo_ingreso_iud" (
		pm_id_usuario integer,
		pm_ip_origen varchar,
		pm_mac_maquina macaddr,
		pm_codigo_procedimiento varchar,
		pm_proc_almacenado varchar,
		al_id_motivo_ingreso  int4,
		al_nombre  varchar,
...
***************************************************************************
 SCRIPT: 		f_tal_motivo_ingreso_iud
 DESCRIPCIÓN: 	Permite registrar en la tabla tal_motivo_ingreso
 AUTOR: 		(generado automaticamente)
 FECHA:			2007-10-11 12:14:40
 COMENTARIOS:	
***************************************************************************
...
---*** INICIACIÓN DE VARIABLES 
    ...
    IF pm_codigo_procedimiento = 'AL_MOTING_INS' THEN

        BEGIN
        	
            INSERT INTO tal_motivo_ingreso(
		nombre,
		descripcion,
		estado_registro,
		fecha_reg,
		...
		al_id_cuenta
            ...
            g_descripcion_log_error := 'Registro exitoso en tal_motivo_ingreso';
  ...
  --procedimiento de modificacion      
   ...
   ELSIF pm_codigo_procedimiento = 'AL_MOTING_UPD' THEN

        BEGIN
            --VERIFICA EXISTENCIA DEL REGISTRO
            IF NOT EXISTS(SELECT 1 FROM tal_motivo_ingreso
                          WHERE tal_motivo_ingreso.id_motivo_ingreso=al_id_motivo_ingreso) THEN
                              
                g_descripcion_log_error := 'Modificación no realizada: no existe el registro de tal_motivo_ingresono existente';
                g_nivel_error := '4';
                g_respuesta := f_pm_mensaje_error(g_descripcion_log_error, g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
		...
		id_cuenta=al_id_cuenta
				...
				WHERE tal_motivo_ingreso.id_motivo_ingreso= al_id_motivo_ingreso;

            -- DESCRIPCIÓN DE ÉXITO PARA GUARDAR EN EL LOG
            g_descripcion_log_error := 'Modificación exitosa en tal_motivo_ingreso';
            g_respuesta := 't'||g_separador||g_descripcion_log_error;

        END;
        
        ELSIF pm_codigo_procedimiento = 'AL_MOTING_DEL' THEN
    ...
    BEGIN
            ...
            IF NOT EXISTS(SELECT 1 FROM tal_motivo_ingreso
                ...
                g_nivel_error := '4';
                g_descripcion_log_error := 'Eliminación no realizada: registro en tal_motivo_ingreso inexistente';
                g_respuesta := f_pm_mensaje_error(g_descripcion_log_error, g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
                RETURN 'f'||g_separador||g_respuesta;
         ...
         --   IF EXISTS(SELECT 1 FROM tal_motivo_ingreso
         --            INNER JOIN tal_id1 ON tal_motivo_ingreso.id_subgrupo = tal_id1.id_subgrupo
         --            WHERE tal_motivo_ingreso.id_motivo_ingreso = al_id_motivo_ingreso) THEN
         --            
         --       g_nivel_error := '4';
         --       g_descripcion_log_error := 'Eliminación no realizada: El registro en tal_motivo_ingreso tiene regitros asociados en XXX';
         --       g_respuesta := f_pm_mensaje_error(g_descripcion_log_error, g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
         --      RETURN 'f'||g_separador||g_respuesta;
         ...
         -- BORRADO DE DATO
            DELETE FROM tal_motivo_ingreso WHERE tal_motivo_ingreso.id_motivo_ingreso = al_id_motivo_ingreso;

            -- DESCRIPCIÓN DE ÉXITO PARA GUARDAR EN EL LOG
            g_descripcion_log_error := 'Eliminación exitosa del registro en tal_motivo_ingreso';
            g_respuesta := 't'||g_separador||g_descripcion_log_error;
        END;
