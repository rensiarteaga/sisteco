	...
	CREATE OR REPLACE FUNCTION "public"."f_tal_motivo_ingreso_sel" (
...
***************************************************************************
 SCRIPT: 		f_tal_motivo_ingreso
 DESCRIPCIÓN: 	Devuelve las consultas a la tabla tal_motivo_ingreso
...
BEGIN
    ...
    g_nombre_funcion := 'f_tal_motivo_ingreso_sel';
    ...
    IF pm_codigo_procedimiento  = 'AL_MOTING_SEL' THEN
        BEGIN
            g_consulta := 'SELECT 
							MOTING.id_motivo_ingreso,
							MOTING.nombre,
							MOTING.descripcion,
							MOTING.estado_registro,
							MOTING.fecha_reg,
							MOTING.id_cuenta,
							CUENTA. as desc_cuenta

						  FROM tal_motivo_ingreso MOTING
						  
	  		 INNER JOIN tct_cuenta CUENTA
             ON CUENTA.id_cuenta=MOTING.id_cuenta
...
/*            -- SE AUMENTA LAS RESTRICCIONES DE LA ESTRUCTURA PROGRAMÁTICA
    ...
    ELSIF pm_codigo_procedimiento  = 'AL_MOTING_COUNT' THEN

        BEGIN
        --Cuenta todos los registros
            g_consulta := 'SELECT
                          COUNT(MOTING.id_motivo_ingreso) AS total
                          FROM tal_motivo_ingreso MOTING
