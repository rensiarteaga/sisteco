	...
	CREATE OR REPLACE FUNCTION "public"."f_tal_responsable_almacen_sel" (
...
***************************************************************************
 SCRIPT: 		f_tal_responsable_almacen
 DESCRIPCIÓN: 	Devuelve las consultas a la tabla tal_responsable_almacen
 AUTOR: 		(Generado Automaticamente)
 FECHA:			2007-10-12 15:53:17
 COMENTARIOS:	
***************************************************************************
...
BEGIN
    ...
    g_nombre_funcion := 'f_tal_responsable_almacen_sel';
    ...
    IF pm_codigo_procedimiento  = 'AL_RESALM_SEL' THEN
        BEGIN
            g_consulta := 'SELECT 
							RESALM.id_responsable_almacen,
							RESALM.estado,
							RESALM.cargo,
							RESALM.fecha_asignacion,
							RESALM.fecha_reg,
							RESALM.observaciones,
							RESALM.id_almacen,
							ALMACE.codigo as desc_almacen,
							RESALM.id_empleado,
							EMPLEA.id_persona as desc_empleado
							FROM tal_responsable_almacen RESALM
						  	INNER JOIN tal_almacen ALMACE
            				ON ALMACE.id_almacen=RESALM.id_almacen
							INNER JOIN tkp_empleado EMPLEA
...
/*            -- SE AUMENTA LAS RESTRICCIONES DE LA ESTRUCTURA PROGRAMÁTICA
    ...
    ELSIF pm_codigo_procedimiento  = 'AL_RESALM_COUNT' THEN

        BEGIN
        --Cuenta todos los registros
            g_consulta := 	'SELECT
                          	COUNT(RESALM.id_responsable_almacen) AS total
                          	FROM tal_responsable_almacen RESALM
						  	INNER JOIN tal_almacen ALMACE
            				ON ALMACE.id_almacen=RESALM.id_almacen
							INNER JOIN tkp_empleado EMPLEA
