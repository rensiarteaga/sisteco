	...
	CREATE OR REPLACE FUNCTION "public"."f_tal_tipo_sector_iud" (
...
***************************************************************************
 SCRIPT: 		f_tal_tipo_sector_iud
 DESCRIPCIÓN: 	Permite registrar en la tabla tal_tipo_sector
 AUTOR: 		(generado automaticamente)
 FECHA:			2007-10-11 16:16:47
 COMENTARIOS:	
***************************************************************************
...
---*** INICIACIÓN DE VARIABLES 
    ...
    IF pm_codigo_procedimiento = 'AL_TIPSEC_INS' THEN

        BEGIN
        	
            INSERT INTO tal_tipo_sector(
		codigo,
		descripcion,
		observaciones,
		estado_registro,
		fecha_reg
		 ...
		 now()
            ...
            g_descripcion_log_error := 'Registro exitoso en tal_tipo_sector';
  ...
  --procedimiento de modificacion      
   ...
   ELSIF pm_codigo_procedimiento = 'AL_TIPSEC_UPD' THEN

        BEGIN
            --VERIFICA EXISTENCIA DEL REGISTRO
            IF NOT EXISTS(SELECT 1 FROM tal_tipo_sector
                          WHERE tal_tipo_sector.id_tipo_sector=al_id_tipo_sector) THEN
                              
                g_descripcion_log_error := 'Modificación no realizada: no existe el registro de tal_tipo_sectorno existente';
                g_nivel_error := '4';
                g_respuesta := f_pm_mensaje_error(g_descripcion_log_error, g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
		...
		fecha_reg=al_fecha_reg
				...
				WHERE tal_tipo_sector.id_tipo_sector= al_id_tipo_sector;

            -- DESCRIPCIÓN DE ÉXITO PARA GUARDAR EN EL LOG
            g_descripcion_log_error := 'Modificación exitosa en tal_tipo_sector';
            g_respuesta := 't'||g_separador||g_descripcion_log_error;

        END;
        
        ELSIF pm_codigo_procedimiento = 'AL_TIPSEC_DEL' THEN
    ...
    BEGIN
            ...
            IF NOT EXISTS(SELECT 1 FROM tal_tipo_sector
                ...
                g_nivel_error := '4';
                g_descripcion_log_error := 'Eliminación no realizada: registro en tal_tipo_sector inexistente';
                g_respuesta := f_pm_mensaje_error(g_descripcion_log_error, g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
                RETURN 'f'||g_separador||g_respuesta;
         ...
         --   IF EXISTS(SELECT 1 FROM tal_tipo_sector
         --            INNER JOIN tal_id1 ON tal_tipo_sector.id_subgrupo = tal_id1.id_subgrupo
         --            WHERE tal_tipo_sector.id_tipo_sector = al_id_tipo_sector) THEN
         --            
         --       g_nivel_error := '4';
         --       g_descripcion_log_error := 'Eliminación no realizada: El registro en tal_tipo_sector tiene regitros asociados en XXX';
         --       g_respuesta := f_pm_mensaje_error(g_descripcion_log_error, g_nombre_funcion, g_nivel_error, pm_codigo_procedimiento);
         --      RETURN 'f'||g_separador||g_respuesta;
         ...
         -- BORRADO DE DATO
            DELETE FROM tal_tipo_sector WHERE tal_tipo_sector.id_tipo_sector = al_id_tipo_sector;

            -- DESCRIPCIÓN DE ÉXITO PARA GUARDAR EN EL LOG
            g_descripcion_log_error := 'Eliminación exitosa del registro en tal_tipo_sector';
            g_respuesta := 't'||g_separador||g_descripcion_log_error;
        END;
