<?php
function crearArchivo_BDSel($direccion,$db,$table,$prefijo,$codigo,$meta){

	$num_campos = sizeof($meta); //cantidad de columnas que tiene la tabla
	$table = "t".strtolower($prefijo)."_".$table;


	$fp_handler=fopen("$direccion/f_".$table."_sel.sql","w+");
	$fecha=date("Y-m-d H:i:s");

	$sql = "
	CREATE OR REPLACE FUNCTION \"public\".\"f_".$table."_sel\" (
...
***************************************************************************
 SCRIPT: 		f_".$table."
 DESCRIPCIÓN: 	Devuelve las consultas a la tabla ".$table."
...
BEGIN
    ...
    g_nombre_funcion := 'f_".$table."_sel';
    ...
    IF pm_codigo_procedimiento  = '".strtoupper($prefijo)."_".$codigo."_SEL' THEN
        BEGIN
            g_consulta := 'SELECT \n";
	...
	for($i=0;$i<=$num_campos -2; $i ++){
		...
		if($meta[$i]["referenciado"] != null){ //si es llave en otro lado sacamos la desripción de la tabla madre
			...
			$consulta.= "\t\t\t\t\t\t\t$codigo_ref.".$campo_ref." as desc_$table_ref_sp,\n";

			$inner_join.="\tINNER JOIN $table_ref $codigo_ref
            \t\t\t\tON $codigo_ref.$id_table_ref=$codigo.".$meta[$i]["campo"]."\n";	
		}
	}
	...
	if($meta[$i]["referenciado"] != null){ //si es llave en otro lado sacamos la desripción de la tabla madre
		...
		echo "campo_ref".$campo_ref."<br>";

		echo "<pre>";
		print_r($descripcion_tabla_ref);
		echo "</pre>";
		...
		$consulta.= ",\n\t\t\t\t\t\t\t$codigo_ref.".$campo_ref." as desc_$table_ref_sp\n";
		//----inner join -----/
	...
	$sql.= $consulta."\t\t\t\t\t\t\tFROM ".$table." $codigo
						  $inner_join\t\t\t\t\t\t\tWHERE ';
...
/*            -- SE AUMENTA LAS RESTRICCIONES DE LA ESTRUCTURA PROGRAMÁTICA
    ...
    ELSIF pm_codigo_procedimiento  = '".strtoupper($prefijo)."_".$codigo."_COUNT' THEN

        BEGIN
        --Cuenta todos los registros
            g_consulta := \t'SELECT
                          \tCOUNT($codigo.".$meta[0]["campo"].") AS total
                          \tFROM ".$table." $codigo
						  $inner_join\t\t\t\t\t\t\tWHERE ';
            g_consulta := g_consulta || pm_criterio_filtro;
