	...
	CREATE OR REPLACE FUNCTION "public"."f_tsg_subsistema_sel" (
...
***************************************************************************
 SCRIPT: 		f_tsg_subsistema
 DESCRIPCIÓN: 	Devuelve las consultas a la tabla tsg_subsistema
...
BEGIN
    ...
    g_nombre_funcion := 'f_tsg_subsistema_sel';
    ...
    IF pm_codigo_procedimiento  = 'SG_SUBSIS_SEL' THEN
        BEGIN
            g_consulta := 'SELECT 
							...
							SUBSIS.nombre_largo,
							SUBSIS.descripcion,
							SUBSIS.version_desarrollo,
							SUBSIS.desarrolladores,
							SUBSIS.fecha_reg,
							SUBSIS.hora_reg,
							SUBSIS.fecha_ultima_modificacion,
							SUBSIS.hora_ultima_modificacion,
							SUBSIS.observaciones

						  FROM tsg_subsistema SUBSIS
...
/*            -- SE AUMENTA LAS RESTRICCIONES DE LA ESTRUCTURA PROGRAMÁTICA
    ...
    ELSIF pm_codigo_procedimiento  = 'SG_SUBSIS_COUNT' THEN

        BEGIN
        --Cuenta todos los registros
            g_consulta := 'SELECT
                          COUNT(SUBSIS.id_subsistema) AS total
                          FROM tsg_subsistema SUBSIS
                          WHERE ';
            g_consulta := g_consulta || pm_criterio_filtro;
