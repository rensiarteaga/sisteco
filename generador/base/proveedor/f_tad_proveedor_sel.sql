	...
	CREATE OR REPLACE FUNCTION "public"."f_tad_proveedor_sel" (
...
***************************************************************************
 SCRIPT: 		f_tad_proveedor
 DESCRIPCIÓN: 	Devuelve las consultas a la tabla tad_proveedor
...
BEGIN
    ...
    g_nombre_funcion := 'f_tad_proveedor_sel';
    ...
    IF pm_codigo_procedimiento  = 'AD_PROVEE_SEL' THEN
        BEGIN
            g_consulta := 'SELECT 
							PROVEE.id_proveedor,
							PROVEE.nombre,
							PROVEE.descripcion,
							PROVEE.telefono_ref,
							PROVEE.observaciones,
							PROVEE.fecha_reg
							FROM tad_proveedor PROVEE
						  							WHERE ';
...
/*            -- SE AUMENTA LAS RESTRICCIONES DE LA ESTRUCTURA PROGRAMÁTICA
    ...
    ELSIF pm_codigo_procedimiento  = 'AD_PROVEE_COUNT' THEN

        BEGIN
        --Cuenta todos los registros
            g_consulta := 	'SELECT
                          	COUNT(PROVEE.id_proveedor) AS total
                          	FROM tad_proveedor PROVEE
						  							WHERE ';
            g_consulta := g_consulta || pm_criterio_filtro;
