...
{    //Cargar los datos
	...
	$this->Ln(8);
		...
		$this->Cell(200,10,'MARCAS REGISTRADAS EN DISPOSITIVO',0,0,'C');
		$this->Ln(7);
		...
		$this->Cell(40,8,'FECHA',1,0,'C',1);
		$this->Cell(40,8,'HORA',1,0,'C',1);
		$this->Cell(40,8,'TIPO MOVIMIENTO',1,0,'C',1);
		$this->Ln(8);
	}
	...
	{     $cant=10000;
		  $puntero=0;
	      $sortcol='rel.fecha,rel.hora';
	      $sortdir='asc';
		  $criterio_filtro="rel.id_empleado=".$id_empleado;
		  $criterio_filtro=$criterio_filtro." AND rel.fecha >= ''$fecha_ini''";
	      $criterio_filtro=$criterio_filtro." AND rel.fecha <= ''$fecha_fin''";			
		 
		
           	//Leer las líneas del fichero
	
	$Custom=new cls_CustomDBControlAsistencia();
	$Custom->ListarLecturaRelojOrig($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);
	$var1=$Custom->salida;
	return $var1;
	}
		...
		$w=array(40,40,40);
	        ...
	        foreach($data as $row){
	        	$relleno=0;
	        	    ...
	        	    $this->Cell($w[1],5,cambiarFormatoFecha($row[2],0),1,0,'C',$relleno);//desc_valor_activo 
				    $this->Cell($w[0],5,$row[3],1,0,'C',$relleno);//desc_detalle
