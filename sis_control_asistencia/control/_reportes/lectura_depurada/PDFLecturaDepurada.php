...
{    //Cargar los datos
	...
	$this->Ln(8);
		...
		$this->Cell(200,10,'MARCAS DEPURADAS',0,0,'C');
		$this->Ln(7);
		...
		$this->Cell(30,8,'TIPO MOVIMIENTO',1,0,'C',1);
		$this->Cell(70,8,'OBSERVACIONES',1,0,'C',1);
		$this->Ln(8);
	}
	...
	{     $cant=10000;
		  $puntero=0;
	      $sortcol='LECDEP.fecha,LECDEP.hora';
	      $sortdir='asc';
		  $criterio_filtro="LECDEP.id_empleado=".$id_empleado;
		  $criterio_filtro=$criterio_filtro." AND LECDEP.fecha >= ''$fecha_ini''";
	      $criterio_filtro=$criterio_filtro." AND LECDEP.fecha <= ''$fecha_fin''";			
		 
		
           	//Leer las líneas del fichero
	
	$Custom=new cls_CustomDBControlAsistencia();
	$Custom->ListarLecturaDepuradaAprobado($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);
	$var1=$Custom->salida;
	return $var1;
	}
		...
		$w=array(30,30,30,70);
	        ...
	        foreach($data as $row){
	        	$relleno=0;
	        	    ...
	        	    $this->Cell($w[1],5,cambiarFormatoFecha($row[4],0),1,0,'C',$relleno);//desc_valor_activo 
				    $this->Cell($w[0],5,$row[5],1,0,'C',$relleno);//desc_detalle
					$this->Cell($w[2],5,$row[6],1,0,'C',$relleno);//desc_ret_incor
					$this->Cell($w[3],5,$row[7],1,0,'C',$relleno);//desc_ret_incor
					$this->Ln(5);
					$i=$i+1;
			}
