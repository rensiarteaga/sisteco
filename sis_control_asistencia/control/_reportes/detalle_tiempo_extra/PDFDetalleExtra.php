...
{    //Cargar los datos
	...
	$this->Ln(8);
		...
		$this->Cell(200,10,'DETALLE DIARIO DE TIEMPO EXTRA TRABAJADO',0,0,'C');
		$this->Ln(7);
		...
		$this->Cell(35,8,'TIEMPO EXTRA AM',1,0,'C',1);
		$this->Cell(35,8,'TIEMPO EXTRA PM',1,0,'C',1);
		$this->Cell(65,8,'TOTAL TIEMPO EXTRA TRABAJADO',1,0,'C',1);
		$this->Ln(8);
	}
	...
	$Custom->ListarDetalleExtra($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);
	$var1=$Custom->salida;
	return $var1;
	}
		...
		$w=array(25,35,35,65);
	        ...
	        foreach($data as $row){
	        	$relleno=0;
					...
					$this->Cell($w[2],5,$row[3],1,0,'C',$relleno);//desc_ret_incor
					$this->Cell($w[3],5,$row[4],1,0,'C',$relleno);//desc_ret_incor
					$this->Ln(5);
					$i=$i+1;
			}
