...
class cls_DBFeriado
{
	...
	var $nombre_archivo = "cls_DBFeriado.php";
	...
	{
		$this->salida = "";
		$this->nombre_funcion = 'casis.f_tca_feriado_sel';
		...
		$this->var->add_def_cols('id_feriado','integer');
		$this->var->add_def_cols('fecha_feriado','date');
		$this->var->add_def_cols('motivo','varchar');
		$this->var->add_def_cols('feriado_nacional','varchar');
	...
	function ContarListaFeriado($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad)
{
	...
	/*
	**********************************************************
	Nombre de la función:	CrearFeriado()

	Propósito:				Se utiliza esta función para insertar un nuevo Feriado en la base de datos
	...
	**********************************************************
	*/
	function CrearFeriado($id_feriado,$fecha_feriado,$motivo,$feriado_nacional,$id_lugar)
	{
		$this->salida = "";
		$this->nombre_funcion = 'casis.f_tca_feriado_iud';
		...
		$this->var->add_param("NULL");//id_feriado
		$this->var->add_param("'$fecha_feriado'");//fecha_feriado
		$this->var->add_param("'$motivo'");//motivo
		$this->var->add_param("'$feriado_nacional'");//feriado_nacional
        $this->var->add_param("'$id_lugar'");//id_lugar		
	...
	function  EliminarFeriado($id_feriado)
	{
		...
		//Carga parámetros específicos (no incluyen los parámetros fijos)
		$this->var->add_param("NULL");//id_fina_regi_prog_proy_acti
		$this->var->add_param("'$id_feriado'");
		$this->var->add_param('NULL');//fecha_feriado
		$this->var->add_param('NULL');//motivo
		$this->var->add_param('NULL');//feriado_nacional
	...
	function  ModificarFeriado($id_feriado,$fecha_feriado,$motivo,$feriado_nacional,$id_lugar)
	{
		$this->salida="";
		$this->nombre_funcion = 'casis.f_tca_feriado_iud';
		...
		//Carga parámetros específicos (no incluyen los parámetros fijos)
		$this->var->add_param("NULL");//id_fina_regi_prog_proy_acti
		$this->var->add_param("'$id_feriado'");//id_feriado
		$this->var->add_param("'$fecha_feriado'");//fecha_feriado
		$this->var->add_param("'$motivo'");//motivo
		$this->var->add_param("'$feriado_nacional'");//feriado_nacional
	    $this->var->add_param("'$id_lugar'");//id_lugar
	 ...
	 * @return unknown
	 */
	function ValidarFeriado($operacion_sql,$id_feriado,$fecha_feriado,$motivo,$feriado_nacional,$id_lugar)
	{
		...
		switch ($operacion_sql)
		{
			case 'insert' :
				...
				$tipo_dato->set_Columna("fecha_feriado");	
				 
				if(!$valid->verifica_dato($tipo_dato->TipoDatoDate(), "fecha_feriado", $fecha_feriado))
				...
				//Validar feriado_nacional - tipo texto
				$tipo_dato->_reiniciar_valor();
				$tipo_dato->set_Columna("feriado_nacional");	
				$tipo_dato->set_MaxLength(2);
				$tipo_dato->set_MinLength(2);				
				
				if(!$valid->verifica_dato($tipo_dato->TipoDatoText(), "feriado_nacional", $feriado_nacional))				
			...
			case 'update' :
				...
				$tipo_dato->_reiniciar_valor();
				$tipo_dato->set_Columna("id_feriado");	
				$tipo_dato->set_MaxLength(10);
				$tipo_dato->set_MinLength(0);				
				
				if(!$valid->verifica_dato($tipo_dato->TipoDatoInteger(), "id_feriado", $id_feriado))					
				...
				//Validar feriado_nacional - tipo varchar
				$tipo_dato->_reiniciar_valor();
				$tipo_dato->set_Columna("feriado_nacional");	
				$tipo_dato->set_MaxLength(2);
				$tipo_dato->set_MinLength(2);				
