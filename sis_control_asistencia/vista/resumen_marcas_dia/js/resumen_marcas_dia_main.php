<?php 
/**
 * Nombre:		  	    resumen_marcas_dia_main.php
...
var elemento={pagina:new pagina_resumen_marcas_dia(idContenedor,direccion,paramConfig),idContenedor:idContenedor};
...
function pagina_resumen_marcas_dia(idContenedor,direccion,paramConfig){
	...
	function finaliza_aprobacion(apro){
		...
		Ext.MessageBox.hide();
		if(apro.responseXML != undefined && apro.responseXML != null && apro.responseXML.documentElement != null && apro.responseXML.documentElement != undefined){
			var root = apro.responseXML.documentElement;
			if((root.getElementsByTagName('error')[0].firstChild.nodeValue)=='false'){
		//if(apro==1)
				Ext.MessageBox.alert('Estado','El resumen de marcas se procesó satisfactoriamente');	
				ClaseMadre_btnActualizar();
		//ClaseMadre_btnActualizar();
			}
			else{
				Ext.MessageBox.alert('Estado','El resumen de marcas NO se procesó');
				ClaseMadre_btnActualizar();
