...
var elemento={pagina:new pagina_columna_tipo(idContenedor,direccion,paramConfig),idContenedor:idContenedor};
...
{ 
	var Atributos=new Array,sw=0, kp_gestion;
	...
	var ds = new Ext.data.Store({
		...
		'desc_cuenta_pasivo',
		'desc_auxiliar_pasivo','id_gestion','id_tipo_obligacion','desc_tipo_obligacion',
		'movimiento_contable',		
		'prorratea'
		]),remoteSort:true});
		...
		var ds_tipo_descuento_bono=new Ext.data.Store({proxy:new Ext.data.HttpProxy({url:direccion+"../../../../sis_kardex_personal/control/tipo_descuento_bono/ActionListarTipoDescuentoBono.php?accion=des_bono"}),
		...
		var ds_tipo_obligacion=new Ext.data.Store({proxy: new Ext.data.HttpProxy({url: direccion+'../../../../sis_kardex_personal/control/tipo_obligacion/ActionListarTipoObligacion.php'}),
		reader:new Ext.data.XmlReader({record:'ROWS',id:'id_cuenta',totalRecords: 'TotalCount'},['id_tipo_obligacion','codigo','nombre'])
		});
		...
		function render_id_tipo_obligacion(value,p,record){return String.format('{0}',record.data['desc_tipo_obligacion'])}
		...
		Atributos[9]= {
			validacion: {
				...
				allowBlank:true,
				emptyText:'Auxiliar...',
				desc:'desc_auxiliar_pasivo', //indica la columna del store principal ds del que proviane la descripcion
				store:ds_auxiliar,
		...
		Atributos[19]={
			validacion:{
				...
				desc:'desc_tipo_obligacion',
				store:ds_tipo_obligacion,
				valueField:'id_tipo_obligacion',
				displayField:'nombre',
				queryParam:'filterValue_0',
				filterCol:'TOB.nombre',
				typeAhead:true,
				forceSelection:true,
			...
			save_as:'id_tipo_obligacion',
			tipo:'ComboBox',
			filtro_0:true,
			filtro_1:true,
			id_grupo:1,
			filterColValue:'TIPOBL.nombre'
		};
		...
		Atributos[21]= {
				validacion:{
					...
					fieldLabel:'Movimiento Contable',
					allowBlank:true,
					typeAhead:true,
					loadMask:true,
					triggerAction:'all',
					store:new Ext.data.SimpleStore({fields:['id','valor'],data:[['DEBE','DEBE'],['HABER','HABER']]}), 	//llamada a _combo.js para cargar datos al combo
					valueField:'id',
					displayField:'valor',
					lazyRender:true,
					grid_visible:true,
					grid_editable:false,
					forceSelection:true,
					//emptyText:'Seleccione un Movimiento...',
					width:200
				},
				tipo: 'ComboBox',
				form:true,
				filtro_0:true,
				id_grupo:1,
				filterColValue:'COLTIP.movimiento_contable',
				save_as:'movimiento_contable'
			};
		Atributos[22]= {
				validacion:{
					name:'prorratea',
					...
					loadMask:true,
					triggerAction:'all',
					store:new Ext.data.SimpleStore({fields:['id','valor'],data:[['SI','SI'],['NO','NO']]}), 	//llamada a _combo.js para cargar datos al combo
					valueField:'id',
					displayField:'valor',
					lazyRender:true,
					grid_visible:true,
					grid_editable:false,
					forceSelection:true,
					//emptyText:'Seleccione Prorratea...',
					width:200
				},
				tipo: 'ComboBox',
				form: true,
				filtro_0:true,
				id_grupo:1,
				filterColValue:'COLTIP.prorratea',
				save_as:'prorratea'
			};
		...
		var config={titulo_maestro:'Tipo de Columna',grid_maestro:'grid-'+idContenedor,urlHijo:'../../../sis_kardex_personal/vista/tipo_columna_base/tipo_columna_base.php'};
		
		var layout_columna_tipo=new DocsLayoutMaestroDeatalle(idContenedor);
...
var cm_EnableSelect=this.EnableSelect;
	...
	this.EnableSelect=function(sm,row,rec)
	{
		...
		_CP.getPagina(layout_columna_tipo.getIdContentHijo()).pagina.reload(rec.data);
		if(rec.data.compromete=='si'){
			_CP.getPagina(layout_columna_tipo.getIdContentHijo()).pagina.bloquearMenu();
		}else{
			
			_CP.getPagina(layout_columna_tipo.getIdContentHijo()).pagina.desbloquearMenu();
		}
		//enable(sm,row,rec);
