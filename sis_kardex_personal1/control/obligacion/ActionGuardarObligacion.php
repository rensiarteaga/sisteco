...
if($_SESSION["autentificado"]=="SI")
{
	...
	for($j = 0;$j < $cont; $j++)
	{
		if ($get)
		{
			...
			$obs_pago=$_GET["obs_pago_$j"];
			$id_gestion=$_GET["id_gestion_$j"];
			$acreedor=$_GET["acreedor_$j"];
			
			$id_lugar=$_GET["id_lugar_$j"];
			$id_institucion_acreedor=$_GET["id_institucion_acreedor_$j"];
		...
		else
		{
			...
			$obs_pago=$_POST["obs_pago_$j"];
			$id_gestion=$_POST["id_gestion_$j"];
			$acreedor=$_POST["acreedor_$j"];
			$id_lugar=$_POST["id_lugar_$j"];
			$id_institucion_acreedor=$_POST["id_institucion_acreedor_$j"];
	...
	//}// end for -- 28feb11 
		...
		if ($id_obligacion == "undefined" || $id_obligacion== "")
		{
			...
			$res = $Custom -> InsertarObligacion($id_obligacion,$id_tipo_obligacion,$id_planilla,$id_comprobante,$monto,$estado_reg,$observaciones,$id_cuenta_bancaria,$tipo_pago,$id_gestion);
	...
	if($accion=='pago'){ //if($_SESSION["ss_id_usuario"]==120){ echo $fecha_pago; exit;} 
		if ($id_institucion_acreedor>0 && $id_persona>0){
			$estado_reg='doble';
		}else{
				if($id_institucion_acreedor>0){
					$id_acreedor=$id_institucion_acreedor;
					$estado_reg='ins';
				}else{
					if($id_persona>0){
						$id_acreedor=$id_persona;
						$estado_reg='per';
					}
				}
		...
		, $fecha_pago,$obs_pago,$id_acreedor,$id_lugar
			...
			$res = $Custom->ModificarObligacion($id_obligacion,$id_tipo_obligacion,$id_planilla,$id_comprobante,$monto,$estado_reg,$observaciones,$id_cuenta_bancaria,$tipo_pago,$id_cuenta,$id_auxiliar,$id_gestion);
