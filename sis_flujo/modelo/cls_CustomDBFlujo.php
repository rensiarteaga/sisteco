 ...
 * Prop�sito:				Interfaz del modelo del Sistema de Flujo
 * todos los metodos existentes pasan por aqui
 * Fecha de Creaci�n:		22-12-2010
...
class cls_CustomDBFlujo
{
	...
	{	
		...
		*/
		...
		include_once("cls_DBHojaRuta.php");
			...
			include_once("cls_DBEstadistica.php");
	}
	...
	*/
	...
	function InsertarAnexo($id_anexo,$numero,$nombre_foto,$extension,$foto)
	{
		$this->salida = "";
		$dbflujo = new cls_DBAnexo($this->decodificar);
		$res = $dbflujo -> InsertarAnexo($id_anexo,$numero,$nombre_foto,$extension,$foto);
		$this->salida = $dbflujo ->salida;
		$this->query = $dbflujo ->query;
		return $res;
	}
	
	function EliminarAnexo($id_anexo)
	{
		$this->salida = "";
		$dbflujo = new cls_DBAnexo($this->decodificar);
		$res = $dbflujo -> EliminarAnexo($id_anexo);
		$this->salida = $dbflujo ->salida;
		$this->query = $dbflujo ->query;
		return $res;
	}
	
	function ModificarAnexo($id_anexo,$nombre_foto,$numero,$extension,$foto)
	{
		$this->salida = "";
		$dbflujo = new cls_DBAnexo($this->decodificar);
		$res = $dbflujo -> ModificarAnexo($id_anexo,$nombre_foto,$numero,$extension,$foto);
		$this->salida = $dbflujo ->salida;
		$this->query = $dbflujo ->query;
		return $res;
	}
	
	function ValidarAnexo($operacion_sql,$id_anexo,$nombre_foto,$numero,$extension)
	{
		$this->salida = "";
		$dbflujo = new cls_DBAnexo($this->decodificar);
		$res = $dbflujo -> ValidarAnexo($operacion_sql,$id_anexo,$nombre_foto,$numero,$extension);
		$this->salida = $dbflujo ->salida;
		$this->query = $dbflujo ->query;
		return $res;
	}
...
//------------------- fin tfl_Adjunto ------------------\\
	...
	/*----------------mflores para ordenar la correspondencia de acuerdo al tipo*/
	function OrdenarCorrespondencia($cant,$puntero,$sortcol,$sortdir,$criterio_filtro)
	{
		$this->salida = "";
		$dbCorrespondencia = new cls_DBCorrespondencia($this->decodificar);
		$res = $dbCorrespondencia ->OrdenarCorrespondencia($cant,$puntero,$sortcol,$sortdir,$criterio_filtro);
		$this->salida = $dbCorrespondencia ->salida;
		$this->query = $dbCorrespondencia ->query;
		return $res;
	}
	
	function ListarCorrespondencia($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad,$vista,$excel='')
	{
		$this->salida = "";
		$dbCorrespondencia = new cls_DBCorrespondencia($this->decodificar);
		$res = $dbCorrespondencia ->ListarCorrespondencia($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad,$vista,$excel);
		$this->salida = $dbCorrespondencia ->salida;
		$this->query = $dbCorrespondencia ->query;
		return $res;
	}
	...
	*/
	...
	function EmpleadoGteGral($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_empleado)
	{
		$this->salida = "";
		$dbCorrespondencia = new cls_DBCorrespondencia($this->decodificar);
		$res = $dbCorrespondencia -> EmpleadoGteGral($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_empleado);
		$this->salida = $dbCorrespondencia ->salida;
		$this->query = $dbCorrespondencia ->query;
		return $res;
	}
	...
	function PlantillasRTF($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_correspondencia)
	{
		$this->salida = "";
		$dbCorrespondencia = new cls_DBCorrespondencia($this->decodificar);
		$res = $dbCorrespondencia -> PlantillasRTF($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_correspondencia);
		$this->salida = $dbCorrespondencia ->salida;
		$this->query = $dbCorrespondencia ->query;
		return $res;
	}
	
	function Destinatarios($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_correspondencia,$var)
	{
		$this->salida = "";
		$dbCorrespondencia = new cls_DBCorrespondencia($this->decodificar);
		$res = $dbCorrespondencia -> Destinatarios($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_correspondencia,$var);
		$this->salida = $dbCorrespondencia ->salida;
		$this->query = $dbCorrespondencia ->query;
		return $res;
	}
	...
	function ListarEstadoCorrespondencia($cant,$puntero,$sortcol,$sortdir,$criterio_filtro)
	{
		$this->salida = "";
		$dbCorrespondencia = new cls_DBCorrespondencia($this->decodificar);
		$res = $dbCorrespondencia -> ListarEstadoCorrespondencia($cant,$puntero,$sortcol,$sortdir,$criterio_filtro);
		$this->salida = $dbCorrespondencia ->salida;
		$this->query = $dbCorrespondencia ->query;
		return $res;
	}
	...
	/*jrr: Para reportes estad�sticos de correspondencia*/
	...
	function ListarEstadisticasExternasGlob($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad){
		$this->salida = "";
		$dbEstadistica = new cls_DBEstadistica($this->decodificar);
		$res = $dbEstadistica -> ListarEstadisticasExternasGlob($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);
		$this->salida = $dbEstadistica ->salida;
		$this->query = $dbEstadistica ->query;
		return $res;	
	}
	...
	function ListarEstadisticasExternasDet($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad){
		$this->salida = "";
		$dbEstadistica = new cls_DBEstadistica($this->decodificar);
		$res = $dbEstadistica -> ListarEstadisticasExternasDet($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);
		$this->salida = $dbEstadistica ->salida;
		$this->query = $dbEstadistica ->query;
		return $res;	
	}
	function ListarEstadisticasEnviadasGlob($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad){
		$this->salida = "";
		$dbEstadistica = new cls_DBEstadistica($this->decodificar);
		$res = $dbEstadistica -> ListarEstadisticasEnviadasGlob($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);
		$this->salida = $dbEstadistica ->salida;
		$this->query = $dbEstadistica ->query;
		return $res;	
	}
	...
	function ListarEstadisticasEnviadasDet($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad){
		$this->salida = "";
		$dbEstadistica = new cls_DBEstadistica($this->decodificar);
		$res = $dbEstadistica -> ListarEstadisticasEnviadasDet($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);
		$this->salida = $dbEstadistica ->salida;
		$this->query = $dbEstadistica ->query;
		return $res;	
	}
	function ListarEstadisticasRecibidasGlob($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad){
		$this->salida = "";
		$dbEstadistica = new cls_DBEstadistica($this->decodificar);
		$res = $dbEstadistica -> ListarEstadisticasRecibidasGlob($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);
		$this->salida = $dbEstadistica ->salida;
		$this->query = $dbEstadistica ->query;
		return $res;	
	}
	...
	function ListarEstadisticasRecibidasDet($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad){
		$this->salida = "";
		$dbEstadistica = new cls_DBEstadistica($this->decodificar);
		$res = $dbEstadistica -> ListarEstadisticasRecibidasDet($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);
		$this->salida = $dbEstadistica ->salida;
		$this->query = $dbEstadistica ->query;
		return $res;	
	}
	...
	//12.12.2014
	function ListarCorrespondenciaENDE($criterio)
	{
		$this->salida = "";
		$dbCorrespondencia = new cls_DBCorrespondencia($this->decodificar);
		$res = $dbCorrespondencia ->ListarCorrespondenciaENDE($criterio);
		$this->salida = $dbCorrespondencia ->salida;
		$this->query = $dbCorrespondencia ->query;
		return $res;
	}
	 ...
	 //12.12.2014
	function ModificarCorrespondenciaENDE($id_usuario, $id_ip_origen,$id_correspondencia)
	{ 
		$this->salida = "";
		$dbCorrespondencia = new cls_DBCorrespondencia($this->decodificar);
		$res = $dbCorrespondencia ->ModificarCorrespondenciaENDE($id_usuario, $id_ip_origen,$id_correspondencia);
	...
	function ListarAdjuntoENDE($id_correspondencia)
	{
		$this->salida = "";
		$dbflujo = new cls_DBAdjunto($this->decodificar);
		$res = $dbflujo -> ListarAdjuntoENDE($id_correspondencia);
		$this->salida = $dbflujo ->salida;
		$this->query = $dbflujo ->query;
		return $res;
	} 
