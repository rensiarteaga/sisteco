...
class cls_DBCorrespondencia
{
	...
	function ListarCorrespondencia($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad,$vista,$excel)
	{
		...
		elseif($vista=='enviado')
			$this->codigo_procedimiento = "'FL_CORENV_SEL'";
		elseif($vista=='solicitud_autorizacion')
			$this->codigo_procedimiento = "'FL_CORENV_INTERNA'";
		...
		$this->var->add_param($func->iif($excel == '',"'%'","'$excel'"));//id_financiador
		...
		if ($vista=='enviado' || $vista=='administracion'|| $vista=='solicitud_autorizacion')
		{
		...
		if ($vista=='externo_recibido' || $vista=='externo_derivado' || $vista=='detalle_externo')
		{
		  $this->var->add_def_cols('fecha_fin','date');
		  $this->var->add_def_cols('respuestas','varchar');
		}
		$this->var->add_def_cols('archivado','integer');
		//Ejecuta la funci�n de consulta
		$res = $this->var->exec_query();
	...
	function OrdenarCorrespondencia($cant,$puntero,$sortcol,$sortdir,$criterio_filtro)
	{
		...
		$this->salida = "";
		$this->nombre_funcion = 'f_tfl_corresordenada_sel';
		$this->codigo_procedimiento = "'FL_CORORD_SEL'";
...
//		exit;
	...
	function ListarCorrespondenciaPDF($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad,$id_correspondencia)
	{
		...
		//$this->var->add_def_cols('hora_destino','time');
		$this->var->add_def_cols('hora_destino','text');
		$this->var->add_def_cols('documento','varchar');
	...
	function ContarCorrespondencia($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad,$vista)
	{
		...
		else if($vista=='solicitud_autorizacion')
			$this->codigo_procedimiento = "'FL_CORENV_INTERNA_COUNT'";
		elseif ($vista=='detalle')
...
//		var_dump($this->query);
	...
	function ModificarCorrespondencia($id_correspondencia,$id_depto,$id_documento,$id_empleado,$id_uo,$id_institucion,$id_persona,$referencia,$fecha_origen,$fecha_destino,$hora_destino,$id_tipo_accion,$empleados,$tipo,$nuevo,$mensaje,$accion_proceso,$url_archivo,$extension,$id_correspondencia_fk,$acciones,$observaciones,$observaciones_estado,$cite,$id_nivel_seguridad,$nivel_prioridad,$fecha_max_res,$id_correspondencia_asociada)
	{
		...
		elseif($accion_proceso=='activar_anulado')
		    $this->codigo_procedimiento = "'FL_CORREST_UPD'";
		else
			$this->codigo_procedimiento = "'FL_COROBSES_UPD'";
        ...
        if($accion_proceso=='modificar_enviado' && isset($id_correspondencia_asociada)){
		   $this->var->add_param_array($id_correspondencia_asociada);
		}
		else{
			$this->var->add_param_array("NULL");//id_correspondencia_asociada
	...
	/**
	 * Nombre de la funci�n:	MarcarResponsable
	 * Prop�sito:				Permite definir un responsable para la posible respuesta de la correspondencia
	 * Autor:				    (autogenerado)
	 * Fecha de creaci�n:		2011-02-11 10:52:58
	 */
	...
	function EmpleadoGteGral($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_empleado)
	{
		$this->salida = "";
		$this->nombre_funcion = 'f_tfl_obtener_cargo_emp_remit';
		$this->codigo_procedimiento = "'FL_GTE_GRAL'";
	...
	{
		$this->salida = "";
		$this->nombre_funcion = 'f_tfl_plantillas_rtf';
		$this->codigo_procedimiento = "'FL_PLANT_RTF'";
		...
		//Carga la definici�n de columnas con sus tipos de datos
		$this->var->add_def_cols('numero','varchar');
		$this->var->add_def_cols('id_corespondencia_fk','integer');
		$this->var->add_def_cols('documento','varchar');
		$this->var->add_def_cols('codigo','varchar');
		$this->var->add_def_cols('nivel_academico','varchar');
		$this->var->add_def_cols('cargo_rem','varchar');
		$this->var->add_def_cols('remitente','text');
		$this->var->add_def_cols('fecha_origen','date');
		$this->var->add_def_cols('tipo','varchar');
		$this->var->add_def_cols('institucion','varchar');
		$this->var->add_def_cols('persona','text');
		$this->var->add_def_cols('referencia','text');
		$this->var->add_def_cols('acciones','varchar');
	...
	function Destinatarios($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_correspondencia,$var)
	{
		//echo $var; exit;
		$this->salida = "";
		$this->nombre_funcion = 'f_tfl_destinatarios_plant_corr';
		$this->codigo_procedimiento = "'FL_PLAN_CORR'";
	...
	{
		$this->salida = "";
		$this->nombre_funcion = 'f_tfl_estado_correspondencia';
		$this->codigo_procedimiento = "'FL_EST_CORR'";
		...
		$this->var->add_def_cols('numero','varchar');
		$this->var->add_def_cols('estado','varchar');
		$this->var->add_def_cols('url_archivo','text');
		$this->var->add_def_cols('referencia','text');
		$this->var->add_def_cols('nombre_completo','text');
	...
	/**************/
	 function ListarCorrespondenciaENDE($criterio_filtro)
	{
		
		$this->salida = "";
		$this->nombre_funcion = 'f_fl_webservices_ende_cadeb_sel';
		
		$this->codigo_procedimiento = "'FL_CORRES_ARCHIVADA_SEL'";
		...
		$this->var->add_param("'$criterio_filtro'");
		//Carga la definici�n de columnas con sus tipos de datos
		$this->var->add_def_cols('codigo','int4');
		$this->var->add_def_cols('numero_correspondencia','varchar');
		$this->var->add_def_cols('codigo_tipo_documento', 'integer');
		$this->var->add_def_cols('descripcion_tipo_documento', 'varchar');
		$this->var->add_def_cols('tipo_correspondencia' ,'varchar');
		$this->var->add_def_cols('nombre_tipo_correspondencia', 'varchar');
		$this->var->add_def_cols('codigo_institucion', 'integer');
		$this->var->add_def_cols('nombre_institucion', 'varchar');
		$this->var->add_def_cols('referencia', 'varchar');
		$this->var->add_def_cols('descripcion', 'varchar');
		$this->var->add_def_cols('emisor', 'integer');
		$this->var->add_def_cols('nombre_emisor', 'varchar');
		$this->var->add_def_cols('receptor', 'integer');
		$this->var->add_def_cols('nombre_receptor', 'varchar');
		$this->var->add_def_cols('fecha_recepcion' ,'date');
		$this->var->add_def_cols('cantidad_hojas', 'integer');
		$this->var->add_def_cols('nombre_archivo', 'varchar');
		$this->var->add_def_cols('tiene_adj', 'integer');
			//Ejecuta la funci�n de consulta
	  ...
	  function ModificarCorrespondenciaENDE($id_usuario, $id_ip_origen,$id_correspondencia)
	{
		...
		$this->salida = "";
		$this->nombre_funcion = 'f_fl_webservices_ende_cadeb_iud';
		
		$this->codigo_procedimiento = "'FL_CORRES_ARCHIVADA_UPD'";
		...
		$this->var->add_param("'$id_ip_origen'");
		$this->var->add_param($this->codigo_procedimiento);
