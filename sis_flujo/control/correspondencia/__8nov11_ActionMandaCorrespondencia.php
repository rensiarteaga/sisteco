...
Fecha de Creación:		2011-02-27 10:52:59
Versión:				1.0.0
	...
	if($res){
		...
		foreach ($Custom->salida as $f){
			...
			$mensaje= '<p>
						...
						<p>del tipo: <b>"'.$f["desc_documento"].'"</b> con el número <b>"'.$f["numero"].'"</b>
						y la acción: <b>'.$f["nombre_accion"].'</b><p>
						<br/>
						<p>'.$f["mensaje"].'</p>
						<br/>
						<p>Originado por:</p>';
			...
			if($f['tipo']=='interna'){
				...
				if(strlen($f["fecha_max_res"])>1){
					$mensaje.='<br/>
						<p>Fecha límite para responder: '.$f["fecha_max_res"].'</p>';
				}
				
			}
			$var_cor = $correo ->EnviarCorreodeFlujo('endesis@ende.bo',$f["email1"],'FLUJO: '.$f["referencia"],$mensaje);
		}
