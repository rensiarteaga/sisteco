...
if($_SESSION['autentificado']=='SI')
{
	...
	//echo $id_correspondencia.'-'.$id_correspondencia_fk; exit;
	...
	$cargo_rem = acentos_enies($Custom->salida[0]['nombre_cargo']);
	...
	{		
		if($var == 1)	
		{			
			$v_empleado = explode(' ',acentos_enies($Custom->salida[$i]['empleado']));
			...
			$destinatarios = $destinatarios.'<strong>'.$nivel_acad1.$Custom->salida[$i]['empleado'].' - '.acentos_enies($Custom->salida[$i]['nombre_cargo']).'</strong>'.chr(13).chr(10).'<TAB><TAB><TAB>'; 
		}
		else
		{
	...
	{					
		...
		//echo $tam_desc_doc = strlen($desc_documento); exit;
		
		/*if($desc_documento == 'RESOLUCION DE VIAJES EN FERIADOS O FINES DE SEMANA')
		{
			
			$fuente = 'fonts_ttf/ariblk.ttf';
			imagettftext($titulo_doc, 14, 0, 125, 78, $plomo, $fuente, 'RESOLUCION DE VIAJES EN');
			
			imagettftext($titulo_doc, 14, 0, 125, 95, $plomo, $fuente, 'FERIADOS O FINES DE SEMANA');
		}	*/	
		...
		if($tam_desc_doc >= 25)
		{
			...
			$fuente = 'fonts_ttf/ariblk.ttf';
			imagettftext($titulo_doc, 14, 0, 125, 70, $negro, $fuente, $desc_documento);
		}
		
		else 
		{
			$fuente = 'fonts_ttf/ariblk.ttf';
			imagettftext($titulo_doc, 18, 0, 125, 88, $negro, $fuente, $desc_documento);			
		}
		...
		imagettftext($titulo_doc, 10, 0, 33, 15, $blanco, $fuente, 'ENDESIS');
		imagejpeg($titulo_doc,"/tmp/titulo_$id_correspondencia.jpg");
		
		$fuente = 'fonts_ttf/tahomabd.ttf';
		//ImageString($titulo_doc, 4, 190, 118, $lugar, '');
		imagettftext($titulo_doc, 10, 0, 190, 130, $negro, $fuente, $lugar);
		imagejpeg($titulo_doc,"/tmp/titulo_$id_correspondencia.jpg");
			
		//ImageString($titulo_doc, 4, 440, 118, $titulo, '');
		imagettftext($titulo_doc, 10, 0, 440, 130, $negro, $fuente, $fecha_origen);
		imagejpeg($titulo_doc,"/tmp/titulo_$id_correspondencia.jpg");
		
		//ImageString($titulo_doc, 4, 554, 118, $titulo, '');
		imagettftext($titulo_doc, 10, 0, 554, 130, $negro, $fuente, $numero);
		imagejpeg($titulo_doc,"/tmp/titulo_$id_correspondencia.jpg");
	...
	else //externas
	{
		...
		$Rtf->addText('<TAB><TAB>'.ucwords(strtolower($nivel_acad)).$desc_empleado);		
