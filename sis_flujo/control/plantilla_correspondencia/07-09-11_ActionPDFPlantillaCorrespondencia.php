...
if($_SESSION['autentificado']=='SI')
{
	...
	//echo $desc_documento.'-'.$tipo; exit;
	
	if (eregi('GGN', $numero)) 
	{
		$gerencia = 'GGN';
		$responsable = 'GERENTE GENERAL<BR>RESPONSABLE DEL PROCESO DE<BR>CONTRATACI�N DIRECTA - RPCD';
	}
	else 
	{
		$gerencia = 'RPCD';
		$responsable = 'RESPONSABLE DEL PROCESO DE<BR>CONTRATACI�N DIRECTA - RPCD';
	}
	...
	$res = $Custom-> TipoDocumento($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_correspondencia);	
	$tipo_doc = $Custom->salida[0]['descripcion']; 
	...
	else 
	{
		...
		{		
			...
			switch ($accion1[$i])
			{
				...
				case 'Para su consideraci�n': $para_consideracion = 'X';
								break;
			}		
		}
	}
	...
	{					
		...
		ImageString($titulo_doc, 1, 45, 0, $cod_hash1, '');		//hash en im�gen de logo
		imagejpeg($titulo_doc,"/tmp/HASH.jpg");
		
		ImageString($titulo_doc, 1, 45, 10, $cod_hash2, '');		//hash en im�gen de logo
		imagejpeg($titulo_doc,"/tmp/HASH.jpg");
		...
		/*if($tipo_doc == 'CI' || $tipo_doc == 'CIR' || $tipo_doc == 'CM' || $tipo_doc == 'IL' || $tipo_doc == 'IN' || $tipo_doc == 'IV' || $tipo_doc == 'ME')
		{
			//generaci�n normal de imagen
		}
		else //caso que no tengan imagenes predise�adas para el titulo se genera titulo e inserta en la imagen
		{*/
		...
		ImageString($titulo_doc, 1, 5, 0, $cod_hash1, $blanco);		//hash en im�gen de logo
		imagejpeg($titulo_doc,"/tmp/titulo_$id_correspondencia.jpg");
		
		ImageString($titulo_doc, 1, 5, 10, $cod_hash2, $blanco);		//hash en im�gen de logo
		imagejpeg($titulo_doc,"/tmp/titulo_$id_correspondencia.jpg");*/
		...
		imagettftext($titulo_doc, 10, 0, 460, 175, $negro, $fuente, $fecha_origen);
		imagejpeg($titulo_doc,"/tmp/titulo_$id_correspondencia.jpg");
		
		//ImageString($titulo_doc, 4, 554, 118, $titulo, '');
		imagettftext($titulo_doc, 10, 0, 587, 173, $negro, $fuente, $numero);
		imagejpeg($titulo_doc,"/tmp/titulo_$id_correspondencia.jpg");
				
		/*// Crear instancias de im�genes
	...
	//echo $tipo.'-'.$tipo_documento; exit;
		
	if($tipo == 'interna') //internas
	{
		...
		{
			$Rtf->setPaperSize(1);
			$Rtf->tablaRemision($numero, $tipo_documento,$dia, $mes, $anio, $nivel_acad, $desc_empleado, $empleado_remision, 'img/Log peque�o.jpg', $cargo_rem, $cargo_remision);			
			$Rtf->getDocument("$numero.rtf");
		}	
		...
		elseif($tipo_documento == 'ALCANCE DE TRABAJO')
		{
			...
			$Rtf->rtfImage("/tmp/titulo_$id_correspondencia.jpg",'jpg'); //inserta una im�gen en el encabezado
		...
		{
			$Rtf->setPaperSize(1); //tama�o carta			
			$Rtf->margenes(2050,'izq');
			$Rtf->margenes(1000,'der');			
			//$Rtf->addText_fuente_tam(3,18,'<strong><BR>A:</strong><TAB><TAB><TAB>'.$destinatarios.'<BR>'); //para cambiar tama�o de la fuente en relacion a la que tiene el documento
			...
			$Rtf->addText("\n");
			$Rtf->paginacion(); //inserta n�mero de p�gina
			$Rtf->rtfImage("/tmp/titulo_$id_correspondencia.jpg",'jpg'); //inserta una im�gen en el encabezado
			$Rtf->getDocument("$numero.rtf");
		}
	}
