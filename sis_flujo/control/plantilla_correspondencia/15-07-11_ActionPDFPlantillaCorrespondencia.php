...
if($_SESSION['autentificado']=='SI')
{
	...
	if (eregi('GGN', $numero) || eregi('DAL', $numero)) 
	{
		$gerencia = 'GGN';
	...
	$empleado_remision = mb_strtoupper($Custom->salida[0]['nivel_academico']).mb_strtoupper($Custom->salida[0]['empleado']);
	...
	{					
		...
		if($tam_desc_doc >= 25)
		{
			...
			imagettftext($titulo_doc, 14, 0, 125, 78, $negro, $fuente, $desc_documento);
		}		
		else 
		{
			$fuente = 'fonts_ttf/ariblk.ttf';
	...
	$empleado = explode(' ',$desc_empleado); //1er nombre completo 2do abrev. 1er ap pat completo 2do ap mat abrev.
	...
	if($var == 1) //internas
	{
		if($desc_documento == 'RESOLUCION')
		{
			...
			$Rtf->addText("<strong><u>$referencia</u></strong><BR>");
			$Rtf->addText("\n");
		...
		if($desc_documento == 'REMISION DE DOCUMENTOS')
		{
			$Rtf->setPaperSize(1);
