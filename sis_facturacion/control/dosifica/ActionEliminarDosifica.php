...
/**
**********************************************************
Nombre de archivo:	    ActionEliminarDosifica.php
Propósito:				Permite eliminar registros de la tabla tfv_dosifica
Tabla:					tfv_dosifica
Parámetros:				$hidden_id_dosifica	--> id del codigo irregular
...
$nombre_archivo = 'ActionEliminarDosifica.php';
...
if($_SESSION['autentificado']=="SI")
{
	...
	for($j = 0;$j < $cont; $j++)
	{
		...
		{
			$resp = new cls_manejo_mensajes(true, "406");
			$resp->mensaje_error = "MENSAJE ERROR = No existe la Dosificación especificada para eliminar.";
			$resp->origen = "ORIGEN = $nombre_archivo";
			$resp->proc = "PROC = $nombre_archivo";
			$resp->nivel = "NIVEL = 4";
			echo $resp->get_mensaje();
		...
		{	//Eliminación
			$res = $Custom-> EliminarDosifica($hidden_id_dosifica);
			if(!$res)
			{
				$resp = new cls_manejo_mensajes(true, "406");
