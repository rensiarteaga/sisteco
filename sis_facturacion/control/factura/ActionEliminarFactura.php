...
/*
**********************************************************
Nombre de archivo:	    ActionEliminarFactura.php
Propósito:				Permite eliminar registros de la tabla tfv_factura
Tabla:					tfv_factura
...
$nombre_archivo = 'ActionEliminarFactura.php';
...
if($_SESSION['autentificado']=="SI")
{
	...
	for($j = 0;$j < $cont; $j++)
	{
		...
		{
			$resp = new cls_manejo_mensajes(true, "406");
			$resp->mensaje_error = "MENSAJE ERROR = No existe la Factura especificada para eliminar.";
			$resp->origen = "ORIGEN = $nombre_archivo";
			$resp->proc = "PROC = $nombre_archivo";
			$resp->nivel = "NIVEL = 4";
			echo $resp->get_mensaje();
		...
		{	//Eliminación
			$res = $Custom-> EliminarFactura($id_factura);
			if(!$res)
			{
				$resp = new cls_manejo_mensajes(true, "406");
