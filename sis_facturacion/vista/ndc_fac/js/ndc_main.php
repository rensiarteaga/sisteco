...
function main(){
	...
	var elemento={pagina:new pagina_ndc(idContenedor,direccion,paramConfig),idContenedor:idContenedor};
...
function pagina_ndc(idContenedor,direccion,paramConfig){
	...
	function btn_nulndc(resp){
		...
		if(NumSelect!=0){
			if(confirm('¿Está seguro de ANULAR la NDC?')){
				Ext.MessageBox.show({
					title: 'Procesando',
				...
				Ext.Ajax.request({
					url:direccion+"../../../control/ndc_fac/ActionProcesNdc.php",
					method:'POST',
					params:{cantidad_ids:'1',id_ndc:SelRecord.data.id_ndc,accion:'anular'},
					success:prg_prores,
					failure:CM_conexionFailure,
					timeout:100000000
				});
	...
	this.EnableSelect=function(sm,row,rec){
		...
		var btn_nulndc = CM_getBoton('ndc_nul-'+ idContenedor);
		var btn_modndc = CM_getBoton('ndc_mod-'+ idContenedor);
	...
	this.AdicionarBoton('../../../lib/imagenes/gtuc/images/grid/drop-no.gif','Anular la Factura',btn_nulndc,true,'ndc_nul','Anular');
	this.AdicionarBoton('../../../lib/imagenes/pencil.png','Editar la Factura',btn_modndc,true,'ndc_mod','Editar');
