<?php
/**
 * Nombre de la Clase:	cls_DBNdcDet
 * Propósito:			Permite ejecutar la funcionalidad de la tabla tfv_ndc_det
 * Autor:				MTSL
...
class cls_DBNdcDet
{
	...
	//Nombre del archivo
	var $nombre_archivo = "cls_DBNdcDet.php";
	...
	/**
	 * Nombre de la función:	ListarNdcDet
	...
	{
		$this->salida = "";
		$this->nombre_funcion = 'f_tfv_ndc_det_sel';
		$this->codigo_procedimiento = "'FV_NDCDET_SEL'";
		...
		$this->var->add_def_cols('id_ndc_det','integer');
		...
		$this->var->add_def_cols('ndc_obsdet','varchar');
	...
	function ContarNdcDet($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad)
	{
		$this->salida = "";
		$this->nombre_funcion = 'f_tfv_ndc_det_sel';
		$this->codigo_procedimiento = "'FV_NDCDET_COUNT'";
	...
	/**
	 * Nombre de la función:	InsertarNdcDet
	 * Propósito:				Permite ejecutar la función de inserción de la tabla tfv_ndc_det
	...
	{
		$this->salida = "";
		$this->nombre_funcion = 'f_tfv_ndc_det_iud';
		$this->codigo_procedimiento = "'FV_NDCDET_INS'";

		//Instancia la clase midlle para la ejecución de la función de la BD
		$this->var = new cls_middle($this->nombre_funcion,$this->codigo_procedimiento,$this->decodificar);

		//Carga parámetros específicos (no incluyen los parámetros fijos)
		$this->var->add_param("NULL");//id ndc_det
		$this->var->add_param("$id_ndc");
		$this->var->add_param("$id_factura_det");
		$this->var->add_param("$ndc_importe");
		$this->var->add_param("'$ndc_obsdet'");
	...
	/**
	 * Nombre de la función:	ModificarNdcDet
	 * Propósito:				Permite ejecutar la función de modificación de la tabla tfv_ndc_det
	 * con los parámetros requeridos
	 * Autor:					MTSL
	 * Fecha de creación:		2014.05
	 */
	function ModificarNdcDet($id_ndc_det, $id_ndc, $id_factura_det, $ndc_importe, $ndc_obsdet)
	{
		$this->salida = "";
		$this->nombre_funcion = 'f_tfv_ndc_det_iud';
		$this->codigo_procedimiento = "'FV_NDCDET_UPD'";

		//Instancia la clase midlle para la ejecución de la función de la BD
		$this->var = new cls_middle($this->nombre_funcion,$this->codigo_procedimiento,$this->decodificar);
		
		//Carga parámetros específicos (no incluyen los parámetros fijos)
		$this->var->add_param("$id_ndc_det");//id de FacturaDet
		$this->var->add_param("$id_ndc");
		$this->var->add_param("$id_factura_det");
		$this->var->add_param("$ndc_importe");
		$this->var->add_param("'$ndc_obsdet'");
	...
	function EditarNdcDet($id_ndc_det, $ndc_obsdet)
	{
		$this->salida = "";
		$this->nombre_funcion = 'f_tfv_ndc_det_iud';
		$this->codigo_procedimiento = "'FV_NDCDET_MOD'";
	...
	/**
	 * Nombre de la función:	EliminarNdcDet
	 * Propósito:				Permite ejecutar la función de eliminación de la tabla tfv_ndc_det
	 * con los parámetros requeridos
	 * Autor:					MTSL
	 * Fecha de creación:		2014.05
	 */
	function EliminarNdcDet($id_ndc_det)
	{
		$this->salida = "";
		$this->nombre_funcion = 'f_tfv_ndc_det_iud';
		$this->codigo_procedimiento = "'FV_NDCDET_DEL'";
	...
	/**
	 * Nombre de la función:	ValidarNdcDet
		...
		switch ($operacion_sql) {
			case 'insert' or 'update':
				...
				{				
					...
					$tipo_dato->set_Columna("id_ndc_det");	
					$tipo_dato->set_MaxLength(15);
					$tipo_dato->set_MinLength(0);
					$tipo_dato->set_Signo('2');
					 
					if(!$valid->verifica_dato($tipo_dato->TipoDatoInteger(), "id_ndc_det", $id_ndc_det))
					{
