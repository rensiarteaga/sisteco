	...
	$data->read('archivos/'.$nombre_archivo);	
		for ($h=0;$h<count($data->sheets);$h++){
			...
			for ($i = 1; $i <= $data->sheets[$h]['numRows']; $i++) {
				...
				$res = $Custom -> InsertarSietCbtePartidaExcel($id_siet_cbte,$importe,$codigo_partida,$codigo_oec);
				if(!$res)
				{
					//Se produjo un error
					$resp = new cls_manejo_mensajes(true, "406");
  	...
  	for ($h=0;$h<count($data->sheets);$h++){
  		...
  		for ($i = 1; $i <= $data->sheets[$h]['numRows']; $i++) {
  			...
  			$res = $Custom ->InsertarSietCbteOecExcel($id_siet_cbte_partida,$importe,$codigo_oec);
  			if(!$res)
  			{
  				//Se produjo un error
  				$resp = new cls_manejo_mensajes(true, "406");
