...
$nombre_archivo = 'ActionListarParFieldGasto.php';
...
if ($sw_reg_comp=='si'&& $m_id_presupuesto){
	$criterio_filtro=$criterio_filtro." AND  (PARTID.id_partida in (select id_partida from presto.tpr_partida_presupuesto 
																		   where id_presupuesto =".$m_id_presupuesto.") or 
											 (PARTID.id_partida  in 				   (select id_partida   from presto.tpr_partida where sw_movimiento=2 and  
...
/*if ($m_sw_partida_cuenta=='si'&& $m_id_gestion){
 ...
 /* echo "Gestion:".$m_id_gestion;
  exit;*/
	...
	$crit_sort = new cls_criterio_sort($sortcol,$sortdir,'ParFieldGasto');
	$sortcol = $crit_sort->get_criterio_sort();
    //Obtiene el total de los registros
	$res = $Custom -> ContarParFieldGasto($cant ,$puntero,$sortcol,$sortdir,$criterio_filtro,$hidden_ep_id_financiador,$hidden_ep_id_regional,$hidden_ep_id_programa,$hidden_ep_id_proyecto,$hidden_ep_id_actividad);

	if($res) $total_registros= $Custom->salida;
	//Obtiene el conjunto de datos de la consulta
	$res = $Custom->ListarParFieldGasto($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$hidden_ep_id_financiador,$hidden_ep_id_regional,$hidden_ep_id_programa,$hidden_ep_id_proyecto,$hidden_ep_id_actividad);
