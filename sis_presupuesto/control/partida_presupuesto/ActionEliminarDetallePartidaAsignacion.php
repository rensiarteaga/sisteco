...
/**
**********************************************************
Nombre de archivo:	    ActionEliminarDetallePartidaFormulacion.php
Propósito:				Permite eliminar registros de la tabla tpr_partida_presupuesto
...
$nombre_archivo = "ActionEliminarDetallePartidaFormulacion.php";
...
if($_SESSION["autentificado"]=="SI")
{
	...
	for($j = 0;$j < $cont; $j++)
	{
		...
		if (($id_partida!= "undefined" && $id_partida!= "")&&($id_presupuesto != "undefined" && $id_presupuesto != "")) {
		
			$res = $Custom->  EliminarDetallePartidaAsignacion($id_partida,$id_presupuesto);
			if(!$res)
			{
				$resp = new cls_manejo_mensajes(true, "406");
		...
		elseif ($id_partida_presupuesto == "undefined" || $id_partida_presupuesto == "")
		{
			$resp = new cls_manejo_mensajes(true, "406");
		...
		{	//Eliminación
			$res = $Custom-> EliminarDetallePartidaFormulacion($id_partida_presupuesto);
			if(!$res)
			{
				$resp = new cls_manejo_mensajes(true, "406");
				$resp->mensaje_error = $Custom->salida[1] ;
