...
class PDF extends FPDF
{
	...
	function Header()
	{
	    ...
	    $this->Ln();
	    $this->Cell(0,3,'Del '.$_SESSION['PDF_fecha_ini'].' Al '.$_SESSION['PDF_fecha_fin']." ",0,0,'C'); //dibuja una celad con contenido y orientacion  x, y 
	    $this->Ln();
		...
		else 	//PRESUPUESTOS DE RECURSOS
	    {
		    ...
		    $this->Cell(25,4,'INGRESAR','B',0,'C',0);		//SALDO POR INGREZAR
	...
	function maestro()
	{
		...
		for($i=0;$i<$_SESSION['PDF_CantFiltros'];$i++)
		{
		 ...
		 $res = $Custom->ListarEjecucionPartida($_SESSION['PDF_limit'],$_SESSION['PDF_start'],'codigo_partida','asc',$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad ,
																			...
																			$_SESSION['PDF_sw_vista'],$_SESSION['PDF_ids_concepto_colectivo'],$fecha_fin,$fecha_ini,$_SESSION['PDF_id_categoria_prog'],$_SESSION['PDF_filtro_niveles']);
			...
			{	 
				 IF($_SESSION['PDF_tipo_pres']>1)	//Presupuestos de gasto o inversion
				 {
					if($row[5]!=1)	//PARA LAS PARTIDAS TOTALITARIAS
					{
						...
						$this->Cell(10,5,number_format( ($row[28]*100) / ($row[34] + 0.01) , 1).' %','LR',0,'R',$fill);//PORCENTAJE DE EJECUCION
