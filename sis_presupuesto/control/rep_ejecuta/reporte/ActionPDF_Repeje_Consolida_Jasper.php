<?php
	...
	if($_SESSION['autentificado']=='SI'){
		...
		if(($sw_ejecuta=='5' || $sw_ejecuta=='8') && $tipo_reporte=='xls' && $sw_sql=='PP'){$sw_sql = 'PPX';}
		...
		if($tipo_reporte=='xls'){
			...
			if($sw_vista=='ejecutar'){
				...
				if($sw_ejecuta=='5' && $sw_eplis!='EP' && $sw_sql=='PPX'){$reporte=new ReportDriver('repejec_ConsGesPP_xls.jasper','sci',$tipo_reporte);}
				if($sw_ejecuta=='5' && $sw_eplis!='EP' && $sw_sql!='PPX'){$reporte=new ReportDriver('repejec_ConsGes_xls.jasper','sci',$tipo_reporte);}
				...
				if($sw_ejecuta=='8' && $sw_eplis!='EP' && $sw_sql=='PPX'){$reporte=new ReportDriver('repejec_ConsGesPP_xls.jasper','sci',$tipo_reporte);}
				if($sw_ejecuta=='8' && $sw_eplis!='EP' && $sw_sql!='PPX'){$reporte=new ReportDriver('repejec_ConsGes_xls.jasper','sci',$tipo_reporte);}
				...
				if($sw_ejecuta=='12'){$reporte=new ReportDriver('repejec_ConsPeri_xls.jasper','sci',$tipo_reporte);}
			}
		}else{
			$sw_det = $tipo_reporte;
			$tipo_reporte = 'pdf';
			...
			if($sw_vista=='ejecutar'){
				...
				if($sw_ejecuta=='4' && $id_tipo_pres!='1' && $sw_impre!=2){$reporte=new ReportDriver('repejec_ConsPFech.jasper','sci',$tipo_reporte);}
				if($sw_ejecuta=='4' && $id_tipo_pres!='1' && $sw_impre==2){$reporte=new ReportDriver('repejec_ConsPFech_dec.jasper','sci',$tipo_reporte);}
				if($sw_ejecuta=='4' && $id_tipo_pres=='1' && $sw_impre!=2){$reporte=new ReportDriver('repejec_ConsPFech_rec.jasper','sci',$tipo_reporte);}
				if($sw_ejecuta=='4' && $id_tipo_pres=='1' && $sw_impre==2){$reporte=new ReportDriver('repejec_ConsPFech_decr.jasper','sci',$tipo_reporte);}
				...
				if($sw_ejecuta=='12' && $id_tipo_pres!='1' && $sw_impre!=2){$reporte=new ReportDriver('repejec_ConsPeri.jasper','sci',$tipo_reporte);}
				if($sw_ejecuta=='12' && $id_tipo_pres!='1' && $sw_impre==2){$reporte=new ReportDriver('repejec_ConsPeri_dec.jasper','sci',$tipo_reporte);}
				if($sw_ejecuta=='12' && $id_tipo_pres=='1' && $sw_impre!=2){$reporte=new ReportDriver('repejec_ConsPeri_rec.jasper','sci',$tipo_reporte);}
				if($sw_ejecuta=='12' && $id_tipo_pres=='1' && $sw_impre==2){$reporte=new ReportDriver('repejec_ConsPeri_decr.jasper','sci',$tipo_reporte);}
			}
		...
		if($sw_vista=='ejecutar'){
			...
			if($sw_ejecuta=='5' || $sw_ejecuta=='6' || $sw_ejecuta=='7' || $sw_ejecuta=='8' || $sw_ejecuta=='12'){$reporte->addParametro('sw_sql',$sw_sql);}
			...
			if($sw_ejecuta=='12'){$reporte->addParametro('id_codproc','PR_EJEPERI_SEL');}
	...
	/*	if ($_SESSION["ss_id_usuario"]==120){
			echo 'opcion:',$sw_ejecuta. '$sw_sql:'. $sw_sql. '$sw_mes:'.$sw_mes.'  $sw_trim:'.$sw_trim .' $sw_impre:'.$sw_impre .', $id_parametro:'.$id_parametro.
			'$id_tipo_pres:'.$id_tipo_pres.' ,$sw_nivel:'.$sw_nivel. ',$sw_ppto:'.$sw_ppto .', $sw_cprog:'.$sw_cprog.' ,$sw_ep_fina:'.$sw_ep_fina
			.', $sw_ep_regi'.$sw_ep_regi.' ,$sw_ep_prog:'.$sw_ep_prog.' ,$sw_ep_proy:'.$sw_ep_proy.' ,$sw_ep_acti:'. $sw_ep_acti. ' ,$sw_ep_uo:'.$sw_ep_uo
			.' ,$sw_cp_prog:'.$sw_cp_prog.' ,$sw_cp_proy:'.$sw_cp_proy. ', $sw_cp_acti:'.$sw_cp_acti. ',$sw_cp_fuen:'.$sw_cp_fuen.
			' ,$sw_cp_orga:'.$sw_cp_orga. ' ,$ids_ppto:'.$ids_ppto.' ,$ids_cprog:'.$ids_cprog. ' ,$ids_ep_fina:'.$ids_ep_fina. ' ,$ids_ep_regi:'.$ids_ep_regi
			.' ,$ids_ep_prog:'.$ids_ep_prog. ' ,$ids_ep_proy:'.$ids_ep_proy. ' ,$ids_ep_acti:'.$ids_ep_acti. ' ,$ids_ep_uo:'.$ids_ep_uo.
			' ,$ids_cp_prog:'.$ids_cp_prog.',$ids_cp_proy:'.$ids_cp_proy.' ,$ids_cp_acti:'.$ids_cp_acti.' ,$ids_cp_fuen:'.$ids_cp_fuen.
			' ,$ids_cp_orga:'.$ids_cp_orga; exit;
		}*/
		if($tipo_reporte=='pdf'){
