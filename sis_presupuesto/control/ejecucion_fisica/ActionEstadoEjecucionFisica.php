...
$nombre_archivo = "ActionEstadoEjecucionFisica.php";
...
if($_SESSION["autentificado"]=="SI")
{
	...
	for($j = 0;$j < $cont; $j++)
	{
		if ($get)
		{ 		
			$id_ejecucion_fisica = $_GET["id_ejecucion_fisica"];
			$accion = $_GET["accion"];
		...
		{
			$id_ejecucion_fisica = $_POST["id_ejecucion_fisica"];
			$accion = $_POST["accion"];			
		}
		...
		{	
			...
			$res = $Custom-> CambiarEstadoEjecucionFisica($id_ejecucion_fisica,$accion);
			if(!$res)
			{	
				$resp = new cls_manejo_mensajes(true, "406");
