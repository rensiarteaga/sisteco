...
/**** fin de REP **/
	...
	if($vista=='rep_formulacion_ejecucion'){
     $criterio_filtro="  PRESUP.id_parametro=$m_id_parametro_g and tipo_press=$m_tipo_pres_g
                                 and (SELECT
                                    sum(mes_01)as Enero
 ...
 if ($reporte_excel=='si')
	{	//recupera los valores de las columnas
		for($i=0;$i<$nro_columnas;$i++){
			...
			//echo " v=".$_GET["valor_$i"]." c=".$_GET["columna_$i"]." a=".$_GET["align_$i"]." w=".$_GET["width_$i"]." i=".$i."</br>";
		}	
		//exit;
...
//--jgl fin
	//avq adición de filtro para presupuesto
