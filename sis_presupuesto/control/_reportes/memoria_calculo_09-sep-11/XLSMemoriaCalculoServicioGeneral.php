<?php
	...
	$nombre_archivo='MEMORIA_CALC_SERV_GRAL.xls';
	$docexcel = new Spreadsheet_Excel_Writer(); //creamos una instancia de excel
	$nuevahoja =& $docexcel->addWorksheet($_SESSION['rep_mem_serv_codigo_partida']); //colocamos nombre a la pestaña
	...
	$nuevahoja->write(2,2,'DIRECCIÓN GENERAL DE ASUNTOS ADMINISTRATIVOS',$titulo1); //dibuja una celad con contenido y orientacion  x, y 
	$nuevahoja->write(3,2,'MEMORIAS DE CÁLCULO',$titulo1);	
	$nuevahoja->write(4,2,'ANTEPROYECTO PRESUPUESTO GESTIÓN '.$_SESSION['rep_mem_serv_gestion_pres'],$titulo2);
	$nuevahoja->write(5,2,'COSTOS ESTIMADOS POR PARTIDA DE GASTO',$titulo2);	
	$nuevahoja->write(7,2,'PARTIDA '.$_SESSION['rep_mem_serv_codigo_partida'].' "'.$_SESSION['rep_mem_serv_nombre_partida'].'"',$negritaCentradoSubrayado);	
	...
	else
	{
		...
		$nuevahoja->write(10,0,"COD. PROGRAMA: ");
		$nuevahoja->write(10,1,$cod_prog);
		
		$nuevahoja->write(11,0,"COD. PROYECTO: ");
		$nuevahoja->write(11,1,$cod_proy);
		
		$nuevahoja->write(12,0,"COD. ACTIVIDAD: ");
		$nuevahoja->write(12,1,$cod_act);
		
		$nuevahoja->write(13,0,"COD. ORGANISMO FINANCIADOR: ");
		$nuevahoja->write(13,1,$cod_organismo);
			
		$nuevahoja->write(14,0,'COD. FUENTE DE FINANCIAMIENTO: ');
		$nuevahoja->write(14,1,$cod_fuente_fin);
	}
	
	
	
	$nuevahoja->write(12,4,$_SESSION['rep_mem_serv_cod_formulario_gasto'],$bordesSimplesNegrita);
	$nuevahoja->write(14,4,'GESTIÓN:     '.$_SESSION['rep_mem_serv_gestion_pres'],$bordesSimplesNegrita);
	$nuevahoja->write(15,4,'FECHA ELAB.: '.$_SESSION['rep_mem_serv_fecha_elaboracion'],$bordesSimplesNegrita);	

	
	//cabeceras de las columnas	
    $nuevahoja->write(18,0,'Nro.',$negritaCentrado);
    $nuevahoja->write(18,1,'DESCRIPCIÓN DEL SERVICIO',$negritaCentrado);
    $nuevahoja->write(18,2,'COSTO MENSUAL',$negritaCentrado);
    $nuevahoja->write(18,3,'COSTO ANUAL',$negritaCentrado);
    $nuevahoja->write(18,4,'JUSTIFICACIÓN',$negritaCentrado);	
	...
	$res = $Custom->ListarRepGralMemoriaServicio($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);		
	...
	if($res)
	{
		...
		$res2 = $CustomS->SumaCostoMemoriaServicio(15,0,'','',$criterio_suma,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);		
