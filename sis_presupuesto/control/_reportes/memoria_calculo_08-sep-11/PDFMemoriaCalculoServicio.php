...
class PDF extends FPDF
{
	...
	$this->Ln(5);
	$this->Cell(200,10,'ANTEPROYECTO PRESUPUESTO GESTIÓN '.$_SESSION['rep_mem_serv_gestion_pres'],0,0,'C');
	$this->Ln(5);
	$this->Cell(200,10,'COSTOS ESTIMADOS POR PARTIDA DE GASTO',0,0,'C');
	$this->Ln(5);
	$this->Cell(200,10,'PARTIDA '.$_SESSION['rep_mem_serv_codigo_partida'].' "'.$_SESSION['rep_mem_serv_nombre_partida'].'"',0,0,'C');
	$this->Ln(10);
	$this->SetFont('Arial','B',7);
	$this->Cell(22,4,'REGIONAL','LTR',0,'L');
	$this->Cell(78,4,$_SESSION['rep_mem_serv_regional'],'TR',0,'L');
	$this->Cell(25,4,'',0,0,'L');
	$this->Cell(45,4,'',0,0,'L');
	$this->Ln(4);
	$this->Cell(22,4,'ORG. FINANC.','LTR',0,'L');
	$this->Cell(78,4,$_SESSION['rep_mem_serv_financiador'],'TR',0,'L');
	$this->Cell(25,4,'',0,0,'L');
	$this->Cell(45,4,'',0,0,'L');
	$this->Ln(4);
	$this->Cell(22,4,'PROGRAMA','LTR',0,'L');
	$this->Cell(78,4,$_SESSION['rep_mem_serv_programa'],'TR',0,'L');
	$this->Cell(45,4,'',0,0,'L');
	$this->Cell(45,4,$_SESSION['rep_mem_serv_cod_formulario_gasto'],'LTRB',0,'C');
	$this->Ln(4);
	$this->Cell(22,4,'SUBPROGRAMA','LTR',0,'L');
	$this->Cell(78,4,$_SESSION['rep_mem_serv_proyecto'],'TR',0,'L');
	$this->Cell(25,4,'',0,0,'L');
	$this->Cell(45,4,'',0,0,'L');
	$this->Ln(4);
	$this->Cell(22,4,'ACTIVIDAD','LTR',0,'L');
	$this->Cell(78,4,$_SESSION['rep_mem_serv_actividad'],'TR',0,'L');
	$this->Cell(25,4,'',0,0,'L');
	$this->Cell(45,4,'',0,0,'L');
	$this->Ln(4);
	...
	$this->Cell(78,4,$_SESSION['rep_mem_serv_unidad_organizacional'],'TRB',0,'L');
	$this->Cell(45,4,'',0,0,'L');
	...
	$this->Cell(65,4,'DESCRIPCIÓN DEL SERVICIO','R',0,'C');
	$this->Cell(25,4,'COSTO MENSUAL','R',0,'C');
	$this->Cell(25,4,'PERIODO','R',0,'C');
	$this->Cell(65,4,'JUSTIFICACIÓN','R',0,'C');	
	$this->Ln(4);
	$this->SetFont('Arial','B',4);
	$this->Cell(10,4,'','LR',0,'L');
	$this->Cell(65,4,'','R',0,'L');
	$this->Cell(25,4,'('.$_SESSION['rep_mem_serv_simbolo'].')','R',0,'C');
	...
	function Footer(){
		...
		$this->ln(4);
		$this->Cell(140,4,'','L',0,'C');
		$this->Cell(50,4,'FIRMA Y SELLO','R',0,'C');
		$this->ln(4);
		$this->Cell(125,3,'','LTR',0,'L');
		$this->Cell(65,3,'','LTR',0,'C');
		$this->ln(3);
		$this->Cell(20,5,'Elaborado Por: ','L',0,'L');
		$this->Cell(103,5,'','B',0,'L');
		$this->Cell(2,5,'','R',0,'L');
		$this->Cell(2,5,'','L',0,'L');
		$this->Cell(61,5,'','B',0,'L');
		$this->Cell(2,5,'','R',0,'L');
		$this->ln(5);
		...
		$this->Cell(2,5,'','R',0,'L');
		$this->ln(5);
		$this->Cell(125,3,'','LBR',0,'L');
		$this->Cell(65,3,'','LBR',0,'C');
		$this->ln(5);
	...
	function LoadData($id_partida_presupuesto,$id_presupuesto,$tipo_memoria,$id_partida,$id_moneda,$tipo_pres){
		$cant=1000;
		//$cant=15;
	    $puntero=0;
	    $sortcol='MEMSER.periodo_pres';
	    $sortdir='asc';	    
		...
		$Custom->ListarRepMemoriaServicio($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);		
	...
	//Tabla coloreada
	function FancyTable($data,$id_partida_presupuesto,$id_presupuesto,$tipo_memoria,$id_partida,$id_moneda,$tipo_pres){
	...
	/*	$this->SetFillColor(255,255,255);
		...
		$CustomC->ContarRepMemoriaServicio(15,0,'','',$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);		
	    
		$cantidad=sizeof($CustomC->salida);
		
	    $CustomS= new cls_CustomDBPresupuesto();
		$CustomS->SumaCostoMemoriaServicio(15,0,'','',$criterio_suma,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);		
		...
		$w=array(10,65,25);
			...
			//Datos			    
				$numero=1;
				$this->SetWidths(array(10,65,25,25,65));
				$this->SetFills(array(0,0,0,0,0));
				$this->SetAligns(array('C','L','R','C','J'));
		       ...
		       /*foreach($data as $row){       
					...
					$this->Cell($w[2],4,$row[2],'R',0,'L');
					$this->MultiCell($w[1],4,$row[3],'RL','J',0);
					//$this->Ln(4);
					$numero=$numero+1;
			    }
			   ...
			   $this->Cell($w[2],4,number_format($suma_total, 2, '.', ','),'LTRB',0,'R');
...
$pdf->AliasNbPages();
//Carga de datos
$data=$pdf->LoadData($id_partida_presupuesto,$id_presupuesto,$tipo_memoria,$id_partida,$id_moneda,$tipo_pres);
...
$pdf->AddPage();
$pdf->FancyTable($data,$id_partida_presupuesto,$id_presupuesto,$tipo_memoria,$id_partida,$id_moneda,$tipo_pres);
$pdf->Output();
?>