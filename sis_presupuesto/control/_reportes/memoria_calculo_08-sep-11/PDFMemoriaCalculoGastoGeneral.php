...
class PDF extends FPDF
{
	...
	$this->Cell(25,4,'('.$_SESSION['rep_mem_cal_simbolo'].')','R',0,'C');
	$this->Cell(25,4,'('.$_SESSION['rep_mem_cal_simbolo'].')','R',0,'C');
	$this->Cell(65,4,'','R',0,'L');
	$this->Ln(3);
	$this->Cell(10,4,'','LTR',0,'L');
	...
	function LoadData($id_partida_presupuesto,$id_presupuesto,$tipo_memoria,$id_partida,$id_moneda,$tipo_pres){
		...
		$Custom->ListarRepGralMemoriaGasto($cant,$puntero,$sortcol,$sortdir,$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);		
	...
	/*	$this->SetFillColor(255,255,255);
		...
		$CustomC->ContarRepGralMemoriaGasto(15,0,'','',$criterio_filtro,$id_financiador,$id_regional,$id_programa,$id_proyecto,$id_actividad);		
       ...
       /*foreach($data as $row){       
			...
			//$this->MultiCell($w[1],4,$row[3],'R','L'); //justificacion
	   ...
	   for($i=1;$i<=$restante;$i++){
			...
			$this->Cell($w[2],4,'','R',0,'C');
			//$this->MultiCell($w[1],4,'','R','C');
			$this->MultiCell($w[1],4,'','RL','J',0);
			//$this->Ln(4);
			$numero=$numero+1;
	   }*/
	   $this->SetFont('Arial','',7);
