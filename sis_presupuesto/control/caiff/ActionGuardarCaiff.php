...
if($_SESSION["autentificado"]=="SI")
{
	...
	for($j = 0;$j < $cont; $j++)
	{
		if ($get)
		{
			...
			$id_gestion= $_GET["h_id_gestion_$j"];
			$id_periodo= $_GET["h_id_periodo_$j"];
			$descripcion= $_GET["txt_descripcion_$j"];
			$fecha_ini= $_GET["txt_fecha_inicio_$j"];
			$fecha_fin= $_GET["txt_fecha_fin_$j"];
		}
		else
		{
			$id_caiff= $_POST["h_id_caiff_$j"];
			$id_gestion= $_POST["h_id_gestion_$j"];
			$id_periodo= $_POST["h_id_periodo_$j"];
			$descripcion= $_POST["txt_descripcion_$j"];
			$fecha_ini= $_POST["txt_fecha_inicio_$j"];
			$fecha_fin= $_POST["txt_fecha_fin_$j"];
		}
		...
		if ($id_caiff == "undefined" || $id_caiff == "")
		{
			...
			$res = $Custom->ValidarCaiff("insert",$id_caiff,$id_gestion,$id_periodo,$descripcion);
			...
			//Validaci�n satisfactoria, se ejecuta la inserci�n en la tabla tpr_usuario_autorizado
			$res = $Custom -> InsertarCaiff($id_gestion,$id_periodo,$descripcion,$fecha_ini,$fecha_fin);
		...
		{	///////////////////////Modificaci�n////////////////////
			...
			//Validaci�n de datos (del lado del servidor)
			$res = $Custom->ValidarCaiff("update",$id_caiff,$id_gestion,$id_periodo,$descripcion);
			...
			$res = $Custom->ModificarCaiff($id_caiff,$id_gestion,$id_periodo,$descripcion,$fecha_ini,$fecha_fin);
