...
function pagina_repeje_consolida(idContenedor, direccion, paramConfig, sw_vista) {
	...
	// Definición de datos //
	/////////////////////////  ['14','Comparativo'],   ,['15','Mensual - Inversión']
	vectorAtributos[0]={
		validacion:{
			name: 'sw_ejecuta',
			...
			store:new Ext.data.SimpleStore({fields:['ID','valor'],data:[['1','© | A una Fecha'],['2','© | Entre Fechas'],['3','© | Fechas - Acumulado'],['4','© | Periodo - Acumulado'],
			                                                            ...
			                                                            ['12','® | A un Periodo'],['5','® | Anual'],['6','® | Trimestral'],['7','® | Mensual'],['8','® | Mensual - Acumulado']]}),
			valueField:'ID',
			displayField:'valor',
			lazyRender:true,
			forceSelection:true,
	...
	vectorAtributos[39]={
		validacion:{
			...
			loadMask:true,
			triggerAction:'all',
			store:new Ext.data.SimpleStore({fields:['id','valor'],data:[['PG','Programa'],['YE','Proyecto'],['AT','Actividad'],['FF','Fuente de Financiamiento'],['OF','Organismo Financiador'],['GY','Programa - Proyecto']]}),
			valueField:'id',
			displayField:'valor',
			lazyRender:true,
			forceSelection:true,
	...
	function f_evento_sw_ejecuta(combo, record, index){
		...
		/*if (record.data.ID == '14'){
			CM_mostrarComponente(comp_sw_mes);
			comp_sw_mes.allowBlank=false;
		}
		if (record.data.ID == '15'){
			CM_mostrarComponente(comp_fecha_ini);
			CM_mostrarComponente(comp_fecha_fin);
			comp_fecha_ini.allowBlank=false;
			comp_fecha_fin.allowBlank=false;
	...
	function f_evento_sw_filtro(combo, record, index){
		...
		if (record.data.ID == '3'){
			...
			if (comp_sw_ejecuta.getValue() == '5' || comp_sw_ejecuta.getValue() == '6' || comp_sw_ejecuta.getValue() == '7' || comp_sw_ejecuta.getValue() == '8' || comp_sw_ejecuta.getValue() == '12'){
