...
function pagina_siet_cbte_partida(idContenedor,direccion,paramConfig,maestro,idContenedorPadre)
{
	...
	ds.load({
	 ...
	 var ds_partida = new Ext.data.Store({proxy: new Ext.data.HttpProxy({url: direccion+'../../../control/partida/ActionListarPartida.php?sw_vista_reporte=rep_ejecucion_partida&id_tipo_pres='+maestro.tipo_declara+'&id_parametro='+maestro.id_parametro}),
	 ...
	 var ds_entidad_transf = new Ext.data.Store({proxy: new Ext.data.HttpProxy({url: direccion+'../../../control/interfaz_siet/ActionListarEntidadTransf.php'}),
			reader: new Ext.data.XmlReader({record:'ROWS',id:'id_siet_ent_ua_transf',totalRecords:'TotalCount'},['id_siet_ent_ua_transf','id_siet_entidad_transf','codigo','denominacion','sigla','codigo_ua','denominacion_ua','sigla_ua','desc_entidad'])
			//baseParams:{sw_vista_reporte:'rep_ejecucion_partida',sid_tipo_pres:'2',$id_parametro:maestro.id_parametro}
	});
	...
	function render_id_siet_entidad_transf(value, p, record){return String.format('{0}', record.data['desc_entidad']);}
	var tpl_id_siet_entidad_transf=new Ext.Template('<div class="search-item">','<b><i>Entidad Transferencia: {desc_entidad}</i></b>','</div>');
	...
	this.reload=function(params){
		...
		ds_partida.baseParams={
				sw_vista_reporte:'rep_ejecucion_partida',
				id_tipo_pres:maestro.tipo_declara,
