/**
 * Nombre:		  	    pagina_partida_incremento.js
 * Propósito: 			pagina objeto principal
 * Autor:				Generado Automaticamente
 * Fecha creación:		2009-02-04 19:45:09
 */
function pagina_partida_incremento(idContenedor,direccion,paramConfig){
	...
	var ds = new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({url: direccion+'../../../control/partida_traspaso/ActionListarPartidaIncremento.php'}),
		...
		baseParams:{filtro_incremento:'si'},
		remoteSort:true});
...
// txt id_partida_ejecucion_destino
	...
	Atributos[5] = {
		validacion: {
			...
			fieldLabel:'Estado Incremento',
			vtype:'texto',
			//emptyText:'Estado Incr...',
			allowBlank: true,
			typeAhead: true,
			loadMask: true,
			triggerAction: 'all',
...
// txt justificacion
	...
	var config={titulo_maestro:'Incrementos',grid_maestro:'grid-'+idContenedor};
	var layout_partida_incremento=new DocsLayoutMaestro(idContenedor);
	layout_partida_incremento.init(config);
	...
	this.pagina(paramConfig,Atributos,ds,layout_partida_incremento,idContenedor);
    ...
    {	
		...
		{			
			Ext.MessageBox.alert('...', 'Solo incrementos en estado VERIFICACION o RECHAZADO pueden ser modificados.');
		}
	}
    ...
    {		
		...
		{				
			Ext.MessageBox.alert('...', 'Solo incrementos en estado VERIFICACION o RECHAZADO pueden ser eliminados.');
		}
	}	
	    
    function InitPaginaPartidaIncremento()
    {						
	...
	{
			componentes[13].store.baseParams={sw_traspaso:'si',m_id_parametro:componentes[6].getValue(),m_tipo_pres:componentes[19].getValue(),m_id_unidad_organizacional:record.data.id_unidad_organizacional};
			componentes[13].modificado=true;
	...
	function btn_autorizacion_incremento()
	{
		...
		{			
			...
			//{
				var sw=false;
				if(confirm('Esta seguro de enviar para AUTORIZACIÓN los Incrementos seleccionados?'))
						{sw=true}
				if(sw)
				{
					
					var SelectionsRecord=sm.getSelections(); 			
		 			var arr_id_partida_traspasos = new Array;
		 			for(var i=0 ; i<NumSelect ; i++)
		 			{
					    arr_id_partida_traspasos[i]=SelectionsRecord[i].data.id_partida_traspaso;
					    Ext.Ajax.request({
						url:direccion+"../../../control/partida_traspaso/ActionEstadoPartidaTraspaso.php",
						method:'POST',
						params:{cantidad_ids:NumSelect, id_partida_traspaso:arr_id_partida_traspasos[i], accion:'enviar_autorizar'},  //estado traspaso el estado esta en concluido		
		...
		{
			Ext.MessageBox.alert('...', 'Antes debe seleccionar por lo menos un registro.');
		}	
	}	

	/*function btn_concluir_incremento()
	{
		...
		{			
			...
			{
				var sw=false;
				if(confirm('Esta seguro de CONCLUIR el Incremento?'))
						{sw=true}
				if(sw)
				{
					var concluir='si';
					...
					var SelectionsRecord=sm.getSelected();			
	 				...
	 				data=data+'&incremento=si';
					//window.open(direccion+'../../../control/_reportes/partida_traspaso/PartidaIncremento.php?'+data, "Incremento");				
				}
				ClaseMadre_btnActualizar();				
			}
			else
			{
				Ext.MessageBox.alert('...', 'Solo incrementos en estado APROBADO pueden ser concluidos.');
			}	
		}
		else
		{
	...
	function btn_concluir_incremento()
	{
		...
		{	
			var sw=false;
			if(confirm('Esta seguro de CONCLUIR los registros seleccionados?'))
					{sw=true}
			if(sw)
			{					
				var SelectionsRecord=sm.getSelections(); 			
				...
				for(var i=0 ; i<NumSelect ; i++)
				{
					...
					url:direccion+"../../../control/partida_traspaso/ActionEstadoPartidaTraspaso.php",
					method:'POST',
					params:{cantidad_ids:NumSelect, id_partida_traspaso:arr_id_partida_traspasos[i], accion:'concluir_modificacion'},  //estado traspaso el estado esta en concluido		
					success:modificacion_Success,						
					failure:ClaseMadre_conexionFailure,
					timeout:100000000
					});						    
	...
	/*function btn_reporte_incremento()
	{
		var sm=getSelectionModel();
		var filas=ds.getModifiedRecords();
		var cont=filas.length;
		...
		if(NumSelect!=0)
		{
			if(componentes[5].getValue()==2)	//estado traspaso concluido
			{
				...
				window.open(direccion+'../../../control/_reportes/partida_traspaso/PartidaTraspaso.php?'+data, "Incremento");	
			}
			else
			{
				Ext.MessageBox.alert('...', 'Solo incrementos en estado CONCLUIDO pueden ser impresos.');
			}			
		}
		else
		{
	...
	this.getLayout=function(){return layout_partida_incremento.getLayout()};
	...
	this.AdicionarBoton('../../../lib/imagenes/tucrem.png','Autorización Incremento',btn_autorizacion_incremento,true,'autorizacion_incremento','Enviar Para Autorización');
	this.AdicionarBoton('../../../lib/imagenes/book_next.png','Concluir Incremento',btn_concluir_incremento,true,'concluir_incremento','Concluir Incremento');
	//this.AdicionarBoton('../../../lib/imagenes/print.gif','Imprimir Reporte Incremento',btn_reporte_incremento,true,'reporte_incremento','Imprimir Reporte');
	
	CM_getBoton('editar-'+idContenedor).disable();
	CM_getBoton('eliminar-'+idContenedor).disable();
	CM_getBoton('autorizacion_incremento-'+idContenedor).disable();
	CM_getBoton('concluir_incremento-'+idContenedor).disable();	
	...
	{		
		...
		if(rec.data['estado_traspaso']=='1')//Verificación
		{
			...
			CM_getBoton('autorizacion_incremento-'+idContenedor).enable();
			CM_getBoton('concluir_incremento-'+idContenedor).disable();
			//CM_getBoton('reporte_incremento-'+idContenedor).disable();
		}
		...
		if(rec.data['estado_traspaso']=='4')//Aprobado
		{
			...
			CM_getBoton('eliminar-'+idContenedor).disable();
			CM_getBoton('autorizacion_incremento-'+idContenedor).disable();
			CM_getBoton('concluir_incremento-'+idContenedor).enable();
			//CM_getBoton('reporte_incremento-'+idContenedor).disable();
		}
	...
	layout_partida_incremento.getLayout().addListener('layout',this.onResize);//aregla la forma en que se ve el grid dentro del layout
