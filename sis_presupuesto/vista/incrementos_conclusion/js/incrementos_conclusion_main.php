...
var elemento={pagina:new pagina_incrementos_conclusion(idContenedor,direccion,paramConfig),idContenedor:idContenedor};
 ...
 * Fecha creación:		2010-05-10 18:01:22
 */
function pagina_incrementos_conclusion(idContenedor,direccion,paramConfig)
{
	...
	var ds = new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({url: direccion+'../../../control/modificacion/ActionListarModificacion.php?tipo_modificacion=3&conclusion=si'}),
	...
	function render_mod(value, p, record){	
		...
		if(value=='4'){return 'Presupuesto Adicional';}
	}
	...
	Atributos[13]={
		validacion:{
			...
			minLength:0,
			triggerAction:'all',
			store:new Ext.data.SimpleStore({fields:['ID','valor','filtro'],data:[['2','Inter Institucional'],['4','Presupuesto Adicional']]}),
			valueField:'ID',
			displayField:'valor',
			mode:'local',
			lazyRender:true,			
	...
	var layout_incrementos_conclusion=new DocsLayoutMaestro(idContenedor);
	layout_incrementos_conclusion.init(config);
	...
	this.pagina(paramConfig,Atributos,ds,layout_incrementos_conclusion,idContenedor);
	...
	function btnDetalleModificaciones(){
		...
		if(NumSelect!=0){
			...
			layout_incrementos_conclusion.loadWindows(direccion+'../../../../sis_presupuesto/vista/partida_modificacion_dest/partida_modificacion_dest_conclusion.php?'+data,'Disminuciones',ParamVentana);
			layout_incrementos_conclusion.getVentana().on('resize',function(){
				layout_incrementos_conclusion.getLayout().layout();
			});
		}else{
	...
	this.getLayout=function(){return layout_incrementos_conclusion.getLayout()};
	...
	var gestion =new Ext.form.ComboBox({
  	...
  	gestion.on('select',function (combo, record, index){
	  	...
	  	componentes[12].store.baseParams={id_gestion:g_id_gestion}
	...
	layout_incrementos_conclusion.getLayout().addListener('layout',this.onResize);//aregla la forma en que se ve el grid dentro del layout
