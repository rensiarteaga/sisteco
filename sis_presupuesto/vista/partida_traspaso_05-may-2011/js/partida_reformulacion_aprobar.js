/**
 * Nombre:		  	    pagina_partida_reformulacion_aprobar.js
 * Propósito: 			pagina objeto principal
 * Autor:				Generado Automaticamente
 * Fecha creación:		2009-02-04 19:45:09
 */
function pagina_partida_reformulacion_aprobar(idContenedor,direccion,paramConfig){
	...
	var ds = new Ext.data.Store({
		...
		baseParams:{filtro_aprobacion:'si',filtro_refor:'si'},
		remoteSort:true});
...
// txt justificacion
	...
	var config={titulo_maestro:'Aprobación de Traspasos',grid_maestro:'grid-'+idContenedor};
	var layout_partida_traspaso_aprobar=new DocsLayoutMaestro(idContenedor);
	layout_partida_traspaso_aprobar.init(config);
	...
	this.pagina(paramConfig,Atributos,ds,layout_partida_traspaso_aprobar,idContenedor);
	...
	function btn_aprobar_traspaso()
	{
		...
		{			
			...
			{
				var sw=false;
				if(confirm('Esta seguro de APROBAR el Traspaso?'))
						{sw=true}
				if(sw)
				{
					var concluir='no';
			...
			{
				Ext.MessageBox.alert('...', 'Solo traspasos en estado AUTORIZACION pueden ser aprobados.');
			}	
		}
		else
		{
	...
	function btn_rechazar_traspaso()
	{
		...
		{			
			...
			{
				var sw=false;
				if(confirm('Esta seguro de RECHAZAR el Traspaso?'))
						{sw=true}
				if(sw)
				{
					var concluir='no';					
			...
			{
				Ext.MessageBox.alert('...', 'Solo traspasos en estado AUTORIZACION pueden ser rechazados.');
			}	
		}
		else
		{
	...
	this.getLayout=function(){return layout_partida_traspaso_aprobar.getLayout()};
	...
	this.AdicionarBoton('../../../lib/imagenes/ok.png','Aprobar Traspaso',btn_aprobar_traspaso,true,'aprobar_traspaso','Aprobar Traspaso');
	this.AdicionarBoton('../../../lib/imagenes/cross.gif','Aprobar Traspaso',btn_rechazar_traspaso,true,'rechazar_traspaso','Rechazar Traspaso');
	
	this.iniciaFormulario();
	InitPaginaPartidaTraspaso();
	iniciarEventosFormularios();
	layout_partida_traspaso_aprobar.getLayout().addListener('layout',this.onResize);//aregla la forma en que se ve el grid dentro del layout
