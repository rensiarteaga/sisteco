...
//alert ('llega a la vista4');
	...
	this.btnEdit=function()
    {
		...
		params:{
			...
			id_moneda:componentes[7].getValue(),
	...
	function evento_presupuesto( combo, record, index )
	{
 	...
 	function evento_partida (combo, record, index)
 	{
 		...
 		componentes[7].on('select',evento_moneda); //moneda*/
   ...
   function  miSave(a,b){
	...
	{		
		...
		if(rec.data['estado']=='Validado'  || rec.data['estado']=='Concluido')//conta_rendicion
		{
			//alert("llega disa");
			CM_getBoton('nuevo-'+idContenedor).disable();
			CM_getBoton('editar-'+idContenedor).disable();
	...
	this.iniciaFormulario({width:800,legend:'Distribución Partida Origen', id:'grilla_adicional'+idContenedor});
    ...
    grid2.on('beforeedit',function(e)
    {
      ...
      {       	
	     ...
	     {   	
	     	...
	     	//if( (e.value*(-1)) <= ds2.getAt(0).data[e.field] && e.record.data.fila!='total' && e.value <= 0)
