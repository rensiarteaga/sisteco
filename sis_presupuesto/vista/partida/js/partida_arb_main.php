...
var elemento={pagina:new pagina_partida_arb(idContenedor,direccion,paramConfig),idContenedor:idContenedor};
ContenedorPrincipal.setPagina(elemento);
}
Ext.onReady(main,main);

function pagina_partida_arb(idContenedor,direccion,paramConfig){
	var Atributos=[];
	var ds_parametro;
	var layout_partida_arb;
	...
	Atributos[9]={
		validacion:{
			...
			loadMask:true,
			triggerAction:'all',
			store:new Ext.data.SimpleStore({fields:['id','valor'],data:[['2','Gastos x Item'],['3','Inversión'],['4','Pasajes'],['5','Viajes'],['6','RRHH'],['7','Servicios'],['9','Combustible'],['8','Otros Gastos']]}),
			valueField:'id',
			displayField:'valor',
			lazyRender:true,
			forceSelection:true,
	...
	layout_partida_arb=new DocsLayoutArb(idContenedor);
	layout_partida_arb.init(config);
	
	/// HEREDAMOS DE LA CLASE MADRE
	this.pagina=PaginaArb;
	//-- pagina (array PARAMETROS DE CONFIGURACION, array DEFINICION DE ATRIBUTOS, SELECTION MODEL, DATA STORE, COLUM MODEL)
	this.pagina(paramConfig,Atributos,layout_partida_arb,idContenedor,DatosNodo,DatosDefecto);
	...
	var paramFunciones={
		...
		Listar:{url:direccion+'../../../../sis_presupuesto/control/partida/ActionListarPartidaArb.php',raiz:'agrupador',baseParams:{},clearOnLoad:true,enableDD:false}
	};
	...
	function btn_concepto_ingas(){
	   ...
	   if(!n || n.attributes.nivel_partida==0){
			Ext.MessageBox.alert('...','Seleccione una partida de tipo Movimiento.')
		}else{
			if(n&&n.attributes.sw_transaccional==2){
				Ext.MessageBox.alert('...','Debe seleccionar una partida de tipo Movimiento.')
		    }else{
				...
				var ven=layout_partida_arb.loadWindows(direccion+'../../../../sis_presupuesto/vista/concepto_ingas/concepto_ingas.php?'+data,'Conceptos de Gasto',Param)
		   }
		}		
	}
   ...
   this.getLayout=function(){
		return layout_partida_arb.getLayout()
	};
