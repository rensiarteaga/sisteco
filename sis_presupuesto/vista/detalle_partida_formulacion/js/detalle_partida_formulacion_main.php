...
var elemento={idContenedor:idContenedor,pagina:new pagina_detalle_partida_formulacion(idContenedor,direccion,paramConfig,maestro,idContenedorPadre)};
...
function pagina_detalle_partida_formulacion(idContenedor,direccion,paramConfig,maestro,idContenedorPadre)
{
	...
	function render_id_partida(value, p, record)
	{
		...
		return String.format('{0}', '<span style="font-size:8pt" title="Descripción Partida '+record.data['codigo_partida']+'  -  '+record.data['partida_descripcion']+'">'+record.data['desc_partida']+ "</span>");
	}
	...
	Atributos[2]={
			...
			filterCol:'PARTID.nombre_partida',	//busqueda en el formulario
			typeAhead:true,
			tpl:tpl_id_partida,
			forceSelection:true,
			mode:'remote',
		...
		form: true,
		filtro_0:true,
		filterColValue:'PARTID.nombre_partida',	//busqueda en la tabla
		save_as:'id_partida',
		id_grupo:1
	};
	...
	Atributos[21] = {
		validacion: {
			...
			store: new Ext.data.SimpleStore({
				fields: ['ID', 'valor'],			
				data :[['2','Gastos x Item'],['3','Inversión'],['4','Pasajes'],['5','Viajes'],['6','RRHH'],['7','Servicios'],['8','Otros Gastos'],['9','Combustibles']]  // from states.js
			}),
			valueField:'ID',
			displayField:'valor',
				...
				layout_detalle_partida_formulacion.getLayout().layout()  });
		   	  			...
		   	  			if(SelectionsRecord.data.tipo_memoria==7){ //servicio
								...
								data+='&m_nombre_regional='+escape(maestro.nombre_regional);
			 					...
			 					data+='&m_tipo_memoria='+escape(SelectionsRecord.data.tipo_memoria);			
								data+='&m_id_partida='+escape(SelectionsRecord.data.id_partida);			
								data+='&m_desc_partida='+escape(SelectionsRecord.data.codigo_partida+' - '+SelectionsRecord.data.desc_partida.replace(/^\s+/, "").replace(/\s+$/, ""));									data+='&m_tipo_vista='+escape(maestro.tipo_vista);
								var ParamVentana={Ventana:{width:'70%',height:'90%'}}
		...
		if(NumSelect!=0)
		{
			if(SelectionsRecord.data.total==0)
			{
				...
				if(SelectionsRecord.data.tipo_memoria==2){
			       ...
			       data+='&tipo_pres='+fragmentoTexto[0];
